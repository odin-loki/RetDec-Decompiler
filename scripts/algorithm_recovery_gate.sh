...
python3 - "${RESULTS}" "${MIN_DECOMPILED}" "${MIN_MEAN_F1}" "${MIN_MEAN_F1_RAW}" <<'PY'
import json, sys

path, min_dec, min_f1, min_f1_raw = sys.argv[1:5]
min_dec = int(min_dec)
min_f1 = float(min_f1)
min_f1_raw = float(min_f1_raw) if min_f1_raw else None
...
mean_f1_raw = summary.get("mean_f1_raw")
...
if min_f1_raw is not None and mean_f1_raw is not None and float(mean_f1_raw) < min_f1_raw:
    failures.append(f"mean_f1_raw {float(mean_f1_raw):.4f} < min {min_f1_raw:.4f}")

print(f"algorithm_recovery: decompiled={decompiled} mean_f1={mean_f1:.4f} mean_f1_raw={mean_f1_raw}")
if failures:
