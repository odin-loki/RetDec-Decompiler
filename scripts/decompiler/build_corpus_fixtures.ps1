...
function Compile-CSource {
    ...
    if ($cl) {
        ...
        & cl /nologo /Od /Ob0 /Oi- /GS- /Oy- /guard:cf- /D_NO_CRT_STDIO_INLINE /Fe:"$Dest" "$Src" $stub /link /INCREMENTAL:NO /GUARD:NO /CETCOMPAT:NO /ENTRY:main /NODEFAULTLIB /SUBSYSTEM:CONSOLE /EXPORT:main 2>$null
        if (Test-Path -LiteralPath $Dest) { return $true }
    }
