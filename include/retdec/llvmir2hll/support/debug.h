...
// In release builds, soft checks (ASSERT, INVARIANT, POSTCONDITION) are
// elided for performance—they verify properties that were proven correct
// at design time and have no observable side effects.
//
// Hard checks (FAIL, PRECONDITION) are programmer errors that must never
// occur on any valid input; they always abort so that we never silently
...
#define PRECONDITION(TEST, MSG) \
	((TEST) ? static_cast<void>(0) : ABORT_IMPL("Precondition failed: `" << #TEST << "`: " << MSG))
#define PRECONDITION_NON_NULL(TEST) \
	((TEST) ? static_cast<void>(0) : ABORT_IMPL("Precondition failed: `" << #TEST << "` is null"))
#define PRECONDITION_FAILED(MSG) \
	ABORT_IMPL("Precondition failed: " << MSG)
...
// In the google test framework, there already is a FAIL macro, so when we use
// this framework, use the FAIL macro from there.  Outside tests, FAIL marks
// an unreachable path that must always terminate the process.
#ifndef FAIL
