...
private:
	...
	*
	* Uses the two-iterator form of erase so the entire [newEnd, end()) range
	* of moved-from weak_ptrs is physically removed, not just the first one.
	* The single-iterator form left stale moved-from weak_ptrs in the vector,
	* whose corrupt control blocks caused a SIGSEGV in _M_weak_release() when
