...
private:
	...
	void precomputeFunction(ShPtr<Function> funcToCompute);
...
private:
	...
	/// Set of functions that have been fully precomputed (all variables
	/// for that function are present in @c cache).  Used by the lazy
	/// per-function precompute path in getUses().
	std::set<Function*> precomputedFunctions;
};
