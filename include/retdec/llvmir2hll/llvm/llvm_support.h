...
public:
	...
	/// @name Loop shape (Category 1.2 — interval / multi-exit awareness)
	/// @{
	/// Number of distinct basic blocks that are exits from @a loop (LLVM LoopInfo).
	static std::size_t countDistinctLoopExits(const llvm::Loop *loop);
	static bool loopHasMultipleExits(const llvm::Loop *loop);
	/// @}
