...
struct fixNumberOfEntries<std::uint32_t>
{
	...
	static void fix(ResourceNode* node)
	{
		node->header.NumberOfIdEntries = static_cast<std::uint16_t>(
			node->children.size() - std::count_if(node->children.begin(), node->children.end(), [](const auto& i) {
				return i.isNamedResource();
			}));
	}
};
...
struct fixNumberOfEntries<std::string>
{
	...
	static void fix(ResourceNode* node)
	{
		node->header.NumberOfNamedEntries =
			static_cast<std::uint16_t>(std::count_if(node->children.begin(), node->children.end(), [](const auto& i) {
				return i.isNamedResource();
			}));
	}
};
...
public:
	...
	/// Returns the data of a certain resource.
	void
	getResourceData(std::uint32_t dwResTypeId, const std::string& strResName, std::vector<std::uint8_t>& data) const;
	/// Returns the data of a certain resource.
	void
	getResourceData(const std::string& strResTypeName, std::uint32_t dwResId, std::vector<std::uint8_t>& data) const;
	/// Returns the data of a certain resource.
	...
	/// Returns the data of a certain resource.
	void
	getResourceDataByIndex(unsigned int uiResTypeIndex, unsigned int uiResIndex, std::vector<std::uint8_t>& data) const;
	...
	/// Sets the data of a certain resource.
	void
	setResourceData(const std::string& strResTypeName, const std::string& strResName, std::vector<std::uint8_t>& data);
	...
	/// Sets the name of a certain resource.
	void
	setResourceName(const std::string& strResTypeName, const std::string& strResName, const std::string& strNewResName);
...
{
	auto Iter = std::find_if(m_rnRoot.children.begin(), m_rnRoot.children.end(), [&](const auto& res) {
		return res.hasEqual(restypeid);
	});
	...
	return std::find_if(currNode->children.begin(), currNode->children.end(), [&](const auto& res) {
		return res.hasEqual(resid);
	});
}
...
int ResourceDirectory::addResourceT(S restypeid, T resid, ResourceChild& rc)
{
	...
	ResourceNode* currNode = static_cast<ResourceNode*>(Iter->child);
	auto ResIter = std::find_if(currNode->children.begin(), currNode->children.end(), [&](const auto& res) {
		return res.hasEqual(resid);
	});
