...
#pragma warning( \
	disable : 4244) // C4244: 'argument': conversion from 'uint64_t' to 'unsigned int', possible loss of data
#endif
...
enum LoaderError
{
	...
	LDR_ERROR_SECTION_ALIGNMENT_TOO_SMALL,  // IMAGE_OPTIONAL_HEADER::SectionAlignment is less than
											// IMAGE_OPTIONAL_HEADER::FileAlignment
	LDR_ERROR_SECTION_ALIGNMENT_INVALID,    // IMAGE_OPTIONAL_HEADER::SectionAlignment must be equal to
											// IMAGE_OPTIONAL_HEADER::FileAlignment if (FileAlignment < 512)
	LDR_ERROR_SIZE_OF_IMAGE_TOO_BIG,        // IMAGE_OPTIONAL_HEADER::SizeOfImage is too big
	LDR_ERROR_INVALID_MACHINE32,            // IMAGE_FILE_HEADER::Machine is invalid for IMAGE_OPTIONAL_HEADER::Magic ==
											// IMAGE_NT_OPTIONAL_HDR32_MAGIC
	LDR_ERROR_INVALID_MACHINE64,            // IMAGE_FILE_HEADER::Machine is invalid for IMAGE_OPTIONAL_HEADER::Magic ==
											// IMAGE_NT_OPTIONAL_HDR64_MAGIC
	LDR_ERROR_SIZE_OF_HEADERS_INVALID,      // IMAGE_OPTIONAL_HEADER::SizeOfHeaders is greater than
											// IMAGE_OPTIONAL_HEADER::SizeOfImage
	LDR_ERROR_SIZE_OF_OPTHDR_NOT_ALIGNED,   // IMAGE_OPTIONAL_HEADER::SizeOfHeaders is not aligned to 8 (64-bit Windows
											// only)
...
const std::uint32_t PELIB_MAX_IMPORTED_FUNCTIONS =
	0x1000; // Maximum number of exported functions (per DLL) that we support
...
private:
	const std::vector<PELIB_IMAGE_FILE_MACHINE> all = {
		PELIB_IMAGE_FILE_MACHINE_UNKNOWN, PELIB_IMAGE_FILE_MACHINE_I386,      PELIB_IMAGE_FILE_MACHINE_I486,
		PELIB_IMAGE_FILE_MACHINE_PENTIUM, PELIB_IMAGE_FILE_MACHINE_R3000_BIG, PELIB_IMAGE_FILE_MACHINE_R3000_LITTLE,
		PELIB_IMAGE_FILE_MACHINE_R4000,   PELIB_IMAGE_FILE_MACHINE_R10000,    PELIB_IMAGE_FILE_MACHINE_WCEMIPSV2,
		PELIB_IMAGE_FILE_MACHINE_ALPHA,   PELIB_IMAGE_FILE_MACHINE_SH3,       PELIB_IMAGE_FILE_MACHINE_SH3DSP,
		PELIB_IMAGE_FILE_MACHINE_SH3E,    PELIB_IMAGE_FILE_MACHINE_SH4,       PELIB_IMAGE_FILE_MACHINE_SH5,
		PELIB_IMAGE_FILE_MACHINE_ARM,     PELIB_IMAGE_FILE_MACHINE_THUMB,     PELIB_IMAGE_FILE_MACHINE_ARMNT,
		PELIB_IMAGE_FILE_MACHINE_AM33,    PELIB_IMAGE_FILE_MACHINE_POWERPC,   PELIB_IMAGE_FILE_MACHINE_POWERPCFP,
		PELIB_IMAGE_FILE_MACHINE_IA64,    PELIB_IMAGE_FILE_MACHINE_MIPS16,    PELIB_IMAGE_FILE_MACHINE_MOTOROLA68000,
		PELIB_IMAGE_FILE_MACHINE_PARISC,  PELIB_IMAGE_FILE_MACHINE_ALPHA64,   PELIB_IMAGE_FILE_MACHINE_AXP64,
		PELIB_IMAGE_FILE_MACHINE_MIPSFPU, PELIB_IMAGE_FILE_MACHINE_MIPSFPU16, PELIB_IMAGE_FILE_MACHINE_TRICORE,
		PELIB_IMAGE_FILE_MACHINE_EBC,     PELIB_IMAGE_FILE_MACHINE_AMD64,     PELIB_IMAGE_FILE_MACHINE_M32R,
		PELIB_IMAGE_FILE_MACHINE_ARM64,   PELIB_IMAGE_FILE_MACHINE_MSIL};
...
struct PELIB_IMAGE_NT_HEADERS_EX
{
	...
	{
		return sizeOfSignature() + PELIB_IMAGE_FILE_HEADER::size() + sizeof(PELIB_IMAGE_OPTIONAL_HEADER)
...
struct PELIB_SECTION_HEADER : public PELIB_IMAGE_SECTION_HEADER
{
	...
	{
		if (newVirtualAddress != UINT32_MAX) VirtualAddress = newVirtualAddress;
		if (newVirtualSize != UINT32_MAX) VirtualSize = newVirtualSize;
	}

	void setRawDataRange(std::uint32_t newPointerToRawData, std::uint32_t newSizeOfRawData)
	{
		if (newPointerToRawData != UINT32_MAX) PointerToRawData = newPointerToRawData;
		if (newSizeOfRawData != UINT32_MAX) SizeOfRawData = newSizeOfRawData;
	}
...
struct PELIB_EXP_FUNC_INFORMATION
{
	...
	inline unsigned int size() const
	{
		...
		if (!funcname.empty()) uiSize = (unsigned int)(uiSize + 4 + funcname.size() + 1);
		return uiSize;
	}
};
...
struct PELIB_IMAGE_EXP_DIRECTORY
{
	...
	{
		return (
			PELIB_IMAGE_EXPORT_DIRECTORY::size() + name.size() + 1
...
struct PELIB_IMAGE_COFF_SYMBOL
{
	...
	PELIB_IMAGE_COFF_SYMBOL():
		Index(0), Value(0), SectionNumber(0), TypeComplex(0), TypeSimple(0), StorageClass(0), NumberOfAuxSymbols(0)
	{}
};
...
public:
	IStreamWrapper(std::istream& stream): _stream(stream)
	{
		_pos = _stream.tellg();
