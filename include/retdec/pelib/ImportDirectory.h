...
private:
	...
	bool hasFunction(std::string strFilename, T value, bool (PELIB_THUNK_DATA::*comp)(T) const) const
	{
		...
		for (int i = 0; i <= 1; i++) // Loop once for m_vOldiid and once for m_vNewiid
		{
			do
			{
				FileIter = std::find_if(FileIter, EndIter, [&](const auto& i) { return i == strFilename; });
...
public:
	...
	unsigned int
	getFunctionIndex(const std::string& strFilename, const std::string& strFuncname, bool newDir) const; // EXPORT
	...
	void setFunctionName(
		std::uint32_t dwFilenr, std::uint32_t dwFuncnr, bool newDir, const std::string& functionName); // EXPORT
	...
	int write(
		...
		std::uint32_t uiRva,
		std::uint32_t pointerSize); // EXPORT
	...
	void
	setFirstThunk(std::uint32_t dwFilenr, std::uint32_t dwFuncnr, bool newDir, std::uint32_t value); // EXPORT _byNumber
	/// Returns the OriginalFirstThunk value of a function.
	std::uint32_t
	getOriginalFirstThunk(std::uint32_t dwFilenr, std::uint32_t dwFuncnr, bool newDir) const; // EXPORT _byNumber
	void
	setOriginalFirstThunk(std::uint32_t dwFilenr, std::uint32_t dwFuncnr, bool newDir, std::uint32_t value); // EXPORT

	//		  std::uint64_t getFirstThunk(const std::string& strFilename, const std::string& strFuncname, bool newDir)
	// const throw (PeLibException); 		  std::uint64_t getOriginalFirstThunk(const std::string& strFilename, const
	// std::string& strFuncname, bool newDir) const throw (PeLibException);
	...
	//		  std::uint16_t getFunctionHint(const std::string& strFilename, const std::string& strFuncname, bool newDir)
	// const throw (PeLibException);
 ...
 * OriginalFirstThunk is valid if it has value higher than file alignment and its RVA can be translated to some offset
 * in the file.
 *
...
{
	return (
		imageLoader.getSizeOfHeaders() <= impDesc.OriginalFirstThunk
		&& impDesc.OriginalFirstThunk < imageLoader.getSizeOfImage());
}
 ...
 **/
inline int ImportDirectory::addFunction(const std::string& strFilename, std::uint16_t wHint)
{
	...
	ImpDirFileIterator FileIter =
		std::find_if(m_vNewiid.begin(), m_vNewiid.end(), [&](const auto& i) { return i == strFilename; });
 ...
 **/
inline int ImportDirectory::addFunction(const std::string& strFilename, const std::string& strFuncname)
{
 ...
 **/
inline unsigned int ImportDirectory::getFileIndex(const std::string& strFilename, bool newDir) const
{
	auto* il = &getImportList(newDir);

	ConstImpDirFileIterator FileIter =
		std::find_if(il->begin(), il->end(), [&](const auto& i) { return i == strFilename; });
 ...
 **/
inline std::string ImportDirectory::getFileName(std::uint32_t dwFilenr, bool newDir) const
{
	return getImportList(newDir)[dwFilenr].name;
}

inline void ImportDirectory::setFileName(std::uint32_t filenr, bool newDir, const std::string& name)
{
 ...
 **/
inline std::string ImportDirectory::getFunctionName(std::uint32_t dwFilenr, std::uint32_t dwFuncnr, bool newDir) const
{
	auto& il = getImportList(newDir);
...
inline void ImportDirectory::setFunctionName(
	std::uint32_t dwFilenr, std::uint32_t dwFuncnr, bool newDir, const std::string& functionName)
{
	auto& il = getImportList(newDir);
 ...
 **/
inline LoaderError ImportDirectory::loaderError() const
{
	return m_ldrError;
}

inline void ImportDirectory::setLoaderError(LoaderError ldrError)
{
 ...
 **/
inline std::uint32_t ImportDirectory::getNumberOfFiles(bool newDir) const
{
 ...
 **/
inline std::uint32_t ImportDirectory::getNumberOfFunctions(std::size_t dwFilenr, bool newDir) const
{
	auto& il = getImportList(newDir);
	std::uint32_t numFuncs = 0;

	if (dwFilenr < il.size()) numFuncs = static_cast<std::uint32_t>(il[dwFilenr].thunk_data.size());
	return numFuncs;
}
 ...
 **/
inline bool ImportDirectory::getImportedFunction(
	std::size_t dwFilenr,
	std::size_t dwFuncnr,
	std::string& importName,
	std::uint16_t& importHint,
 ...
 **/
inline std::uint16_t ImportDirectory::getFunctionHint(std::uint32_t dwFilenr, std::uint32_t dwFuncnr, bool newDir) const
{
	auto& il = getImportList(newDir);
	std::uint16_t hint = 0;
...
inline void
ImportDirectory::setFunctionHint(std::uint32_t dwFilenr, std::uint32_t dwFuncnr, bool newDir, std::uint16_t value)
{
	auto& il = getImportList(newDir);
...
inline bool isBadImportName(const std::string& importName)
{
	...
	if (importName.size() == 0) return true;
	...
	if (importName[0] <= 0x20) return true;

	// All characters of the name must be a valid (printable) ASCII char
	// Sample:
	// retdec-regression-tests\tools\fileinfo\features\malformed-imports-exports\7CE5BB5CA99B3570514AF03782545D41213A77A0F93D4AAC8269823A8D3A58EF.dat
	for (unsigned char singleChar: importName)
	{
		if (singleChar < 0x20 || singleChar >= 0x7f) return true;
	}
 ...
 **/
inline int ImportDirectory::read(ImageLoader& imageLoader)
{
	...
	for (;;)
	{
		...
		if (iidCurr.impdesc.Name == 0 || iidCurr.impdesc.FirstThunk == 0) break;
		...
		if (!(m_occupiedAddresses.back().second & 1)) m_occupiedAddresses.back().second += 1;
	...
	for (std::size_t i = 0; i < vOldIidCurr.size(); i++)
	{
		...
		if (seenOffsets.count(firstThunk)) continue;
		...
		for (uiIndex = 0;; uiIndex++)
		{
			...
			if (!imageLoader.readPointer(originalThunk, thunkData.itd.Ordinal)) break;

			// Are we at the end of the list?
			if (thunkData.itd.Ordinal == 0) break;
			...
			if ((thunkData.itd.Ordinal & OrdinalMask) == 0)
			{
				...
				if (thunkData.itd.Ordinal < imageLoader.getSizeOfImage())
				{
					...
					if (imageLoader.readImage(&thunkData.hint, thunkData.itd.Ordinal, sizeof(std::uint16_t))
						!= sizeof(std::uint16_t))
					{
						setLoaderError(LDR_ERROR_IMPDIR_THUNK_RVA_INVALID);
						thunkData.hint = 0;
					...
					imageLoader.readString(
						thunkData.fname, thunkData.itd.Ordinal + sizeof(std::uint16_t), IMPORT_SYMBOL_MAX_LENGTH);
					...
					m_occupiedAddresses.emplace_back(
						...
						static_cast<unsigned int>(
							thunkData.itd.Ordinal + sizeof(thunkData.hint) + thunkData.fname.length() + 1));
 ...
 **/
inline void ImportDirectory::rebuild(std::vector<std::uint8_t>& vBuffer, std::uint32_t dwRva, bool fixEntries)
{
	unsigned int uiImprva = dwRva;
	unsigned int uiSizeofdescriptors =
		(static_cast<unsigned int>(m_vNewiid.size() + m_vOldiid.size()) + 1) * PELIB_IMAGE_IMPORT_DESCRIPTOR::size();
	...
	//		for (unsigned int i=0;i<m_vNewiid.size();i++)
	//		{
	//			uiSizeofoft += (static_cast<unsigned int>(m_vNewiid[i].thunk_data.size())+1) *
	// PELIB_IMAGE_THUNK_DATA::size();
	//		}
	...
	for (unsigned int i = 0; i < m_vNewiid.size(); i++)
	{
		for (unsigned int j = 0; j < m_vNewiid[i].thunk_data.size(); j++)
		{
			...
			obBuffer.add(
				m_vNewiid[i].thunk_data[j].fname.c_str(),
				static_cast<unsigned int>(m_vNewiid[i].thunk_data[j].fname.size()) + 1);
		}
	}
}
 ...
 **/
inline int ImportDirectory::removeFile(const std::string& strFilename)
{
	unsigned int oldSize = static_cast<unsigned int>(m_vNewiid.size());

	m_vNewiid.erase(
		std::remove_if(m_vNewiid.begin(), m_vNewiid.end(), [&](const auto& i) { return i == strFilename; }),
		m_vNewiid.end());
 ...
 * @param strFuncname Name of the imported function.
 **/
inline int ImportDirectory::removeFunction(const std::string& strFilename, const std::string& strFuncname)
{
 ...
 **/
inline int ImportDirectory::removeFunction(const std::string& strFilename, std::uint16_t wHint)
{
	ImpDirFileIterator viPos = m_vNewiid.begin();
	int notFound = 1;
 ...
 **/
inline int ImportDirectory::write(
 ...
 **/
inline std::uint32_t ImportDirectory::calculateSize(std::uint32_t pointerSize) const
{
 ...
 **/
inline std::uint32_t ImportDirectory::getFirstThunk(const std::string& strFilename, bool newDir) const
{
 ...
 **/
inline std::uint32_t ImportDirectory::getOriginalFirstThunk(const std::string& strFilename, bool newDir) const
{
 ...
 **/
inline std::uint32_t ImportDirectory::getForwarderChain(const std::string& strFilename, bool newDir) const
{
 ...
 **/
inline std::uint32_t ImportDirectory::getTimeDateStamp(const std::string& strFilename, bool newDir) const
{
	return getImportList(newDir)[getFileIndex(strFilename, newDir)].impdesc.TimeDateStamp;
}

inline std::uint32_t ImportDirectory::getRvaOfName(const std::string& strFilename, bool newDir) const
{
 ...
 * @return FirstThunk value of an imported file.
 **/
inline std::uint32_t ImportDirectory::getFirstThunk(std::uint32_t dwFilenr, bool newDir) const
{
	return getImportList(newDir)[dwFilenr].impdesc.FirstThunk;
}

inline void ImportDirectory::setFirstThunk(std::uint32_t dwFilenr, bool newDir, std::uint32_t value)
{
 ...
 * @return OriginalFirstThunk value of an imported file.
 **/
inline std::uint32_t ImportDirectory::getOriginalFirstThunk(std::uint32_t dwFilenr, bool newDir) const
{
	return getImportList(newDir)[dwFilenr].impdesc.OriginalFirstThunk;
}

inline void ImportDirectory::setOriginalFirstThunk(std::uint32_t dwFilenr, bool newDir, std::uint32_t value)
{
...
inline std::uint32_t ImportDirectory::getForwarderChain(std::uint32_t dwFilenr, bool newDir) const
{
	return getImportList(newDir)[dwFilenr].impdesc.ForwarderChain;
}

inline void ImportDirectory::setForwarderChain(std::uint32_t dwFilenr, bool newDir, std::uint32_t value)
{
 ...
 * @return TimeDateStamp value of an imported file.
 **/
inline std::uint32_t ImportDirectory::getTimeDateStamp(std::uint32_t dwFilenr, bool newDir) const
{
	return getImportList(newDir)[dwFilenr].impdesc.TimeDateStamp;
}

inline void ImportDirectory::setTimeDateStamp(std::uint32_t dwFilenr, bool newDir, std::uint32_t value)
{
	getImportList(newDir)[dwFilenr].impdesc.TimeDateStamp = value;
}

inline std::uint32_t ImportDirectory::getRvaOfName(std::uint32_t dwFilenr, bool newDir) const
{
	return getImportList(newDir)[dwFilenr].impdesc.Name;
}

inline void ImportDirectory::setRvaOfName(std::uint32_t dwFilenr, bool newDir, std::uint32_t value)
{
 ...
 **/
inline std::uint32_t ImportDirectory::getFirstThunk(std::uint32_t dwFilenr, std::uint32_t dwFuncnr, bool newDir) const
{
	return getImportList(newDir)[dwFilenr].thunk_data[dwFuncnr].itd.Ordinal;
}

inline void
ImportDirectory::setFirstThunk(std::uint32_t dwFilenr, std::uint32_t dwFuncnr, bool newDir, std::uint32_t value)
{
 ...
 **/
inline std::uint32_t
ImportDirectory::getOriginalFirstThunk(std::uint32_t dwFilenr, std::uint32_t dwFuncnr, bool newDir) const
{
	if (newDir == false)
	{
...
inline void
ImportDirectory::setOriginalFirstThunk(std::uint32_t dwFilenr, std::uint32_t dwFuncnr, bool newDir, std::uint32_t value)
{
	getImportList(newDir)[dwFilenr].thunk_data[dwFuncnr].itd.Ordinal = value;
}

inline const std::vector<std::pair<unsigned int, unsigned int>>& ImportDirectory::getOccupiedAddresses() const
{
