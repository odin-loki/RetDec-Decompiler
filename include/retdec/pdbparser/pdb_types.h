...
{
	PDBTYPE_BASE,
	PDBTYPE_FIELDLIST, // Auxiliary only
	PDBTYPE_ENUM,
	PDBTYPE_ARRAY,
	PDBTYPE_POINTER,
	PDBTYPE_CONST,
...
protected:
	PDBTypeDef(int ind, ePDBTypeClass c, int bts = 0): type_index(ind), type_class(c), size_bytes(bts){};
...
public:
	...
	PDBTypeBase(int ind, ePDBBaseType t, bool ptr, int bits, const char* desc):
		PDBTypeDef(ind, PDBTYPE_BASE, (ptr) ? 4 : bits / 8), base_type(t), is_pointer(ptr), size_bits(bits),
		description(desc){};
...
public:
	// Constructor and destructor
	PDBTypeFieldList(int ind): PDBTypeDef(ind, PDBTYPE_FIELDLIST){};
...
public:
	...
	PDBTypeArray(int ind):
		PDBTypeDef(ind, PDBTYPE_ARRAY), array_elemtype_index(0), array_elemtype_def(nullptr), array_idxtype_index(0),
		array_idxtype_def(nullptr), array_count(0){};
...
public:
	// Constructor and destructor
	PDBTypePointer(int ind): PDBTypeDef(ind, PDBTYPE_POINTER), ptr_utype_index(0), ptr_utype_def(nullptr){};
...
public:
	// Constructor and destructor
	PDBTypeConst(int ind): PDBTypeDef(ind, PDBTYPE_CONST), const_utype_index(0), const_utype_def(nullptr){};
...
public:
	...
	virtual void dump(bool nested = false)
	{
		...
		if (!nested) printf("/* Argument list */\n");
	};
	virtual bool is_fully_defined(void)
	{
		return false;
...
public:
	...
	PDBTypeFunction(int ind):
		PDBTypeDef(ind, PDBTYPE_FUNCTION), func_rettype_index(0), func_rettype_def(nullptr), func_calltype(0),
		func_args_count(0), func_args(nullptr), func_is_variadic(false), func_is_clsmember(false),
		func_clstype_index(0), func_clstype_def(nullptr), func_thistype_index(0), func_thistype_def(nullptr){};
	virtual ~PDBTypeFunction(void)
	{
		if (func_args != nullptr) delete[] func_args;
	};
...
public:
	// Constructor and destructor
	PDBTypeStruct(int ind): PDBTypeDef(ind, PDBTYPE_STRUCT), struct_count(0){};
...
public:
	...
	PDBTypes(PDBStream* s):
		pdb_tpi_size(s->size), pdb_tpi_data(s->data), parsed(false), tpi_header(reinterpret_cast<HDR*>(s->data)){};
	~PDBTypes(void);
