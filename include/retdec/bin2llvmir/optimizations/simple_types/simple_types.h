...
class EquationEntry
{
	public:
		...
		/// Same relation type, but @a other points to @a newOther (used when merging EqSets).
		EquationEntry withRemappedOther(EqSet* newOther) const;
...
class SimpleTypesAnalysis : public llvm::ModulePass
{
	public:
		...
		/// Reset full-vs-partial run alternation (for unit tests; call from test SetUp).
		static void resetAlternationForTests();

	private:
		void buildEqSets(llvm::Module& M);
		/// Remove EqSets that became empty after cross-root merges (no @c processedObjs reference).
		void pruneEmptyEqSets();
		void buildEquations();
		void processRoot(llvm::Value* root);
		void processValue(std::queue<llvm::Value*>& toProcess, EqSet*& eqSetPtr);
		void mergeEqSetInto(EqSet& sink, EqSet& victim);
		void remapEquationOtherPointers(EqSet* from, EqSet* to);
