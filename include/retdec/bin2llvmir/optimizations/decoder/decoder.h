...
private:
	...
	void initJumpTargetsTls();
	void initJumpTargetsDebug();
	void initJumpTargetsPdata();   // Stage 15: PE .pdata exception directory
	void initJumpTargetsEhFrame(); // Stage 15: ELF .eh_frame exception handling
...
private:
	...
	translate(ByteData& bytes, common::Address& addr, llvm::IRBuilder<>& irb);
...
private:
	...
	/// Stage 8: Convergent boundaries — higher priority overrides (DEBUG > PDATA > SYMBOL > CONFIG)
	enum class FunctionSizePriority
	{
		CONFIG,
		SYMBOL,
		PDATA,
	...
	void addFunctionSize(
		llvm::Function* f, std::optional<std::size_t> sz, FunctionSizePriority prio = FunctionSizePriority::CONFIG);
	...
	// Stage 8: Convergent — DEBUG > SYMBOL > CONFIG
	//
	...
	//
	std::map<llvm::Function*, std::pair<std::size_t, FunctionSizePriority>> _fnc2sz;
...
private:
	...
	std::size_t decodeJumpTargetDryRun_arm(
		const JumpTarget& jt, ByteData bytes, cs_mode mode, std::size_t& decodedSz, bool strict = false);
...
private:
	...
	llvm::CallInst*
	transformToCondCall(llvm::CallInst* pseudo, llvm::Value* cond, llvm::Function* callee, llvm::BasicBlock* falseBb);
