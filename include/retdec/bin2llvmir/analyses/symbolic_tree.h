...
class SymbolicTree
{
	...
	public:
		...
		/**
		 * Returns @c true if this node could not be expanded because there were
		 * too many reaching definitions.  Callers should treat an invalid tree
		 * as "value unknown" and bail out of any pattern-matching that requires
		 * a concrete result.
		 */
	...
	private:
		unsigned _level = 1;
		/// Set to @c true when expansion was aborted because the number of
		/// reaching definitions exceeded @c _naryLimit.  The @c ops vector
		/// is left empty; callers must check @c isInvalid() before using the
		/// tree as meaningful analysis output.
		bool _invalid = false;
};
