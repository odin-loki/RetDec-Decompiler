...
//
	public:
		...
		/**
		 * Stage 7: Semantics-First Instruction Decoding.
		 * Validates that translation invariants hold after translating an
		 * instruction. Override to add semantic checks (e.g. storeOp produces
		 * StoreInst, loadOp produces LoadInst); default returns true.
		 */
		virtual bool validateTranslationSemantics(
				const TranslationResultOne& res) const
		{
			return true;
		}
//
