...
public:
	...
	virtual bool
	getXByte(std::uint64_t address, std::uint64_t x, std::uint64_t& res, Endianness e = Endianness::UNKNOWN) const = 0;
	virtual bool getXBytes(std::uint64_t address, std::uint64_t x, std::vector<std::uint8_t>& res) const = 0;

	virtual bool
	setXByte(std::uint64_t address, std::uint64_t x, std::uint64_t val, Endianness e = Endianness::UNKNOWN) = 0;
	...
	bool getWordArray(
		std::uint64_t address, std::vector<std::uint64_t>& res, std::size_t, Endianness e = Endianness::UNKNOWN) const;
...
protected:
	...
	static bool extendedBytesFor(long double val, bool hasLongDouble, std::vector<std::uint8_t>& out);
