...
public:
	...
	std::vector<SigMatchResult> batchMatch(
		const std::vector<std::string>& patterns, std::size_t startOffset = 0, std::size_t stopOffset = SIZE_MAX) const;
	 ...
	 */
	double fileEntropy(std::size_t startOffset = 0, std::size_t stopOffset = SIZE_MAX) const;
