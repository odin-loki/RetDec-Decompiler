...
class BasicBlockItem : public QGraphicsObject {
	Q_OBJECT
public:
	explicit BasicBlockItem(const BasicBlockData& data, QGraphicsItem* parent = nullptr);
...
public:
	CFGEdgeItem(QPointF from, QPointF to, EdgeKind kind, bool isCurvedBack = false, QGraphicsItem* parent = nullptr);
...
class MiniMapView : public QGraphicsView {
	Q_OBJECT
public:
	MiniMapView(QGraphicsScene* scene, QGraphicsView* mainView, QWidget* parent = nullptr);
...
public:
	...
	void setMiniMap(MiniMapView* mm)
	{
		miniMap_ = mm;
	}
 ...
 * Clicking a compressed node, its context menu, or Expand chains
 * reloads the full graph.
 */
...
public:
	...
	void loadCFG(const std::vector<BasicBlockData>& blocks, const std::vector<CFGEdgeData>& edges);
	...
	int nodeCount() const
	{
		return static_cast<int>(blockItems_.size());
	}
