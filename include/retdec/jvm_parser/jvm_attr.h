...
struct AnnotationElemArray
{
	std::vector<AnnotationElem> values;
};
...
struct NestHostAttr
{
	uint16_t hostClassIndex = 0;
};
struct NestMembersAttr
{
	std::vector<uint16_t> memberIndices;
};
 ...
 */
ParsedAttr parseAttribute(BinaryReader& r, const ConstPool& pool, const std::string& context = "unknown");

/**
 * @brief Parse `count` attributes.
 */
std::vector<ParsedAttr>
parseAttributes(BinaryReader& r, const ConstPool& pool, uint16_t count, const std::string& context = "unknown");
...
getParamAnnotations(const std::vector<ParsedAttr>& attrs, const ConstPool& pool);
