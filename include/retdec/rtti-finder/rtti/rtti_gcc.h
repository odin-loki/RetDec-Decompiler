 ...
 *
 * The Itanium ABI defines additional `type_info` subclasses that describe
 * language built-in types: `__fundamental_type_info` (scalars), `__array_type_info`,
 * `__function_type_info`, `__enum_type_info`, and `__pbase_type_info`
 * (pointer/pointer-to-member base).  These are not attached to user-defined
 * virtual classes and therefore are not useful for vtable reconstruction.
 * They can appear in RTTI chains as base types when recovering the exception-
 * handling type-filter table, but that use is handled separately by the
 * `eh_reconstruct` module.  This file intentionally covers only class and
 * class-hierarchy type_info objects.
 */
