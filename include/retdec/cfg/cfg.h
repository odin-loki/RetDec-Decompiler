...
struct CFGEdge
{
	...
	{
		return type == EdgeType::LoopBackEdge;
	}
	bool isCallEdge() const noexcept
	{
		return type == EdgeType::DirectCall || type == EdgeType::TailCall || type == EdgeType::VirtualCallEdge;
	}
};
...
public:
	 ...
	 */
	CFGBuilder(uint64_t imageBase, const uint8_t* data, std::size_t size, bool is64Bit = true);
	 ...
	 */
	void addFunction(uint64_t funcStart, uint64_t funcEnd, const std::vector<InstrSummary>& instrs);
	...
	const CFGGraph& graph() const noexcept
	{
		return _graph;
	}
	CFGGraph& graph() noexcept
	{
		return _graph;
	}
...
private:
	...
	void dfsVisit(uint64_t blockAddr, std::unordered_map<uint64_t, int>& colour);
