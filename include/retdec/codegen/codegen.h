...
struct CExpr
{
	...
	static std::shared_ptr<CExpr> var(std::string n, uint32_t vid = UINT32_MAX)
	{
		auto e = std::make_shared<CExpr>();
		e->kind = Kind::Var;
		e->varName = std::move(n);
		e->ssaValueId = vid;
		return e;
	}
	static std::shared_ptr<CExpr> binop(BinOpKind op, std::shared_ptr<CExpr> lhs, std::shared_ptr<CExpr> rhs)
	{
		auto e = std::make_shared<CExpr>();
		e->kind = Kind::BinOp;
		e->binOp = op;
	...
	static std::shared_ptr<CExpr> cast(std::shared_ptr<CType> t, std::shared_ptr<CExpr> sub)
	{
		auto e = std::make_shared<CExpr>();
		e->kind = Kind::Cast;
		e->castType = std::move(t);
		e->children.push_back(std::move(sub));
		return e;
	}
	static std::shared_ptr<CExpr> index(std::shared_ptr<CExpr> base, std::shared_ptr<CExpr> idx)
	{
		auto e = std::make_shared<CExpr>();
		e->kind = Kind::Index;
	...
	static std::shared_ptr<CExpr> member(std::shared_ptr<CExpr> base, std::string field, bool arrow)
	{
		auto e = std::make_shared<CExpr>();
		e->kind = Kind::Member;
		e->fieldName = std::move(field);
		e->arrowAccess = arrow;
		e->children.push_back(std::move(base));
		return e;
	}
	static std::shared_ptr<CExpr> call(std::string fn, std::vector<std::shared_ptr<CExpr>> args)
	{
		auto e = std::make_shared<CExpr>();
		e->kind = Kind::Call;
		e->callee = std::move(fn);
		e->children = std::move(args);
		return e;
	}
	static std::shared_ptr<CExpr>
	ternary(std::shared_ptr<CExpr> cond, std::shared_ptr<CExpr> then, std::shared_ptr<CExpr> els)
	{
		auto e = std::make_shared<CExpr>();
		e->kind = Kind::Ternary;
...
struct CStmt
{
	...
	{
		auto s = std::make_shared<CStmt>();
		s->kind = Kind::Block;
		return s;
	}
	static std::shared_ptr<CStmt> assign(std::shared_ptr<CExpr> l, std::shared_ptr<CExpr> r)
	{
		auto s = std::make_shared<CStmt>();
		s->kind = Kind::Assign;
		s->lhs = std::move(l);
	...
	static std::shared_ptr<CStmt>
	forStmt(std::shared_ptr<CExpr> init, std::shared_ptr<CExpr> cond, std::shared_ptr<CExpr> incr)
	{
		auto s = std::make_shared<CStmt>();
		s->kind = Kind::For;
	...
	static std::shared_ptr<CStmt>
	declStmt(std::string name, std::shared_ptr<CType> type, std::shared_ptr<CExpr> init = nullptr)
	{
		auto s = std::make_shared<CStmt>();
		s->kind = Kind::Decl;
...
public:
	...
	Result run(const ssa::SSAFunction& fn, const dce::DeadCodeResult& dce) const;

private:
	std::shared_ptr<CExpr> buildExpr(
		uint32_t valueId, const ssa::SSAFunction& fn, Result& res, std::unordered_set<uint32_t>& inProgress) const;
...
private:
	...
	void countGotos(const CStmt* s, std::unordered_map<std::string, GotoInfo>& info) const;
	std::shared_ptr<CStmt> rewrite(
...
private:
	...
	std::shared_ptr<CExpr>
	valueToExpr(uint32_t valueId, const ExprCoalescer::Result& exprs, const ssa::SSAFunction& fn) const;
};
