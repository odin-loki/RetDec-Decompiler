...
class FileFormat : public retdec::utils::ByteValueStorage, private retdec::utils::NonCopyable
{
	...
	public:
		...
		/// Stage 12: Reference-anchored string — get string at address if valid (for code-referenced addresses)
		std::optional<String> getStringAtAddress(std::uint64_t address, StringType type = StringType::Ascii) const;
