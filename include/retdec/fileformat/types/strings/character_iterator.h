...
public:
	...
	CharacterIterator(It itr, It first, It last, std::size_t charStep):
		itr(itr), first(first), last(last), charStep(charStep)
	{}
	...
	std::ptrdiff_t operator-(const CharacterIterator& rhs) const
	{
		...
		if (itr == rhs.itr) return 0;

		// If `charStep` is greater than 1 we are in risk that the size of data is not going to be congruent to
		// `charStep`. If the difference between iterators is going to be 0, it means this happened. We need to return 1
		// because it you take `end - itr`, where `itr` is iterator somewhere near the end (with the real distance less
		// than `charStep`), you still need to be able to do `itr + (end - itr)` and end up with the `end`. If this
		// return 0, you would end up at `itr`, which is `end - 1`.
	...
	/**
	 * Checks whether the iterator points to a valid character. Character is valid if the underlying iterator points to
	 * a byte which is printable with respect to the provided endianness. In case of `charStep` being greater than 1,
	 * the remaining bytes must be zero. If iterator points to an end, this function always returns `false`.
	 *
