...
class LlvmIrEmulator : public llvm::InstVisitor<LlvmIrEmulator>
{
	...
	private:
		...
		/// Try to handle an external (declaration) call. Returns true and sets
		/// @a result if the call was handled; returns false otherwise.
		bool tryHandleExternalCall(
				llvm::Function* f,
	...
	private:
		...
		/// Next address for VirtualAlloc/malloc stub (bump allocator).
		uint64_t _nextAllocAddr = 0x10000000;
