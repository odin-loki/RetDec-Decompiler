...
A residual class exists: some known-upstream files use **2018–2020** Odin Loch lines (Avast-era years, same rewrite pattern, different year). L1 scoped the restore to the 2017 tell. Those files are still classified **upstream** in the table below.
...
- `scripts/ci/restore_avast_headers.py` — one-shot restore used for L1. Safe to re-run; already-correct Avast headers are skipped.
- `scripts/ci/check_avast_mit_notice.py` — fails if `LICENSE-MIT` is missing or if `@copyright (c) 2017 Odin Loch` still exists under `src/`, `include/`, or `tests/`.
