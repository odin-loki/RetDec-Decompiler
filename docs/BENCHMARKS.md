...
was measured at 0% before the cause was fixed; that figure is stale and is
being re-taken (see `docs/BENCHMARKS_TABLE.md`). Buildable sidecars
...
`mean_f1` **1.0** is the withdrawn stem-tuned score — do not
advertise it.
...
Sentinel heapsort O0/O2 stay 1.000. Micro fp is **0** after the
identity-copy Mul/Xor gate.
