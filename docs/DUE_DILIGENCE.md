...
| B8 | No git tags / no downloadable release | [QUICKSTART.md](../QUICKSTART.md) exists (`REL-07`). Tag `v2.0.21` (`REL-01`) published `retdec-2.0.21-linux-x64.tar.gz` (~262 MiB) plus `install.sh` / `uninstall.sh`. CycloneDX of CMake pins attaches to the release (`REL-06`) and is keyless-signed. `sign-release-sbom.yml` also signs the Linux tarball when the bundle is missing. `docker-publish.yml` can push GHCR (`REL-02`); Docker Hub `imortek/retdec` is still unpublished. | Authenticode remains. Windows NSIS/zip still building. AppImage was skipped. `docker pull imortek/retdec` is not live. |
