...
| B8 | No git tags / no downloadable release | [QUICKSTART.md](../QUICKSTART.md) exists (`REL-07`). Tag `v2.0.21` published Linux tarball + AppImage (both keyless-signed), Windows NSIS `setup.exe` and portable zip, CycloneDX JSON (keyless-signed). `docker-from-release.yml` pushed `ghcr.io/odin-loki/retdec:v2.0.21`; unauthenticated pulls still return 401. Docker Hub `imortek/retdec` is unpublished. | Authenticode remains. Windows zip/NSIS were uploaded unsigned (cosign YAML landed after the tag; leftover `sign-release-sbom.yml` job). `docker pull imortek/retdec` is not live. |
