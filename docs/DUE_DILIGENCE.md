...
| B8 | No git tags / no downloadable release | [QUICKSTART.md](../QUICKSTART.md) exists (`REL-07`). Tag `v2.0.21` (`REL-01`) published `retdec-2.0.21-linux-x64.tar.gz` (~262 MiB) plus `install.sh` / `uninstall.sh`. CycloneDX of CMake pins and the Linux tarball both have keyless `.sigstore.json` bundles on the Release (`REL-05`/`REL-06`). `docker-from-release.yml` pushed `ghcr.io/odin-loki/retdec:v2.0.21` from that tarball (`REL-02`); unauthenticated pulls still return 401 (package visibility is not public). Docker Hub `imortek/retdec` is unpublished. | Authenticode remains. Windows NSIS/zip still building. AppImage-from-release is red (logs need a signed-in GitHub session). `docker pull imortek/retdec` is not live. |
