...
| B3 | No CLA; dual-licence relicensing is not wired | [CLA.md](../CLA.md) + PR template / CONTRIBUTING outbound grant (`LEG-05`). `.github/workflows/cla.yml` runs CLA-assistant on `pull_request_target` (`LEG-06`). | Making **CLA Assistant** a required branch-protection check is a GitHub UI setting. |
...
| B5 | Documented `retdec-qwen3-runner` / `--model` did not exist | Purged from README, user manual, Windows/MinGW docs, whitepaper. Neural path is `RETDEC_NEURAL_REFINE` + `RETDEC_NEURAL_MODEL`. `scripts/README.md` no longer advertises the phantom runner. `build-install-run-windows.ps1` defaults to `retdec-decompiler`. | `run-qwen3-trace.ps1` remains as a leftover that errors; it is not a CMake target. |
...
| B8 | No git tags / no downloadable release | [QUICKSTART.md](../QUICKSTART.md) exists (`REL-07`). Tag `v2.0.21` published Linux tarball + AppImage + Windows NSIS `setup.exe` + portable zip + CycloneDX JSON + `fib_smoke` / `fib_smoke.exe` samples; signed blobs have keyless `.sigstore.json`. `docker-from-release.yml` pushed `ghcr.io/odin-loki/retdec:v2.0.21` with `/opt/retdec/share/fib_smoke`; unauthenticated pulls still return 401. Docker Hub `imortek/retdec` is unpublished. | Authenticode remains. `docker pull imortek/retdec` is not live. |
| B9 | AGPL is a procurement blocker for the stated buyer | Dual-licence text remains; commercial terms no longer publish a price list (`LEG-10`). [LICENSING_FAQ.md](../LICENSING_FAQ.md) answers air-gap / corresponding-source questions from the commercial text (`LEG-09`). | Keystone is excluded from commercial install by CMake and `check_keystone_not_installed.py` (`LEG-11`). |
