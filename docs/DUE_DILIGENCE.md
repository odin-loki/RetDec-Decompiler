...
| B6 | Incremental cache can return wrong results; on by default | `CACHE-01`: `computeFunctionBodyHash` now includes integer immediate operand values (`BodyHashDistinguishesConstantOperands`). `ctest-linux` runs cache-off vs cache-on on `fib_smoke` (`CACHE-05`). | Detector-version tokens and BLAKE3 (`CACHE-02`, `CACHE-03`) remain. Full 216-binary corpus differential is not yet gated. |
