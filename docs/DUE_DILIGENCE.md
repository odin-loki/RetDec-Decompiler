...
| B8 | No git tags / no downloadable release | [QUICKSTART.md](../QUICKSTART.md) exists (`REL-07`). Tag `v2.0.21` (`REL-01`) fires `release-installers.yml`. CycloneDX of CMake pins attaches to the release (`REL-06`). Later commits add keyless cosign on future artefacts (`REL-05`) and `sign-release-sbom.yml` for the already-published JSON. `docker-publish.yml` can push GHCR (`REL-02`); Docker Hub `imortek/retdec` is still unpublished. | Authenticode remains. Linux/Windows installer artefacts depend on the in-progress tag workflow. `docker pull imortek/retdec` is not live. |
