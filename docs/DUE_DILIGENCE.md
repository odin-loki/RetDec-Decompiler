...
| B8 | No git tags / no downloadable release | [QUICKSTART.md](../QUICKSTART.md) exists (`REL-07`). Tag `v2.0.21` published Linux tarball + AppImage + Windows NSIS `setup.exe` + portable zip + CycloneDX JSON + `fib_smoke` sample ELF; signed blobs have keyless `.sigstore.json`. `docker-from-release.yml` pushed `ghcr.io/odin-loki/retdec:v2.0.21`; unauthenticated pulls still return 401. Docker Hub `imortek/retdec` is unpublished. | Authenticode remains. `docker pull imortek/retdec` is not live. |
