...
| B2 | 151 upstream files lack the Avast copyright line | Rewrite years **2017–2020** restored under `src/`/`include/`/`tests/`. CI fails on leftover Odin Loch rewrite lines (`check_avast_mit_notice.py`). | File-level generated provenance (`LEG-03`) still module-level. |
