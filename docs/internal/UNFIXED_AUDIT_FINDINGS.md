...
`retdec-decompiler` aborts on `generated_quicksort-gcc-O0`. Found by
`ctest-linux` run 34156903969, which is the first time that workflow has run on
this branch: the whole LLVM build, clang-tidy, the GUI suite, the unit suites
and the integration suites all pass, and the algorithm-recovery gate then fails
because one of its nine corpus binaries does not decompile.

```
extract: generated_quicksort-gcc-O0 rc=-6: Stack dump:
  0.  Program arguments: retdec-decompiler .../generated_quicksort-gcc-O0 --output ...
  1.  Running pass 'LLVM instruction optimization using RDA' on module ''
extract: decompile failed generated_quicksort-gcc-O0
GATE FAIL: decompiled 8 < min 9
```

rc=-6 is SIGABRT, and `ctest-linux` configures `full-linux-debug`, so `NDEBUG`
is off and an `assert` fires rather than being skipped. The four candidates are
in `src/bin2llvmir/analyses/reaching_definitions.cpp` (lines 248, 277, 343 and
347), all of the shape "we should have this basic block in bbMap". In a Release
build the same input reaches undefined behaviour instead of aborting, which is
the worse half of this.

The input is an ordinary `gcc -O0` quicksort generated by
`tests/algorithm_recovery/sources/generated`. Not diagnosed further here: it
needs the pinned LLVM build to reproduce, and this environment cannot run it.
The gate itself is correct to fail -- `MIN_DECOMPILED=9` against nine binaries
means "all of them", and eight decompiled.
