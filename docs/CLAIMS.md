...
| C-B8-LOOP | Loop-containing negatives; FP rate on strlen/atoi/dfs/sort heuristics | `RETDEC_AUDIT.md` B8 leftover | `scripts/ci/run_b8_loop_negatives.sh`; `results/b8-loop-negatives.md`. After Quicksort self-call + Mergesort floor removal, FP **0.200** (20/100). Sort labels gone; remaining FPs are ring_buffer and copy. Not a product figure. | demonstrated |
...
| C-B9 | Adversarial-positive recall on idiosyncratic implementations | `RETDEC_AUDIT.md` B9 | `scripts/ci/run_b9_adversarial_corpus.sh`; `results/b9-adversarial-positive.md`. Name-blind mean F1 **0.093** on 18 binaries after the sort-gate remasure. Not a product figure. | demonstrated |
