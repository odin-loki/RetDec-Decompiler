...
| C-B7 | Symbol-name hits tagged and excluded from headline F1 | `RETDEC_AUDIT.md` B7 | Concurrency, open-addressing, named-variant introsort/heapsort/mergesort, name-only partition swap, name-only unordered-map hash, name-only shared_ptr atomic, name-only list alloc, and name-only vector growth export tag `evidence:symbol_name`; extract skips them. Extract already drops `std::partition` labels and does not map `kind=="crypto"` / `kind=="serial"` / `kind=="pattern"`. ci-core **0.126**; full 216 **0.056**. `results/b7-name-evidence.md`. AES-NI-below-floor/unordered/self-recursion still leftover. Serial name-only and RAII table hits are now tagged. | demonstrated |
...
| C-N16 | Neural refine prompt includes existing semantic context | `RETDEC_AUDIT.md` N16 | `src/neural/decompile_hook.cpp` (`serializeSemanticContext`): function fields including start/end address, known calling convention, role flags, return/parameter/frame-base storage, parameter real names, and source line numbers, RTTI class members, vtable items (name/address/target/thumb), YARA patterns, tools (including confidence), languages, architecture (including endian), file format, file class bits, file type, callers/callees. Comments, locals, globals, and pattern descriptions are not dumped. Crypto, serial, and design-pattern detections are now pipeline annotations (`kind="crypto"` / `kind="serial"` / `kind="pattern"`). No invented caller-buffer fields. Tests in `tests/neural/mock_test.cpp`. Off unless `RETDEC_NEURAL_REFINE`. | opt-in |
| C-N17 | Per-suggestion logprob; abstain below threshold | `RETDEC_AUDIT.md` N17 | `src/neural/llama_inference.cpp` (`llama_token_data.p` after `llama_sampler_apply`). Manifest `mean_token_p`. Abstain only if `RETDEC_NEURAL_MIN_MEAN_P` is set. Mock test `LowMeanTokenProbAbstains`. Not per-identifier logprob. | opt-in |
