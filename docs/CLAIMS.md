...
| C-B7 | Symbol-name hits tagged and excluded from headline F1 | `RETDEC_AUDIT.md` B7 | Concurrency, open-addressing, named-variant introsort/heapsort/mergesort, name-only partition swap, name-only unordered-map hash, name-only shared_ptr atomic, name-only list alloc, and name-only vector growth export tag `evidence:symbol_name`; extract skips them. Extract already drops `std::partition` labels and does not map `kind=="crypto"`. ci-core **0.126**; full 216 **0.056**. `results/b7-name-evidence.md`. Serial/pattern/AES-NI-below-floor/unordered/self-recursion still leftover. | demonstrated |
