...
| C-N11 | Prompt that exceeds `n_ctx` is refused, not silently truncated | `RETDEC_AUDIT.md` N11 | `src/neural/llama_inference.cpp` (`llama_n_ctx` vs tokenize count + `maxTokens`). Piece buffer retry on negative `llama_token_to_piece`. No deadline or GUI cancel. | demonstrated |
