...
| C-N14 | Neural unit tests cover gates, injection strip, context overflow, and manifest schema | `RETDEC_AUDIT.md` N14 | `tests/neural/mock_test.cpp`: spawn-call reject, string-literal strip, context-budget retry, `ManifestSchemaHasRequiredKeys`, `EachTierHasDistinctInstruction`. No adversarial-binary injection corpus yet. | demonstrated |
