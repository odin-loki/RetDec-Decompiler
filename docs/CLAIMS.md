...
| C-N14 | Neural unit tests cover gates, injection strip, context overflow, and manifest schema | `RETDEC_AUDIT.md` N14 | `tests/neural/mock_test.cpp`: spawn-call reject, string-literal strip, comment-body strip, context-budget retry, `ManifestSchemaHasRequiredKeys`, `EachTierHasDistinctInstruction`, `ConcurrentIndependentRefinesDoNotCrash`. C-source fixtures only; no adversarial-binary injection corpus. Does not claim llama.cpp is thread-safe. | demonstrated |
