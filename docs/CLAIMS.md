...
| C-B7 | Symbol-name hits tagged and excluded from headline F1 | `RETDEC_AUDIT.md` B7 | Concurrency, open-addressing, named-variant introsort/heapsort/mergesort, name-only partition swap, and name-only unordered-map hash export tag `evidence:symbol_name`; extract skips them. Extract already drops `std::partition` labels. ci-core **0.126**; full 216 **0.056**. `results/b7-name-evidence.md`. Serial/unordered/self-recursion still untagged. | demonstrated |
