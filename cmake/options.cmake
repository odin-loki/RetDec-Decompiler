...
# Profiling and sanitizer options (for bug detection).
option(RETDEC_USE_ADDRESS_SANITIZER "Build with AddressSanitizer (memory errors)." OFF)
option(RETDEC_USE_UNDEFINED_BEHAVIOR_SANITIZER "Build with UndefinedBehaviorSanitizer." OFF)
...
# OpenSSL: default ON when targeting Windows (MinGW / cross from Unix). Bundled build is MinGW+make only (not MSVC).
if(WIN32)
	set(_RETDEC_BUNDLE_SSL_DEFAULT ON)
elseif(CMAKE_CROSSCOMPILING AND CMAKE_SYSTEM_NAME STREQUAL "Windows")
	set(_RETDEC_BUNDLE_SSL_DEFAULT ON)
else()
	set(_RETDEC_BUNDLE_SSL_DEFAULT OFF)
endif()
option(RETDEC_BUNDLED_OPENSSL "Build OpenSSL from source into the build tree (external/openssl-ep + external/openssl-install)" ${_RETDEC_BUNDLE_SSL_DEFAULT})

if(CMAKE_CROSSCOMPILING AND CMAKE_SYSTEM_NAME STREQUAL "Windows")
	set(_RETDEC_BUNDLE_ZLIB_CROSS_DEFAULT ON)
else()
	set(_RETDEC_BUNDLE_ZLIB_CROSS_DEFAULT OFF)
endif()
option(RETDEC_BUNDLED_ZLIB_WIN_CROSS "Build zlib into the tree for LLVM when cross-compiling to Windows (MinGW)" ${_RETDEC_BUNDLE_ZLIB_CROSS_DEFAULT})
...
# retdec/utils/bounds.h and leb128.h are header-only, so this suite needs no
# library beyond GoogleTest. It gates on RETDEC_TESTS alone.
set_if_all_set(RETDEC_ENABLE_BOUNDS_TESTS
		RETDEC_TESTS)
set_if_all_set(RETDEC_ENABLE_CONFIG_TESTS
		RETDEC_TESTS
		RETDEC_ENABLE_CONFIG)
set_if_all_set(RETDEC_ENABLE_CPDETECT_TESTS
		RETDEC_TESTS
		RETDEC_ENABLE_CPDETECT)
...
set_if_at_least_one_set(RETDEC_ENABLE_GOOGLETEST
		...
		RETDEC_ENABLE_BOUNDS_TESTS
		RETDEC_ENABLE_CAPSTONE2LLVMIR_TESTS
		RETDEC_ENABLE_COMMON_TESTS
		RETDEC_ENABLE_CONFIG_TESTS
		RETDEC_ENABLE_CPDETECT_TESTS
