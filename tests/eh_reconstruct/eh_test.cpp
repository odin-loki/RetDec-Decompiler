...
public:
	...
	FlatBin()
	{
		data_.resize(0x10000, 0);
	}

	uint64_t imageBase() const noexcept override
	{
		return base_;
	}
	...
	// ── Write helpers ──
	uint64_t off(uint64_t vma) const
	{
		return vma - base_;
	}

	void writeU8(uint64_t vma, uint8_t v)
	{
		data_[off(vma)] = v;
	}
	...
	void writeI32(uint64_t vma, int32_t v)
	{
		writeU32(vma, (uint32_t)v);
	}
 ...
 */
static uint64_t writeMsvcUnwindInfo(
	FlatBin& fb,
	uint64_t unwindVma,
 ...
 */
static void writeRuntimeFn(FlatBin& fb, uint64_t rfVma, uint32_t beginRVA, uint32_t endRVA, uint32_t unwindRVA)
{
...
TEST(MsvcEH, AllocSmall)
{
	...
	std::vector<uint16_t> codes = {(uint16_t)(5 | (((1 << 4) | 2) << 8))};
...
TEST(MsvcEH, TwoRuntimeFunctions)
{
	...
	writeRuntimeFn(fb, pdataVma + 0, (uint32_t)(fn1Begin - base), (uint32_t)(fn1End - base), (uint32_t)(unw1 - base));
	writeRuntimeFn(fb, pdataVma + 12, (uint32_t)(fn2Begin - base), (uint32_t)(fn2End - base), (uint32_t)(unw2 - base));
 ...
 */
static uint64_t writeCIE(FlatBin& fb, uint64_t sectionVma, uint64_t& cur, const char* augmentation = "")
{
...
struct LsdaBuilder
{
	...
	uint64_t build(uint64_t funcStart)
	{
		...
		for (auto& s: sites)
		{
			fb.writeU32(cur, (uint32_t)s.start);
			cur += 4;
			fb.writeU32(cur, (uint32_t)s.len);
			cur += 4;
			fb.writeU32(cur, (uint32_t)s.lp);
			cur += 4;
...
TEST(ItaniumEH, CatchAll_TypeFilter0)
{
	...
	lsdaCur += 4;                           // landing pad offset from func
...
TEST(EHReconstructor, FindFunction_ReturnsCorrectFunction)
{
	...
	{
		std::size_t readBytes(uint64_t, uint8_t*, std::size_t) const override
		{
			return 0;
		}
		bool isMapped(uint64_t) const override
		{
			return false;
		}
		bool isDataSection(uint64_t) const override
		{
			return false;
		}
		uint64_t imageBase() const noexcept override
		...
		uint64_t sectionVma(const char*) const noexcept override
		{
			return 0;
		}
		std::size_t sectionSize(const char*) const noexcept override
		{
			return 0;
		}
	} nv;
...
TEST(EHReconstructor, Callback_CalledForEachFunction)
{
	struct FakeParser final : public IEHParser
	{
		...
		{
			EHFunction a, b;
			a.functionVma = 0x1000;
			a.functionEnd = 0x1100;
			b.functionVma = 0x2000;
			b.functionEnd = 0x2100;
			return {a, b};
		}
	};
...
static void writeExidxEntry(FlatBin& fb, uint64_t entryVma, uint64_t fnVma, uint32_t word1)
{
...
static void writeBorlandProlog(FlatBin& fb, uint64_t prologVma, uint32_t handlerAddr)
{
	// PUSH imm32 (handler)
...
static void writeBorlandExceptRec(
	FlatBin& fb, uint64_t vma, uint32_t tryStart, uint32_t tryEnd, uint32_t catchOff, uint32_t typePtr)
{
...
static void writeDmcScopeEntry(FlatBin& fb, uint64_t vma, int32_t encLevel, uint32_t filterFn, uint32_t handlerFn)
{
...
static void writeWatcomEntry(FlatBin& fb, uint64_t vma, uint32_t fnStartRVA, uint32_t fnEndRVA, uint32_t htRVA)
{
	fb.writeU32(vma + 0, fnStartRVA);
	fb.writeU32(vma + 4, fnEndRVA);
	fb.writeU32(vma + 8, htRVA);
}
static void writeWatcomHandlerEntry(
	FlatBin& fb, uint64_t vma, uint16_t tryStart, uint16_t tryEnd, uint16_t catchOff, uint16_t flags, uint32_t typeRVA)
{
