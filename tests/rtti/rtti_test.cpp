...
public:
	...
	explicit FlatBinaryBuilder(bool is64bit = true): is64bit_(is64bit) {}
	...
	{
		return kBase + execBuf_.size() + dataBuf_.size();
	}
	...
	void writePtrAt(uint64_t vma, uint64_t val)
	{
		if (is64bit_)
			...
			writeAt(vma, static_cast<uint32_t>(val));
	}
	...
	uint64_t execVma() const
	{
		return kBase + execBuf_.size();
	}
...
static ClassHierarchyGraph makeSampleGraph()
{
	...
	ClassNode base;
	base.name = "Base";
	base.typeInfoVma = 0x1000;
	g.classes["Base"] = base;

	ClassNode derived;
	derived.name = "Derived";
	derived.typeInfoVma = 0x2000;
	InheritanceEdge e;
	e.baseName = "Base";
	...
	InheritanceEdge e2;
	e2.baseName = "Base";
	e2.kind = InheritanceKind::Direct;
	InheritanceEdge e3;
	e3.baseName = "Derived";
	e3.kind = InheritanceKind::Virtual;
	multi.bases = {e2, e3};
			...
			for (const auto& slot: vt.slots)
			{
				...
				if (!slot.isNull && !slot.isPureVirtual)
				{
					EXPECT_TRUE(view.inExecutable(slot.targetVma))
						<< "Slot target 0x" << std::hex << slot.targetVma << " not in exec section";
				}
			}
		}
	}
...
TEST(MsvcReconstructor, Reconstruct_classIsPolymorphic)
{
	...
	for (const auto& kv: g.classes)
	{
		if (!kv.second.vtables.empty()) EXPECT_TRUE(kv.second.isPolymorphic);
	}
}
