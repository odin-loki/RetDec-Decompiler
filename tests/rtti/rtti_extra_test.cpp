...
public:
	...
	{
		uint64_t v = execVma();
		execBuf_.resize(execBuf_.size() + n, fill);
		return v;
	}
	...
	{
		uint64_t v = dataVma();
		dataBuf_.resize(dataBuf_.size() + n, fill);
		return v;
	}
...
struct DmcFactory
{
	...
	void buildClass(const std::string& className, bool hasBase = false, const std::string& baseName = "")
	{
		funcVma = b.allocExec(16);
		...
		uint64_t nameVma = b.allocData(className.size() + 1);
		{
			...
			wstr(s, className);
			s.pop_back(); // wstr adds null already
			s.push_back(0);
			b.writeAt(nameVma, s);
		}
...
struct SymbianFactory
{
	...
	void buildClass(const std::string& className, uint32_t iSize = 32, uint64_t parentVma = 0)
	{
		funcVma = b.allocExec(16);
