...
TEST(CsTypeEmitter, EmitsGenericConstraints) {
    CsWriter w;
    CsExprEmitter ex(w);
    CsStmtEmitter st(w, ex);
    CsTypeEmitter te(w, ex, st);

    BcClass cls = makeSimpleClass("SortedList");
    cls.typeParams = {"E extends Comparable<E>"};
    ...
    std::string s = w.str();
    EXPECT_TRUE(contains(s, "SortedList<E>"));
    EXPECT_TRUE(contains(s, "where E :"));
    EXPECT_TRUE(contains(s, "Comparable<E>"));
}
...
TEST(CsTypeEmitter, EmitsClassAttribute) {
    CsWriter w;
    CsExprEmitter ex(w);
    CsStmtEmitter st(w, ex);
    CsTypeEmitter te(w, ex, st);
    ...
    std::string s = w.str();
    EXPECT_TRUE(contains(s, "[Obsolete]"));
}

TEST(CsTypeEmitter, EmitsAttributeArguments) {
    CsWriter w;
    CsExprEmitter ex(w);
    CsStmtEmitter st(w, ex);
    CsTypeEmitter te(w, ex, st);
    ...
    BcClass cls = makeSimpleClass("MyClass");
    BcAnnotation ann;
    ann.typeName = "System.Runtime.InteropServices.DllImportAttribute";
    BcAnnotationValue dll;
    dll.kind = BcAnnotationValue::Kind::String;
    dll.stringValue = "kernel32";
    ann.elements["Value"] = dll;
    BcAnnotationValue ep;
    ep.kind = BcAnnotationValue::Kind::String;
    ep.stringValue = "Foo";
    ann.elements["EntryPoint"] = ep;
    cls.annotations.push_back(ann);
    ...
    std::string s = w.str();
    EXPECT_TRUE(contains(s, "[DllImport(\"kernel32\""));
    EXPECT_TRUE(contains(s, "EntryPoint = \"Foo\""));
}

TEST(CsTypeEmitter, EmitsParamAttribute) {
    CsWriter w;
    CsExprEmitter ex(w);
    CsStmtEmitter st(w, ex);
    CsTypeEmitter te(w, ex, st);
    ...
    std::string s = w.str();
    EXPECT_TRUE(contains(s, "[Obsolete] int"));
}
...
TEST(CsFileEmitter, EmitsAssemblyVersion) {
    BcModule module("TestLib", SourceLang::CSharp);
    module.version = "1.2.3.4";
    BcAnnotation ann;
    ann.typeName = "System.Runtime.CompilerServices.CompilationRelaxationsAttribute";
    module.moduleAnnotations.push_back(ann);
    BcClass& cls = module.addClass(makeSimpleClass("Foo", "TestLib"));
    (void)cls;
    ...
    EXPECT_TRUE(contains(src, "// Version: 1.2.3.4"));
    EXPECT_TRUE(contains(src, "[assembly: CompilationRelaxations]"));
}
