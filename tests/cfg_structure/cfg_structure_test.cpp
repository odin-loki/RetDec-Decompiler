	...
	std::vector<uint32_t> addBlocks = {})
{
	...
	for (std::size_t i = 0; i < numBlocks; ++i)
	{
		...
		if (!bb) continue;
		bool hasCond = std::find(condBlocks.begin(), condBlocks.end(), static_cast<uint32_t>(i)) != condBlocks.end();
		bool hasAdd = std::find(addBlocks.begin(), addBlocks.end(), static_cast<uint32_t>(i)) != addBlocks.end();
		if (!hasCond)
		{
			if (hasAdd)
			{
...
TEST(CompilerStructurer, SingleBlock_EmitsBlock)
{
	...
	bool ok =
		(root->kind == StructNode::Kind::Block) || (root->kind == StructNode::Kind::Sequence && root->children.empty());
	EXPECT_TRUE(ok);
}
...
TEST(CompilerStructurer, WhileLoop_Detected)
{
	...
	std::function<void(const StructNode*)> scan = [&](const StructNode* n) {
		if (!n) return;
		if (n->kind == StructNode::Kind::While || n->kind == StructNode::Kind::For) foundLoop = true;
		for (const auto& c: n->children)
			scan(c.get());
	};
	scan(root.get());
...
// ─── What comes out of a loop, and what comes after it ───────────────────────
...
// Two defects met here. NaturalLoop::exits holds the body blocks that HAVE an
// edge leaving the loop, not the blocks those edges go to; every one of them is
// in the body by construction, so the "skip any exits that are still inside the
// body" filter could never fire and the walk resumed at a body block it had
// already visited -- everything after a while or for loop became one goto. And
// pre-marking the latch visited made it the body entry of an ordinary two-block
...
// and ends at the latch. Bounding it by the header instead let the walk run
// past the latch's exit edge and pull the block after the loop inside it --
