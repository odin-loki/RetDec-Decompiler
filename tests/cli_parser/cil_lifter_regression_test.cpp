...
void u8(std::vector<uint8_t>& v, uint8_t x)
{
	v.push_back(x);
}
...
std::vector<uint8_t> fatBodyWithSections(const std::vector<uint8_t>& code, const std::vector<uint8_t>& sections)
{
	std::vector<uint8_t> body;
...
TEST(CILLifterEhSectionRegression, ZeroDataSizeDeclaresNoClauses)
{
	...
	u16(sect, 0);   // reserved
	for (int i = 0; i < 12; ++i)
		u8(sect, 0xAB); // one small clause of bytes
...
TEST(CILLifterEhRegionRegression, TryRegionThatWrapsIsNotRecorded)
{
	...
	fatClause(
		sect,
		/*kind=*/0,
		/*tryOffset=*/310550527u,
		/*tryLength=*/3984416769u,
		/*handlerOffset=*/0,
		/*handlerLength=*/1,
		/*classToken=*/0);
