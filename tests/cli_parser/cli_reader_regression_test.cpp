...
void putU8(std::vector<uint8_t>& v, uint8_t x)
{
	v.push_back(x);
}
...
std::vector<uint8_t> buildStringsHeap()
{
	...
	assert(v[kStrModule] == '<' && v[kStrClass] == 'C' && v[kStrField] == 'K' && v[kStrAsmName] == 't');
	return v;
}
...
std::vector<uint8_t> buildTildeStream(uint8_t elementType, uint16_t blobIdx, int32_t thirdRowLists = -1)
{
	...
	putU8(v, elementType);
	putU8(v, 0);
	putU16(v, static_cast<uint16_t>((1u << kHasConstantTagBits) | kHasConstantFieldTag)); // Field row 1
	putU16(v, blobIdx);
...
std::vector<uint8_t> buildMetadataRoot(const std::vector<StreamSpec>& streams)
{
	...
	putU16(md, 1);
	putU16(md, 1);                                 // Major/MinorVersion
	putU32(md, 0);                                 // Reserved
	putU32(md, static_cast<uint32_t>(versionLen)); // VersionLength
	for (char c: version)
		md.push_back(static_cast<uint8_t>(c));
	while (md.size() < 16 + versionLen)
		md.push_back(0);
	putU16(md, 0);                                     // Flags
...
/// returns a value.
bool readTheOnlyField(const std::vector<uint8_t>& buf, BcField& out, std::string& why)
{
	CLIReader reader;
	...
	{
		why = "expected 1 class, got " + std::to_string(result.module.classes().size());
		return false;
	}
...
TEST(CLIReaderConstantTest, StringConstantSurrogatePairBecomesOneCharacter)
{
	...
	BcField f;
	std::string why;
	ASSERT_TRUE(readTheOnlyField(buildAssemblyWithConstant(ElementType::String, blob), f, why)) << why;
	ASSERT_TRUE(f.constantStrValue.has_value());

	const std::string expected = "\xF0\x9F\x98\x80";
	EXPECT_EQ(expected, *f.constantStrValue)
		<< "got " << hexOf(*f.constantStrValue) << ", expected F0 9F 98 80 (U+1F600)";
...
TEST(CLIReaderConstantTest, StringConstantLoneSurrogateBecomesReplacement)
{
	...
	EXPECT_EQ(expected, *f.constantStrValue) << "got " << hexOf(*f.constantStrValue) << ", expected EF BF BD (U+FFFD)";
}
...
TEST(CLIReaderConstantTest, StringConstantLoneLowSurrogateBecomesReplacement)
{
	...
	ASSERT_TRUE(readTheOnlyField(buildAssemblyWithConstant(ElementType::String, blob), f, why)) << why;
	ASSERT_TRUE(f.constantStrValue.has_value());
	EXPECT_EQ(std::string("\xEF\xBF\xBD"), *f.constantStrValue) << "got " << hexOf(*f.constantStrValue);
}
...
TEST(CLIReaderConstantTest, StringConstantNonSurrogatesAreUnchanged)
{
	...
	for (const auto& c: cases)
	{
		BcField f;
		std::string why;
		ASSERT_TRUE(readTheOnlyField(buildAssemblyWithConstant(ElementType::String, c.blob), f, why))
			<< c.what << ": " << why;
		ASSERT_TRUE(f.constantStrValue.has_value()) << c.what;
		EXPECT_EQ(c.expected, *f.constantStrValue) << c.what << ": got " << hexOf(*f.constantStrValue);
	}
}
...
TEST(CLIReaderConstantTest, FixedWidthConstantsKeepTheirValues)
{
	...
	const Case cases[] = {
		...
		{ElementType::I8, {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF}, -1},
		{ElementType::I8, {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80}, INT64_MIN},
	...
	for (const auto& c: cases)
	{
		BcField f;
		std::string why;
		ASSERT_TRUE(readTheOnlyField(buildAssemblyWithConstant(c.ty, c.blob), f, why)) << nameOf(c.ty) << ": " << why;
...
TEST(CLIReaderConstantTest, CharConstantIsAnUnsignedCodeUnit)
{
	...
	for (const auto& c: cases)
	{
		BcField f;
		std::string why;
		ASSERT_TRUE(readTheOnlyField(buildAssemblyWithConstant(ElementType::Char, c.blob), f, why))
			<< c.what << ": " << why;
		ASSERT_TRUE(f.constantIntValue.has_value()) << c.what;
		EXPECT_EQ(c.expected, *f.constantIntValue)
			<< c.what << " -- a Char is unsigned, so no #Blob code unit " << "decodes negative";
...
TEST(CLIReaderConstantTest, FloatConstantsAreReadLittleEndian)
{
	...
	for (const auto& c: r4)
	{
		...
		BcField f;
		std::string why;
		ASSERT_TRUE(readTheOnlyField(buildAssemblyWithConstant(ElementType::R4, blob), f, why))
			<< c.what << ": " << why;
		ASSERT_TRUE(f.constantFltValue.has_value()) << c.what;
		EXPECT_EQ(static_cast<double>(floatFromBits(c.bits)), *f.constantFltValue)
			<< c.what << ": R4 blob holds the four bytes of " << std::hex << c.bits << " little-endian";
	...
	for (const auto& c: r8)
	{
		...
		BcField f;
		std::string why;
		ASSERT_TRUE(readTheOnlyField(buildAssemblyWithConstant(ElementType::R8, blob), f, why))
			<< c.what << ": " << why;
		ASSERT_TRUE(f.constantFltValue.has_value()) << c.what;
		EXPECT_EQ(doubleFromBits(c.bits), *f.constantFltValue)
			<< c.what << ": R8 blob holds the eight bytes of " << std::hex << c.bits << " little-endian";
		EXPECT_FALSE(f.constantIntValue.has_value()) << c.what;
	}
}
