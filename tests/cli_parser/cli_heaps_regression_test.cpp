...
// wrong.
MetadataToken splitCodedToken(std::uint32_t raw, const std::uint8_t* tableIds, std::size_t count, unsigned tagBits);
} // namespace cli_parser
} // namespace retdec
...
{
	if (g_countingAllocations.load(std::memory_order_relaxed)) g_bytesAllocated.fetch_add(n, std::memory_order_relaxed);
...
TEST(CliHeapsGuidRegression, IndexAt16MDoesNotWrapOntoTheFirstGuid)
{
	...
	EXPECT_TRUE(isAllZero(wrapped)) << "index 0x10000001 aliased onto heap offset 0";
}
...
TEST(CliHeapsGuidRegression, WrappedIndexDoesNotAliasTheLastGuid)
{
	...
	EXPECT_TRUE(isAllZero(wrapped)) << "index 805306623 aliased onto heap offset 4064";
}
...
TEST(CliHeapsCompressedIntRegression, RefusalLeavesTheCursorAlone)
{
	...
	std::size_t pos = 0;
	EXPECT_FALSE(decodeCompressedUInt({truncated.data(), truncated.size()}, pos).has_value());
	EXPECT_EQ(0u, pos);

	std::vector<uint8_t> badTag = {0xE0}; // 111xxxxx is not a compressed integer
	pos = 0;
	EXPECT_FALSE(decodeCompressedUInt({badTag.data(), badTag.size()}, pos).has_value());
	EXPECT_EQ(0u, pos);
}
...
TEST(CliHeapsUserStringsRegression, StringAtNonZeroOffsetUsesItsOwnLengthPrefix)
{
	std::vector<uint8_t> heap = {
		...
		0x69,
		0x00,
		0x00, // 6..11  len 5, "Hi", flag
		0x05,
		0x58,
		0x00,
		0x59,
		0x00,
		0x00, // 12..17 len 5, "XY", flag
	};
...
TEST(CliHeapsUserStringsRegression, NonSurrogateUnitsAreUnchanged)
{
	...
	std::vector<uint8_t> heap = {0x07, 0x41, 0x00, 0xE9, 0x00, 0xAC, 0x20, 0x00};
	UserStringsHeap h({heap.data(), heap.size()});
...
TEST(CliSigCountRegression, GenericArgCountIsBoundedByTheBytesLeft)
{
	...
	EXPECT_EQ(0u, ct->base.ref().typeArgs.size()) << "1000 generic arguments declared by a 5-byte blob";
	...
	EXPECT_NE(static_cast<int>(BcRefKind::Generic), static_cast<int>(ct->base.ref().kind))
...
TEST(CliSigCountRegression, ArrayShapeSizeCountIsBoundedByTheBytesLeft)
{
	...
	EXPECT_LT(used, 256u * 1024u) << "a 7-byte array signature committed " << used << " bytes";
}
...
TEST(CliSigCountRegression, WellFormedArrayShapeStillDecodes)
{
	...
	std::vector<uint8_t> blob = {0x14, 0x08, 0x02, 0x02, 0x04, 0x04, 0x02, 0x01, 0x00};
	CliSigDecoder dec;
	auto ct = dec.decodeTypeSpec({blob.data(), blob.size()});
	ASSERT_TRUE(ct.has_value());
	ASSERT_TRUE(ct->base.isRef());
...
TEST(CliSigCountRegression, ArrayShapeIsWalkedToItsEnd)
{
	...
	//   local 1: STRING
	std::vector<uint8_t> blob = {0x07, 0x02, 0x14, 0x08, 0x02, 0x02, 0x04, 0x04, 0x02, 0x01, 0x00, 0x0E};
	CliSigDecoder dec;
...
std::size_t arrayNesting(const BcType& t)
{
	std::size_t n = 0;
	const BcType* cur = &t;
	while (cur->isRef() && cur->ref().kind == BcRefKind::Array && cur->ref().elementType)
	{
		++n;
...
TEST(CliSigDepthRegression, NestedTypeDepthDoesNotFollowTheBlobLength)
{
	...
	EXPECT_LT(longNesting, kLong) << "a " << kLong << "-byte SZARRAY chain nested " << longNesting
	...
	EXPECT_EQ(shortNesting, longNesting) << kShort << " bytes nested " << shortNesting << " deep and " << kLong
										 << " bytes nested " << longNesting
										 << " deep, so nothing bounds the "
...
public:
	...
	void setDecoder(const CliSigDecoder* d)
	{
		decoder_ = d;
	}
	long calls() const
	...
	std::string typeDefName(uint32_t) const override
	{
		return "TypeDef";
	}
	std::string typeRefName(uint32_t) const override
	{
		return "TypeRef";
	}
...
TEST(CliTablesCodedTokenRegression, TagWiderThanTheWordIsRefused)
{
	for (unsigned tagBits: {32u, 65u, 255u})
	{
		...
		{
			MetadataToken tok = splitCodedToken(raw, kTypeDefOrRefTables, 3, tagBits);
			EXPECT_EQ(0xFF, unsigned(tok.table))
				<< "tagBits=" << tagBits << " raw=" << raw << " named table " << unsigned(tok.table);
