...
static BcInstruction makeInvokeVirtual(
	...
	uint32_t offset = 0)
{
	...
	ref.owner = owner;
	ref.name = name;
	auto retType0 = (retDesc == "V") ? types::Void() : (retDesc == "I") ? types::Int() : types::Class(retDesc);
...
static BcInstruction
makeInvokeInterface(const std::string& owner, const std::string& name, const std::string& retDesc, uint32_t offset = 0)
{
	auto i = makeInsn(BcOpcode::InvokeInterface, offset);
	BcMethodRef ref;
	ref.owner = owner;
	ref.name = name;
	auto retType1 = (retDesc == "Z") ? types::Bool() : (retDesc == "V") ? types::Void() : types::Class(retDesc);
	ref.descriptor.returnType = std::make_shared<BcType>(retType1);
	i.operands.push_back(ref);
	return i;
}

static BcInstruction makeInvokeDynamic(const std::string& bsmOwner, const std::string& bsmName, uint32_t offset = 0)
{
...
static BcInstruction makeInvokeStatic(const std::string& owner, const std::string& name, uint32_t offset = 0)
{
...
static BcInstruction makeLoadLocal(uint32_t idx, BcType type = types::Int(), uint32_t offset = 0)
{
...
static BcMethod makeMethod(const std::string& name = "test", bool isStatic = true)
{
	BcMethod m;
	m.name = name;
...
TEST(JvmStackSim, ExceptionHandlerGetsExceptionSlot)
{
	...
	if (it != result.blockEntryStates.end()) EXPECT_GE(it->second.size(), 1u);
}
...
TEST(PatternLifter, DetectsLambdaInvokeDynamic)
{
	...
	if (!patterns.lambdas.empty())
	{
		...
		EXPECT_EQ("java.util.function.Predicate", patterns.lambdas[0].functionalInterface);
	}
}
...
TEST(PatternLifter, DetectsIteratorForEach)
{
	...
	body.instrs.push_back(makeInvokeInterface("java.util.Iterator", "next", "java.lang.Object"));
	...
	if (!patterns.forEachLoops.empty()) EXPECT_EQ(ForEachKind::Iterator, patterns.forEachLoops[0].kind);
}
