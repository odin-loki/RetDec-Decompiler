...
TEST(CryptoResultTest, AlgorithmNames)
{
	...
	EXPECT_EQ(r.algorithmName(), "AES");
	r.algorithm = CryptoAlgorithm::SHA256;
	EXPECT_EQ(r.algorithmName(), "SHA-256");
	r.algorithm = CryptoAlgorithm::SHA1;
	EXPECT_EQ(r.algorithmName(), "SHA-1");
	r.algorithm = CryptoAlgorithm::ChaCha20;
	EXPECT_EQ(r.algorithmName(), "ChaCha20");
	r.algorithm = CryptoAlgorithm::HMAC;
	EXPECT_EQ(r.algorithmName(), "HMAC");
	r.algorithm = CryptoAlgorithm::RSA;
	EXPECT_EQ(r.algorithmName(), "RSA");
	r.algorithm = CryptoAlgorithm::RC4;
	EXPECT_EQ(r.algorithmName(), "RC4");
	r.algorithm = CryptoAlgorithm::MD5;
	EXPECT_EQ(r.algorithmName(), "MD5");
	r.algorithm = CryptoAlgorithm::CRC;
	EXPECT_EQ(r.algorithmName(), "CRC");
	r.algorithm = CryptoAlgorithm::Blowfish;
	EXPECT_EQ(r.algorithmName(), "Blowfish");
	r.algorithm = CryptoAlgorithm::DES;
	EXPECT_EQ(r.algorithmName(), "DES");
	r.algorithm = CryptoAlgorithm::Salsa20;
	EXPECT_EQ(r.algorithmName(), "Salsa20");
	r.algorithm = CryptoAlgorithm::Poly1305;
	EXPECT_EQ(r.algorithmName(), "Poly1305");
	r.algorithm = CryptoAlgorithm::Unknown;
...
TEST(CryptoResultTest, VariantNames)
{
	...
	EXPECT_EQ(r.variantName(), "AES-128");
	r.variant = CryptoVariant::SHA256_256;
	EXPECT_EQ(r.variantName(), "SHA-256");
	r.variant = CryptoVariant::SHA1_160;
	EXPECT_EQ(r.variantName(), "SHA-1");
	r.variant = CryptoVariant::RSA2048;
	EXPECT_EQ(r.variantName(), "RSA-2048");
	r.variant = CryptoVariant::P256;
	EXPECT_EQ(r.variantName(), "P-256");
	r.variant = CryptoVariant::Unknown;
	EXPECT_EQ(r.variantName(), "");
}
...
TEST(CryptoResultTest, ModeNames)
{
	...
	EXPECT_EQ(r.modeName(), "CBC");
	r.mode = CryptoMode::CTR;
	EXPECT_EQ(r.modeName(), "CTR");
	r.mode = CryptoMode::GCM;
	EXPECT_EQ(r.modeName(), "GCM");
	r.mode = CryptoMode::ECB;
	EXPECT_EQ(r.modeName(), "ECB");
	r.mode = CryptoMode::Unknown;
	EXPECT_EQ(r.modeName(), "");
}
...
TEST(ChaCha20DetectorTest, AllFourRotationConstants)
{
	...
	for (int k = 0; k < 4; ++k)
		addInstr(*fn, blk, IrInstr::Op::Xor);
	for (uint64_t rot: {16u, 12u, 8u, 7u})
		addRotate32(*fn, blk, rot);
	auto r = det.detect(*fn);
...
TEST(Salsa20DetectorTest, AllFourRotationConstants)
{
	...
	for (int k = 0; k < 4; ++k)
		addInstr(*fn, blk, IrInstr::Op::Xor);
	for (uint64_t rot: {7u, 9u, 13u, 18u})
		addRotate32(*fn, blk, rot);
	auto r = det.detect(*fn);
	EXPECT_NEAR(r.confidence, 1.0f, 0.01f);
...
TEST(RSADetectorTest, MontgomeryMultiplyDetected)
{
	...
	for (int k = 0; k < 2; ++k)
		addInstr(*fn, blk1, IrInstr::Op::Mul);
	for (int k = 0; k < 5; ++k)
		addInstr(*fn, blk1, IrInstr::Op::Add);
...
TEST(CryptoDetectorTest, ModuleDetectAggregatesResults)
{
	...
	addSBoxRun(*fn2, blk2);
	for (int k = 0; k < 4; ++k)
		addInstr(*fn2, blk2, IrInstr::Op::Xor);
...
TEST(CryptoDetectorTest, PlainStringHashIsNotAnyCipher)
{
	...
	for (const auto& r: det.detect(*fn))
		EXPECT_TRUE(r.emittedAnnotation.empty()) << "annotated as a cipher: " << r.emittedAnnotation;
}
...
{
	CryptoDetector det;
	auto fn1 = makeEmptyFn("f1");
	addBlock(*fn1);
	auto fn2 = makeEmptyFn("f2");
	addBlock(*fn2);
	det.detect(*fn1);
	det.detect(*fn2);
