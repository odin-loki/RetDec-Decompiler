...
inline std::string printValue(char* v)
{
	return printValue(static_cast<const char*>(v));
}
inline std::string printValue(const std::string& v)
{
	return "\"" + v + "\"";
}
inline std::string printValue(std::nullptr_t)
{
	return "nullptr";
}
...
public:
	...
	Message& operator<<(std::ostream& (*m)(std::ostream&))
	{
		os_ << m;
		return *this;
	}
...
public:
	...
	{
		state().traces.push_back(std::string(file) + ":" + std::to_string(line) + ": " + std::move(label));
	}
	~ScopedTrace()
	{
...
public:
	...
	{
		state().skipped = true;
		state().skipReason = extra.str().empty() ? std::string(file_) + ":" + std::to_string(line_) : extra.str();
	}
...
bool eqCompare(const A& a, const B& b)
{
	...
	if constexpr (
		(std::is_same_v<DA, const char*> || std::is_same_v<DA, char*>)
		&& (std::is_same_v<DB, const char*> || std::is_same_v<DB, char*>))
	{
		if (a == nullptr || b == nullptr) return static_cast<const void*>(a) == static_cast<const void*>(b);
...
{
	if (!eqCompare(a, b)) return Result(true, "");
	return Result(false, std::string("Expected: (") + ta + ") != (" + tb + "), actual: both are " + printValue(a));
}
...
#define RETDEC_GTL_REL_CMP(fnName, op, opText)                                                               \
	...
	{                                                                                                        \
		...
		return Result(                                                                                       \
			false,                                                                                           \
			std::string("Expected: (") + ta + ") " opText " (" + tb + "), actual: " + printValue(a) + " vs " \
				+ printValue(b));                                                                            \
	}
...
inline Result cmpStr(const char* a, const char* b, bool wantEqual, const char* ta, const char* tb)
{
	...
	return Result(
		false,
		std::string("Expected: (") + ta + ") " + (wantEqual ? "==" : "!=") + " (" + tb + "), actual: " + printValue(a)
			+ " vs " + printValue(b));
}
...
Result cmpUlp(F a, F b, const char* ta, const char* tb)
{
	...
	const auto biased = [signBit](Bits x) {
		return (x & signBit) ? static_cast<Bits>(signBit - (x & ~signBit)) : static_cast<Bits>(signBit + x);
	};
...
std::vector<T> ValuesIn(const T (&arr)[N])
{
	return std::vector<T>(arr, arr + N);
}

template <typename It>
auto ValuesIn(It first, It last) -> std::vector<typename std::iterator_traits<It>::value_type>
{
...
#define EXPECT_TRUE(c) \
	GTEST_LITE_ASSERT_(::testing::lite::cmpBool(static_cast<bool>(c), true, #c), GTEST_LITE_NONFATAL_)
#define EXPECT_FALSE(c) \
	GTEST_LITE_ASSERT_(::testing::lite::cmpBool(static_cast<bool>(c), false, #c), GTEST_LITE_NONFATAL_)
...
#define EXPECT_NEAR(a, b, t) \
	GTEST_LITE_ASSERT_(::testing::lite::cmpNear((a), (b), (t), #a, #b, #t), GTEST_LITE_NONFATAL_)
#define ASSERT_NEAR(a, b, t) GTEST_LITE_ASSERT_(::testing::lite::cmpNear((a), (b), (t), #a, #b, #t), GTEST_LITE_FATAL_)

#define EXPECT_DOUBLE_EQ(a, b) \
	GTEST_LITE_ASSERT_(::testing::lite::cmpUlp<double>((a), (b), #a, #b), GTEST_LITE_NONFATAL_)
...
#define GTEST_LITE_NO_THROW_(stmt, on_failure)                                                                      \
	GTEST_LITE_ASSERT_(                                                                                             \
		[&]() -> ::testing::lite::Result {                                                                          \
			...
			{                                                                                                       \
				return ::testing::lite::Result(                                                                     \
					false, std::string("Expected: " #stmt " does not throw.\n  Actual: it throws: ") + e.what());   \
			}                                                                                                       \
			catch (...)                                                                                             \
			{                                                                                                       \
				return ::testing::lite::Result(false, "Expected: " #stmt " does not throw.\n  Actual: it throws."); \
			}                                                                                                       \
...
#define GTEST_LITE_ANY_THROW_(stmt, on_failure)                                                   \
	GTEST_LITE_ASSERT_(                                                                           \
		[&]() -> ::testing::lite::Result {                                                        \
			...
			return ::testing::lite::Result(                                                       \
				false, "Expected: " #stmt " throws an exception.\n  Actual: it throws nothing."); \
		}(),                                                                                      \
		on_failure)
...
#define TEST_P(fixture, name)                                                                                    \
	...
	static const int GTEST_LITE_CONCAT_(gtlParamReg_, __LINE__) = []() {                                         \
		::testing::lite::paramBodies<fixture>().push_back({#name, &GTEST_LITE_CLASS_(fixture, name)::gtlEntry}); \
		return 0;                                                                                                \
	}();                                                                                                         \
	void GTEST_LITE_CLASS_(fixture, name)::gtlBody()
		...
		{                                                                                                              \
			...
			{                                                                                                          \
				...
				::testing::lite::registry().push_back(::testing::lite::TestCase{                                       \
					std::string(#prefix "/" #fixture), body.name + "/" + std::to_string(i), [fn, pp]() { fn(*pp); }}); \
			}                                                                                                          \
		}                                                                                                              \
		return 0;                                                                                                      \
	}()
