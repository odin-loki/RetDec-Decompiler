...
// ── hasFunctionPrologue's bounds ──────────────────────────────────────────

// One `i + 4 <= size` guarded all three prologue patterns, so the three-byte
// `55 8B EC` and two-byte `40 55` forms were never tried in the last three
// bytes of a section. A section that is nothing but a short prologue looked
// like code with no prologue at all, which is one more mismatched section and
