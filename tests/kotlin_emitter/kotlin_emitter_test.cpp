...
static BcClass makeClass(const std::string& name, const std::string& fqName = "")
{
	BcClass cls;
	cls.name = name;
...
static BcAnnotation makeKotlinMetadata(int kind, const std::string& d1 = "", const std::vector<std::string>& d2 = {})
{
	BcAnnotation ann;
...
{
	std::string proto = encodeVarintField(1, 10) + encodeVarintField(3, 7) + encodeVarintField(3, 0);
	ProtobufReader r(proto);
	auto fields = r.readAll();
...
static KotlinClassMetadata makeMetaForClass(
	const std::string& fqName, const KotlinClassFlags& flags = {}, const std::vector<std::string>& stringTable = {})
{
