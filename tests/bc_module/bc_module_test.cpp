...
TEST(BcCFG, AddEdge)
{
	...
	EXPECT_EQ(b0.succs.size(), 1u);
	EXPECT_EQ(b0.succs[0], 1u);
	EXPECT_EQ(b1.preds.size(), 1u);
	EXPECT_EQ(b1.preds[0], 0u);
}
...
TEST(GoldenModule, CSharpHelloWorld)
{
	...
	main.access = BcAccess::Public | BcAccess::Static;
	main.paramNames.push_back("args");
	main.descriptor.params.push_back(std::make_shared<BcType>(Array(ClrString())));
	cls.methods.push_back(std::move(main));
