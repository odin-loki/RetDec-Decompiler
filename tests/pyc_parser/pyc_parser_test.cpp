...
{
	for (const auto& ver: {PythonVersion{3, 8, 0, ""}, PythonVersion{3, 9, 0, ""}, PythonVersion{3, 10, 0, ""}})
	{
...
static void checkOpcodeNames(const PythonVersion& ver, const OpcodeNameRow* rows, size_t n)
{
	for (size_t i = 0; i < n; ++i)
	{
		...
		EXPECT_EQ(std::string(rows[i].name), std::string(info.name))
			<< "opcode " << static_cast<int>(rows[i].opcode) << " in Python " << ver.major << "." << ver.minor;
	}
}
