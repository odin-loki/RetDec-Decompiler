...
// This used to assert that opcode 90 is EXTENDED_ARG below 3.11, which is
// where the defect lived: 90 is STORE_NAME, and it is also HAVE_ARGUMENT,
// which is what the table was really encoding. EXTENDED_ARG is 144 --
// 0x90, not 0x5A -- in every version this parser reads. Treating 90 as a
// prefix swallowed every module-level and class-level assignment.
TEST(OpcodeInfo, Opcode90IsStoreNameNotExtendedArg)
{
	for (const auto& ver: {PythonVersion{3, 8, 0, ""}, PythonVersion{3, 9, 0, ""},
			PythonVersion{3, 10, 0, ""}})
	{
		auto info = opcodeInfo(90, ver);
		EXPECT_EQ(std::string("STORE_NAME"), info.name);
		EXPECT_NE(OpcodeKind::ExtendedArg, info.kind);
		EXPECT_TRUE(info.hasArg) << "90 is HAVE_ARGUMENT itself";
	}
}

TEST(OpcodeInfo, ExtendedArgIs144InEveryVersion)
{
	for (const auto& ver: {PythonVersion{3, 8, 0, ""}, PythonVersion{3, 9, 0, ""},
			PythonVersion{3, 10, 0, ""}, PythonVersion{3, 11, 0, ""},
			PythonVersion{3, 12, 0, ""}})
	{
		auto info = opcodeInfo(144, ver);
		EXPECT_EQ(OpcodeKind::ExtendedArg, info.kind);
		EXPECT_EQ(std::string("EXTENDED_ARG"), info.name);
	}
}
...
// ── The opcode table, pinned against CPython ─────────────────────────────────
//
// A twenty-one-row audit of this table against CPython's own Lib/opcode.py
// found twenty-one entries wrong for 3.8-3.10 -- three separate off-by-one
// runs (59/65/66/67, 141/142/143, 154/155/156), five 3.10 opcodes missing
// altogether, and 3.11/3.12 names sitting on numbers 3.10 uses for something
// else. Every one of them silently mis-decodes real bytecode: BINARY_XOR read
// as BINARY_OR, CALL_FUNCTION_KW as CALL_FUNCTION_EX, SETUP_WITH as
// EXTENDED_ARG.
//
// So the table is pinned entry by entry, from Lib/opcode.py on the 3.8, 3.9,
// 3.10, 3.11 and 3.12 branches. Nothing here is judgement: it is CPython's
// own numbering, and a row that disagrees with it is a decoding bug.
namespace {
struct OpcodeNameRow { uint8_t opcode; const char* name; };

const OpcodeNameRow kCPython38[] = {
	{  1, "POP_TOP"},
	{  2, "ROT_TWO"},
	{  3, "ROT_THREE"},
	{  4, "DUP_TOP"},
...
const OpcodeNameRow kCPython39[] = {
	{  1, "POP_TOP"},
	{  2, "ROT_TWO"},
	{  3, "ROT_THREE"},
	{  4, "DUP_TOP"},
...
const OpcodeNameRow kCPython310[] = {
	{  1, "POP_TOP"},
	{  2, "ROT_TWO"},
	{  3, "ROT_THREE"},
	{  4, "DUP_TOP"},
...
const OpcodeNameRow kCPython311[] = {
	{  0, "CACHE"},
	{  1, "POP_TOP"},
	{  2, "PUSH_NULL"},
	{  9, "NOP"},
...
const OpcodeNameRow kCPython312[] = {
	{  0, "CACHE"},
	{  1, "POP_TOP"},
	{  2, "PUSH_NULL"},
	{  3, "INTERPRETER_EXIT"},
...
static void checkOpcodeNames(const PythonVersion& ver, const OpcodeNameRow* rows, size_t n)
{
	for (size_t i = 0; i < n; ++i)
	{
		auto info = opcodeInfo(rows[i].opcode, ver);
		EXPECT_EQ(std::string(rows[i].name), std::string(info.name))
			<< "opcode " << static_cast<int>(rows[i].opcode) << " in Python "
			<< ver.major << "." << ver.minor;
	}
}

TEST(OpcodeTable, MatchesCPython38)
{
	PythonVersion ver{3, 8, 0, ""};
	checkOpcodeNames(ver, kCPython38, sizeof(kCPython38) / sizeof(kCPython38[0]));
}

TEST(OpcodeTable, MatchesCPython39)
{
	PythonVersion ver{3, 9, 0, ""};
	checkOpcodeNames(ver, kCPython39, sizeof(kCPython39) / sizeof(kCPython39[0]));
}

TEST(OpcodeTable, MatchesCPython310)
{
	PythonVersion ver{3, 10, 0, ""};
	checkOpcodeNames(ver, kCPython310, sizeof(kCPython310) / sizeof(kCPython310[0]));
}

TEST(OpcodeTable, MatchesCPython311)
{
	PythonVersion ver{3, 11, 0, ""};
	checkOpcodeNames(ver, kCPython311, sizeof(kCPython311) / sizeof(kCPython311[0]));
}

TEST(OpcodeTable, MatchesCPython312)
{
