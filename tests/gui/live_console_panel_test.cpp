...
{
	panel->appendChunk(LiveConsolePanel::Stream::Stdout, QByteArrayLiteral("line one\nline two\n"));
	QApplication::processEvents();
...
TEST_F(LiveConsolePanelTest, StderrTextAppearsWithoutPrefix)
{
	...
	// by stream tag.
	panel->appendChunk(LiveConsolePanel::Stream::Stderr, QByteArrayLiteral("boom\n"));
	QApplication::processEvents();
	const QString text = editor()->toPlainText();
...
{
	panel->appendChunk(LiveConsolePanel::Stream::Stdout, QByteArrayLiteral("\x1b[31mred\x1b[0m text\n"));
	QApplication::processEvents();
	const QString text = editor()->toPlainText();
...
TEST_F(LiveConsolePanelTest, AttachedProcessOutputAppearsLive)
{
	...
	proc.setArguments(QStringList{QStringLiteral("/c"), QStringLiteral("echo first& echo second")});
#else
	proc.setProgram(QStringLiteral("/bin/sh"));
	proc.setArguments(QStringList{QStringLiteral("-c"), QStringLiteral("printf 'first\\nsecond\\n'")});
#endif
	...
	EXPECT_TRUE(
		waitUntil([this] { return editor() && editor()->toPlainText().contains(QStringLiteral("second")); }, 5000));
...
TEST_F(LiveConsolePanelTest, PerCallStaysUnderFrameBudgetForRealisticChunks)
{
	...
	EXPECT_LT(worstUs, 16'000) << "16 KiB stdout insert exceeded 16 ms frame budget (worst=" << worstUs << " us)";
}
