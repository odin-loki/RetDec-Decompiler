...
QList<QAction*> allMenuActionsRecursive(QMenuBar* bar)
{
	...
	{
		if (auto* m = topLevel->menu()) out << menuActions(m);
	}
	return out;
}
...
protected:
	...
	QString writeFakeBinary(const QString& name = QStringLiteral("fake.bin"), int sizeBytes = 256)
	{
...
TEST_F(ComprehensiveSmokeTest, RepeatedOpenBinaryCyclesAreLeakFree)
{
	...
	for (int i = 0; i < 50; ++i)
	{
		const QString p = writeFakeBinary(QStringLiteral("cycle_%1.bin").arg(i), 64 + (i & 0xFF));
		win->openBinary(p);
		if ((i % 10) == 0) pump(5);
	}
	pump(50);
...
TEST_F(ComprehensiveSmokeTest, LiveConsoleHandlesPathologicalInputs)
{
	...
	c->appendChunk(panels::LiveConsolePanel::Stream::Stdout, QByteArrayLiteral("naïve €100 \x01\x02\x07 \xff\xfe\n"));
	// Long no-newline line
	c->appendChunk(panels::LiveConsolePanel::Stream::Stdout, QByteArray(8192, 'A'));
	c->appendChunk(panels::LiveConsolePanel::Stream::Stdout, QByteArrayLiteral("\n"));
	pump(50);
...
TEST_F(ComprehensiveSmokeTest, CommandLogScrollbackStaysBoundedUnderFlood)
{
	...
	{
		log.appendRun(QStringLiteral("tool"), {QString::number(i)}, QStringLiteral("/tmp"), 0, i, QStringLiteral("ok"));
	}
	pump(40);
