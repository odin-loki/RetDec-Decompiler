...
public:
	...
	{
		return std::any_of(constraints.begin(), constraints.end(), [k](const TypeConstraint& c) {
			return c.kind == k;
		});
	}
...
INSTANTIATE_TEST_SUITE_P(ItaniumSymbols, ItaniumCorpusTest, ::testing::ValuesIn(kItaniumCorpus));
...
INSTANTIATE_TEST_SUITE_P(MsvcSymbols, MsvcCorpusTest, ::testing::ValuesIn(kMsvcCorpus));
...
INSTANTIATE_TEST_SUITE_P(RustSymbols, RustCorpusTest, ::testing::ValuesIn(kRustCorpus));
