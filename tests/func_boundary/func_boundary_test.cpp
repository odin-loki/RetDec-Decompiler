...
static void writeAt(std::vector<uint8_t>& img, uint64_t va, std::initializer_list<uint8_t> bytes)
{
...
static FuncBoundaryDetector makeDetector(const std::vector<uint8_t>& img, bool addExecSection = true)
{
	FuncBoundaryDetector det(kBase, img.data(), img.size(), true);
	if (addExecSection) det.addExecutableSection(kCode, kBase + img.size());
	return det;
}
...
TEST(EvidenceConfidence, OrderingCorrect)
{
	...
	EXPECT_GT(evidenceConfidence(EvidenceSource::EntryPoint), evidenceConfidence(EvidenceSource::DebugSymbol));
	EXPECT_GT(evidenceConfidence(EvidenceSource::Export), evidenceConfidence(EvidenceSource::PrologueFull));
	EXPECT_GT(evidenceConfidence(EvidenceSource::PrologueFull), evidenceConfidence(EvidenceSource::ProloguePartial));
	EXPECT_GT(evidenceConfidence(EvidenceSource::ProloguePartial), evidenceConfidence(EvidenceSource::Heuristic));
}
...
TEST(ProloguePattern, GCCPatternsPresent)
{
	...
	{
		if (!p.bytes.empty() && p.bytes[0] == 0x55)
		{
			found = true;
			break;
		}
...
static void writeRaw(std::vector<uint8_t>& img, std::size_t off, std::initializer_list<uint8_t> bytes)
{
	for (uint8_t b: bytes)
	{
		if (off < img.size()) img[off++] = b;
...
TEST(SectionMapping, PrologueFoundThroughRawOffset)
{
	...
	FuncBoundaryDetector det(kBase, img.data(), img.size(), true);
	det.addExecutableSection(kTextVa, kTextVa + kTextRawSize, kTextRawOff, kTextRawSize);
	det.runPass2(CompilerHint::GCC);
...
TEST(SectionMapping, VirtualTailWithoutFileBytesIsNotScanned)
{
	...
	FuncBoundaryDetector det(kBase, img.data(), img.size(), true);
	det.addExecutableSection(kTextVa, kTextVa + 0x1000, kTextRawOff, kTextRawSize);
