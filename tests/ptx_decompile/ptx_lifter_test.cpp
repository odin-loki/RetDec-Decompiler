...
static PtxInstr makeInstr(const std::string& mn, PtxType ty, std::vector<PtxOperand> ops, const std::string& mod = "")
{
	PtxInstr i;
	i.mnemonic = mn;
	i.type = ty;
...
{
	PtxOperand o;
	o.kind = PtxOperandKind::Register;
	o.name = "%" + name;
	return o;
}
...
{
	PtxOperand o;
	o.kind = PtxOperandKind::Immediate;
	o.immVal = v;
	return o;
}
...
{
	PtxOperand o;
	o.kind = PtxOperandKind::SpecialReg;
	o.name = name;
	return o;
}
...
{
	InstrLifter lft;
	auto i = makeInstr("selp", PtxType::F32, {regOp("f0"), regOp("f1"), regOp("f2"), regOp("p0")});
	auto s = lft.lift(i, "");
	EXPECT_NE(s.find("?"), std::string::npos);
	EXPECT_NE(s.find(":"), std::string::npos);
}
