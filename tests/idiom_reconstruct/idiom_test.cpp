...
inline u128 operator+(uint64_t a, u128 b)
{
	return u128(a) + b;
}
inline u128 operator-(u128 a, uint64_t b)
...
static IdiomInstr make(IdiomOp op, uint32_t dst, uint32_t s0, int64_t imm, uint32_t width = 32)
{
	IdiomInstr ins;
	ins.op = op;
...
static IdiomInstr makeRR(IdiomOp op, uint32_t dst, uint32_t s0, uint32_t s1, uint32_t width = 32)
{
	IdiomInstr ins;
	ins.op = op;
	ins.dst = IdiomOperand::reg32(dst);
...
static InstrWindow buildUnsignedSimple(uint32_t xReg, uint32_t& qReg, uint64_t M, int k, uint32_t w = 32)
{
	uint32_t t = nextReg();
	qReg = nextReg();
	InstrWindow w2;
...
static InstrWindow buildUnsignedFixup(uint32_t xReg, uint32_t& qReg, uint64_t M, int k, uint32_t w = 32)
{
...
static InstrWindow buildSigned(uint32_t xReg, uint32_t& qReg, int64_t M, int k, bool postAdd, uint32_t w = 32)
{
	uint32_t t0 = nextReg(), t1 = nextReg(), t2 = nextReg(), t3 = nextReg();
	qReg = nextReg();
	InstrWindow r;
...
TEST_P(DivisionUnsignedTest, RecoversDivisorCorrectly)
{
	...
	ASSERT_FALSE(results.empty()) << "No idiom matched for K=" << K << " M=0x" << std::hex << magic.M
								  << " k=" << std::dec << magic.k << " fixup=" << magic.fixup;
	EXPECT_EQ(results[0].kind, ReplacementKind::DivUnsigned) << "K=" << K;
	EXPECT_EQ((uint64_t)results[0].divisor, K)
		<< "Wrong divisor for K=" << K << " got=" << results[0].divisor << " M=" << magic.M << " k=" << magic.k;
}
...
INSTANTIATE_TEST_SUITE_P(
	Unsigned32,
	DivisionUnsignedTest,
	::testing::ValuesIn(unsignedDivisors()),
	[](const ::testing::TestParamInfo<uint64_t>& i) { return "K" + std::to_string(i.param); });
...
TEST_P(DivisionSignedTest, RecoversDivisorCorrectly)
{
	...
	ASSERT_FALSE(results.empty()) << "No idiom matched for K=" << K << " M=" << magic.M << " k=" << magic.k;
	EXPECT_EQ(results[0].kind, ReplacementKind::DivSigned) << "K=" << K;
	EXPECT_EQ(results[0].divisor, K) << "Wrong divisor for K=" << K << " got=" << results[0].divisor;
}

static std::vector<int64_t> signedDivisors()
{
	std::vector<int64_t> v;
	for (int64_t k = 2; k <= 100; ++k)
		v.push_back(k);
	return v;
}
...
INSTANTIATE_TEST_SUITE_P(
	...
	[](const ::testing::TestParamInfo<int64_t>& i) { return "K" + std::to_string(i.param); });
...
TEST(AbsTest, FormA_CDQ)
{
	...
	IdiomInstr cdq;
	cdq.op = IdiomOp::Cdq;
	cdq.dst = IdiomOperand::reg32(edx);
	cdq.src0 = IdiomOperand::reg32(eax);
	InstrWindow w = {
		cdq,
...
TEST(AbsTest, FormD_MOVSX_SAR_XOR_SUB)
{
	...
	IdiomInstr movsx;
	movsx.op = IdiomOp::Movsx;
	movsx.dst = IdiomOperand::reg64(ext);
	movsx.src0 = IdiomOperand::reg32(x);
	InstrWindow w = {
		movsx,
...
TEST(BitTest, LowestSetBit)
{
	...
	IdiomInstr neg;
	neg.op = IdiomOp::Neg;
	neg.dst = IdiomOperand::reg32(t);
	neg.src0 = IdiomOperand::reg32(x);
...
TEST(BitTest, IsZero_NOT_AND1)
{
	...
	IdiomInstr not_;
	not_.op = IdiomOp::Not;
	not_.dst = IdiomOperand::reg32(t);
	not_.src0 = IdiomOperand::reg32(x);
...
TEST(BitTest, ToBool_NEG_SBB_NEG)
{
	...
	neg1.op = IdiomOp::Neg;
	neg1.dst = IdiomOperand::reg32(t);
	neg1.src0 = IdiomOperand::reg32(x);
	sbb.op = IdiomOp::Sub;
	sbb.dst = IdiomOperand::reg32(sb);
	sbb.src0 = IdiomOperand::reg32(sb);
	sbb.src1 = IdiomOperand::reg32(sb);
	neg2.op = IdiomOp::Neg;
	neg2.dst = IdiomOperand::reg32(r_);
	neg2.src0 = IdiomOperand::reg32(sb);
...
TEST(BitTest, ByteSwap16_ROR8)
{
	...
	IdiomInstr ror;
	ror.op = IdiomOp::Ror;
	ror.dst = IdiomOperand::reg32(r_);
	ror.dst.width = 16;
	ror.src0 = IdiomOperand::reg32(x);
	ror.src0.width = 16;
...
TEST(BitTest, ByteSwap32_Software)
{
	...
	IdiomInstr ror;
	ror.op = IdiomOp::Ror;
	ror.dst = IdiomOperand::reg32(t0);
	ror.src0 = IdiomOperand::reg32(x);
	ror.src1 = IdiomOperand::makeImm(16);
	InstrWindow w = {
		ror,
...
{
	IdiomInstr ins;
	ins.op = IdiomOp::VecSet;
	ins.dst = IdiomOperand::vreg(dst, vecW * 8);
	ins.src0 = IdiomOperand::makeImm(val, 32);
	ins.vecWidth = vecW;
	return ins;
}
