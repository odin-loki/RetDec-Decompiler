...
{
	double ms =
		Profiler::instance().time("timed_fn", [] { std::this_thread::sleep_for(std::chrono::microseconds(500)); });
	EXPECT_GE(ms, 0.0);
}
...
TEST_F(ProfilingTest, HistogramBucketsSumToTotalSamples)
{
	FunctionHistogram h;
	for (int i = 0; i < 100; ++i)
		h.add(static_cast<Nanos>(i) * 100'000);
	int64_t sum = 0;
...
TEST_F(ProfilingTest, HistogramPercentile)
{
	FunctionHistogram h;
	for (int i = 1; i <= 100; ++i)
		h.add(static_cast<Nanos>(i) * 1'000'000);
...
TEST_F(ProfilingTest, HistogramBarIsValidUtf8FullBlock)
{
	...
	ASSERT_NE(out.find("\u2588"), std::string::npos) << "no FULL BLOCK in histogram output";
