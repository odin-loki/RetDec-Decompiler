...
def main():
    ...
    skip_verify = sha256hash_ref.upper() in ('SKIP', 'NONE', 'OFF', '0', '')
    if not skip_verify:
        ...
        if sha256hash != sha256hash_ref:
            ...
            print('       computed:', sha256hash)
            print('       expected:', sha256hash_ref)
            cleanup(support_dir)
            sys.exit(1)
    else:
        print('WARNING: skipping SHA-256 verification (RETDEC_SUPPORT_PKG_VERIFY_SHA256=OFF or SKIP);'
              ' use only with trusted support package sources.')
