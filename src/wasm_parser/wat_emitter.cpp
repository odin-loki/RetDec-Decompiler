...
std::string WatEmitter::limitsStr(const Limits& lim) const
{
	...
	if (lim.max.has_value()) s += " " + std::to_string(*lim.max);
...
std::string WatEmitter::dataStr(const std::vector<uint8_t>& bytes) const
{
	...
	for (uint8_t b: bytes)
	{
		...
		{
			ss << '\\' << std::hex << std::setw(2) << std::setfill('0') << (unsigned)b << std::dec;
		}
	}
	ss << '"';
	return ss.str();
...
std::string WatEmitter::constExprStr(const WasmModule& /*mod*/, const std::vector<uint8_t>& expr) const
{
...
std::string WatEmitter::localId(const WasmModule& mod, uint32_t funcIdx, uint32_t localIdx) const
{
	if (!opts_.useNames || !mod.names.has_value()) return std::to_string(localIdx);
...
std::string WatEmitter::funcTypeSig(const FuncType& ft) const
{
	...
	for (auto p: ft.params)
		s += " (param " + valTypeName(p) + ")";
	for (auto r: ft.results)
		s += " (result " + valTypeName(r) + ")";
	return s;
}

std::string WatEmitter::funcTypeRef(const WasmModule& mod, uint32_t typeIdx) const
{
...
std::string WatEmitter::exportAnnotations(const WasmModule& mod, ExternKind kind, uint32_t idx) const
{
	if (!opts_.inlineExports) return "";
	std::string s;
	for (const auto& exp: mod.exports)
	{
		if (exp.kind == kind && exp.index == idx) s += " (export " + dataStr({exp.name.begin(), exp.name.end()}) + ")";
	}
	return s;
}
...
void WatEmitter::emitTypeSection(const WasmModule& mod, std::ostream& out, int indent) const
{
	if (!opts_.foldTypes) return;
	for (size_t i = 0; i < mod.types.size(); ++i)
	{
		out << indentStr(indent) << "(type (;" << i << ";) (func" << funcTypeSig(mod.types[i]) << "))\n";
	}
}

void WatEmitter::emitImports(const WasmModule& mod, std::ostream& out, int indent) const
{
	if (!opts_.inlineImports)
	{
		...
		for (const auto& imp: mod.imports)
		{
			...
			switch (imp.kind)
			{
			case ExternKind::Func: out << " (func " << funcTypeRef(mod, imp.index) << ")"; break;
			case ExternKind::Table:
				if (imp.tableType)
					out << " (table " << limitsStr(imp.tableType->limits) << " " << valTypeName(imp.tableType->refType)
						<< ")";
				break;
			case ExternKind::Memory:
				if (imp.memType) out << " (memory " << limitsStr(imp.memType->limits) << ")";
				break;
			case ExternKind::Global:
				if (imp.globalType)
					out << " (global " << (imp.globalType->isMutable ? "(mut " : "")
						<< valTypeName(imp.globalType->valType) << (imp.globalType->isMutable ? ")" : "") << ")";
				break;
			}
			out << ")\n";
		}
...
void WatEmitter::emitFunctions(const WasmModule& mod, std::ostream& out, int indent) const
{
	...
	if (opts_.inlineImports)
	{
		for (const auto& imp: mod.imports)
		{
			...
			out << indentStr(indent) << "(func " << funcId(mod, impIdx) << importAnnotation(imp)
				<< exportAnnotations(mod, ExternKind::Func, impIdx) << " " << funcTypeRef(mod, imp.index) << ")\n";
			++impIdx;
		}
	}
	...
	for (size_t ci = 0; ci < mod.codes.size(); ++ci)
	{
		...
		uint32_t typeIdx = funcIdx < mod.totalFuncCount() ? mod.funcTypeIndex(funcIdx) : 0;
		const FuncCode& fc = mod.codes[ci];

		out << indentStr(indent) << "(func " << funcId(mod, funcIdx)
			<< exportAnnotations(mod, ExternKind::Func, funcIdx) << " " << funcTypeRef(mod, typeIdx);
		...
		if (typeIdx < mod.types.size())
		{
			...
			for (uint32_t pi = 0; pi < paramCount; ++pi)
			{
				...
				out << "\n" << indentStr(indent + 1) << "(param " << lid << " " << valTypeName(ft.params[pi]) << ")";
			}
...
void WatEmitter::emitTables(const WasmModule& mod, std::ostream& out, int indent) const
{
	...
	if (opts_.inlineImports)
	{
		...
		for (const auto& imp: mod.imports)
		{
			...
			out << indentStr(indent) << "(table " << tableId(mod, tIdx) << importAnnotation(imp)
				<< exportAnnotations(mod, ExternKind::Table, tIdx);
			if (imp.tableType)
				out << " " << limitsStr(imp.tableType->limits) << " " << valTypeName(imp.tableType->refType);
			out << ")\n";
			++tIdx;
		}
	}
	...
	for (size_t i = 0; i < mod.tables.size(); ++i)
	{
		...
		out << indentStr(indent) << "(table " << tableId(mod, tIdx) << exportAnnotations(mod, ExternKind::Table, tIdx)
			<< " " << limitsStr(tt.limits) << " " << valTypeName(tt.refType) << ")\n";
	}
}

void WatEmitter::emitMemories(const WasmModule& mod, std::ostream& out, int indent) const
{
	if (opts_.inlineImports)
	{
		uint32_t mIdx = 0;
		for (const auto& imp: mod.imports)
		{
			...
			out << indentStr(indent) << "(memory " << memId(mod, mIdx) << importAnnotation(imp)
				<< exportAnnotations(mod, ExternKind::Memory, mIdx);
			if (imp.memType) out << " " << limitsStr(imp.memType->limits);
			out << ")\n";
			++mIdx;
		}
	}
	...
	for (size_t i = 0; i < mod.memories.size(); ++i)
	{
		...
		out << indentStr(indent) << "(memory " << memId(mod, mIdx) << exportAnnotations(mod, ExternKind::Memory, mIdx)
			<< " " << limitsStr(mod.memories[i].limits) << ")\n";
	}
}

void WatEmitter::emitGlobals(const WasmModule& mod, std::ostream& out, int indent) const
{
	if (opts_.inlineImports)
	{
		uint32_t gIdx = 0;
		for (const auto& imp: mod.imports)
		{
			...
			out << indentStr(indent) << "(global " << globalId(mod, gIdx) << importAnnotation(imp)
				<< exportAnnotations(mod, ExternKind::Global, gIdx);
			if (imp.globalType)
			{
				const auto& gt = *imp.globalType;
				if (gt.isMutable)
					out << " (mut " << valTypeName(gt.valType) << ")";
				else
					out << " " << valTypeName(gt.valType);
			}
			out << ")\n";
			++gIdx;
		}
...
void WatEmitter::emitExports(const WasmModule& mod, std::ostream& out, int indent) const
{
	...
	for (const auto& exp: mod.exports)
	{
		...
		out << indentStr(indent) << "(export " << dataStr({exp.name.begin(), exp.name.end()}) << " (" << kindStr << " "
			<< exp.index << "))\n";
	}
}

void WatEmitter::emitStart(const WasmModule& mod, std::ostream& out, int indent) const
{
	if (mod.startFunc.has_value()) out << indentStr(indent) << "(start " << funcId(mod, *mod.startFunc) << ")\n";
}

void WatEmitter::emitElements(const WasmModule& mod, std::ostream& out, int indent) const
{
	for (size_t i = 0; i < mod.elements.size(); ++i)
	{
		...
		if (!seg.isPassive && !seg.isDeclarative)
		{
			...
			if (!seg.offsetExpr.empty()) out << " (" << constExprStr(mod, seg.offsetExpr) << ")";
		}
...
void WatEmitter::emitDataSegments(const WasmModule& mod, std::ostream& out, int indent) const
{
...
std::string WatEmitter::decodeInstr(DisState& st, const std::vector<uint8_t>& code, size_t& pc, int indent) const
{
	if (pc >= code.size()) return "";
	uint8_t op = code[pc++];
	...
	case 0x43: {
		float v = readVecF32(code, pc);
		line << "f32.const " << v;
		break;
	}
	case 0x44: {
		double v = readVecF64(code, pc);
		line << "f64.const " << v;
		break;
	}
	...
	case 0xFC: {
		...
		case 8: {
			uint32_t si = readVecULEB(code, pc);
		...
		case 12: {
			uint32_t ei = readVecULEB(code, pc);
			readVecULEB(code, pc);
			line << "table.init " << ei;
			break;
		}
	...
	case 0xFD: {
		...
		case 0:
			line.str("");
			line << indentStr(indent) << "v128.load" << memArg(code, pc);
			break;
		case 11:
			line.str("");
			line << indentStr(indent) << "v128.store" << memArg(code, pc);
			break;
		case 12: {
			// v128.const (16 bytes)
			if (pc + 16 <= code.size())
			{
				line.str("");
				line << indentStr(indent) << "v128.const i8x16";
	...
	default: line << ";; unknown_op 0x" << std::hex << (int)op << std::dec; break;
	}
...
void WatEmitter::emitModule(const WasmModule& mod, std::ostream& out) const
{
	...
	if (opts_.inlineImports)
		emitFunctions(mod, out, indent);
	else
	{
		emitImports(mod, out, indent);
		emitFunctions(mod, out, indent);
	}
