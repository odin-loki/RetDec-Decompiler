...
/// Whether the decompiled output is a source listing, as opposed to one of the
/// machine-readable formats.
///
/// `-f json` and `-f json-human` route llvmir2hll through a JsonOutputManager,
/// so the output file holds JSON. Nothing here consulted the format: the
/// semantic comments were inserted as `// ...` lines into that JSON, and the
/// buildable sidecars were generated by parsing it as C. A machine-readable
/// mode that emits invalid JSON whenever a detection fires is worse than one
/// that emits no comments.
bool outputIsSourceListing(const config::Config& config)
{
	const std::string& format = config.parameters.getOutputFormat();
	return format != "json" && format != "json-human";
}

/// Every function defined in @a lines, mapped to its 1-based line. Defined
/// below, after the source-scanning helpers it shares with the sidecar
/// generator are declared.
std::map<std::string, std::size_t> definitionLines(const std::vector<std::string>& lines);

/// Inserts a comment above each detected function's definition.
/// @return whether anything was inserted.
bool injectSemanticCommentsIntoLines(std::vector<std::string>& lines, const config::Config& config)
{
	const std::string& outputLang = config.parameters.getOutputLang();

	// Where each function is in THIS output, not where it was in a source file
	// the binary may never have carried. See definitionLines.
	const auto defined = definitionLines(lines);

	std::map<int, std::vector<std::string>> inserts;
	for (const auto& fn: config.functions)
	{
		if (fn.semanticDetections.empty())
		{
			continue;
		}

		// getRealName() as well, because that is the identifier the emitter
		// writes when a function was renamed from its config name.
		auto at = defined.find(fn.getName());
		if (at == defined.end())
		{
			at = defined.find(fn.getRealName());
		}
		if (at == defined.end())
		{
			continue;
		}
		const int line = static_cast<int>(at->second);
		if (line <= 0)
		{
			continue;
		}
...
/// Every function DEFINED in @a lines, mapped to its 1-based line.
///
/// The comments used to be placed at common::Function::getStartLine(), which is
/// the line DWARF or PDB recorded in the ORIGINAL source, used as an index into
/// the decompiler's own output. Two unrelated coordinate systems:
///
///   * on a stripped binary -- the normal input -- the debug line is undefined,
///     every function was skipped, and the feature emitted nothing at all;
///   * where debug info exists the comment landed at whatever line of the
///     emitted C happened to share a number with the original source line.
///
/// The name is in the output, because the emitter takes it from the same config
/// these detections are attached to. Finding it is what the placement should
/// have been doing.
///
/// The declarator tests are the ones collectCallAndDefinedNames uses, so "this
/// is a definition" means the same thing here as it does to the sidecar
/// generator rather than being a second, differently-wrong parser. One pass for
/// the whole output rather than one per detected function: the latter is
/// O(functions x output size), and both are numbers a large binary chooses.
std::map<std::string, std::size_t> definitionLines(const std::vector<std::string>& lines)
{
	std::map<std::string, std::size_t> found;

	// One buffer, so the scanner sees the surrounding source that
	// looksLikeDeclarator() needs, plus where each line began in it.
	std::string src;
	std::vector<std::size_t> lineStarts;
	lineStarts.reserve(lines.size());
	for (const auto& line: lines)
	{
		lineStarts.push_back(src.size());
		src += line;
		src += '\n';
	}
...
{
	// Source comments belong in a source listing. See outputIsSourceListing.
	if (!outputIsSourceListing(config))
	{
		return;
	}
	...
	// std::getline drops the separator, so whether the last line had one is not
	// recoverable from `lines` and has to be noted while reading.
	...
	else if (outString && !outString->empty())
	{
		...
		trailingNewline = outString->back() == '\n';
	}
	...
	if (!injectSemanticCommentsIntoLines(lines, config))
	{
		// Nothing to say about this output. Rewriting it anyway is not free:
		// the loops below join with '\n' BETWEEN lines and none after, so a
		// file that ends in a newline -- every file the emitter writes -- came
		// back one byte shorter for having been read.
		return;
	}

	// Preserved for the same reason: this function adds comments, it does not
	// decide how the file ends.
	const bool endedWithNewline = trailingNewline;
	...
	if (!outPath.empty())
	{
		...
		{
			out << lines[i];
			if (i + 1 < lines.size() || endedWithNewline)
			{
				out << '\n';
			}
		}
...
{
	// The sidecars are C, parsed out of the output file. With -f json that file
	// is JSON and every name in it is a string, so the generator emitted a
	// header and a stub set derived from the decompiler's own JSON keys. See
	// outputIsSourceListing.
	if (!outputIsSourceListing(config))
	{
		return;
	}
	maybeWriteBuildableSidecars(config.parameters.getOutputFile(), cSource);
}
