		...
		eOpConv ct)
{
	...
	if (r != pr)
	{
		// A narrow view of a wider register. The FP views are typed float and
		// double, so the truncation has to happen on an integer of the right
		// width first -- there is no `trunc i128 to double`.
		unsigned bits = getRegisterBitSize(r);
		if (bits && bits < llvmReg->getValueType()->getPrimitiveSizeInBits())
		{
			llvm::Value* asInt =
				llvmReg->getValueType()->isIntegerTy()
					? ret
					: irb.CreateBitCast(ret, irb.getIntNTy(llvmReg->getValueType()->getPrimitiveSizeInBits()));
			ret = irb.CreateTrunc(asInt, irb.getIntNTy(bits));
		}
		if (ret->getType() != rt)
		{
			ret = rt->isIntegerTy() ? irb.CreateZExtOrTrunc(ret, rt) : irb.CreateBitCast(ret, rt);
		}
	}
 ...
 * translators that can reach them need the test without the action.
 */
bool Capstone2LlvmIrTranslatorArm64_impl::hasVectorOperand(cs_arm64* ai) const
{
	for (unsigned j = 0; j < ai->op_count; ++j)
	{
		if (isVectorRegister(ai->operands[j]))
		{
			return true;
		}
	}
...
/**
 * A NEON register read as a vector of \p lanes lanes of \p laneBits bits.
 */
llvm::Value* Capstone2LlvmIrTranslatorArm64_impl::loadNeonVector(
	uint32_t reg, unsigned laneBits, unsigned lanes, llvm::IRBuilder<>& irb)
{
	auto* w = irb.getIntNTy(laneBits * lanes);
	auto* vecTy = llvm::FixedVectorType::get(irb.getIntNTy(laneBits), lanes);

	return irb.CreateBitCast(irb.CreateZExtOrTrunc(loadRegister(reg, irb), w), vecTy);
}

/**
 * ARM64_INS_ADD, ARM64_INS_SUB, ARM64_INS_MUL,
 * ARM64_INS_UMAX, ARM64_INS_UMIN, ARM64_INS_SMAX, ARM64_INS_SMIN,
 ...
 *
 * ADD, SUB and MUL were not nullptr entries: they had translators, and those
 * translators opened with
 *
 *     if (ifVectorGeneratePseudo(i, ai, irb)) return;
 *
 * so a vector operand meant "give up" and `add v0.4s, v1.4s, v2.4s` came out
 * as __asm_add -- 588 occurrences in the static parity corpus, and COV-01
 * scores every one of them covered because the dispatch table has a function
 * pointer for ADD. That guard was right when it was written: the alternative
 * at the time was CreateAdd on a 128-bit register, which is not a lane-wise
 * add of anything and dumped core on `.4s` operands. It is the wrong answer
 * now that the lane model exists.
 *
 * The three families differ only in how the lanes are put together:
 *
 *   lane-wise    the operation between corresponding lanes
 *   pairwise     the operation between ADJACENT lanes of vn followed by vm,
 *                which is the concatenation's even lanes against its odd ones
 *   permute      no operation at all, only a choice of lanes
 *
 * so all three are one shufflevector away from each other and are written as
 * one function. Getting a permute's mask backwards produces a vector with the
 * right lanes in the wrong order, which is why each permute has its own test
 * with lanes that are all different.
 */
void Capstone2LlvmIrTranslatorArm64_impl::translateNeonLaneBinary(cs_insn* i, cs_arm64* ai, llvm::IRBuilder<>& irb)
{
	unsigned bytes = 0;
	...
	// The permutes choose lanes out of the concatenation of the two sources
	// and do no arithmetic. The masks are over 0..2*lanes-1, where
	// lanes..2*lanes-1 is vm. Getting one backwards produces a vector with
	// the right lanes in the wrong order, so each has its own test with
	// lanes that are all different.
	...
	for (unsigned k = 0; k < lanes; ++k)
	{
		switch (i->id)
		{
		// vd[k] = concat[2k]           the concatenation's even lanes
		case ARM64_INS_UZP1: mask.push_back(2 * k); break;
		// vd[k] = concat[2k+1]         its odd ones
		case ARM64_INS_UZP2: mask.push_back(2 * k + 1); break;
		// vd[2k] = vn[k], vd[2k+1] = vm[k]        the lower halves
		case ARM64_INS_ZIP1: mask.push_back((k % 2) ? int(lanes + k / 2) : int(k / 2)); break;
		// the upper halves
		case ARM64_INS_ZIP2: mask.push_back((k % 2) ? int(lanes + half + k / 2) : int(half + k / 2)); break;
		// vd[2k] = vn[2k], vd[2k+1] = vm[2k]      the even lanes of each
		case ARM64_INS_TRN1: mask.push_back((k % 2) ? int(lanes + k - 1) : int(k)); break;
		// vd[2k] = vn[2k+1], vd[2k+1] = vm[2k+1]  the odd lanes of each
		case ARM64_INS_TRN2: mask.push_back((k % 2) ? int(lanes + k) : int(k + 1)); break;
		default: isPermute = false; break;
		}
		if (!isPermute)
		{
			break;
	...
	if (isPermute)
	{
		storeRegister(
			ai->operands[0].reg,
			irb.CreateBitCast(irb.CreateShuffleVector(a, b, mask), w),
			irb,
			eOpConv::ZEXT_TRUNC_OR_BITCAST);
		return;
	}

	// The pairwise forms operate on adjacent lanes of [vn, vm] rather than on
	// corresponding lanes of vn and vm. Splitting the concatenation into its
	// even and odd lanes turns them back into the lane-wise case.
	bool pairwise = i->id == ARM64_INS_ADDP || i->id == ARM64_INS_UMAXP || i->id == ARM64_INS_UMINP
				 || i->id == ARM64_INS_SMAXP || i->id == ARM64_INS_SMINP;
	if (pairwise)
	{
		...
		llvm::Value* ab0 = irb.CreateShuffleVector(a, b, evens);
		llvm::Value* ab1 = irb.CreateShuffleVector(a, b, odds);
		a = ab0;
		b = ab1;
	}
	...
	{
	case ARM64_INS_ADD:
	case ARM64_INS_ADDP: res = irb.CreateAdd(a, b); break;
	case ARM64_INS_SUB: res = irb.CreateSub(a, b); break;
	case ARM64_INS_MUL: res = irb.CreateMul(a, b); break;
	case ARM64_INS_UMAX:
	case ARM64_INS_UMAXP: res = irb.CreateSelect(irb.CreateICmpUGT(a, b), a, b); break;
	case ARM64_INS_UMIN:
	case ARM64_INS_UMINP: res = irb.CreateSelect(irb.CreateICmpULT(a, b), a, b); break;
	case ARM64_INS_SMAX:
 ...
 *
 * The right shifts take an amount of 1..laneBits, and laneBits is a legal
 * encoding -- `ushr v0.16b, v1.16b, #8` is how you zero a register lane by
 * lane, and `sshr` by 8 broadcasts each byte's sign. A shift equal to the
...
{
	unsigned bytes = 0;
	if (ai->op_count != 3 || ai->operands[2].type != ARM64_OP_IMM || !neonSameWidthRegs(ai, 2, bytes))
	{
		translatePseudoAsmGeneric(i, ai, irb);
		return;
	}
	...
	llvm::Value* res = nullptr;
	if (i->id == ARM64_INS_USHR && amount == laneBits)
	{
		res = llvm::Constant::getNullValue(vecTy);
	}
	else
	{
		uint64_t n = (i->id == ARM64_INS_SSHR && amount == laneBits) ? laneBits - 1 : amount;
		auto* splat = llvm::ConstantVector::getSplat(
			llvm::ElementCount::getFixed(lanes), llvm::ConstantInt::get(irb.getIntNTy(laneBits), n));

		switch (i->id)
		{
		case ARM64_INS_SHL: res = irb.CreateShl(a, splat); break;
		case ARM64_INS_USHR: res = irb.CreateLShr(a, splat); break;
		case ARM64_INS_SSHR: res = irb.CreateAShr(a, splat); break;
		default: translatePseudoAsmGeneric(i, ai, irb); return;
		}
	}
...
/**
 * ARM64_INS_UMOV, ARM64_INS_SMOV
 *
 * One lane out of a NEON register into a general-purpose one: `umov w0,
 * v1.b[3]` zero-extends, `smov x0, v1.h[2]` sign-extends. It is how the result
 * of a lane compare gets back into scalar code, so it sits at the end of every
 * NEON string routine.
 *
 * The lane width comes from the operand's arrangement -- Capstone reports
 * ARM64_VAS_1B for `.b[3]`, a one-lane arrangement, with vector_index 3 -- so
 * it is read from vasLanes() of that arrangement rather than from the total
 * register width, and the index is bounds-checked against the register rather
 * than against the arrangement.
 */
void Capstone2LlvmIrTranslatorArm64_impl::translateNeonLaneMove(cs_insn* i, cs_arm64* ai, llvm::IRBuilder<>& irb)
{
	EXPECT_IS_BINARY(i, ai, irb);
	...
	// Capstone reports the SINGLE-lane arrangements here -- ARM64_VAS_1B for
	// `.b[3]` -- which vasLanes() does not map, and widening vasLanes() would
	// also widen neonSameWidthRegs(), which depends on it. An explicit switch
	// instead.
	unsigned laneBits = 0;
	switch (src.vas)
	{
	case ARM64_VAS_1B: laneBits = 8; break;
	case ARM64_VAS_1H: laneBits = 16; break;
	case ARM64_VAS_1S: laneBits = 32; break;
	case ARM64_VAS_1D: laneBits = 64; break;
	default: break;
	}
	if (laneBits == 0)
	{
	...
	if (static_cast<unsigned>(src.vector_index) >= lanes)
	{
		translatePseudoAsmGeneric(i, ai, irb);
		return;
	}

	llvm::Value* v = loadNeonVector(src.reg, laneBits, lanes, irb);
	llvm::Value* lane = irb.CreateExtractElement(v, llvm::ConstantInt::get(irb.getInt32Ty(), src.vector_index));

	storeOp(
		ai->operands[0],
		lane,
		irb,
		i->id == ARM64_INS_SMOV ? eOpConv::SEXT_TRUNC_OR_BITCAST : eOpConv::ZEXT_TRUNC_OR_BITCAST);
}
		...
		eOpConv ct)
{
	...
	// Writing a narrow view ZEROES the rest of the register -- the rule for
	// every ARM64 sub-register, unlike x86 where an 8- or 16-bit write merges.
	// The value is converted to the VIEW's own type, using whatever the caller
	// asked for, and only then widened as bits: converting straight to the
	// parent's type would turn `ucvtf d0, w1` into a bitcast of w1's bits.
	bool narrowView = r != pr;
	auto* targetTy = narrowView ? getRegisterType(r) : llvmReg->getValueType();
	...
	if (targetTy->isFloatingPointTy())
	{
		switch (ct)
		{
			...
			case eOpConv::UITOFP_OR_FPCAST: val = generateTypeConversion(irb, val, targetTy, ct); break;
			default: val = generateTypeConversion(irb, val, targetTy, eOpConv::FPCAST_OR_BITCAST);
		}
	}
	else
	{
		switch (ct)
		{
			...
			case eOpConv::ZEXT_TRUNC_OR_BITCAST: val = generateTypeConversion(irb, val, targetTy, ct); break;
			default: val = generateTypeConversion(irb, val, targetTy, eOpConv::SEXT_TRUNC_OR_BITCAST);
			}
	}

	if (narrowView)
	{
		unsigned viewBits = getRegisterBitSize(r);
		unsigned wholeBits = llvmReg->getValueType()->getPrimitiveSizeInBits();
		if (!val->getType()->isIntegerTy())
		{
			val = irb.CreateBitCast(val, irb.getIntNTy(viewBits));
		}
		val = irb.CreateZExt(val, irb.getIntNTy(wholeBits));
		if (!llvmReg->getValueType()->isIntegerTy())
		{
			val = irb.CreateBitCast(val, llvmReg->getValueType());
		}
	}
...
void Capstone2LlvmIrTranslatorArm64_impl::translateAdd(cs_insn* i, cs_arm64* ai, llvm::IRBuilder<>& irb)
{
	...
	// arithmetic.
	// A vector operand used to mean "give up" and emit __asm_add. The
	// lane-wise form is modelled now; anything translateNeonLaneBinary does
	// not recognise still falls through to the pseudo-assembly path inside
	// it.
	if (hasVectorOperand(ai))
	{
		translateNeonLaneBinary(i, ai, irb);
		return;
	}
...
/**
 * ARM64_INS_MOVI, ARM64_INS_MVNI -- the Advanced SIMD modified immediate.
 *
 * 1,599 sites in the static corpus and every one of them fell back, because
 * translateMovi() handed any operand with a vector arrangement to
 * pseudo-assembly. `movi v0.4s, #0` is how a compiler zeroes a vector, so
 * this was the third most common unmodelled instruction on the architecture.
 *
 * Capstone reports the RAW imm8 plus the shift, not the element value, except
 * for the `.2d` form where it reports the already-expanded 64 bits.
 *
 *   movi v0.4s, #0x33, lsl #8   ->  I(0x33) {shift LSL 8}
 *   movi v0.2d, #0xff00ff00ff   ->  I(0xff00ff00ff) {no shift}
 *
 * MSL -- "shift ones" -- fills the vacated bits with ones rather than zeroes,
 * which is the one shift type an ordinary left shift gets wrong.
 */
void Capstone2LlvmIrTranslatorArm64_impl::translateNeonMovi(cs_insn* i, cs_arm64* ai, llvm::IRBuilder<>& irb)
{
	unsigned laneBits = 0, lanes = 0;
	if (ai->op_count != 2 || ai->operands[1].type != ARM64_OP_IMM || !neonArrangement(ai->operands[0], laneBits, lanes))
	{
		translateMovi(i, ai, irb);
		return;
	}

	uint64_t imm = static_cast<uint64_t>(ai->operands[1].imm);
	unsigned shift = ai->operands[1].shift.value;

	// The .2d form is already the whole element; every other arrangement
	// takes an 8-bit immediate and a shift.
	if (laneBits != 64)
	{
		if (ai->operands[1].shift.type == ARM64_SFT_LSL)
		{
			imm <<= shift;
		}
		else if (ai->operands[1].shift.type == ARM64_SFT_MSL)
		{
			imm = (imm << shift) | ((1ULL << shift) - 1);
		}
		else if (ai->operands[1].shift.type != ARM64_SFT_INVALID)
		{
			translateMovi(i, ai, irb);
			return;
		}
	...
	// Replicate across the arrangement.
	llvm::APInt whole(laneBits * lanes, 0);
	for (unsigned k = 0; k < lanes; ++k)
	{
		whole |= llvm::APInt(laneBits * lanes, imm).shl(k * laneBits);
	}

	auto* w = irb.getIntNTy(laneBits * lanes);
	storeRegister(ai->operands[0].reg, llvm::ConstantInt::get(w, whole), irb, eOpConv::ZEXT_TRUNC_OR_BITCAST);
}

/**
 * ARM64_INS_REV16, ARM64_INS_REV32 on general-purpose registers.
 *
 * REV16 is wired to the NEON lane translator, which declines anything with no
 * vector operand -- so `rev16 w2, w2`, the scalar form, fell back 338 times.
 * It reverses the bytes WITHIN each halfword, which is not the same as
 * reversing the register: `0x11223344` becomes `0x22114433`, not `0x44332211`.
 */
void Capstone2LlvmIrTranslatorArm64_impl::translateScalarRev(cs_insn* i, cs_arm64* ai, llvm::IRBuilder<>& irb)
{
	if (ai->op_count != 2 || hasVectorOperand(ai))
	{
		translateNeonLaneUnary(i, ai, irb);
		return;
	}

	unsigned regBits = getRegisterBitSize(ai->operands[0].reg);
	unsigned groupBits = i->id == ARM64_INS_REV16 ? 16 : 32;
	if (regBits == 0 || groupBits > regBits)
	{
		translatePseudoAsmGeneric(i, ai, irb);
		return;
	}

	auto* ty = irb.getIntNTy(regBits);
	llvm::Value* x = irb.CreateZExtOrTrunc(loadOp(ai->operands[1], irb), ty);

	// Reverse the bytes within each group by swapping blocks of 1 byte, then
	// 2, and so on up to half the group. Written as masked shifts rather
	// than a vector llvm.bswap because the emulator reads that intrinsic's
	// operand as a plain integer and asserts on a vector type -- a wrong IR
	// shape that shows up as a crash rather than a wrong answer.
	for (unsigned blockBytes = 1; blockBytes * 16 <= groupBits; blockBytes *= 2)
	{
		unsigned blockBits = blockBytes * 8;
		llvm::APInt mask(regBits, 0);
		for (unsigned bit = 0; bit < regBits; bit += 2 * blockBits)
		{
			mask |= llvm::APInt::getLowBitsSet(regBits, blockBits).shl(bit);
		}
		auto* m = llvm::ConstantInt::get(ty, mask);
		auto* sh = llvm::ConstantInt::get(ty, blockBits);
		x = irb.CreateOr(irb.CreateShl(irb.CreateAnd(x, m), sh), irb.CreateAnd(irb.CreateLShr(x, sh), m));
	}

	storeOp(ai->operands[0], x, irb, eOpConv::ZEXT_TRUNC_OR_BITCAST);
}

/**
 * ARM64_INS_FMOV between a general-purpose register and a vector LANE.
 *
 * `fmov x3, v0.d[1]` moves the high half of a vector register, and is the one
 * FMOV form whose source is not simply the register the operand names.
 * translateFMov() declined anything touching a vector register, so this fell
 * back.
 */
void Capstone2LlvmIrTranslatorArm64_impl::translateFMovLane(cs_insn* i, cs_arm64* ai, llvm::IRBuilder<>& irb)
{
	if (ai->op_count != 2)
	{
		translateFMov(i, ai, irb);
		return;
	}

	auto& dst = ai->operands[0];
	auto& src = ai->operands[1];
	bool fromLane = isVectorRegister(src) && src.vector_index >= 0;
	bool toLane = isVectorRegister(dst) && dst.vector_index >= 0;
	if (!fromLane && !toLane)
	{
		translateFMov(i, ai, irb);
		return;
	...
	// The lane form only ever moves a 64-bit half.
	auto* i64 = irb.getInt64Ty();
	auto* vecTy = llvm::FixedVectorType::get(i64, 2);

	if (fromLane)
	{
		llvm::Value* v =
			irb.CreateBitCast(irb.CreateZExtOrTrunc(loadRegister(src.reg, irb), irb.getIntNTy(128)), vecTy);
		llvm::Value* lane = irb.CreateExtractElement(v, irb.getInt32(src.vector_index));
		storeOp(dst, lane, irb, eOpConv::ZEXT_TRUNC_OR_BITCAST);
		return;
	}

	llvm::Value* v = irb.CreateBitCast(irb.CreateZExtOrTrunc(loadRegister(dst.reg, irb), irb.getIntNTy(128)), vecTy);
	llvm::Value* val = irb.CreateZExtOrTrunc(loadOp(src, irb), i64);
	v = irb.CreateInsertElement(v, val, irb.getInt32(dst.vector_index));
	storeRegister(dst.reg, irb.CreateBitCast(v, irb.getIntNTy(128)), irb, eOpConv::ZEXT_TRUNC_OR_BITCAST);
}
...
{
	unsigned laneBits = 0, lanes = 0;
	if (ai->op_count != 2 || !neonArrangement(ai->operands[1], laneBits, lanes) || lanes < 2)
	{
		translatePseudoAsmGeneric(i, ai, irb);
		return;
	}
	...
	for (unsigned k = 1; k < lanes; ++k)
	{
		...
		switch (i->id)
		{
		case ARM64_INS_ADDV: acc = irb.CreateAdd(acc, e); break;
		case ARM64_INS_SMAXV: acc = irb.CreateSelect(irb.CreateICmpSGT(acc, e), acc, e); break;
	...
	// The scalar destination is written at its own width and the bits above
	// it cleared, the same rule every NEON scalar write follows.
