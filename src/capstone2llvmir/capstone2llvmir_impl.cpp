		...
		llvm::IRBuilder<>& irb)
{
	...
	if (disasmRes)
	{
		...
		// Stage 7: Optional semantic validation (override validateTranslationSemantics)
		(void)validateTranslationSemantics(res);
...
template <typename CInsn, typename CInsnOp>
bool Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::validateTranslationSemantics(
		const TranslationResultOne& res) const
{
	if (!res.llvmInsn)
		return true;
	return llvm::isa<llvm::StoreInst>(res.llvmInsn);
}
