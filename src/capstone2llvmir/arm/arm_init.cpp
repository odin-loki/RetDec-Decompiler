...
std::map<std::size_t, void (Capstone2LlvmIrTranslatorArm_impl::*)(cs_insn* i, cs_arm*, llvm::IRBuilder<>&)>
	Capstone2LlvmIrTranslatorArm_impl::_i2fm = {
		...
		// Capstone decodes `vmrs APSR_nzcv, fpscr` as FMSTAT, the pre-UAL
		// alias, and prints it as "vmrs". That is the id compilers produce;
		// VMRS above is the general `vmrs rN, fpscr` form. This key was
		// absent from the table entirely, so the encoding gcc emits could
		// not be translated no matter what VMRS pointed at.
		...
		{ARM_INS_NEG, &Capstone2LlvmIrTranslatorArm_impl::translateSub},
