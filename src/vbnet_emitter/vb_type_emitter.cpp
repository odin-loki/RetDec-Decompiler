...
VbTypeEmitter::VbTypeEmitter(VbWriter& writer, Options opts): writer_(writer), opts_(std::move(opts)) {}
...
static std::string vbAttrLit(const BcAnnotationValue& val)
{
	...
	default: return "\"" + val.stringValue + "\"";
	}
}
...
static void emitVbAttributes(VbWriter& w, const std::vector<BcAnnotation>& anns)
{
...
std::string VbTypeEmitter::methodModifiers(const BcMethod& m) const
{
	...
	if (hasFlag(m.access, BcAccess::Virtual) && !m.isAbstract && !hasFlag(m.access, BcAccess::Override))
		s += "Overridable ";
	if (hasFlag(m.access, BcAccess::Final) && !hasFlag(m.access, BcAccess::Static)) s += "NotOverridable ";
	return s;
}
...
std::string VbTypeEmitter::fieldModifiers(const BcField& f) const
{
	...
	if (hasFlag(f.access, BcAccess::Readonly) || hasFlag(f.access, BcAccess::Final)) s += "ReadOnly ";
	return s;
}
...
{
	return cls.methods.size() == 1 && cls.methods[0].name == "Invoke" && cls.superClass.has_value()
...
bool VbTypeEmitter::isCompilerGenerated(const BcClass& cls) const
{
	...
	return !n.empty() && (n[0] == '<' || n.find("__") == 0 || n.find("d__") != std::string::npos);
}
...
{
	return m.name.size() > 4 && (m.name.substr(0, 4) == "add_" || m.name.substr(0, 7) == "remove_");
}

// ─── collectProperties ───────────────────────────────────────────────────────

std::vector<VbTypeEmitter::PropGroup> VbTypeEmitter::collectProperties(const BcClass& cls) const
{
...
void VbTypeEmitter::emitProperty(
	const std::string& propName, const BcType& propType, const BcMethod* getter, const BcMethod* setter, bool isStatic)
{
	std::string shared = isStatic ? "Shared " : "";
	std::string ro = setter ? "" : "ReadOnly ";
	std::string type = typeStr(propType);

	writer_.line("Public " + shared + ro + "Property " + VbWriter::safeName(propName) + " As " + type);
	writer_.indent();
	if (getter)
	{
		writer_.line("Get");
...
void VbTypeEmitter::emitAbstractMethod(const BcMethod& m)
{
	...
	{
		writer_.line(
			acc + "MustOverride Function " + name + params + " As "
			+ typeStr(m.descriptor.returnType ? *m.descriptor.returnType : BcType{}));
	}
}
...
void VbTypeEmitter::emitMethod(const BcClass& /*cls*/, const BcMethod& m)
{
	...
	{
		writer_.line(
			acc + mods + "Function " + name + params + " As "
			+ typeStr(m.descriptor.returnType ? *m.descriptor.returnType : BcType{}));
	}
	writer_.indent();
	if (m.cfg.blockCount() == 0)
	{
		if (!isVoid_) writer_.line("Throw New NotImplementedException()");
	}
	else
	{
		if (!isVoid_) writer_.line("Throw New NotImplementedException()");
...
void VbTypeEmitter::emitEnum(const BcClass& cls)
{
	...
	else
	{
		...
		{
			if (f.constantIntValue.has_value())
				writer_.line(VbWriter::safeName(f.name) + " = " + std::to_string(*f.constantIntValue));
			else
...
void VbTypeEmitter::emitDelegate(const BcClass& cls)
{
	...
	{
		writer_.line(
			"Public Delegate Function " + VbWriter::safeName(cls.name) + params + " As "
...
void VbTypeEmitter::emitInterface(const BcClass& cls)
{
	...
	for (const auto& m: cls.methods)
	{
		...
		if (isPropertyGetter(m))
		{
			...
			writer_.line(
				ro + "Property " + VbWriter::safeName(propName) + " As "
				+ typeStr(m.descriptor.returnType ? *m.descriptor.returnType : BcType{}));
			continue;
		}
		...
		if (isVoid(m))
			...
			writer_.line(
				"Function " + VbWriter::safeName(m.name) + paramList(m) + " As "
				+ typeStr(m.descriptor.returnType ? *m.descriptor.returnType : BcType{}));
	}
	writer_.dedent();
...
void VbTypeEmitter::emitRegularClass(const BcClass& cls)
{
	...
	std::string hdr;
	// Access
	if (hasFlag(cls.access, BcAccess::Public))
		hdr += "Public ";
	else if (hasFlag(cls.access, BcAccess::Internal))
		hdr += "Friend ";
	...
	if (cls.superClass.has_value())
	{
		...
		if (base != "Object" && base != "System.Object") writer_.line("Inherits " + base);
	}
	for (const auto& iface: cls.interfaces)
		writer_.line("Implements " + typeStr(iface));

	if (cls.superClass.has_value() || !cls.interfaces.empty()) writer_.blank();
...
void VbTypeEmitter::emitClass(const BcClass& cls, const BcModule& /*module*/)
{
	...
	bool isStruct = hasFlag(cls.access, BcAccess::Readonly) || cls.name.find("Struct") != std::string::npos;
	if (isStruct)
	{
		emitStructure(cls);
		return;
