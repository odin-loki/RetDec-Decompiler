	...
	while (std::getline(ss, line))
	{
		...
		if (!line.empty() && line[0] != ' ' && line[0] != '\t')
		{
			...
			if (arrow != std::string::npos)
			{
				...
				if (!obf.empty() && obf.back() == ':') obf.pop_back();
		...
		else
		{
			...
			if (colon2 != std::string::npos)
			{
				...
				if (colon3 != std::string::npos) lhs = lhs.substr(colon3 + 1);
			}
			...
			if (paren != std::string::npos) origMember = origMember.substr(0, paren);
...
std::vector<ApkReader::ZipEntry> ApkReader::parseCentralDirectory(const uint8_t* data, size_t size)
{
...
std::vector<uint8_t> ApkReader::extractEntry(const uint8_t* data, size_t size, const ZipEntry& entry)
{
	...
	const size_t dataOff = lfhOff + 30 + static_cast<size_t>(nameLen) + static_cast<size_t>(extraLen);
	...
	if (entry.method == 0)
	{
		...
		return std::vector<uint8_t>(data + dataOff, data + dataOff + entry.uncompressedSize);
	}
	if (entry.method == 8)
	{
		// Raw DEFLATE (ZIP method 8), not a zlib wrapper.
		constexpr uint32_t kMaxInflate = 16u * 1024u * 1024u;
		if (entry.uncompressedSize == 0 || entry.uncompressedSize > kMaxInflate) return {};
...
ApkReader::ApkReader(ApkReadOptions opts): opts_(std::move(opts)) {}

bool ApkReader::isClassesDex(const std::string& name)
{
	if (name == "classes.dex") return true;
	// classes2.dex, classes3.dex, etc.
	if (name.size() > 11 && name.substr(0, 7) == "classes" && name.substr(name.size() - 4) == ".dex") return true;
	return false;
}

bool ApkReader::isMappingFile(const std::string& name)
{
	return name == "mapping.txt" || name == "proguard/mapping.txt" || name == "META-INF/mapping.txt";
}

void ApkReader::processDex(
	const uint8_t* dexData,
	size_t dexSize,
	...
	std::vector<std::string>& warnings)
{
	try
	{
		...
		for (uint32_t i = 0; i < dex.classCount(); ++i)
		{
			...
			{
				warnings.push_back(dexName + ": class " + std::to_string(i) + ": " + result.error);
			}
		}
	}
...
ApkReadResult ApkReader::readDex(const uint8_t* data, size_t size, const std::string& name)
{
	...
	if (!result.warnings.empty()) result.status = ApkReadResult::PartialError;
	return result;
}
...
ApkReadResult ApkReader::readApk(const uint8_t* data, size_t size)
{
	...
	std::sort(dexEntries.begin(), dexEntries.end(), [](const ZipEntry* a, const ZipEntry* b) {
	...
	if (mappingEntry)
	{
		...
		if (!rawMapping.empty())
		{
			std::string text(reinterpret_cast<const char*>(rawMapping.data()), rawMapping.size());
	...
	{
		if (!opts_.parseAllDexFiles && entry->name != "classes.dex") break;
		...
		processDex(dexData.data(), dexData.size(), entry->name, result.module, result.warnings);
	}

	// Apply ProGuard mapping.
	if (result.hadMapping) applyProGuardMapping(result.module, result.mapping);
...
ApkReadResult ApkReader::readOat(const uint8_t* data, size_t size)
{
	...
	{
		if (std::memcmp(data + i, kDexMagicBuf, 4) != 0) continue;
...
void ApkReader::applyProGuardMapping(bc_module::BcModule& module, const ProGuardMapping& mapping)
{
	for (auto& cls: module.classes())
	{
		...
		if (memberIt != mapping.memberMap.end())
		{
			...
			for (auto& method: cls.methods)
			{
				...
				if (it != members.end()) method.name = it->second.originalName;
			}
			for (auto& field: cls.fields)
			{
				auto it = members.find(field.name);
				if (it != members.end()) field.name = it->second.originalName;
			}
		}
		...
		{
			if (cls.fqName == obfName) cls.fqName = classIt->second;
