...
static void skipEncodedValue(DexReader& br, unsigned depth)
{
	if (depth >= kMaxEncodedValueDepth) throw DexParseError("encoded_value nested deeper than the parser accepts");
	...
	case 0x1d: // VALUE_ANNOTATION
		...
		return;
	default: br.skip(static_cast<size_t>(arg) + 1); return;
	}
}
...
static uint64_t readEncodedBits(DexReader& br, uint8_t n)
{
	if (n == 0) return 0;
	uint64_t v = 0;
	if (n > kMaxEncodedValueBytes || !utils::byteorder::readLE(br.data(), br.size(), br.pos(), n, v))
		throw DexParseError("encoded_value payload of " + std::to_string(n) + " bytes runs past the end of the file");
	br.skip(n);
	return v;
}
...
static void applyEncodedValue(BcField& field, DexReader& br, const DexFile& dex)
{
	...
	case 0x04:
	case 0x06:
		field.constantIntValue = utils::byteorder::signExtendFrom(readEncodedBits(br, nbytes), payloadBits);
		break;
	...
	case 0x03:
		field.constantIntValue =
			static_cast<int64_t>(utils::byteorder::zeroExtendFrom(readEncodedBits(br, nbytes), payloadBits));
		break;
	case 0x10: {
		...
		uint32_t bits = static_cast<uint32_t>(utils::byteorder::extendHigh(raw, supplied, kFloatBytes));
		float f = 0.0f;
	...
	case 0x17: {
		...
		if (idx < dex.stringCount()) field.constantStrValue = dex.string(static_cast<uint32_t>(idx));
		break;
	}
	case 0x1e: break;
	case 0x1f: field.constantIntValue = arg != 0 ? 1 : 0; break;
	case 0x1c: skipEncodedArray(br, 0); break;
	case 0x1d: skipEncodedAnnotation(br, 0); break;
	default: br.skip(nbytes); break;
	}
}

static void fillStaticConstants(BcClass& cls, const DexFile& dex, uint32_t off, size_t nStatic)
{
	if (off == 0 || nStatic == 0) return;
	try
	{
...
static std::vector<BcAnnotation> readAnnotationSet(const DexFile& dex, uint32_t setOff)
{
	std::vector<BcAnnotation> out;
	if (setOff == 0) return out;
	...
	for (uint32_t i = 0; i < annSetSize; ++i)
	{
		...
		for (uint32_t e = 0; e < numElems; ++e)
		{
			...
			std::string key = (nameIdx < dex.stringCount()) ? dex.string(nameIdx) : "";
			skipEncodedValue(br, 0);
...
BcType DexClassParser::descriptorToType(const std::string& desc) const
{
	...
	while (dims < desc.size() && desc[dims] == '[')
		++dims;
	if (dims > kMaxArrayDimensions) return types::Void();
...
{
	std::string s = desc;
	if (!s.empty() && s[0] == 'L' && s.back() == ';') s = s.substr(1, s.size() - 2);
	for (char& c: s)
		if (c == '/') c = '.';
	return s;
}
...
void DexClassParser::parseFields(BcClass& cls, const std::vector<EncodedField>& fields, bool isStatic)
{
	for (const auto& ef: fields)
	{
		if (ef.fieldIdx >= dex_.fieldCount()) continue;
		BcField field;
		...
		if (isStatic) field.access = field.access | BcAccess::Static;
...
void DexClassParser::parseMethods(BcClass& cls, const std::vector<EncodedMethod>& methods)
{
	for (const auto& em: methods)
	{
		if (em.methodIdx >= dex_.methodCount()) continue;
		...
		if (proto.parametersOff != 0)
		{
			...
			{
				funcType.params.push_back(std::make_shared<BcType>(descriptorToType(dex_.typeName(typeIdx))));
			}
		}
		...
		if (opts_.parseBytecode && em.codeOff != 0)
		{
			try
			{
				...
				if (liftResult.status == DexLiftResult::OK) method.cfg = std::move(liftResult.cfg);
			}
...
{
	if (!opts_.parseAnnotations || annotationsOff == 0) return;
	...
	try
	{
		...
		if (classAnnotOff != 0)
		{
			...
			cls.annotations.insert(cls.annotations.end(), anns.begin(), anns.end());
		}
		...
		for (uint32_t i = 0; i < fieldsSize; ++i)
		{
			...
			if (fieldIdx >= dex_.fieldCount()) continue;
			if (BcField* f = cls.findField(dex_.fieldName(fieldIdx)))
			{
				f->annotations = readAnnotationSet(dex_, setOff);
				if (opts_.resolveGenerics) f->signature = resolveGenericSignature(annotationsOff, fieldIdx, false);
			}
		}
		for (uint32_t i = 0; i < methodsSize; ++i)
		{
			...
			uint32_t setOff = r.u4();
			if (methodIdx >= dex_.methodCount()) continue;
			if (BcMethod* m = cls.findMethod(dex_.methodName(methodIdx), dex_.methodProto(methodIdx)))
			{
				m->annotations = readAnnotationSet(dex_, setOff);
				if (opts_.resolveGenerics) m->signature = resolveGenericSignature(annotationsOff, methodIdx, true);
			}
		}
		for (uint32_t i = 0; i < paramsSize; ++i)
		{
			...
			if (methodIdx >= dex_.methodCount() || listOff == 0) continue;
...
std::string DexClassParser::resolveGenericSignature(uint32_t annotationsOff, uint32_t memberIdx, bool isMethod) const
{
	if (!opts_.resolveGenerics || annotationsOff == 0) return {};
	...
	try
	{
			...
			for (uint32_t i = 0; i < annSetSize; ++i)
			{
				...
				for (uint32_t e = 0; e < numElems; ++e)
				{
					...
					for (uint32_t j = 0; j < arrSize; ++j)
					{
						...
						uint64_t strIdx = readEncodedBits(br, static_cast<uint8_t>(argBits + 1));
						if (strIdx < dex_.stringCount()) sig += dex_.string(static_cast<uint32_t>(strIdx));
					}
					return sig;
				}
			}
		...
		if (!isMethod && memberIdx == 0) return sigFromSet(classAnnotOff);
		...
		if (!isMethod)
		{
			for (uint32_t i = 0; i < fieldsSize; ++i)
			{
				uint32_t fidx = r.u4();
				uint32_t off = r.u4();
				if (fidx == memberIdx) return sigFromSet(off);
			}
			return {};
		}
		...
		for (uint32_t i = 0; i < methodsSize; ++i)
		{
			uint32_t midx = r.u4();
			uint32_t off = r.u4();
			if (midx == memberIdx) return sigFromSet(off);
		}
	}
	catch (const std::exception&)
	{}
...
DexClassResult DexClassParser::parseClass(uint32_t classDefIdx)
{
	...
	if (lastDot != std::string::npos) bcClass->packageName = dotted.substr(0, lastDot);
	...
	if (opts_.parseAnnotations) parseAnnotations(*bcClass, cd.annotationsOff);

	if (opts_.resolveGenerics) bcClass->signature = resolveGenericSignature(cd.annotationsOff, 0, false);
