...
bool AbiArtifactMarker::isRspAlignInstr(const ssa::SSAFunction& fn, InstrId id) const
{
...
bool AbiArtifactMarker::isPrologueBlock(BlockId blk, const ssa::SSAFunction& fn) const
{
...
std::vector<AbiArtifact> AbiArtifactMarker::markStackAlign(const ssa::SSAFunction& fn) const
{
...
std::vector<AbiArtifact> AbiArtifactMarker::markPrologueEpilogue(const ssa::SSAFunction& fn) const
{
	std::vector<AbiArtifact> result;
	...
	for (const auto& blk: fn.blocks())
	{
		...
		for (int i = static_cast<int>(blk->instrs.size()) - 2; i >= 0; --i)
		{
			...
			if (instr->op != ssa::IrInstr::Op::Store && instr->op != ssa::IrInstr::Op::Load) break;
		}
	}
...
std::vector<AbiArtifact> AbiArtifactMarker::markShadowSpace(const ssa::SSAFunction& fn) const
{
	std::vector<AbiArtifact> result;
...
std::vector<AbiArtifact> AbiArtifactMarker::markCalleeSavePairs(const ssa::SSAFunction& fn, const Config& cfg) const
{
	std::vector<AbiArtifact> result;
...
std::vector<AbiArtifact> AbiArtifactMarker::markRedZone(const ssa::SSAFunction& fn) const
{
	std::vector<AbiArtifact> result;
				...
				for (const auto& rb: fn.blocks())
				{
					...
					for (const ssa::IrInstr* ri: rb->instrs)
					{
						...
						for (const auto& use: ri->uses)
						{
							...
							if (rv && rv->kind == ssa::ValueKind::MemRef && rv->memIsStack
...
std::vector<AbiArtifact> AbiArtifactMarker::run(const ssa::SSAFunction& fn, const Config& cfg) const
{
