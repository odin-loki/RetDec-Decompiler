...
void LiveRootCollector::collectGlobalWrites(const ssa::SSAFunction& fn, std::vector<LiveRoot>& roots) const
{
	for (const auto& blk: fn.blocks())
	{
		if (!blk) continue;
...
void LiveRootCollector::collectIoSideEffects(const ssa::SSAFunction& fn, std::vector<LiveRoot>& roots) const
{
	for (const auto& blk: fn.blocks())
	{
		if (!blk) continue;
