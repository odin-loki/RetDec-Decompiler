...
LoaderSim::LoaderSim(const uint8_t* data, std::size_t size, uint64_t imageBase, bool is64Bit, bool isELF, bool isLE):
	_data(data), _size(size), _imageBase(imageBase), _is64Bit(is64Bit), _isELF(isELF), _isLE(isLE)
{}
...
uint16_t LoaderSim::r16(std::size_t off) const noexcept
{
	...
	if (_isLE) return static_cast<uint16_t>(_data[off]) | (static_cast<uint16_t>(_data[off + 1]) << 8);
	return (static_cast<uint16_t>(_data[off]) << 8) | static_cast<uint16_t>(_data[off + 1]);
}
...
uint32_t LoaderSim::r32(std::size_t off) const noexcept
{
	...
	if (_isLE)
		return static_cast<uint32_t>(_data[off]) | (static_cast<uint32_t>(_data[off + 1]) << 8)
			 | (static_cast<uint32_t>(_data[off + 2]) << 16) | (static_cast<uint32_t>(_data[off + 3]) << 24);
	return (static_cast<uint32_t>(_data[off]) << 24) | (static_cast<uint32_t>(_data[off + 1]) << 16)
		 | (static_cast<uint32_t>(_data[off + 2]) << 8) | static_cast<uint32_t>(_data[off + 3]);
}
...
static std::size_t peNtOffset(const uint8_t* data, std::size_t size)
{
	...
	uint32_t e_lfanew = static_cast<uint32_t>(data[0x3C]) | (static_cast<uint32_t>(data[0x3D]) << 8)
					  | (static_cast<uint32_t>(data[0x3E]) << 16) | (static_cast<uint32_t>(data[0x3F]) << 24);
...
std::vector<LoaderSim::PESection> LoaderSim::parsePESections() const
{
	...
	for (uint16_t i = 0; i < numSec; ++i)
	{
		...
		uint64_t base64 = (magic == kOptMagicPE32P) ? r64(optOff + kImageBase64) : r32(optOff + kImageBase32);
...
std::size_t LoaderSim::vaToOffset(uint64_t va, const std::vector<PESection>& secs) const
{
	std::size_t optOff = 0;
	{
		std::size_t ntOff = peNtOffset(_data, _size);
		...
		uint64_t ib =
			(magic == kOptMagicPE32P) ? r64(ntOff + 4 + 20 + kImageBase64) : r32(ntOff + 4 + 20 + kImageBase32);
		...
		const uint64_t off = utils::secmap::addressToOffset(rva, mapped.data(), mapped.size(), _size);
		(void)optOff;
...
uint64_t LoaderSim::peDataDir(int idx, bool& ok) const
{
	...
	std::size_t optOff = ntOff + 4 + 20;
	uint16_t magic = r16(optOff + kMagic);
	std::size_t ddOff = (magic == kOptMagicPE32P) ? (optOff + kDataDir64 + idx * 8) : (optOff + kDataDir32 + idx * 8);
	if (!inBounds(ddOff, 8)) return 0;
	uint32_t rva = r32(ddOff);
	if (rva == 0) return 0;

	uint64_t ib = (magic == kOptMagicPE32P) ? r64(optOff + kImageBase64) : r32(optOff + kImageBase32);
	ok = true;
	return ib + rva;
}
...
std::vector<ImportRef> LoaderSim::resolvePEDelayImports() const
{
	...
	for (;;)
	{
		...
		for (uint32_t entry = 0;; ++entry)
		{
			...
			{
				std::size_t ibnOff = vaToOffset(ib + (val & 0x7FFFFFFF), secs);
				if (inBounds(ibnOff, 2)) imp.symbol = rStr(ibnOff + 2);
			}
			imports.push_back(imp);
		}
	}
...
std::vector<TLSCallback> LoaderSim::parsePETLS() const
{
	...
	uint64_t cbTableVA = _is64Bit ? r64(tlsOff + 24)  // 3rd 8-byte field
	...
	for (uint32_t i = 0;; ++i)
	{
		...
		uint64_t fnVA = _is64Bit ? r64(cbOff + i * ptrSz) : r32(cbOff + i * ptrSz);
		if (fnVA == 0) break;
		TLSCallback cb;
		cb.vma = fnVA;
...
std::vector<LoaderSim::ElfSection> LoaderSim::parseElfSections() const
{
	...
	if (shstrndx < shnum)
	{
		...
		strtabOff = is64 ? static_cast<std::size_t>(r64(sh + kShOffOff64)) : r32(sh + kShOffOff32);
	}
...
LoaderSim::findElfSection(const std::vector<ElfSection>& secs, const std::string& name) const
{
	for (const auto& s: secs)
	{
...
std::vector<ImportRef> LoaderSim::resolveELFImports() const
{
	...
	for (std::size_t i = 0; i < numEnt; ++i)
	{
		...
		uint64_t info = rPtr(eOff + (_is64Bit ? 8u : 4u));
		uint32_t symIdx = _is64Bit ? static_cast<uint32_t>(info >> 32) : static_cast<uint32_t>(info >> 8);
