...
void logArArchiveMemberProbeFromPath(const std::string &path)
{
	...
	if (!retdec::ar_extractor::isArchive(path))
	{
		return;
	}
	...
	if (!in)
	{
		Log::info() << "[AR format probe] could not open input: " << path << std::endl;
		return;
	}
...
void ProgramOptions::load()
{
	for (auto i = _argv.begin(); i != _argv.end();)
	{
		...
		if (isParam(i, "", "--config"))
		{
			...
			// LLVM pipeline. When a list is empty after load, keep the values
			// from the install default config loaded in main().
			auto& p = config.parameters;
			if (p.staticSignaturePaths.empty())
			{
				p.staticSignaturePaths = backup.staticSignaturePaths;
			}
			if (p.libraryTypeInfoPaths.empty())
			{
				p.libraryTypeInfoPaths = backup.libraryTypeInfoPaths;
			}
			if (p.cryptoPatternPaths.empty())
			{
				p.cryptoPatternPaths = backup.cryptoPatternPaths;
			}
			if (p.abiPaths.empty())
			{
				p.abiPaths = backup.abiPaths;
			}
			if (p.llvmPasses.empty())
			{
				p.llvmPasses = backup.llvmPasses;
			}
			if (p.getOrdinalNumbersDirectory().empty())
			{
				p.setOrdinalNumbersDirectory(backup.getOrdinalNumbersDirectory());
			}
		}
		++i;
	}
...
void ProgramOptions::loadOption(std::list<std::string>::iterator& i)
{
	...
	else if (isParam(i, "", "--try-emulation"))
	{
		tryEmulationUnpacking = true;
	}
	else if (isParam(i, "", "--jobs"))
	{
		auto val = getParamOrDie(i);
		try
		{
...
{
	retdec::setLogsFrom(config.parameters);
	...
	if (po.arIdx || !po.arName.empty())
	{
		...
		logArArchiveMemberProbeFromPath(config.parameters.getInputFile());
	...
	{
		Log::phase("Stage 3: Emulation-based unpacking");
		if (retdec::tryEmulationUnpacking(config, config.parameters.getOutputUnpackedFile()))
		{
