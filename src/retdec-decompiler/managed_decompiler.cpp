...
{
	return size >= 4 && d[0] == 0x50 && d[1] == 0x4B && (d[2] == 0x03 || d[2] == 0x05 || d[2] == 0x07)
...
static bool isClassesDexEntry(const std::string& name)
{
	...
	if (name.size() > 10 && name.compare(0, 8, "classes") == 0 && name.substr(name.size() - 4) == ".dex")
	{
...
{
	if (size < 22) return ZipManagedKind::None;

	std::size_t eocdOffset = size - 22;
	while (eocdOffset > 0)
	{
		if (le32(data + eocdOffset) == kZipEOCD) break;
		--eocdOffset;
	}
	if (le32(data + eocdOffset) != kZipEOCD) return ZipManagedKind::None;
	...
	{
		if (le32(data + pos) != kZipCentral) break;
		...
		if (pos + 46 + fnLen <= size)
		{
			...
			if (endsWith(name, ".class")) hasClass = true;
			if (isClassesDexEntry(name)) hasDex = true;
		}
		pos += 46 + fnLen + extraLen + commentLen;
	}

	if (hasDex) return ZipManagedKind::Apk;
	if (hasClass) return ZipManagedKind::JavaJar;
...
{
	if (size < 8 || d[0] != 0xCA || d[1] != 0xFE || d[2] != 0xBA || d[3] != 0xBE) return false;
...
{
	if (!data || size < 4) return ManagedFormat::Unknown;

	const uint8_t* d = data;

	if (isJavaClassMagic(d, size)) return ManagedFormat::JavaClass;

	if (d[0] == 0x64 && d[1] == 0x65 && d[2] == 0x78 && d[3] == 0x0A) return ManagedFormat::Dex;

	if (d[0] == 0x00 && d[1] == 0x61 && d[2] == 0x73 && d[3] == 0x6D) return ManagedFormat::Wasm;

	if (d[0] == 0x1B && d[1] == 0x4C && d[2] == 0x75 && d[3] == 0x61) return ManagedFormat::LuaBytecode;

	if (size >= 4 && d[2] == 0x0D && d[3] == 0x0A)
	{
		const uint16_t magicWord = uint16_t(d[0]) | uint16_t(uint16_t(d[1]) << 8);
		if (magicWord != 0) return ManagedFormat::PythonPyc;
	}
	...
	if (size >= 2 && d[0] == 0x4D && d[1] == 0x5A && probeCliAssembly(d, size)) return ManagedFormat::CliAssembly;
...
const char* managedOutputLangHint(ManagedFormat fmt)
{
	...
	case ManagedFormat::Apk: return "java";
	case ManagedFormat::PythonPyc: return "python";
	case ManagedFormat::LuaBytecode: return "lua";
	case ManagedFormat::Wasm: return "wat";
	case ManagedFormat::CliAssembly: return "csharp";
	default: return nullptr;
	}
}

void logManagedFormatRoute(ManagedFormat fmt, const std::string& path, std::ostream& os)
{
	os << "[managed] Detected format: " << managedFormatName(fmt) << " (" << path << ")\n";
...
void logUnknownManagedFormat(const uint8_t* data, std::size_t size, const std::string& path, std::ostream& os)
{
	...
	if (ext == ".jar" || ext == ".apk" || ext == ".dex" || ext == ".class" || ext == ".pyc" || ext == ".luac"
		|| ext == ".wasm" || ext == ".dll")
	{
...
static int decompileJavaClass(const std::string& inputFile, const std::string& outputFile)
{
	...
	{
		if (!fr.source.empty()) combined += fr.source + "\n";
	}

	if (combined.empty()) std::cerr << "Warning: Java emitter produced no output.\n";

	try
	{
		writeText(outputFile, combined);
	}
	catch (const std::exception& e)
	{
...
static int decompileJavaJar(const std::string& inputFile, const std::string& outputFile)
{
	...
	std::cerr << "[managed] JAR decompilation written to: " << outputFile << " (" << jarResult.classesParsed << "/"
...
static int decompileDex(const std::string& inputFile, const std::string& outputFile)
{
	...
	if (!result.error.empty()) std::cerr << "Warning: " << result.error << "\n";
...
static int decompileApk(const std::string& inputFile, const std::string& outputFile)
{
...
static int decompileCliAssembly(const std::string& inputFile, const std::string& outputFile)
{
	...
	{
		if (!kv.second.empty()) combined += kv.second + "\n";
	}

	if (combined.empty()) std::cerr << "Warning: C# emitter produced no output.\n";
...
static int decompilePyc(const std::string& inputFile, const std::string& outputFile)
{
	...
	auto pyModule =
		reconstructor.reconstruct(*pycResult.root, pycResult.version.major, pycResult.version.minor, inputFile);
...
static int decompileLua(const std::string& inputFile, const std::string& outputFile)
{
...
static int decompileWasm(const std::string& inputFile, const std::string& outputFile)
{
...
int decompileManaged(ManagedFormat fmt, const std::string& inputFile, const std::string& outputFile)
{
	switch (fmt)
	{
	case ManagedFormat::JavaClass: return decompileJavaClass(inputFile, outputFile);
	case ManagedFormat::JavaJar: return decompileJavaJar(inputFile, outputFile);
	case ManagedFormat::Dex: return decompileDex(inputFile, outputFile);
	case ManagedFormat::Apk: return decompileApk(inputFile, outputFile);
	case ManagedFormat::CliAssembly: return decompileCliAssembly(inputFile, outputFile);
	case ManagedFormat::PythonPyc: return decompilePyc(inputFile, outputFile);
	case ManagedFormat::LuaBytecode: return decompileLua(inputFile, outputFile);
	case ManagedFormat::Wasm: return decompileWasm(inputFile, outputFile);
	default: std::cerr << "Error: unknown managed format.\n"; return 1;
	}
}
