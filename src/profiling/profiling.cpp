...
ProfilingReport Profiler::report(bool resetAfter)
{
	...
	for (const auto& [_, k]: kernels_)
		r.kernels.push_back(k);
	r.functionSamples = funcSamples_;

	// Sort stages by total time descending
	std::sort(r.stages.begin(), r.stages.end(), [](const StageRecord& a, const StageRecord& b) {
		return a.totalNs > b.totalNs;
	});
	std::sort(r.kernels.begin(), r.kernels.end(), [](const KernelRecord& a, const KernelRecord& b) {
		return a.totalNs > b.totalNs;
	});
...
std::string ProfilingReport::toText() const
{
	...
	for (const auto& s: stages)
	{
		...
		os << "│ " << std::left << std::setw(24) << s.name.substr(0, 24) << " │" << std::right << std::setw(7)
		   << std::fixed << std::setprecision(1) << s.totalMs() << " │" << std::setw(7) << s.averageMs() << " │"
		   << std::setw(7) << (s.minNs == INT64_MAX ? 0.0 : s.minMs()) << " │" << std::setw(7) << s.maxMs() << " │"
		   << std::setw(5) << s.callCount << " │" << "  " << std::setprecision(1) << frac << "%\n";
	}

	os << "├──────────────────────────┴────────┴────────┴────────┴────────┴──────┤\n";
	os << "│ Total wall time: " << std::fixed << std::setprecision(1) << totalMs << " ms";
	if (peakRssBytes > 0)
	{
	...
	if (!kernels.empty())
	{
		...
		{
			os << "  " << std::left << std::setw(30) << k.name.substr(0, 30) << "  total=" << std::setw(8) << std::fixed
			   << std::setprecision(2) << k.totalMs() << " ms  launches=" << k.launches
...
bool ProfilingReport::toCsv(const std::string& path) const
{
	...
	{
		double frac = totalMs > 0.0 ? s.totalMs() / totalMs * 100.0 : 0.0;
		f << '"' << s.name << '"' << ',' << std::fixed << std::setprecision(3) << s.totalMs() << ',' << s.averageMs()
		  << ',' << (s.minNs == INT64_MAX ? 0.0 : s.minMs()) << ',' << s.maxMs() << ',' << s.callCount << ',' << frac
		  << '\n';
	}
	if (!kernels.empty())
	{
		...
		for (const auto& k: kernels)
			f << '"' << k.name << '"' << ',' << std::fixed << std::setprecision(3) << k.totalMs() << ',' << k.avgMs()
			  << ',' << k.launches << '\n';
	}
	return true;
}
...
{
	std::ostringstream os;
	os << "{\n  \"total_wall_ms\": " << std::fixed << std::setprecision(3) << static_cast<double>(totalWallNs) / 1e6;
	if (peakRssBytes > 0) os << ",\n  \"peak_rss_bytes\": " << peakRssBytes;
	os << ",\n  \"stages\": [\n";
	double totalMs = static_cast<double>(totalWallNs) / 1e6;
	for (size_t i = 0; i < stages.size(); ++i)
	{
		...
		double frac = totalMs > 0.0 ? s.totalMs() / totalMs * 100.0 : 0.0;
		os << "    {\"name\":\"" << s.name << "\"," << "\"total_ms\":" << s.totalMs() << ","
		   << "\"avg_ms\":" << s.averageMs() << "," << "\"calls\":" << s.callCount << "," << "\"fraction_pct\":" << frac
		   << "}";
	...
	if (!functionSamples.empty())
	{
		...
		for (size_t i = 0; i < functionSamples.size(); ++i)
		{
			...
			os << "    {\"name\":\"" << f.key << "\"," << "\"elapsed_ms\":" << static_cast<double>(f.elapsedNs) / 1e6
			   << "}";
	...
	if (!kernels.empty())
	{
		...
		for (size_t i = 0; i < kernels.size(); ++i)
		{
			...
			os << "    {\"name\":\"" << k.name << "\"," << "\"total_ms\":" << k.totalMs() << ","
			   << "\"avg_ms\":" << k.avgMs() << "," << "\"launches\":" << k.launches << "}";
...
double ProfilingReport::stageFraction(const std::string& stageName) const
{
	...
	for (const auto& s: stages)
		if (s.name == stageName) return static_cast<double>(s.totalNs) / totalWallNs;
	return 0.0;
}
...
ScopeTimer::ScopeTimer(std::string stageName, Profiler& profiler):
	stageName_(std::move(stageName)), start_(Clock::now()), profiler_(&profiler)
{}

ScopeTimer::ScopeTimer(ScopeTimer&& other) noexcept:
	stageName_(std::move(other.stageName_)), start_(other.start_), profiler_(other.profiler_), active_(other.active_)
{
	other.active_ = false;
}
...
int64_t RssTracker::peakRssBytes()
{
	...
	{
		// VmPeak:   12345 kB
		if (line.rfind("VmPeak:", 0) == 0) return statusLineKbToBytes(line, sizeof("VmPeak:") - 1);
	}
	return 0;
}
int64_t RssTracker::currentRssBytes()
{
	...
	while (std::getline(f, line))
	{
		if (line.rfind("VmRSS:", 0) == 0) return statusLineKbToBytes(line, sizeof("VmRSS:") - 1);
	}
	return 0;
}
#endif
...
void FunctionHistogram::add(Nanos ns)
{
	...
	if (static_cast<int>(rawSamples_.size()) < kMaxRaw) rawSamples_.push_back(ns);
}
...
std::string FunctionHistogram::format() const
{
	...
	for (const auto& b: buckets_)
	{
		int barLen = maxCount > 0 ? static_cast<int>(40.0 * b.count / maxCount) : 0;
		os << std::setw(8) << static_cast<double>(b.lo) / 1e6 << "ms " << bar(barLen) << " " << b.count << "\n";
	}
	if (!rawSamples_.empty())
	{
		os << "P50=" << percentile(0.5) / 1e3 << "µs  " << "P90=" << percentile(0.9) / 1e3 << "µs  "
