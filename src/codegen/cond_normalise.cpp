...
{
	using B = CExpr::BinOpKind;
	return op == B::Eq || op == B::Ne || op == B::Lt || op == B::Le || op == B::Gt || op == B::Ge;
}

// Forward declaration.
static std::shared_ptr<CExpr> norm(std::shared_ptr<CExpr> e, bool boolCtx, std::size_t* n);

// Count one rewrite, if the caller asked for a count.
...
static std::shared_ptr<CExpr> normChildren(std::shared_ptr<CExpr> e, bool boolCtx, std::size_t* n)
{
	// We always normalise children with boolCtx=false unless e is a boolean op
	// whose children should also be in boolean context.
	bool childBool =
		(e->kind == CExpr::Kind::BinOp && (e->binOp == CExpr::BinOpKind::LAnd || e->binOp == CExpr::BinOpKind::LOr));
	for (auto& c: e->children)
		c = norm(c, childBool, n);
	return e;
}
...
static std::shared_ptr<CExpr> norm(std::shared_ptr<CExpr> e, bool boolCtx, std::size_t* n)
{
	...
	if (boolCtx && e->kind == K::BinOp && (e->binOp == B::Ne || e->binOp == B::Eq) && e->children.size() == 2)
	{
...
CondNormaliser::normalise(std::shared_ptr<CExpr> expr, bool boolContext, std::size_t* rewrites) const
{
	return norm(std::move(expr), boolContext, rewrites);
}
