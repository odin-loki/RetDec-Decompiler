...
std::string Emitter::emitType(const CType& type, const std::string& name) const
{
	...
	if (!name.empty())
	{
		if (type.kind == CType::Kind::Array)
		{
			...
			if (bracket != std::string::npos) return t.substr(0, bracket) + name + t.substr(bracket);
		}
		return t + " " + name;
	}
	return t;
...
std::string Emitter::emitStmt(const CStmt& stmt, int indent, const Config& cfg) const
{
	const std::string I = ind(indent, cfg.indentWidth);

	switch (stmt.kind)
	{
	case CStmt::Kind::Block: return I + emitBlock(stmt, indent, cfg) + "\n";

	case CStmt::Kind::Decl: {
		std::string s = I + emitType(*stmt.declType, stmt.declName);
		if (stmt.declInit) s += " = " + emitExpr(*stmt.declInit);
		return s + ";\n";
	}
	...
	case CStmt::Kind::Case:
		if (stmt.caseValue.has_value()) return I + "case " + std::to_string(*stmt.caseValue) + ":\n";
		return I + "default:\n";

	case CStmt::Kind::Default: return I + "default:\n";
	...
	case CStmt::Kind::Label: return ind(std::max(0, indent - 1), cfg.indentWidth) + stmt.label + ":\n";
	}
	return "";
}
...
static const std::unordered_map<std::string, std::string> kStdIncludes = {
	{"printf", "stdio.h"},   {"fprintf", "stdio.h"}, {"fopen", "stdio.h"},   {"fclose", "stdio.h"},
	{"fread", "stdio.h"},    {"fwrite", "stdio.h"},  {"malloc", "stdlib.h"}, {"calloc", "stdlib.h"},
	{"realloc", "stdlib.h"}, {"free", "stdlib.h"},   {"exit", "stdlib.h"},   {"atoi", "stdlib.h"},
	{"memcpy", "string.h"},  {"memset", "string.h"}, {"strcmp", "string.h"}, {"strlen", "string.h"},
	{"strcpy", "string.h"},  {"strcat", "string.h"}, {"sin", "math.h"},      {"cos", "math.h"},
	{"sqrt", "math.h"},
};
	...
	const PointerSyntax& ptrSyn)
{
	...
	auto getUseExpr = [&](std::size_t i) -> std::shared_ptr<CExpr> {
		...
		return (it != exprs.valueExprs.end()) ? it->second : CExpr::var("v" + std::to_string(uid), uid);
	};
	...
	case O::Call: {
		if (instr->defValue != ssa::kInvalidValue && exprs.inlinedValues.count(instr->defValue))
		...
		if (instr->defValue == ssa::kInvalidValue) return CStmt::exprStmt(callE);
			...
			if (arm)
			{
				...
				if (ab)
				{
					if (ab->kind == CStmt::Kind::Block)
						for (auto& s: ab->children)
							sw->children.push_back(s);
					else
	...
	const Config& cfg) const
{
	...
	case RK::Float: cfn.returnType = CType::make(cc.ret.width <= 32 ? CType::Kind::Float : CType::Kind::Double); break;
	case RK::Struct: cfn.returnType = CType::make(CType::Kind::Int64); break;
	default:
		if (cc.ret.width <= 8)
			cfn.returnType = CType::make(CType::Kind::Int8);
		else if (cc.ret.width <= 16)
			cfn.returnType = CType::make(CType::Kind::Int16);
		else if (cc.ret.width <= 32)
			cfn.returnType = CType::make(CType::Kind::Int32);
		else
			cfn.returnType = CType::make(CType::Kind::Int64);
		break;
	}
	...
	for (std::size_t i = 0; i < cc.args.size(); ++i)
	{
		...
		if (isFp)
			p.type = CType::make(w <= 32 ? CType::Kind::Float : CType::Kind::Double);
		else if (w <= 8)
			p.type = CType::make(CType::Kind::Int8);
		else if (w <= 16)
			p.type = CType::make(CType::Kind::Int16);
		else if (w <= 32)
			p.type = CType::make(CType::Kind::Int32);
		else
			p.type = CType::make(CType::Kind::Int64);
		cfn.params.push_back(std::move(p));
	}

	// Phase 1: Expression coalescing.
	ExprCoalescer coalescer;
	auto exprResult = cfg.enableCoalescing ? coalescer.run(fn, dce) : ExprCoalescer::Result{};
	...
	const Config& cfg) const
{
	...
	for (std::size_t i = 0; i < fns.size(); ++i)
	{
		...
		const auto& cc = ccMap.count(fname) ? ccMap.at(fname) : call_conv::CallingConvention{};
		const auto& dce = dceMap.count(fname) ? dceMap.at(fname) : dce::DeadCodeResult{};
