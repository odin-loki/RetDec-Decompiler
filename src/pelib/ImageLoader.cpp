...
PeLib::ImageLoader::ImageLoader(std::uint32_t versionInfo)
{
	...
	if (windowsBuildNumber != 0)
	{
		...
		// Note that Windows 8-10 do this somewhat randomly; the same image is mapped differently when in different
		// folders
		ssiImageAlignment32 =
			(BuildNumberXP <= windowsBuildNumber && windowsBuildNumber < BuildNumberVista) ? PELIB_SECTOR_SIZE : 1;

		// Max section count is smaller in Windows XP
		maxSectionCount = (BuildNumberXP <= windowsBuildNumber && windowsBuildNumber < BuildNumberVista)
		...
		if (is64BitWindows && windowsBuildNumber >= BuildNumber8) loadItaniumImages = false;
...
bool PeLib::ImageLoader::relocateImage(std::uint64_t newImageBase)
{
	...
	if (newImageBase != optionalHeader.ImageBase)
	{
		...
		if (fileHeader.Characteristics & PELIB_IMAGE_FILE_RELOCS_STRIPPED) return false;

		// Don't relocate 32-bit images to an address greater than 32bits
		if (optionalHeader.Magic == PELIB_IMAGE_NT_OPTIONAL_HDR32_MAGIC && (newImageBase >> 32)) return false;
		...
		if (optionalHeader.NumberOfRvaAndSizes <= PELIB_IMAGE_DIRECTORY_ENTRY_BASERELOC) return false;
		...
		if (VirtualAddress == 0 || Size == 0) return false;

		// Do not relocate images with weird or invalid relocation table
		if (!isValidImageBlock(VirtualAddress, Size)) return false;
...
std::uint32_t PeLib::ImageLoader::readImage(void* buffer, std::uint32_t rva, std::uint32_t bytesToRead)
{
	// If the image was properly mapped, perform an image-read operation
	if (rawFileData.size() == 0) return readWriteImage(buffer, rva, bytesToRead, readFromPage);
...
std::uint32_t PeLib::ImageLoader::writeImage(void* buffer, std::uint32_t rva, std::uint32_t bytesToRead)
{
	// If the image was properly mapped, perform an image-read operation
	if (rawFileData.size() == 0) return readWriteImage(buffer, rva, bytesToRead, writeToPage);
...
std::uint32_t PeLib::ImageLoader::stringLength(std::uint32_t rva, std::uint32_t maxLength) const
{
	...
	if (pages.size())
	{
		...
		if (rvaEnd > getSizeOfImageAligned()) rvaEnd = getSizeOfImageAligned();
	...
	else
	{
		...
		if ((rva = getFileOffsetFromRva(rva)) < rawFileData.size())
		{
			...
			if (stringEnd != nullptr) length = stringEnd - stringPtr;
		}
	}
...
std::uint32_t PeLib::ImageLoader::readString(std::string& str, std::uint32_t rva, std::uint32_t maxLength)
{
...
std::uint32_t PeLib::ImageLoader::readPointer(std::uint32_t rva, std::uint64_t& pointerValue)
{
	...
	{
	case 64:
		if (readImage(&pointerValue, rva, sizeof(std::uint64_t)) == sizeof(std::uint64_t)) return sizeof(std::uint64_t);
		break;
...
std::uint32_t PeLib::ImageLoader::readStringRc(std::string& str, std::uint32_t rva)
{
	...
	bool mustNotBeTooLong)
{
	...
	if (offset < fileData.size())
	{
		...
		if ((offset + maxLength) > fileData.size()) maxLength = fileData.size() - offset;
...
{
	if (optionalHeader.Magic == PELIB_IMAGE_NT_OPTIONAL_HDR64_MAGIC) return 64;
...
{
	if (sectionIndex >= sections.size()) return UINT32_MAX;
	if (optionalHeader.SectionAlignment < PELIB_PAGE_SIZE) return sections[sectionIndex].PointerToRawData;
...
{
	if (sectionIndex >= sections.size()) return UINT32_MAX;
	if (optionalHeader.SectionAlignment < PELIB_PAGE_SIZE) return sections[sectionIndex].SizeOfRawData;

	std::uint32_t beginOfRawData = sections[sectionIndex].PointerToRawData & ~(PELIB_SECTOR_SIZE - 1);
	std::uint32_t endOfRawData = sections[sectionIndex].PointerToRawData
							   + AlignToSize(sections[sectionIndex].SizeOfRawData, optionalHeader.FileAlignment);
...
std::uint32_t PeLib::ImageLoader::getImageProtection(std::uint32_t sectionCharacteristics) const
{
	...
	if (sectionCharacteristics & PELIB_IMAGE_SCN_MEM_EXECUTE) Index |= 1;

	if (sectionCharacteristics & PELIB_IMAGE_SCN_MEM_READ) Index |= 2;

	if (sectionCharacteristics & PELIB_IMAGE_SCN_MEM_WRITE) Index |= 4;

	if (sectionCharacteristics & PELIB_IMAGE_SCN_MEM_SHARED) Index |= 8;
...
std::size_t PeLib::ImageLoader::getSectionIndexByRva(std::uint32_t Rva) const
{
	...
	for (const auto& section: sections)
	{
		if (section.VirtualAddress <= Rva
			&& Rva < AlignToSize(section.VirtualAddress + section.VirtualSize, optionalHeader.SectionAlignment))
			return sectionIndex;
		sectionIndex++;
	}
...
void PeLib::ImageLoader::setSizeOfCode(std::uint32_t sizeOfCode, std::uint32_t baseOfCode)
{
	if (sizeOfCode != UINT32_MAX) optionalHeader.SizeOfCode = sizeOfCode;
	if (baseOfCode != UINT32_MAX) optionalHeader.BaseOfCode = baseOfCode;
}

void PeLib::ImageLoader::setDataDirectory(std::uint32_t entryIndex, std::uint32_t VirtualAddress, std::uint32_t Size)
{
	if (entryIndex < PELIB_IMAGE_NUMBEROF_DIRECTORY_ENTRIES)
	{
		// Make sure there is enough entries
		if (entryIndex >= optionalHeader.NumberOfRvaAndSizes) optionalHeader.NumberOfRvaAndSizes = entryIndex + 1;

		if (VirtualAddress != UINT32_MAX) optionalHeader.DataDirectory[entryIndex].VirtualAddress = VirtualAddress;
		if (Size != UINT32_MAX) optionalHeader.DataDirectory[entryIndex].Size = Size;
	}
}

PeLib::PELIB_IMAGE_SECTION_HEADER* PeLib::ImageLoader::addSection(const char* name, std::uint32_t sectionSize)
{
	if (optionalHeader.FileAlignment == 0) return nullptr;
	if (optionalHeader.SectionAlignment == 0) return nullptr;
	if (sections.size() >= UINT16_MAX) return nullptr;
	...
	SectHdr.Characteristics = PELIB_IMAGE_SCN_MEM_WRITE | PELIB_IMAGE_SCN_MEM_READ
							| PELIB_IMAGE_SCN_CNT_INITIALIZED_DATA | PELIB_IMAGE_SCN_CNT_CODE;
...
void PeLib::ImageLoader::calcNewSectionAddresses(std::uint32_t& Rva, std::uint32_t& RawOffset)
{
...
void PeLib::ImageLoader::setSectionVirtualRange(
	std::size_t sectionIndex, std::uint32_t VirtualAddress, std::uint32_t VirtualSize)
{
	if (sectionIndex < sections.size())
	{
...
void PeLib::ImageLoader::setSectionRawDataRange(
	std::size_t sectionIndex, std::uint32_t PointerToRawData, std::uint32_t SizeOfRawData)
{
	if (sectionIndex < sections.size())
	{
...
void PeLib::ImageLoader::setSectionCharacteristics(std::size_t sectionIndex, std::uint32_t Characteristics)
{
	if (sectionIndex < sections.size())
	{
...
{
	if (!optionalHeader.FileAlignment) return PeLib::ERROR_NO_FILE_ALIGNMENT;
	...
	if (sectionIndex > sections.size()) return PeLib::ERROR_ENTRY_NOT_FOUND;

	// Offset at which the section is going to be split must be multiple of section alignment
	if (splitOffset & (getSectionAlignment() - 1)) return PeLib::ERROR_NOT_ENOUGH_SPACE;
	...
	if (splitOffset >= getSectionHeader(sectionIndex)->VirtualSize) return PeLib::ERROR_NOT_ENOUGH_SPACE;
	...
	setSectionRawDataRange(
		sectionIndex + 1, sections[sectionIndex].PointerToRawData + splitOffset, originalSize - splitOffset);
	setSectionVirtualRange(
		sectionIndex + 1, sections[sectionIndex].VirtualAddress + splitOffset, originalSize - splitOffset);
	setSectionCharacteristics(
		sectionIndex + 1,
		PeLib::PELIB_IMAGE_SCN_MEM_WRITE | PeLib::PELIB_IMAGE_SCN_MEM_READ | PeLib::PELIB_IMAGE_SCN_CNT_INITIALIZED_DATA
...
void PeLib::ImageLoader::enlargeLastSection(std::uint32_t sizeIncrement)
{
	if (sections.size())
	{
		...
		lastSection.VirtualSize = lastSection.SizeOfRawData =
			AlignToSize(lastSection.SizeOfRawData + sizeIncrement, getFileAlignment());
...
{
	if (sectionIndex >= getNumberOfSections()) return ERROR_ENTRY_NOT_FOUND;
...
void PeLib::ImageLoader::makeValid()
{
	...
	fileHeader.Characteristics = (fileHeader.Characteristics != 0)
								   ? fileHeader.Characteristics
								   : PELIB_IMAGE_FILE_EXECUTABLE_IMAGE | PELIB_IMAGE_FILE_32BIT_MACHINE;

	// Fix the IMAGE_OPTIONAL_HEADER
	optionalHeader.Magic =
		(imageBitability == 64) ? PELIB_IMAGE_NT_OPTIONAL_HDR64_MAGIC : PELIB_IMAGE_NT_OPTIONAL_HDR32_MAGIC;
	...
	sizeOfHeaders = dosHeader.e_lfanew + sizeof(PELIB_IMAGE_NT_SIGNATURE) + sizeof(PELIB_IMAGE_FILE_HEADER)
				  + fileHeader.SizeOfOptionalHeader + fileHeader.NumberOfSections * sizeof(PELIB_IMAGE_SECTION_HEADER);
	...
	for (std::uint16_t i = 0; i < fileHeader.NumberOfSections; i++)
	{
		...
		if (dwOffsetDiff) setSectionRawDataRange(i, pSectionHeader->PointerToRawData + dwOffsetDiff);
	}
...
int PeLib::ImageLoader::Load(ByteBuffer& fileData, std::uint32_t loadFlags)
{
	int fileError;
	...
	if (fileError != ERROR_NONE) return fileError;
	...
	if (fileError != ERROR_NONE || ldrError == LDR_ERROR_NTHEADER_OUT_OF_FILE) return fileError;
...
int PeLib::ImageLoader::Load(std::istream& fs, std::streamoff fileOffset, std::uint32_t loadFlags)
{
	ByteBuffer fileData;
	...
	if (fileOffset > fileSize) return ERROR_INVALID_FILE;

	// Windows loader refuses to load any file which is larger than 0xFFFFFFFF
	if (((fileSize - fileOffset) >> 32) != 0) return setLoaderError(LDR_ERROR_FILE_TOO_BIG);
	...
	if ((fileError = verifyDosHeader(fs, fileOffset, fileSize2)) != ERROR_NONE) return fileError;
...
int PeLib::ImageLoader::Load(const char* fileName, std::uint32_t loadFlags)
{
	std::ifstream fs(fileName, std::ifstream::in | std::ifstream::binary);
	if (!fs.is_open()) return ERROR_OPENING_FILE;
...
int PeLib::ImageLoader::Save(std::ostream& fs, std::streamoff fileOffset, std::uint32_t saveFlags)
{
	int fileError;
	...
	if (saveFlags & IoFlagNewFile)
	{
		...
		// Check and capture section headers
		fileOffset = fileOffset + dosHeader.e_lfanew + sizeof(PELIB_IMAGE_NT_SIGNATURE)
				   + sizeof(PELIB_IMAGE_FILE_HEADER) + fileHeader.SizeOfOptionalHeader;
...
int PeLib::ImageLoader::Save(const char* fileName, std::uint32_t saveFlags)
{
...
void PeLib::ImageLoader::readFromPage(
	PELIB_FILE_PAGE& page, void* buffer, std::size_t offsetInPage, std::size_t bytesInPage)
{
...
std::uint32_t
PeLib::ImageLoader::readWriteImage(void* buffer, std::uint32_t rva, std::uint32_t bytesToRead, READWRITE ReadWrite)
{
	std::uint32_t bytesRead = 0;
	...
	if (rva < rvaEnd)
	{
		...
		while (rva < rvaEnd)
		{
			...
			// Perhaps the last page loaded?
			if (bytesInPage > (rvaEnd - rva)) bytesInPage = (rvaEnd - rva);
...
std::uint32_t
PeLib::ImageLoader::readWriteImageFile(void* buffer, std::uint32_t rva, std::uint32_t bytesToRead, bool bReadOperation)
{
	...
	if ((fileOffset + bytesToRead) > rawFileData.size()) bytesToRead = (std::uint32_t)(rawFileData.size() - fileOffset);
...
#define INS_IMM64(Value, TargetValue32, Size, InstPos, ValPos) \
	TargetValue32 =                                            \
		(TargetValue32 & ~(((1 << Size) - 1) << InstPos))      \
		| ((std::uint32_t)((((std::uint64_t)Value >> ValPos) & (((std::uint64_t)1 << Size) - 1))) << InstPos)

bool PeLib::ImageLoader::processImageRelocation_IA64_IMM64(std::uint32_t fixupAddress, std::uint64_t difference)
{
	...
	if (readImage(BundleBlock, fixupAddress, sizeof(BundleBlock)) != sizeof(BundleBlock)) return false;
	...
	EXT_IMM64(
		Value64,
		BundleBlock[EMARCH_ENC_I17_IMM7B_INST_WORD_X],
		EMARCH_ENC_I17_IMM7B_SIZE_X,
		EMARCH_ENC_I17_IMM7B_INST_WORD_POS_X,
		EMARCH_ENC_I17_IMM7B_VAL_POS_X);
	EXT_IMM64(
		Value64,
		BundleBlock[EMARCH_ENC_I17_IMM9D_INST_WORD_X],
		EMARCH_ENC_I17_IMM9D_SIZE_X,
		EMARCH_ENC_I17_IMM9D_INST_WORD_POS_X,
		EMARCH_ENC_I17_IMM9D_VAL_POS_X);
	EXT_IMM64(
		Value64,
		BundleBlock[EMARCH_ENC_I17_IMM5C_INST_WORD_X],
		EMARCH_ENC_I17_IMM5C_SIZE_X,
		EMARCH_ENC_I17_IMM5C_INST_WORD_POS_X,
		EMARCH_ENC_I17_IMM5C_VAL_POS_X);
	EXT_IMM64(
		Value64,
		BundleBlock[EMARCH_ENC_I17_IC_INST_WORD_X],
		EMARCH_ENC_I17_IC_SIZE_X,
		EMARCH_ENC_I17_IC_INST_WORD_POS_X,
		EMARCH_ENC_I17_IC_VAL_POS_X);
	EXT_IMM64(
		Value64,
		BundleBlock[EMARCH_ENC_I17_IMM41a_INST_WORD_X],
		EMARCH_ENC_I17_IMM41a_SIZE_X,
		EMARCH_ENC_I17_IMM41a_INST_WORD_POS_X,
		EMARCH_ENC_I17_IMM41a_VAL_POS_X);
	EXT_IMM64(
		Value64,
		BundleBlock[EMARCH_ENC_I17_IMM41b_INST_WORD_X],
		EMARCH_ENC_I17_IMM41b_SIZE_X,
		EMARCH_ENC_I17_IMM41b_INST_WORD_POS_X,
		EMARCH_ENC_I17_IMM41b_VAL_POS_X);
	EXT_IMM64(
		Value64,
		BundleBlock[EMARCH_ENC_I17_IMM41c_INST_WORD_X],
		EMARCH_ENC_I17_IMM41c_SIZE_X,
		EMARCH_ENC_I17_IMM41c_INST_WORD_POS_X,
		EMARCH_ENC_I17_IMM41c_VAL_POS_X);
	EXT_IMM64(
		Value64,
		BundleBlock[EMARCH_ENC_I17_SIGN_INST_WORD_X],
...
bool PeLib::ImageLoader::processImageRelocations(
	std::uint64_t oldImageBase, std::uint64_t newImageBase, std::uint32_t VirtualAddress, std::uint32_t Size)
{
	...
	if (buffer != nullptr)
	{
		...
		while ((bufferPtr + sizeof(PELIB_IMAGE_BASE_RELOCATION)) <= bufferEnd)
		{
			...
			for (std::uint32_t i = 0; i < numRelocations; i++)
			{
				...
				case PELIB_IMAGE_REL_BASED_ABSOLUTE: break;
...
int PeLib::ImageLoader::captureDosHeader(ByteBuffer& fileData)
{
	...
	if ((fileBegin + sizeof(PELIB_IMAGE_DOS_HEADER)) >= fileEnd) return ERROR_INVALID_FILE;
...
int PeLib::ImageLoader::saveToFile(std::ostream& fs, std::streamoff fileOffset, std::size_t rva, std::size_t length)
{
...
int PeLib::ImageLoader::saveDosHeaderNew(std::ostream& fs, std::streamoff fileOffset)
{
...
int PeLib::ImageLoader::saveDosHeader(std::ostream& fs, std::streamoff fileOffset)
{
	...
	if (dosHeader.e_lfanew > PELIB_PAGE_SIZE * 10) return ERROR_INVALID_FILE;
...
int PeLib::ImageLoader::captureNtHeaders(ByteBuffer& fileData)
{
	...
	{
		std::uint32_t minFileSize = dosHeader.e_lfanew + sizeof(std::uint32_t) + sizeof(PELIB_IMAGE_FILE_HEADER)
								  + sizeof(PELIB_IMAGE_OPTIONAL_HEADER32);

		if ((fileBegin + minFileSize) > fileEnd) return setLoaderError(LDR_ERROR_NTHEADER_OUT_OF_FILE);
	}
	...
	if (fileHeader.NumberOfSections > maxSectionCount) setLoaderError(LDR_ERROR_IMAGE_NON_EXECUTABLE);
	...
	if ((dosHeader.e_lfanew + ntHeaderSize) < dosHeader.e_lfanew) setLoaderError(LDR_ERROR_NTHEADER_OFFSET_OVERFLOW);
	...
	if (architectureSpecificChecks && checkForBadArchitectureSpecific()) setLoaderError(LDR_ERROR_IMAGE_NON_EXECUTABLE);
	...
	if (optionalHeader.FileAlignment == 0) setLoaderError(LDR_ERROR_FILE_ALIGNMENT_ZERO);
	...
	if (optionalHeader.SectionAlignment == 0) setLoaderError(LDR_ERROR_SECTION_ALIGNMENT_ZERO);
	...
	if (optionalHeader.SizeOfImage > PELIB_MM_SIZE_OF_LARGEST_IMAGE) setLoaderError(LDR_ERROR_SIZE_OF_IMAGE_TOO_BIG);
	...
	if (optionalHeader.SizeOfHeaders > optionalHeader.SizeOfImage) setLoaderError(LDR_ERROR_SIZE_OF_HEADERS_INVALID);
	...
	if (BytesToPages(optionalHeader.SizeOfImage) == 0) setLoaderError(LDR_ERROR_SIZE_OF_IMAGE_ZERO);

	// Check for proper alignment of the image base
	if (optionalHeader.ImageBase & (PELIB_SIZE_64KB - 1)) setLoaderError(LDR_ERROR_IMAGE_BASE_NOT_ALIGNED);

	return ERROR_NONE;
}

int PeLib::ImageLoader::saveNtHeadersNew(std::ostream& fs, std::streamoff fileOffset)
{
	// Calculate the size of the optional header. Any version of PE file,
	// 32 or 64-bit, must have this field set to a correct value.
	std::uint32_t sizeOfOptionalHeader = getFieldOffset(PELIB_MEMBER_TYPE::OPTHDR_sizeof_fixed)
									   + optionalHeader.NumberOfRvaAndSizes * sizeof(PELIB_IMAGE_DATA_DIRECTORY);
	...
	if (optionalHeader.Magic == PELIB_IMAGE_NT_OPTIONAL_HDR32_MAGIC)
	{
		...
		if ((optionalHeader.ImageBase >> 0x20) != 0) return ERROR_INVALID_FILE;
...
int PeLib::ImageLoader::saveNtHeaders(std::ostream& fs, std::streamoff fileOffset)
{
	// Calculate the size of the optional header. Any version of PE file,
	// 32 or 64-bit, must have this field set to a correct value.
	std::size_t sizeOfOptionalHeader = getFieldOffset(PELIB_MEMBER_TYPE::OPTHDR_sizeof_fixed)
									 + optionalHeader.NumberOfRvaAndSizes * sizeof(PELIB_IMAGE_DATA_DIRECTORY);
	...
	if (sizeOfHeaders > PELIB_PAGE_SIZE * 10) return ERROR_INVALID_FILE;
	return saveToFile(fs, fileOffset, dosHeader.e_lfanew, sizeOfHeaders);
}

int PeLib::ImageLoader::captureSectionName(ByteBuffer& fileData, std::string& sectionName, const std::uint8_t* Name)
{
	...
	if (fileHeader.PointerToSymbolTable != 0 && Name[0] == '/')
	{
		...
		std::uint32_t stringTableOffset =
			fileHeader.PointerToSymbolTable + fileHeader.NumberOfSymbols * PELIB_IMAGE_SIZEOF_COFF_SYMBOL;
		...
		if (readStringRaw(
				fileData, sectionName, stringTableOffset + stringTableIndex, PELIB_IMAGE_SIZEOF_MAX_NAME, true, true)
			!= 0)
			return ERROR_NONE;
	}
...
int PeLib::ImageLoader::captureSectionHeaders(ByteBuffer& fileData)
{
	...
	if (fileHeader.NumberOfSections == 0) return ERROR_NONE;

	// Check whether the sections are within the file
	filePtr = fileBegin + dosHeader.e_lfanew + sizeof(std::uint32_t) + sizeof(PELIB_IMAGE_FILE_HEADER)
			+ fileHeader.SizeOfOptionalHeader;
	if (filePtr > fileEnd) return setLoaderError(LDR_ERROR_SECTION_HEADERS_OUT_OF_IMAGE);

	// Set the counters
	std::uint32_t NumberOfSectionPTEs =
		AlignToSize(optionalHeader.SizeOfHeaders, optionalHeader.SectionAlignment) / PELIB_PAGE_SIZE;
	...
	if (!SingleSubsection)
	{
		...
		if (NumberOfSectionPTEs > NumberOfPTEs) setLoaderError(LDR_ERROR_SIZE_OF_HEADERS_INVALID);
	...
	for (std::uint16_t i = 0; i < fileHeader.NumberOfSections; i++)
	{
		...
		if ((filePtr + sizeof(PELIB_IMAGE_SECTION_HEADER)) > fileEnd) break;
		...
		// Overflow check
		if ((PointerToRawData + sectHdr.SizeOfRawData) < PointerToRawData) setLoaderError(LDR_ERROR_RAW_DATA_OVERFLOW);
		...
		else
		{
			...
			if (NextVirtualAddress != sectHdr.VirtualAddress) setLoaderError(LDR_ERROR_INVALID_SECTION_VA);
			...
			// Check section size
			if ((VirtualSize + (PELIB_PAGE_SIZE - 1)) <= VirtualSize) setLoaderError(LDR_ERROR_INVALID_SECTION_VSIZE);
			...
			if (NumberOfSectionPTEs > NumberOfPTEs) setLoaderError(LDR_ERROR_INVALID_SECTION_VSIZE);

			NumberOfPTEs -= NumberOfSectionPTEs;

			// Check end of the raw data for the section
			if (((PointerToRawData + sectHdr.SizeOfRawData + FileAlignmentMask) & ~FileAlignmentMask)
				< PointerToRawData)
		...
		if (PointerToRawData != 0 && (fileBegin + EndOfRawData) > fileEnd) bRawDataBeyondEOF = true;
	...
	{
		std::uint32_t ThresholdNumberOfPTEs =
			(SingleSubsection == false) ? (optionalHeader.SectionAlignment / PELIB_PAGE_SIZE) : 1;
...
int PeLib::ImageLoader::saveSectionHeadersNew(std::ostream& fs, std::streamoff fileOffset)
{
...
{
	std::size_t offsetOfHeaders =
		dosHeader.e_lfanew + sizeof(std::uint32_t) + sizeof(PELIB_IMAGE_FILE_HEADER) + fileHeader.SizeOfOptionalHeader;
...
int PeLib::ImageLoader::captureImageSections(ByteBuffer& fileData, std::uint32_t loadFlags)
{
	...
	if (optionalHeader.SectionAlignment >= PELIB_PAGE_SIZE)
	{
		...
		virtualAddress = captureImageSection(
			fileData, virtualAddress, sizeOfHeaders, 0, sizeOfHeaders, PELIB_IMAGE_SCN_MEM_READ, true);
		if (virtualAddress == 0) return ERROR_INVALID_FILE;
		...
		if (sections.size() != 0)
		{
			for (auto& sectionHeader: sections)
			{
				...
				std::uint32_t pointerToRawData =
					(loadFlags & IoFlagLoadAsImage) ? sectionHeader.VirtualAddress : sectionHeader.PointerToRawData;
				std::uint32_t sectionEnd;

				// Capture all pages from the section
				sectionEnd = captureImageSection(
					fileData,
		...
		else
		{
			...
			if (virtualAddress > optionalHeader.SizeOfImage
				|| (optionalHeader.SizeOfImage - virtualAddress) > optionalHeader.SectionAlignment)
			{
				setLoaderError(LDR_ERROR_INVALID_SIZE_OF_IMAGE);
			}
		}
	...
	else
	{
		...
		if (is64BitWindows && alignSingleSectionImagesToPage) sizeOfImage = AlignToSize(sizeOfImage, PELIB_PAGE_SIZE);
		if (sizeOfImage < PELIB_PAGE_SIZE) sizeOfImage = PELIB_PAGE_SIZE;
		...
		virtualAddress = captureImageSection(
			...
			0,
			sizeOfImage,
			PELIB_IMAGE_SCN_MEM_WRITE | PELIB_IMAGE_SCN_MEM_READ | PELIB_IMAGE_SCN_MEM_EXECUTE,
			true);
		if (virtualAddress == 0) return ERROR_INVALID_FILE;
	}
...
{
	if (hdr.e_magic != PELIB_IMAGE_DOS_SIGNATURE) return ERROR_INVALID_FILE;
	if (hdr.e_lfanew & 3) return setLoaderError(LDR_ERROR_E_LFANEW_UNALIGNED);
	if (hdr.e_lfanew > fileSize) return setLoaderError(LDR_ERROR_E_LFANEW_OUT_OF_FILE);
...
int PeLib::ImageLoader::verifyDosHeader(std::istream& fs, std::streamoff fileOffset, std::size_t fileSize)
{
	...
	if ((fileOffset + sizeof(PELIB_IMAGE_DOS_HEADER)) >= fileSize) return ERROR_INVALID_FILE;
	fs.seekg(fileOffset);
	...
	if ((fileError = verifyDosHeader(tempDosHeader, fileSize)) != ERROR_NONE) return fileError;
...
std::uint32_t PeLib::ImageLoader::copyDataDirectories(
	...
	std::size_t optionalHeaderMax, // How many bytes do we have from the beginning of the optional header till the end
								   // of the file
	std::uint32_t numberOfRvaAndSizes)
{
	std::uint8_t* dataDirectoriesEnd =
		dataDirectoriesPtr + PELIB_IMAGE_NUMBEROF_DIRECTORY_ENTRIES * sizeof(PELIB_IMAGE_DATA_DIRECTORY);
...
int PeLib::ImageLoader::captureOptionalHeader64(std::uint8_t* fileBegin, std::uint8_t* filePtr, std::uint8_t* fileEnd)
{
	PELIB_IMAGE_OPTIONAL_HEADER64 optionalHeader64{};
	...
	if ((filePtr + sizeOfOptionalHeader) > fileEnd) sizeOfOptionalHeader = (std::uint32_t)(fileEnd - filePtr);
	memcpy(&optionalHeader64, filePtr, sizeOfOptionalHeader);

	// Verify whether it's 64-bit optional header
	if (optionalHeader64.Magic != PELIB_IMAGE_NT_OPTIONAL_HDR64_MAGIC) return setLoaderError(LDR_ERROR_NO_OPTHDR_MAGIC);
	...
	realNumberOfRvaAndSizes = copyDataDirectories(
		(std::uint8_t*)(&optionalHeader64),
	...
	securityDirFileOffset = (filePtr - fileBegin) + offsetof(PELIB_IMAGE_OPTIONAL_HEADER64, DataDirectory)
						  + (sizeof(PELIB_IMAGE_DATA_DIRECTORY) * PELIB_IMAGE_DIRECTORY_ENTRY_SECURITY);
	return ERROR_NONE;
}

int PeLib::ImageLoader::captureOptionalHeader32(std::uint8_t* fileBegin, std::uint8_t* filePtr, std::uint8_t* fileEnd)
{
	PELIB_IMAGE_OPTIONAL_HEADER32 optionalHeader32{};
	...
	if (optionalHeader32.Magic != PELIB_IMAGE_NT_OPTIONAL_HDR32_MAGIC) setLoaderError(LDR_ERROR_NO_OPTHDR_MAGIC);
	...
	securityDirFileOffset = (filePtr - fileBegin) + offsetof(PELIB_IMAGE_OPTIONAL_HEADER32, DataDirectory)
						  + (sizeof(PELIB_IMAGE_DATA_DIRECTORY) * PELIB_IMAGE_DIRECTORY_ENTRY_SECURITY);
	return ERROR_NONE;
}
	...
	bool isImageHeader)
{
	...
	if (sizeOfRawData > virtualSize) sizeOfRawData = virtualSize;
	...
	if (sizeOfSection < virtualSize) sizeOfSection = virtualSize;
		...
		if (pointerToRawData || isImageHeader)
		{
			...
			while (pageOffset < sizeOfInitializedPages && pageIndex < pages.size())
			{
				...
				if (rawDataPtr < fileEnd)
				{
					...
					if ((rawDataPtr + bytesToCopy) > fileEnd) bytesToCopy = (fileEnd - rawDataPtr);
...
{
	std::uint32_t fileOffsetToSectionHeader =
		dosHeader.e_lfanew + sizeof(std::uint32_t) + sizeof(PELIB_IMAGE_FILE_HEADER) + fileHeader.SizeOfOptionalHeader;
	...
	for (std::size_t i = 0; i < sections.size(); i++, fileOffsetToSectionHeader += sizeof(PELIB_IMAGE_SECTION_HEADER))
	{
		...
		if (sectHdr.SizeOfRawData == 0)
		{
			...
			fileOffsetOfPointerToRawData =
				fileOffsetToSectionHeader + 0x14; // FIELD_OFFSET(PELIB_IMAGE_SECTION_HEADER, PointerToRawData)

			if (fileOffsetOfPointerToRawData <= fileOffset
				&& fileOffset < fileOffsetOfPointerToRawData + sizeof(std::uint32_t))
				return true;
		}
	}
...
{
	if (Machine == PELIB_IMAGE_FILE_MACHINE_I386) return true;
	if (Machine == PELIB_IMAGE_FILE_MACHINE_AMD64) return true;
	return false;
}

bool PeLib::ImageLoader::checkForValid64BitMachine()
{
	if (loadItaniumImages && fileHeader.Machine == PELIB_IMAGE_FILE_MACHINE_IA64) return true;
	if (loadArm64Images && fileHeader.Machine == PELIB_IMAGE_FILE_MACHINE_ARM64) return true;
	return (fileHeader.Machine == PELIB_IMAGE_FILE_MACHINE_AMD64);
}

bool PeLib::ImageLoader::checkForValid32BitMachine()
{
	if (loadArmImages && fileHeader.Machine == PELIB_IMAGE_FILE_MACHINE_ARMNT) return true;
...
bool PeLib::ImageLoader::checkForInvalidImageRange()
{
	...
	if (optionalHeader.Magic == PELIB_IMAGE_NT_OPTIONAL_HDR32_MAGIC)
	{
		...
		if (ImageBase < MmLowestImageBase || ImageBase > MmHighestImageBase
			|| (ImageBase + AlignedSizeOfImage) > MmHighestImageBase)
		{
			return true;
		}
	}
...
bool PeLib::ImageLoader::isValidMachineForCodeIntegrifyCheck(std::uint32_t Bits)
{
	if (Bits & 64)
	{
		...
		if (fileHeader.Machine == PELIB_IMAGE_FILE_MACHINE_AMD64) return true;

		// Returns STATUS_INVALID_IMAGE_FORMAT due to page size being 0x2000
		if (fileHeader.Machine == PELIB_IMAGE_FILE_MACHINE_IA64) return true;

		if (fileHeader.Machine == PELIB_IMAGE_FILE_MACHINE_ARM64) return true;
	}
...
bool PeLib::ImageLoader::checkForBadCodeIntegrityImages(ByteBuffer& fileData)
{
	if (optionalHeader.DllCharacteristics & PELIB_IMAGE_DLLCHARACTERISTICS_FORCE_INTEGRITY)
	{
		...
		std::uint32_t sizeOfNtHeaders =
			sizeof(std::uint32_t) + sizeof(PELIB_IMAGE_FILE_HEADER) + sizeof(PELIB_IMAGE_OPTIONAL_HEADER32);
		std::uint32_t endOfRawData;
		std::size_t peFileSize = fileData.size();

		if (dosHeader.e_lfanew < sizeof(PELIB_IMAGE_DOS_HEADER)) return true;
		if (dosHeader.e_lfanew > optionalHeader.SectionAlignment) return true;
		if ((optionalHeader.SectionAlignment - dosHeader.e_lfanew) <= dosHeader.e_lfanew) return true;
		if ((dosHeader.e_lfanew + sizeOfNtHeaders) > optionalHeader.SectionAlignment) return true;

		if (ntSignature != PELIB_IMAGE_NT_SIGNATURE) return true;
		if (fileHeader.SizeOfOptionalHeader == 0) return true;

		if (!isValidMachineForCodeIntegrifyCheck(32 | 64)) return true;

		if (optionalHeader.MajorLinkerVersion < 3 && optionalHeader.MajorLinkerVersion < 5) return true;
		if (optionalHeader.Magic != PELIB_IMAGE_NT_OPTIONAL_HDR32_MAGIC
		...
		if (optionalHeader.SizeOfHeaders == 0 || optionalHeader.SizeOfHeaders > peFileSize) return true;
		if (optionalHeader.FileAlignment == 0 || (optionalHeader.FileAlignment & (optionalHeader.FileAlignment - 1)))
			return true;
		if (optionalHeader.SectionAlignment & (optionalHeader.SectionAlignment - 1)) return true;
		if (optionalHeader.FileAlignment > optionalHeader.SectionAlignment) return true;
		if ((optionalHeader.FileAlignment & (PELIB_SECTOR_SIZE - 1))
			&& (optionalHeader.FileAlignment != optionalHeader.SectionAlignment))
			return true;

		// End of headers altogether must fit in the first page
		endOfRawData = dosHeader.e_lfanew + sizeof(std::uint32_t) + sizeof(PELIB_IMAGE_FILE_HEADER)
					 + fileHeader.SizeOfOptionalHeader;
		endOfRawData += (fileHeader.NumberOfSections * sizeof(PELIB_IMAGE_SECTION_HEADER));
		if (endOfRawData >= PELIB_PAGE_SIZE) return true;
		...
		for (auto& section: sections)
		{
			...
			if (optionalHeader.Magic == PELIB_IMAGE_NT_OPTIONAL_HDR64_MAGIC)
			{
				if (section.PointerToRawData && section.SizeOfRawData
					&& section.PointerToRawData < optionalHeader.SizeOfHeaders)
				{
					return true;
				}
			}

			if (section.PointerToRawData != 0 && section.PointerToRawData < endOfRawData) return true;
			if ((section.PointerToRawData + section.SizeOfRawData) < section.PointerToRawData) return true;
			if ((section.PointerToRawData + section.SizeOfRawData) > peFileSize) return true;
			if ((section.VirtualAddress + section.SizeOfRawData - 1) < section.SizeOfRawData) return true;
		...
		{
			if (SecurityDir.VirtualAddress < endOfRawData || SecurityDir.VirtualAddress > peFileSize) return true;
			if ((SecurityDir.VirtualAddress + SecurityDir.Size) != peFileSize) return true;
			if ((SecurityDir.VirtualAddress + SecurityDir.Size) < endOfRawData) return true;
			if (SecurityDir.VirtualAddress < optionalHeader.SizeOfHeaders) return true;
			if (SecurityDir.VirtualAddress & 0x03) return true;
		}
		...
		if (forceIntegrityCheckCertificate)
		{
			...
			if (SecurityDir.Size > 2 && certPtr[0] == 0 && certPtr[1] == 0) return true;
		}
	}
...
bool PeLib::ImageLoader::checkForBadLoadConfigXX(std::uint32_t loadConfigRva, std::uint32_t loadConfigSize)
{
	...
	if (loadConfigSize > sizeof(LOAD_CONFIG)) loadConfigSize = sizeof(LOAD_CONFIG);

	// Load the load config directory
	if (readImage(&LoadConfig, loadConfigRva, loadConfigSize) == loadConfigSize)
	{
		if (LoadConfig.DynamicValueRelocTableSection >= fileHeader.NumberOfSections) return true;

		if (LoadConfig.GuardCFFunctionTable > 0)
		{
			if (LoadConfig.GuardCFFunctionTable < optionalHeader.ImageBase) return true;
			if (LoadConfig.GuardCFFunctionCount == 0) return true;
		}
	}
...
bool PeLib::ImageLoader::checkForImageAfterMapping()
{
	...
	if (!isLegacyImageArchitecture(fileHeader.Machine))
	{
		...
		{
			if (readImage(&BaseReloc, baseRelocRVA, sizeof(PELIB_IMAGE_BASE_RELOCATION))
				== sizeof(PELIB_IMAGE_BASE_RELOCATION))
			{
				if (baseRelocSize >= sizeof(PELIB_IMAGE_BASE_RELOCATION) + sizeof(std::uint16_t))
				{
					if (BaseReloc.SizeOfBlock > baseRelocSize) return true;
					if (BaseReloc.SizeOfBlock & 0x01) return true;
					if (BaseReloc.SizeOfBlock < sizeof(PELIB_IMAGE_BASE_RELOCATION)) return true;
				}
				else
				{
...
bool PeLib::ImageLoader::checkForSectionTablesWithinHeader(std::uint32_t e_lfanew)
{
	std::uint32_t OffsetToSectionTable =
		sizeof(std::uint32_t) + sizeof(PELIB_IMAGE_FILE_HEADER) + fileHeader.SizeOfOptionalHeader;
	...
	if ((e_lfanew + OffsetToSectionTable + (NumberOfSubsections + 1) * sizeof(PELIB_IMAGE_SECTION_HEADER))
		<= NtHeaderSize)
		return false;
...
bool PeLib::ImageLoader::isImageMappedOk() const
{
	...
	if (!isImageLoadable()) return false;
	if (pages.size() == 0) return false;
	return true;
}
...
{
	if (Rva >= optionalHeader.SizeOfImage || Size >= optionalHeader.SizeOfImage) return false;
	if ((Rva + Size) < Rva) return false;
	if ((Rva + Size) > optionalHeader.SizeOfImage) return false;
	return true;
}
...
std::size_t PeLib::ImageLoader::getMismatchOffset(void* buffer1, void* buffer2, std::uint32_t rva, std::size_t length)
{
	...
	for (std::size_t i = 0; i < length; i++)
	{
		...
		{
			// Windows loader puts 0 in IMAGE_SECTION_HEADER::PointerToRawData if IMAGE_SECTION_HEADER::SizeOfRawData is
			// also zero. However, this is somewhat random - depends on current memory condition, often dissappears when
			// the sample is copied to another location.
			if (isSectionHeaderPointerToRawData(fileOffset + i)) continue;
			...
			if (byteBuffer1[i] == 0xCC) continue;
...
void PeLib::ImageLoader::compareWithWindowsMappedImage(
	PELIB_IMAGE_COMPARE& ImageCompare, void* imageDataPtr, std::uint32_t imageSize)
{
