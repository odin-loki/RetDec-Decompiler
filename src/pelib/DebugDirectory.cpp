...
int DebugDirectory::read(std::istream& inStream, ImageLoader& imageLoader)
{
	...
	for (auto& debugEntry: debugInfo)
	{
		...
		if (inStream.gcount() != debugEntry.idd.SizeOfData) return ERROR_INVALID_FILE;

		if (debugEntry.idd.SizeOfData > 0)
		{
			m_occupiedAddresses.push_back(std::make_pair(
				debugEntry.idd.AddressOfRawData, debugEntry.idd.AddressOfRawData + debugEntry.idd.SizeOfData - 1));
		}
	}
...
void DebugDirectory::read(
	ImageLoader& imageLoader, std::vector<PELIB_IMG_DEBUG_DIRECTORY>& debugInfo, std::uint32_t rva, std::uint32_t size)
{
	...
	const std::size_t maxEntries = static_cast<std::size_t>(fileSize / PELIB_IMAGE_DEBUG_DIRECTORY::size());
	...
	for (std::size_t i = 0; i < entryCount; i++)
	{
		...
		if (bytesRead != sizeof(PELIB_IMAGE_DEBUG_DIRECTORY)) break;
	...
	if (!debugInfo.empty())
	{
		m_occupiedAddresses.emplace_back(rva, rva + entryCount * PELIB_IMAGE_DEBUG_DIRECTORY::size() - 1);
	}
}
