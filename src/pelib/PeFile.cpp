...
int PeFileT::getFileType() const
{
	...
	if ((machine == PELIB_IMAGE_FILE_MACHINE_AMD64 || machine == PELIB_IMAGE_FILE_MACHINE_IA64)
...
LoaderError PeFileT::loaderError() const
{
	...
	if (ldrError == LDR_ERROR_NONE) ldrError = coffSymTab().loaderError();

	// Check errors in import directory
	if (ldrError == LDR_ERROR_NONE) ldrError = impDir().loaderError();

	// Check errors in resource directory
	if (ldrError == LDR_ERROR_NONE) ldrError = resDir().loaderError();

	// Check errors in relocations directory
	if (ldrError == LDR_ERROR_NONE) ldrError = relocDir().loaderError();

	// Check errors in security directory
	if (ldrError == LDR_ERROR_NONE) ldrError = securityDir().loaderError();

	// Check errors in entry point
	if (ldrError == LDR_ERROR_NONE) ldrError = checkEntryPointErrors();

	// If there was a loaded error, we'll check whether
	// the file can't actually be an in-memory version
	if (ldrError != LDR_ERROR_NONE) ldrError = checkForInMemoryLayout(ldrError);
