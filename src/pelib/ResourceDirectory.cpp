	...
	unsigned depth)
{
	...
	if (entry.OffsetToData == 0 && entry.Size == 0) return ERROR_SKIP_RESOURCE; // Be in sync with YARA
	if (entry.OffsetToData > sizeOfImage || entry.Size > sizeOfImage) return ERROR_NONE;
	...
	if (entry.Size > 0) resDir->addOccupiedAddressRange(entry.OffsetToData, entry.OffsetToData + entry.Size - 1);
	return ERROR_NONE;
}
...
void ResourceLeaf::rebuild(OutputBuffer& obBuffer, unsigned int uiOffset, unsigned int uiRva, const std::string&) const
{
	...
	for (unsigned int i = 0; i < m_data.size(); i++)
	{
		...
		if (entry.OffsetToData < uiRva) continue;
...
void ResourceNode::makeValid()
{
	...
	header.NumberOfNamedEntries = static_cast<std::uint16_t>(
		std::count_if(children.begin(), children.end(), [](const auto& i) { return i.isNamedResource(); }));
 ...
 **/
void ResourceNode::rebuild(
	OutputBuffer& obBuffer, unsigned int uiOffset, unsigned int uiRva, const std::string& pad) const
{
	/*		Log::debug() << std::hex << pad << uiOffset << std::endl;
			...
			Log::debug() << std::hex << pad << "header.NumberOfNamedEntries: "  << header.NumberOfNamedEntries <<
	   std::endl; Log::debug() << std::hex << pad << "header.NumberOfIdEntries: "  << header.NumberOfIdEntries <<
	   std::endl;
	*/
	...
	for (unsigned int i = 0; i < children.size(); i++)
	{
		...
		if (children[i].entry.irde.OffsetToData & PELIB_IMAGE_RESOURCE_DATA_IS_DIRECTORY)
			children[i].child->rebuild(
				obBuffer, children[i].entry.irde.OffsetToData & ~PELIB_IMAGE_RESOURCE_DATA_IS_DIRECTORY, uiRva, pad);
		else
	...
	unsigned depth)
{
	...
	if (imageLoader.readImage(&header, uiRva, PELIB_IMAGE_RESOURCE_DIRECTORY::size())
		!= PELIB_IMAGE_RESOURCE_DIRECTORY::size())
	...
	if (uiNumberOfEntries > 0)
	{
		resDir->addOccupiedAddressRange(
			uiRva, uiRva + uiNumberOfEntries * PELIB_IMAGE_RESOURCE_DIRECTORY_ENTRY::size() - 1);
	}
...
/*	/// Returns the size of a resource node.
	unsigned int ResourceNode::size() const
	{
		if (children.size())
		{
			Log::debug() << std::accumulate(children.begin(), children.end(), 0, accumulate<ResourceChild>) <<
   std::endl; return PELIB_IMAGE_RESOURCE_DIRECTORY::size()
...
{
	std::vector<ResourceChild>::iterator Iter =
		std::find_if(m_rnRoot.children.begin(), m_rnRoot.children.end(), [&](const auto& i) {
			return i.equalId(dwResTypeId);
		});
...
{
	auto Iter = std::find_if(m_rnRoot.children.begin(), m_rnRoot.children.end(), [&](const auto& i) {
		return i.equalName(strResTypeName);
	});
	if (Iter != m_rnRoot.children.end())
	{
		return ERROR_DUPLICATE_ENTRY;
...
int ResourceDirectory::removeResourceType(std::uint32_t dwResTypeId)
{
	...
	if (isNamed)
		m_rnRoot.header.NumberOfNamedEntries = static_cast<std::uint16_t>(m_rnRoot.children.size());
	else
		m_rnRoot.header.NumberOfIdEntries = static_cast<std::uint16_t>(m_rnRoot.children.size());
 ...
 **/
void ResourceDirectory::getResourceData(
	std::uint32_t dwResTypeId, std::uint32_t dwResId, std::vector<std::uint8_t>& data) const
{
 ...
 **/
void ResourceDirectory::getResourceData(
	std::uint32_t dwResTypeId, const std::string& strResName, std::vector<std::uint8_t>& data) const
{
 ...
 **/
void ResourceDirectory::getResourceData(
	const std::string& strResTypeName, std::uint32_t dwResId, std::vector<std::uint8_t>& data) const
{
 ...
 **/
void ResourceDirectory::getResourceData(
	const std::string& strResTypeName, const std::string& strResName, std::vector<std::uint8_t>& data) const
{
 ...
 * @param data Vector where the data is stored.
 **/
void ResourceDirectory::getResourceDataByIndex(
	unsigned int uiResTypeIndex, unsigned int uiResIndex, std::vector<std::uint8_t>& data) const
{
 ...
 **/
void ResourceDirectory::setResourceData(
	std::uint32_t dwResTypeId, std::uint32_t dwResId, std::vector<std::uint8_t>& data)
{
 ...
 **/
void ResourceDirectory::setResourceData(
	std::uint32_t dwResTypeId, const std::string& strResName, std::vector<std::uint8_t>& data)
{
 ...
 **/
void ResourceDirectory::setResourceData(
	const std::string& strResTypeName, const std::string& strResName, std::vector<std::uint8_t>& data)
{
 ...
 * @param data The new resource data.
 **/
void ResourceDirectory::setResourceDataByIndex(
	unsigned int uiResTypeIndex, unsigned int uiResIndex, std::vector<std::uint8_t>& data)
{
	ResourceNode* currNode = static_cast<ResourceNode*>(m_rnRoot.children[uiResTypeIndex].child);
	currNode = static_cast<ResourceNode*>(currNode->children[uiResIndex].child);
	ResourceLeaf* currLeaf = static_cast<ResourceLeaf*>(currNode->children[0].child);
 ...
 * @param dwNewResId New ID of the resource.
 **/
void ResourceDirectory::setResourceId(
	std::uint32_t dwResTypeId, const std::string& strResName, std::uint32_t dwNewResId)
{
 ...
 **/
void ResourceDirectory::setResourceId(
	const std::string& strResTypeName, const std::string& strResName, std::uint32_t dwNewResId)
{
 ...
 **/
void ResourceDirectory::setResourceIdByIndex(
	unsigned int uiResTypeIndex, unsigned int uiResIndex, std::uint32_t dwNewResId)
{
	ResourceNode* currNode = static_cast<ResourceNode*>(m_rnRoot.children[uiResTypeIndex].child);
 ...
 **/
void ResourceDirectory::setResourceName(
	std::uint32_t dwResTypeId, std::uint32_t dwResId, const std::string& strNewResName)
{
 ...
 **/
void ResourceDirectory::setResourceName(
	std::uint32_t dwResTypeId, const std::string& strResName, const std::string& strNewResName)
{
 ...
 **/
void ResourceDirectory::setResourceName(
	const std::string& strResTypeName, std::uint32_t dwResId, const std::string& strNewResName)
{
 ...
 **/
void ResourceDirectory::setResourceName(
	const std::string& strResTypeName, const std::string& strResName, const std::string& strNewResName)
{
 ...
 * @param strNewResName New name of the specified resource.
 **/
void ResourceDirectory::setResourceNameByIndex(
	unsigned int uiResTypeIndex, unsigned int uiResIndex, const std::string& strNewResName)
{
	ResourceNode* currNode = static_cast<ResourceNode*>(m_rnRoot.children[uiResTypeIndex].child);
