...
public:
	...
	ProgramOptions(int argc, char* argv[])
	{
		...
		for (int i = 1; i < argc; ++i)
		{
			...
			else if (c == "-m")
			{
				...
				else if (_basicMode == "thumb")
					basicMode = CS_MODE_THUMB;
				else if (_basicMode == "16")
					basicMode = CS_MODE_16;
				else if (_basicMode == "32")
					basicMode = CS_MODE_32;
				else if (_basicMode == "64")
					basicMode = CS_MODE_64;
				else if (_basicMode == "mips3")
					basicMode = CS_MODE_MIPS3;
				else if (_basicMode == "mips32r6")
					basicMode = CS_MODE_MIPS32R6;
				else if (_basicMode == "mips32")
			...
			else if (c == "-e")
			{
				...
				if (_extraMode == "little")
					extraMode = CS_MODE_LITTLE_ENDIAN;
				else if (_extraMode == "mclass")
					extraMode = CS_MODE_MCLASS;
				else if (_extraMode == "v8")
					extraMode = CS_MODE_V8;
				else if (_extraMode == "micro")
	...
	cs_mode getDefaultBasicModeFromArch(cs_arch a)
	{
		...
		case CS_ARCH_MAX:
		case CS_ARCH_ALL:
		default: Log::error() << "Can not get Capstone arch to default Capstone basic mode." << std::endl; exit(1);
		}
	}
...
void printVersion()
{
	...
	Log::info() << std::endl;
	Log::info() << "Capstone version: " << version << " (major: " << major << ", minor: " << minor << ")" << std::endl;
}
...
ks_arch capstoneArchToKeystoneArch(cs_arch a)
{
	...
	case CS_ARCH_MAX:
	case CS_ARCH_ALL:
	default: Log::error() << "Can not convert Capstone arch to Keystone arch." << std::endl; exit(1);
	}
}
