...
bool DVSA::isCarved(const PrologueInfo& info, int64_t off, uint8_t sz) const
{
...
static bool isBaseRegSP(ssa::VarId reg)
{
	...
	return reg == (ssa::VarId)var_recovery::Reg::RSP || reg == (ssa::VarId)var_recovery::Reg::ESP
		|| reg == (ssa::VarId)var_recovery::Reg::SP_ARM64 || reg == (ssa::VarId)var_recovery::Reg::SP_ARM32;
}
...
DVSA::collectAccesses(const ssa::SSAFunction& fn, const PrologueInfo& prologue) const
{
	...
	std::sort(result.begin(), result.end(), [](const FrameAccess& a, const FrameAccess& b) {
...
std::vector<FrameSlot> DVSA::partition(std::vector<FrameAccess>& accesses) const
{
...
DVSA::Result DVSA::run(const ssa::SSAFunction& fn, const PrologueInfo& prologue) const
{
	Result res;
	...
	std::sort(res.slots.begin(), res.slots.end(), [](const FrameSlot& a, const FrameSlot& b) {
