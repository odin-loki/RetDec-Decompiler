...
PrologueInfo PrologueParser::parseSysVx64(const std::vector<RawInstr>& instrs) const
{
	PrologueInfo info;
	...
	for (int i = 0; i < (int)instrs.size() && i < kMaxPrologueInstrs; ++i)
	{
		...
		{
			Reg r = ins.src;
			if (r == Reg::RBX || r == Reg::R12 || r == Reg::R13 || r == Reg::R14 || r == Reg::R15)
			{
...
PrologueInfo PrologueParser::parseWin64(const std::vector<RawInstr>& instrs) const
{
	PrologueInfo info;
	info.abi = ABI::Win64;
	info.arch = Arch::X86_64;
	...
	for (int i = 0; i < (int)instrs.size() && i < kMaxPrologueInstrs; ++i)
	{
		...
		if (ins.op == RawInstr::Op::Push)
		{
			...
			if (r == Reg::RBX || r == Reg::RDI || r == Reg::RSI || r == Reg::R12 || r == Reg::R13 || r == Reg::R14
				|| r == Reg::R15)
			{
				int64_t off = -(pushCount + 1) * 8;
				info.calleeSaves.push_back({r, off});
...
PrologueInfo PrologueParser::parseSysVx32(const std::vector<RawInstr>& instrs) const
{
	PrologueInfo info;
...
PrologueInfo PrologueParser::parseAArch64(const std::vector<RawInstr>& instrs) const
{
	PrologueInfo info;
	...
	for (int i = 0; i < (int)instrs.size() && i < kMaxPrologueInstrs; ++i)
	{
		...
		if (ins.op == RawInstr::Op::StoreRegPair && ins.dst == Reg::X29 && ins.src == Reg::X30 && ins.hasImm)
		{
...
PrologueInfo PrologueParser::parseARM32(const std::vector<RawInstr>& instrs) const
{
	PrologueInfo info;
	...
	for (int i = 0; i < (int)instrs.size() && i < kMaxPrologueInstrs; ++i)
	{
		...
		if (ins.op == RawInstr::Op::Sub && ins.dst == Reg::SP_ARM32 && ins.hasImm)
		{
			spAdjust = ins.imm;
			break;
		}
...
const char* regName(Reg r) noexcept
{
	...
	case Reg::X29: return "x29";
	case Reg::X30: return "x30";
