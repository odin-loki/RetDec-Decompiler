		...
		for (const auto* instr: blk->instrs)
		{
			...
			if (instr->op == ssa::IrInstr::Op::Shl && !instr->uses.empty())
			{
				...
				for (const auto& u: instr->uses)
				{
					...
					if (sv && sv->kind == ssa::ValueKind::Immediate && sv->imm == 1) return true;
				}
			}
			// Mul x, 2. llvm_to_ssa only attaches ConstantInt uses, so
			// a recovered `i * 2` has one Immediate use, not two.
			if (instr->op == ssa::IrInstr::Op::Mul)
			{
				for (const auto& u: instr->uses)
				{
					const auto* sv = fn.value(u.valueId);
					if (sv && sv->kind == ssa::ValueKind::Immediate && sv->imm == 2) return true;
				}
			}
		}
	}
...
RecursiveHalvingEvidence RecursiveHalvingFingerprint::analyse(const ssa::SSAFunction& fn) const
{
...
InsertionSortEvidence InsertionSortFingerprint::analyse(const ssa::SSAFunction& fn) const
{
