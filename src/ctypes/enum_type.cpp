 ...
 */
EnumType::EnumType(const std::string& name, const Values& values): Type(name, 0), values(values) {}

/**
 * @brief Constructs a new enum value.
 */
EnumType::Value::Value(const std::string& name, ValueType value): name(name), value(value) {}

/// Sets default enum value.
const EnumType::Value::ValueType EnumType::DEFAULT_VALUE = std::numeric_limits<Value::ValueType>::min();
...
EnumType::create(const std::shared_ptr<Context>& context, const std::string& name, const Values& values)
{
