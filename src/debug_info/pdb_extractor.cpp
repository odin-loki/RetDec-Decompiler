...
PdbExtractor::PdbExtractor(std::string pdbPath, uint64_t imageBase):
	impl_(std::make_unique<Impl>()), pdbPath_(std::move(pdbPath)), imageBase_(imageBase)
{
#ifdef RETDEC_USE_LLVM_PDB
	llvm::Error e = llvm::pdb::loadDataForPDB(llvm::pdb::PDB_ReaderType::Native, pdbPath_, impl_->session);
	impl_->loaded = !e;
	...
	if (ifs)
	{
		impl_->rawData.assign(std::istreambuf_iterator<char>(ifs), std::istreambuf_iterator<char>());
	...
	const std::vector<uint8_t>& raw, uint32_t blockSize, const std::vector<uint32_t>& blocks, uint32_t streamSize)
{
	...
	for (uint32_t b: blocks)
	{
		...
		uint32_t take = std::min<uint32_t>(blockSize, static_cast<uint32_t>(raw.size() - off));
...
static DebugType simpleType(uint32_t typeIdx)
{
	...
	static const struct
	{
		...
		{0x0003, "void", 0},          {0x0008, "HRESULT", 4},      {0x0010, "signed char", 1},
		{0x0020, "unsigned char", 1}, {0x0030, "char", 1},         {0x0040, "wchar_t", 2},
		{0x0070, "char8_t", 1},       {0x0080, "char16_t", 2},     {0x00a0, "char32_t", 4},
		{0x0068, "char", 1},          {0x0069, "char", 1},         {0x0071, "int8_t", 1},
		{0x0072, "uint8_t", 1},       {0x0073, "int16_t", 2},      {0x0074, "uint16_t", 2},
		{0x0075, "int32_t", 4},       {0x0076, "uint32_t", 4},     {0x0077, "int64_t", 8},
		{0x0078, "uint64_t", 8},      {0x0041, "short", 2},        {0x0042, "unsigned short", 2},
		{0x0074, "int", 4},           {0x0075, "unsigned int", 4}, {0x0076, "long", 4},
		{0x0077, "unsigned long", 4}, {0x0079, "__int128", 16},    {0x007a, "unsigned __int128", 16},
		{0x0040, "float", 4},         {0x0044, "double", 8},       {0x0045, "long double", 10},
		{0x0030, "bool", 1},
	};
...
static void walkSymbolStream(const std::vector<uint8_t>& syms, uint64_t imageBase, DebugGroundTruth& out)
{
	...
	while (p + 4 <= end)
	{
		...
		else if (kind == S_GDATA32 || kind == S_LDATA32)
		{
			...
			if (out.types.find(ds.typind) == out.types.end() && ds.typind < 0x1000)
			{
...
static void walkTpiStream(const std::vector<uint8_t>& tpi, DebugGroundTruth& out)
{
	...
	while (p + 4 <= end)
	{
		...
		if (dt.kind != DebugTypeKind::Unknown || leaf == LF_STRUCTURE || leaf == LF_CLASS || leaf == LF_UNION
			|| leaf == LF_ENUM)
		{
...
bool PdbExtractor::extract(DebugGroundTruth& out)
{
	...
	const MsfSuperBlock* sb = reinterpret_cast<const MsfSuperBlock*>(raw.data());
	...
	if (mapOk && !dirBlockList.empty()) dir = readStream(raw, blockSize, dirBlockList, numDirBytes);
	...
	{
		dir = readStream(raw, blockSize, std::vector<uint32_t>{blockMapAddr}, numDirBytes);
	}
	...
	uint32_t numStreams = r32(dp);
	dp += 4;
	if (numStreams < 5)
	{ // need at least PDB, TPI, DBI, IPI, GSI
	...
	// TPI stream (index 1)
	if (streams.size() > 1 && !streams[1].empty()) walkTpiStream(streams[1], out);
	...
	if (streams.size() > 2 && !streams[2].empty()) walkSymbolStream(streams[2], imageBase_, out);

	// GSI stream (index 4) — public symbols
	if (streams.size() > 4 && !streams[4].empty()) walkSymbolStream(streams[4], imageBase_, out);
