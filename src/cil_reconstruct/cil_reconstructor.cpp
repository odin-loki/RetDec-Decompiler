...
CilReconstructor::CilReconstructor(CilReconstructOptions opts): opts_(std::move(opts)) {}
		...
		for (uint32_t succ: blk.succs)
		{
			...
			if (it != order.end() && it->second < i)
			{
				...
				for (uint32_t lb: loopBody)
				{
					...
					{
						if (!loopSet.count(lsucc)) exitBlocks.push_back(lsucc);
					}
				}
				return true;
			}
	...
	const BcCFG& cfg) const
{
	...
	for (uint32_t lb: loopBlockIds)
	{
		...
		{
			if (s.kind != StmtKind::Goto || s.blockRef != headerBlock)
			{
...
std::vector<CilStmt>
CilReconstructor::buildSwitch(uint32_t switchBlock, std::vector<CilRecoveredBlock>& blocks, const BcCFG& cfg) const
{
	std::vector<CilStmt> stmts;
...
std::vector<CilStmt> CilReconstructor::structureEH(
	const std::vector<uint32_t>& blockIds, std::vector<CilRecoveredBlock>& blocks, const BcCFG& cfg) const
{
	std::vector<CilStmt> stmts;
	...
	for (const auto& group: grouped)
	{
		...
		for (const auto* eh: group)
		{
			...
			else
			{
				...
				if (eh->catchType.has_value()) cc.catchType = *eh->catchType;
				cc.body = std::move(handlerBody);
				tryStmt.catches.push_back(std::move(cc));
			}
		}

		if (!tryStmt.tryBody.empty() || !tryStmt.catches.empty() || !tryStmt.finallyBody.empty())
		{
...
std::vector<CilStmt> CilReconstructor::structureRegion(
	const std::vector<uint32_t>& blockIds, std::vector<CilRecoveredBlock>& blocks, const BcCFG& cfg, int depth) const
{
	std::vector<CilStmt> stmts;
...
std::vector<CilStmt> CilReconstructor::structureBlocks(std::vector<CilRecoveredBlock>& blocks, const BcCFG& cfg) const
{
...
CilReconstructResult CilReconstructor::reconstruct(const BcMethod& method, const BcModule& module) const
{
...
std::unordered_map<std::string, CilReconstructResult> CilReconstructor::reconstructAll(const BcModule& module) const
{
