...
JavaClassEmitter::JavaClassEmitter(
	...
	const ClassEmitOptions& opts): imports_(imports), tyPrinter_(tyPrinter), reconMap_(reconMap), opts_(opts)
{}
...
std::string JavaClassEmitter::emitAnnotationValue(const BcAnnotationValue& val) const
{
	...
	case BcAnnotationValue::Kind::Enum: return val.enumTypeName + "." + val.enumConstant;
	...
	case BcAnnotationValue::Kind::Annotation: return "@" + val.stringValue;
	default: return "/* ? */";
	}
}
...
void JavaClassEmitter::emitAnnotations(const std::vector<BcAnnotation>& anns, CodeWriter& writer)
{
...
std::string JavaClassEmitter::canonicalModifiers(BcAccess access, bool isInterface, bool isField, bool isMethod)
{
	std::string out;
...
std::string JavaClassEmitter::modifiersFor(BcAccess access, bool isInterface, bool isField, bool isMethod) const
{
...
void JavaClassEmitter::emitField(const BcField& field, const BcClass& /*cls*/, CodeWriter& writer)
{
	if (!opts_.emitSyntheticFields && hasFlag(field.access, BcAccess::Synthetic)) return;
...
std::string JavaClassEmitter::reconKey(const BcMethod& method)
{
	return method.name + method.descriptor.jvmDescriptor();
}

const ReconstructResult* JavaClassEmitter::reconFor(const BcMethod& method) const
{
	if (!reconMap_) return nullptr;
	auto it = reconMap_->find(reconKey(method));
	return (it != reconMap_->end()) ? &it->second : nullptr;
}

std::string JavaClassEmitter::buildParamList(const BcMethod& method, const ReconstructResult* recon) const
{
	std::string out;
	const BcFuncType& desc = method.descriptor;

	size_t startIdx = hasFlag(method.access, BcAccess::Static) ? 0 : 1;
	// `this` is slot 0 on an instance method; parameters follow, each taking
	// one slot or two.
	int slotCursor = static_cast<int>(startIdx);

	for (size_t i = 0; i < desc.params.size(); ++i)
	{
		if (i) out += ", ";
		std::string ptype = desc.params[i] ? tyPrinter_.print(*desc.params[i]) : "Object";

		// Get parameter name.
		std::string pname;
		if (recon && !recon->locals.locals.empty())
		{
			// Look for the param local at this parameter's JVM slot.
			//
			// The cursor advances by the parameter's slot width, not by one:
			// long and double occupy two JVM local slots, exactly as
			// LocalRebuilder assigns them. Computing the slot as startIdx + i
			// meant that for any method with a wide parameter that is not last,
			// the lookup missed for every parameter after it, pname fell back
			// to "p<i>", and the emitted signature declared an identifier the
			// body -- which resolves names through the real slot -- never used.
			uint32_t slot = static_cast<uint32_t>(slotCursor);
		...
		slotCursor += desc.params[i] ? desc.params[i]->jvmSlots() : 1;
		if (pname.empty())
		{
		...
		if (i + 1 == desc.params.size() && hasFlag(method.access, BcAccess::VarArgs) && ptype.size() >= 2
			&& ptype.compare(ptype.size() - 2, 2, "[]") == 0)
...
void JavaClassEmitter::emitMethodSignature(const BcMethod& method, const BcClass& cls, CodeWriter& writer)
{
	...
	{
		retType = (method.descriptor.returnType ? tyPrinter_.print(*method.descriptor.returnType) : "void") + " ";
	}
	...
	std::string params = method.isStaticInit ? "" : buildParamList(method, recon);
...
void JavaClassEmitter::emitMethod(const BcMethod& method, const BcClass& cls, CodeWriter& writer)
{
	// Skip bridge/synthetic if requested.
	if (!opts_.emitBridgeMethods && hasFlag(method.access, BcAccess::Bridge)) return;
	if (hasFlag(method.access, BcAccess::Synthetic) && !method.isStaticInit && !method.isConstructor) return;
	...
	if (!method.cfg.blocks().empty())
	{
		...
		if (recon)
		{
			JavaStmtEmitter stmtEmit(method, *recon, tyPrinter_, opts_.stmtOpts);
			...
			for (size_t i = 0; i < lines.size(); ++i)
			{
				...
				if (l.size() >= 4 && l.substr(0, 4) == "    ") l = l.substr(4);
				writer.writeLine(l);
			}
		}
		else
...
{
	if (!cls.sourceFile.empty()) writer.writeLine("// SourceFile: " + cls.sourceFile);
	...
	if (cls.isAnnotation)
		keyword = "@interface";
	else if (cls.isModule)
		keyword = "module";
	else if (cls.isInterface)
		keyword = "interface";
	else if (cls.isEnum)
		keyword = "enum";
	else if (cls.isRecord && opts_.javaVersion >= 16)
		keyword = "record";
	else
		keyword = "class";
	...
	if (cls.superClass && !cls.isInterface)
	{
		...
		if (superName != "Object" && superName != "java.lang.Object") extendsClause = " extends " + superName;
	}
	...
	std::string header = (mods.empty() ? "" : mods + " ") + keyword + " " + cls.name + typeParams;
	...
	if (!cls.permittedSubclasses.empty())
	{
		...
		{
			if (i) permitsClause += ", ";
			permitsClause += tyPrinter_.print(types::Class(cls.permittedSubclasses[i]));
		}
	}
...
void JavaClassEmitter::emitEnumConstants(const BcClass& cls, CodeWriter& writer)
{
	...
	for (const auto& f: cls.fields)
	{
		if (hasFlag(f.access, BcAccess::Static) && hasFlag(f.access, BcAccess::Final))
		{
			anyConstants = true;
			break;
		}
	...
	for (const auto& f: cls.fields)
	{
		if (!hasFlag(f.access, BcAccess::Static) || !hasFlag(f.access, BcAccess::Final)) continue;
		if (!first) constants += ",\n    ";
		first = false;
		constants += f.name;
	}
	if (!constants.empty()) writer.writeLine("    " + constants + ";");
}

// ─── Record components ────────────────────────────────────────────────────────

void JavaClassEmitter::emitRecordComponents(const BcClass& cls, CodeWriter& writer)
{
	(void)cls;
	(void)writer;
...
void JavaClassEmitter::emitFields(const BcClass& cls, CodeWriter& writer)
{
	...
	{
		if (cls.isEnum && hasFlag(field.access, BcAccess::Static) && hasFlag(field.access, BcAccess::Final))
			continue;                                                           // Already emitted as enum constants.
		if (cls.isRecord && !hasFlag(field.access, BcAccess::Static)) continue; // Record components live in the header.
...
void JavaClassEmitter::emitMethods(const BcClass& cls, CodeWriter& writer)
{
	...
	{
		if (cls.isRecord && method.isConstructor) continue;
...
void JavaClassEmitter::emitInnerClasses(const BcClass& /*cls*/, CodeWriter& /*writer*/)
{
...
void JavaClassEmitter::emitClass(const BcClass& cls, CodeWriter& writer, int depth)
{
	emitClassHeader(cls, writer);
	writer.indent();

	if (cls.isEnum) emitEnumConstants(cls, writer);
