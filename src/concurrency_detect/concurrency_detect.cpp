...
std::vector<CallEntry> extractCalls(const retdec::ssa::BasicBlock& blk)
{
	...
	{
		if (!instr) continue;
		if (instr->op == retdec::ssa::IrInstr::Op::Call && !instr->calleeName.empty())
		{
...
bool tryParseAtomicBuiltin(const std::string& name, uint64_t vma, AtomicEntry& ae)
{
	ae.address = vma;

	// GCC/Clang __atomic_* builtins
	if (name.find("__atomic_load") != std::string::npos)
	{
		ae.op = "load";
	}
	else if (name.find("__atomic_store") != std::string::npos)
	{
		ae.op = "store";
	}
	else if (name.find("__atomic_exchange") != std::string::npos)
	{
		ae.op = "exchange";
	}
	else if (name.find("__atomic_compare_exchange") != std::string::npos)
	{
		ae.op = "cmpxchg";
	}
	else if (name.find("__atomic_fetch_add") != std::string::npos)
	{
		ae.op = "add";
	}
	else if (name.find("__atomic_fetch_sub") != std::string::npos)
	{
		ae.op = "sub";
	}
	else if (name.find("__atomic_fetch_and") != std::string::npos)
	{
		ae.op = "and";
	}
	else if (name.find("__atomic_fetch_or") != std::string::npos)
	{
		ae.op = "or";
	}
	else if (name.find("__atomic_fetch_xor") != std::string::npos)
	{
		ae.op = "xor";
	}
	else if (name.find("__atomic_fetch_nand") != std::string::npos)
	{
		ae.op = "nand";
	}
	// Legacy GCC __sync_* builtins
	else if (name.find("__sync_fetch_and_add") != std::string::npos)
	{
		ae.op = "add";
	}
	else if (name.find("__sync_fetch_and_sub") != std::string::npos)
	{
		ae.op = "sub";
	}
	else if (name.find("__sync_bool_compare_and_swap") != std::string::npos)
	{
		ae.op = "cmpxchg";
	}
	else if (name.find("__sync_val_compare_and_swap") != std::string::npos)
	{
		ae.op = "cmpxchg";
	}
	else if (name.find("__sync_lock_test_and_set") != std::string::npos)
	{
		ae.op = "exchange";
	}
	else if (name.find("__sync_lock_release") != std::string::npos)
	{
		ae.op = "store";
	}
	// C11 stdatomic.h / libatomic entry points
	else if (name.find("atomic_fetch_add") != std::string::npos)
	{
		ae.op = "add";
	}
	else if (name.find("atomic_fetch_sub") != std::string::npos)
	{
		ae.op = "sub";
	}
	else if (name.find("atomic_fetch_and") != std::string::npos)
	{
		ae.op = "and";
	}
	else if (name.find("atomic_fetch_or") != std::string::npos)
	{
		ae.op = "or";
	}
	else if (name.find("atomic_fetch_xor") != std::string::npos)
	{
		ae.op = "xor";
	}
	else if (name.find("atomic_compare_exchange") != std::string::npos)
	{
		ae.op = "cmpxchg";
	}
	else if (name.find("atomic_exchange") != std::string::npos)
	{
		ae.op = "exchange";
	}
	else if (name.find("atomic_load") != std::string::npos)
	{
		ae.op = "load";
	}
	else if (name.find("atomic_store") != std::string::npos)
	{
		ae.op = "store";
	}
	else
	...
	if (name.find("_relaxed") != std::string::npos)
		ae.order = "relaxed";
	else if (name.find("_acquire") != std::string::npos)
		ae.order = "acquire";
	else if (name.find("_release") != std::string::npos)
		ae.order = "release";
	else if (name.find("_acq_rel") != std::string::npos)
		ae.order = "acq_rel";
	else if (name.find("_seq_cst") != std::string::npos)
		ae.order = "seq_cst";
	else
		ae.order = "seq_cst"; // conservative
...
std::vector<AtomicEntry> extractAtomics(const retdec::ssa::BasicBlock& blk)
{
	...
	for (const retdec::ssa::IrInstr* instr: blk.instrs)
	{
		...
		AtomicEntry ae;
		if (tryParseAtomicBuiltin(instr->calleeName, instr->vma, ae)) result.push_back(ae);
	}
	return result;
}

/// The most recent lock recorded in *this* function that has no unlock yet.
///
/// The pairing used to be `out.locks.back()` -- the last lock pushed anywhere
/// in the module, since ConcurrencyDetector::analyseModule threads one
/// ConcurrencyModel through every function with no reset in between. An unlock
/// in function B silently became the unlock of a lock in function A, and
/// reversing the order the functions arrived in changed the model. Scoping the
/// search to the current function costs a lock/unlock pair split across two
/// functions, which was never recoverable from a call list anyway, and buys an
/// answer that does not depend on iteration order.
retdec::concurrency_detect::LockInfo*
openLockIn(retdec::concurrency_detect::ConcurrencyModel& out, const std::string& fnName)
{
	for (auto it = out.locks.rbegin(); it != out.locks.rend(); ++it)
		if (it->funcName == fnName && it->unlockCall == 0) return &*it;
	return nullptr;
}

/// The most recent thread recorded in *this* function. Same reasoning.
retdec::concurrency_detect::ThreadInfo*
threadIn(retdec::concurrency_detect::ConcurrencyModel& out, const std::string& fnName)
{
	for (auto it = out.threads.rbegin(); it != out.threads.rend(); ++it)
		if (it->funcName == fnName) return &*it;
	return nullptr;
}

/// Returns true if the block is a loop header, i.e. one of its successors is
/// itself or an earlier block. Same test the rest of the tree uses
/// (rc4_detect.cpp, aes_detect.cpp, observer_detect.cpp, rsa_detect.cpp).
bool hasBackEdge(const retdec::ssa::BasicBlock& blk)
{
...
/// The address of a block's first instruction, or 0 if it has none.
uint64_t blockVma(const retdec::ssa::BasicBlock& blk)
{
	for (const retdec::ssa::IrInstr* instr: blk.instrs)
		if (instr) return instr->vma;
	return 0;
}

/// Returns true if the block is a spin loop: a CAS-like call in a block that
/// branches back to itself or earlier.
///
/// The loop test used to be missing entirely -- the function only looked for a
/// CAS-shaped callee name -- so a single straight-line compare_exchange was
/// reported as a spinlock, contradicting both this function's name and the
/// pattern the header documents ("tight loop containing CMPXCHG ... until
/// exchange succeeds").
bool hasCasLoop(const retdec::ssa::BasicBlock& blk)
{
	if (!hasBackEdge(blk)) return false;
	for (const retdec::ssa::IrInstr* instr: blk.instrs)
	{
		if (!instr || instr->op != retdec::ssa::IrInstr::Op::Call) continue;
		const auto& n = instr->calleeName;
		if (n.find("compare_exchange") != std::string::npos || n.find("__atomic_compare_exchange") != std::string::npos
...
bool isDCLP(const retdec::ssa::BasicBlock& blk)
{
	...
	for (const retdec::ssa::IrInstr* instr: blk.instrs)
	{
		...
		const auto& n = instr->calleeName;
		if (!hasAtomicLoad
			&& (n.find("__atomic_load") != std::string::npos || n.find("atomic_load") != std::string::npos))
			hasAtomicLoad = true;
		if (!hasMutexLock && (n.find("mutex") != std::string::npos || n.find("lock") != std::string::npos))
			hasMutexLock = true;
	}
...
static bool matchesAny(const std::string& sym, const std::unordered_set<std::string>& set)
{
...
static AtomicOp parseOp(const std::string& s)
{
	...
	if (s == "atomicrmw" || s == "xchg" || s == "exchange") return AtomicOp::Exchange;
	if (s == "cmpxchg" || s == "cmpxchg_weak") return AtomicOp::CompareExchange;
...
void StdThreadDetector::analyseFunction(const ssa::SSAFunction& fn, ConcurrencyModel& out)
{
	...
	for (const auto& blkPtr: fn.blocks())
	{
		for (const auto& call: extractCalls(*blkPtr))
		{
			...
			if (matchesAny(sym, kStdThreadCreate))
			{
				...
				out.isMT = true;
				if (out.primaryLib == ThreadLib::Unknown) out.primaryLib = ThreadLib::StdThread;
			}
			if (matchesAny(sym, kStdThreadJoin))
			{
				if (ThreadInfo* ti = threadIn(out, fnName))
				{
					ti->isJoined = sym.find("join") != std::string::npos;
					ti->isDetached = sym.find("detach") != std::string::npos;
				}
			}
			if (matchesAny(sym, kStdMutexLock))
			{
				...
				li.isLockGuard =
					(sym.find("lock_guard") != std::string::npos || sym.find("scoped_lock") != std::string::npos
					 || sym.find("unique_lock") != std::string::npos);
				out.locks.push_back(li);
			}
			if (matchesAny(sym, kStdMutexUnlock))
			{
				if (LockInfo* li = openLockIn(out, fnName)) li->unlockCall = call.address;
			}
...
void PthreadDetector::analyseFunction(const ssa::SSAFunction& fn, ConcurrencyModel& out)
{
	const std::string& fnName = fn.name();
	for (const auto& blkPtr: fn.blocks())
	{
		for (const auto& call: extractCalls(*blkPtr))
		{
			...
			if (matchesAny(sym, kPthreadCreate))
			{
				...
				out.threads.push_back(ti);
				out.isMT = true;
				if (out.primaryLib == ThreadLib::Unknown) out.primaryLib = ThreadLib::PThread;
			}
			if (matchesAny(sym, kPthreadJoin))
			{
				if (ThreadInfo* ti = threadIn(out, fnName))
				{
					ti->isJoined = (sym == "pthread_join");
					ti->isDetached = (sym == "pthread_detach");
				}
			}
			if (matchesAny(sym, kPthreadMutexLock))
			{
				LockInfo li;
				li.kind = ciContains(sym, "rwlock") ? MutexKind::PthreadRwLock : MutexKind::PthreadMutex;
				li.funcName = fnName;
				li.lockCall = call.address;
...
void Win32ThreadDetector::analyseFunction(const ssa::SSAFunction& fn, ConcurrencyModel& out)
{
	const std::string& fnName = fn.name();
	for (const auto& blkPtr: fn.blocks())
	{
		for (const auto& call: extractCalls(*blkPtr))
		{
			...
			if (matchesAny(sym, kWin32ThreadCreate))
			{
				...
				out.threads.push_back(ti);
				out.isMT = true;
				if (out.primaryLib == ThreadLib::Unknown) out.primaryLib = ThreadLib::Win32;
			}
			...
			if (matchesAny(sym, kWin32Interlocked))
			{
				...
				// Longest name first: InterlockedCompareExchange and
				// InterlockedExchangeAdd both contain "Exchange", and
				// ExchangeAdd is a fetch-add, not an exchange -- it used to
				// fall into the Exchange arm. InterlockedAnd/Or/Xor matched no
				// arm at all and fell to an `else` that hardcoded FetchAdd,
				// which is why AtomicOp::FetchAnd/FetchOr/FetchXor were
				// unreachable from Win32 input.
				if (sym.find("CompareExchange") != std::string::npos)
					ai.op = AtomicOp::CompareExchange;
				else if (sym.find("ExchangeAdd") != std::string::npos)
					ai.op = AtomicOp::FetchAdd;
				else if (sym.find("Exchange") != std::string::npos)
					ai.op = AtomicOp::Exchange;
				else if (sym.find("Increment") != std::string::npos)
					ai.op = AtomicOp::FetchAdd;
				else if (sym.find("Decrement") != std::string::npos)
					ai.op = AtomicOp::FetchSub;
				else if (sym.find("Add") != std::string::npos)
					ai.op = AtomicOp::FetchAdd; // InterlockedAdd
				else if (sym.find("And") != std::string::npos)
					ai.op = AtomicOp::FetchAnd;
				else if (sym.find("Xor") != std::string::npos)
					ai.op = AtomicOp::FetchXor;
				else if (sym.find("Or") != std::string::npos)
					ai.op = AtomicOp::FetchOr;
				else
					ai.op = AtomicOp::Unknown;
				out.atomics.push_back(ai);
				// AtomicDetector sets this for the GCC-builtin path; the Win32
				// path did not, so a lock-free Win32 program was reported as
				// "Multithreaded: no" while listing its atomics.
				if (ai.op != AtomicOp::Unknown) out.isMT = true;
			}
			// WaitForSingleObject / WaitForMultipleObjects on a thread handle
			// is how a Win32 program joins. kWin32ThreadWait was defined and
			// never referenced, so no Win32 thread was ever marked joined.
			if (matchesAny(sym, kWin32ThreadWait))
			{
				if (ThreadInfo* ti = threadIn(out, fnName)) ti->isJoined = true;
			}
		}
	}
}
...
void AtomicDetector::analyseFunction(const ssa::SSAFunction& fn, ConcurrencyModel& out)
{
	const std::string& fnName = fn.name();
	for (const auto& blkPtr: fn.blocks())
	{
		...
		for (const auto& ae: extractAtomics(*blkPtr))
		{
			...
			if (!out.isMT && info.op != AtomicOp::Unknown) out.isMT = true;
		}
	}
}
...
void SpinlockDetector::analyseFunction(const ssa::SSAFunction& fn, ConcurrencyModel& out)
{
	const std::string& fnName = fn.name();
	for (const auto& blkPtr: fn.blocks())
	{
		const bool spin = hasCasLoop(*blkPtr);
		// Double-checked locking is not a spin loop -- it is an if, not a
		// while -- so it is recorded on its own evidence rather than through
		// hasCasLoop, which now requires a back edge.
		const bool dclp = isDCLP(*blkPtr);
		if (!spin && !dclp) continue;

		SpinlockInfo si;
		si.funcName = fnName;
		// The address of the loop header, as SpinlockInfo documents and the
		// emitter prints it. This used to store blkPtr->id, a 0-based block
		// index -- so the emitter printed a block number as a hex VMA, and
		// suppressed it entirely for block 0, whose id is 0. It is left at 0
		// for a DCLP, which has no loop header to name.
		si.loopAddr = spin ? blockVma(*blkPtr) : 0;
		si.isDCLP = dclp;
...
void OpenMPDetector::analyseFunction(const ssa::SSAFunction& fn, ConcurrencyModel& out)
{
	const std::string& fnName = fn.name();
	for (const auto& blkPtr: fn.blocks())
	{
		for (const auto& call: extractCalls(*blkPtr))
		{
			...
			if (matchesAny(sym, kOmpFork))
			{
				...
				out.isMT = true;
				if (out.primaryLib == ThreadLib::Unknown) out.primaryLib = ThreadLib::OpenMP;
			}
...
void TBBDetector::analyseFunction(const ssa::SSAFunction& fn, ConcurrencyModel& out)
{
	const std::string& fnName = fn.name();
	for (const auto& blkPtr: fn.blocks())
	{
			...
			for (const auto& pattern: kTBBPatterns)
			{
				if (call.target.find(pattern) != std::string::npos)
				{
					...
					if (pattern.find("parallel_for") != std::string::npos)
						...
						tp.kind = "task_group";
					else if (pattern.find("mutex") != std::string::npos || pattern.find("lock") != std::string::npos)
						tp.kind = "mutex";
					...
					out.isMT = true;
					if (out.primaryLib == ThreadLib::Unknown) out.primaryLib = ThreadLib::TBB;
					break;
				}
			}
		}
...
std::string ConcurrencyEmitter::emit(const ConcurrencyModel& model) const
{
	...
	if (!model.threads.empty())
	{
		...
		{
			os << "//   [" << threadLibName(t.lib) << "] " << t.funcName << " @ 0x" << std::hex << t.callSite
			   << std::dec;
	...
	if (!model.atomics.empty())
	{
		...
		{
			os << "//   " << atomicOpName(a.op) << " (" << atomicOrderName(a.order) << ")" << " @ 0x" << std::hex
			   << a.address << std::dec << " in " << a.funcName;
	...
	if (!model.semaphores.empty())
	{
		...
		{
			os << "//   " << (s.kind == SemKind::PosixSem ? "sem_t" : "HANDLE(Sem)") << " in " << s.funcName;
	...
	if (!model.barriers.empty())
	{
		...
		{
			os << "//   [" << threadLibName(b.lib) << "] " << b.funcName << " @ 0x" << std::hex << b.callAddr
			   << std::dec << "\n";
		}
		os << "\n";
	}
	...
	if (!model.ompRegions.empty())
	{
		...
		{
			os << "//   #pragma omp " << r.kind << " in " << r.funcName << " @ 0x" << std::hex << r.forkCall << std::dec
			   << "\n";
		}
		os << "\n";
	}
	...
	if (!model.tbbPatterns.empty())
	{
		...
		{
			os << "//   tbb::" << t.kind << " in " << t.funcName << " @ 0x" << std::hex << t.callAddr << std::dec
			   << "\n";
		}
		os << "\n";
	}
...
void ConcurrencyDetector::analyseFunction(const ssa::SSAFunction& fn, ConcurrencyModel& out)
{
