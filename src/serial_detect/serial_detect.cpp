...
static bool hasRightShift(const ssa::SSAFunction& fn, int amount)
{
	...
	for (auto& blk: fn.blocks())
		for (auto* i: blk->instrs)
			if (i->op == ssa::IrInstr::Op::Shr || i->op == ssa::IrInstr::Op::Sar) return true;
	return false;
}
...
static bool hasSelfCall(const ssa::SSAFunction& fn)
{
	for (auto& blk: fn.blocks())
		for (auto* i: blk->instrs)
			if (i->op == ssa::IrInstr::Op::Call && i->calleeName == fn.name()) return true;
	return false;
}
static int selfCallCount(const ssa::SSAFunction& fn)
{
	...
	for (auto& blk: fn.blocks())
		for (auto* i: blk->instrs)
			if (i->op == ssa::IrInstr::Op::Call && i->calleeName == fn.name()) ++n;
	return n;
}
static bool callsFunction(const ssa::SSAFunction& fn, const std::string& name)
{
	for (auto& blk: fn.blocks())
		for (auto* i: blk->instrs)
			if (i->op == ssa::IrInstr::Op::Call && i->calleeName == name) return true;
	return false;
}
static bool hasStructOffset(const ssa::SSAFunction& fn, int offset, int width)
{
	for (auto& v: fn.values())
		if (v->kind == ssa::ValueKind::MemRef && v->memOffset == (int64_t)offset && v->memWidth == (uint8_t)width)
			return true;
	return false;
}
static bool hasSimdInstruction(const ssa::SSAFunction& /*fn*/)
{
	return false;
}
static bool hasMutualRecursion(const ssa::SSAFunction& /*fn*/)
{
	return false;
}
static bool hasStringLiteral(const ssa::SSAFunction& /*fn*/, const std::string& /*s*/)
{
	return false;
}
...
static int loadWidthsPresent(const ssa::SSAFunction& fn)
{
	...
	for (auto& v: fn.values())
		if (v->kind == ssa::ValueKind::MemRef && v->memWidth > 0) widths.insert(v->memWidth);
...
{
	return callsFunction(fn, "malloc") || callsFunction(fn, "operator new") || callsFunction(fn, "calloc");
}
...
std::string symbolToFieldName(const std::string& sym)
{
	...
	static const char* kPrefixes[] = {
		"get_", "set_", "has_", "clear_", "mutable_", "Get", "Set", "Has", "Clear", "Mutable"};
	std::string s = sym;
...
{
	if (scalarType == ProtoScalarType::Message && !nestedMessageName.empty()) return nestedMessageName;
...
bool ProtobufDetector::hasSerializeSymbol(const std::unordered_set<std::string>& sym) const
{
	return symContains(sym, "SerializeToString") || symContains(sym, "SerializeToArray")
		|| symContains(sym, "SerializeToOstream") || symContains(sym, "AppendToString")
		|| symContains(sym, "ByteSizeLong") || symContains(sym, "ByteSize");
}

bool ProtobufDetector::hasParseSymbol(const std::unordered_set<std::string>& sym) const
{
	return symContains(sym, "ParseFromString") || symContains(sym, "ParseFromArray")
		|| symContains(sym, "ParseFromIstream") || symContains(sym, "MergeFromString")
		|| symContains(sym, "MergePartialFromCodedStream");
}

SerialLibrary
ProtobufDetector::detectVersion(const ssa::SSAFunction& fn, const std::unordered_set<std::string>& sym) const
{
	// proto3: no has_* methods for scalar fields, presence via oneof only
	bool hasHasMethods = symContains(sym, "has_") && !symContains(sym, "has_oneof");
	return hasHasMethods ? SerialLibrary::Protobuf2 : SerialLibrary::Protobuf3;
}

ProtoScalarType ProtobufDetector::wireTypeToScalar(ProtoWireType wt, uint8_t accessBytes) const
{
	switch (wt)
	{
	...
	case ProtoWireType::Fixed32: return (accessBytes == 4) ? ProtoScalarType::Float : ProtoScalarType::Fixed32;
	case ProtoWireType::Fixed64: return (accessBytes == 8) ? ProtoScalarType::Double : ProtoScalarType::Fixed64;
	case ProtoWireType::LengthDelimited: return ProtoScalarType::Bytes; // could be string or nested message
	default: return ProtoScalarType::Unknown;
	}
}

ProtoField ProtobufDetector::recoverField(uint32_t fieldNum, ProtoWireType wt, const std::string& symbolicName) const
{
	ProtoField f;
	f.number = fieldNum;
	f.wireType = wt;
	f.scalarType = wireTypeToScalar(wt, 4);
	f.name = symbolicName.empty() ? "field_" + std::to_string(fieldNum) : symbolToFieldName(symbolicName);
	return f;
}

SerialResult ProtobufDetector::detect(const ssa::SSAFunction& fn, const std::unordered_set<std::string>& sym) const
{
	SerialResult res;
...
ProtoSchema ProtobufDetector::reconstructSchema(
	const std::vector<const ssa::SSAFunction*>& classFunctions, const std::unordered_set<std::string>& symTable) const
{
	ProtoSchema schema;
	...
	for (auto* fn: classFunctions)
	{
		...
		if (tg.found && tg.fieldNumber > 0)
		{
			if (!fields.count(tg.fieldNumber))
			{
				...
				fields[tg.fieldNumber] = recoverField(tg.fieldNumber, tg.wireType, symName);
			}
		}
	}
...
{
	return ir_query::callsFunction(fn, "StartTable") || ir_query::callsFunction(fn, "EndTable")
		|| ir_query::callsFunction(fn, "AddElement") || ir_query::callsFunction(fn, "FlatBufferBuilder");
}

bool FlatBuffersDetector::hasVerifier(const ssa::SSAFunction& fn, const std::unordered_set<std::string>& sym) const
{
	return symContains(sym, "Verify") || symContains(sym, "VerifyFlatBuffer") || ir_query::callsFunction(fn, "Verify");
}

std::vector<FbsField> FlatBuffersDetector::recoverFields(const ssa::SSAFunction& fn) const
{
...
SerialResult FlatBuffersDetector::detect(const ssa::SSAFunction& fn, const std::unordered_set<std::string>& sym) const
{
	SerialResult res;
	...
	// Symbol evidence
	if (symContains(sym, "FlatBufferBuilder"))
	{
		score += 0.45f;
		evidence += "FlatBufferBuilder; ";
	}
	if (symContains(sym, "GetRootAs"))
	{
		score += 0.35f;
		evidence += "GetRootAs; ";
	}
...
FbsSchema FlatBuffersDetector::reconstructSchema(
	const std::vector<const ssa::SSAFunction*>& tableFunctions, const std::unordered_set<std::string>& symTable) const
{
	FbsSchema schema;
	...
	if (!schema.tables.empty()) schema.rootType = schema.tables[0].name;
	return schema;
}
...
MsgpackEvidence MessagePackDetector::detectSwitch(const ssa::SSAFunction& fn) const
{
	...
	if (ir_query::hasConstant(fn, 0x80))
	{
		ev.hasFixmap = true;
		ev.confidence += 0.10f;
	}
	if (ir_query::hasConstant(fn, 0x90))
	{
		ev.hasFixarray = true;
		ev.confidence += 0.10f;
	}
	if (ir_query::hasConstant(fn, 0xa0))
	{
		ev.hasFixstr = true;
		ev.confidence += 0.10f;
	}
	if (ir_query::hasConstant(fn, 0x7f))
	{
		ev.hasFixint = true;
		ev.confidence += 0.10f;
	}
...
{
	if (symContains(sym, "msgpack") || symContains(sym, "msgpack_pack")) return SerialLibrary::MessagePack_msgpack_c;
	return SerialLibrary::MessagePack_Generic;
}

SerialResult MessagePackDetector::detect(const ssa::SSAFunction& fn, const std::unordered_set<std::string>& sym) const
{
	SerialResult res;
...
{
	return ir_query::hasConstant(fn, 24) && ir_query::hasConstant(fn, 25) && ir_query::hasConstant(fn, 26)
		&& ir_query::hasConstant(fn, 27);
}

SerialResult CBORDetector::detect(const ssa::SSAFunction& fn, const std::unordered_set<std::string>& sym) const
{
	SerialResult res;
...
bool JSONDetector::hasRapidJsonSymbols(const std::unordered_set<std::string>& sym) const
{
	return symContains(sym, "GenericDocument") || symContains(sym, "GenericStringRef")
		|| symContains(sym, "kObjectType") || symContains(sym, "RapidJSON") || symContains(sym, "rapidjson");
}

bool JSONDetector::hasNlohmannSymbols(const std::unordered_set<std::string>& sym) const
{
	return symContains(sym, "nlohmann") || symContains(sym, "basic_json") || symContains(sym, "json_sax_dom_parser")
...
bool JSONDetector::hasSimdjsonSimd(const ssa::SSAFunction& fn) const
{
	...
	return ir_query::hasSimdInstruction(fn) && ir_query::hasConstant(fn, '{') && ir_query::hasConstant(fn, '[')
		&& ir_query::hasConstant(fn, '}') && ir_query::hasConstant(fn, ']');
}

bool JSONDetector::hasCJsonSymbols(const std::unordered_set<std::string>& sym) const
{
	return symContains(sym, "cJSON_Parse") || symContains(sym, "cJSON_GetObjectItem")
		|| symContains(sym, "cJSON_Delete") || symContains(sym, "cJSON_Print");
}

JsonLibraryEvidence
JSONDetector::detectLibrary(const ssa::SSAFunction& fn, const std::unordered_set<std::string>& sym) const
{
...
SerialResult JSONDetector::detect(const ssa::SSAFunction& fn, const std::unordered_set<std::string>& sym) const
{
	SerialResult res;
	...
	if (libEv.library != SerialLibrary::Unknown && libEv.library != SerialLibrary::JSON_Generic)
	{
...
bool XMLDetector::hasLibxml2Symbols(const std::unordered_set<std::string>& sym) const
{
	return symContains(sym, "xmlReadMemory") || symContains(sym, "xmlParseFile")
		|| symContains(sym, "xmlDocGetRootElement") || symContains(sym, "xmlFreeDoc") || symContains(sym, "xmlNode")
		|| symContains(sym, "libxml");
}

bool XMLDetector::hasExpatSymbols(const std::unordered_set<std::string>& sym) const
{
	return symContains(sym, "XML_ParserCreate") || symContains(sym, "XML_SetElementHandler")
		|| symContains(sym, "XML_SetCharacterDataHandler") || symContains(sym, "XML_Parse")
		|| symContains(sym, "XML_ParserFree");
}

bool XMLDetector::hasTinyXmlSymbols(const std::unordered_set<std::string>& sym) const
{
	return symContains(sym, "TiXml") || symContains(sym, "XMLDocument") || symContains(sym, "XMLElement")
		|| symContains(sym, "tinyxml") || symContains(sym, "TinyXML");
}

bool XMLDetector::hasRapidXmlSymbols(const std::unordered_set<std::string>& sym) const
{
	return symContains(sym, "rapidxml") || symContains(sym, "RapidXML") || symContains(sym, "xml_document")
		|| symContains(sym, "xml_node");
}

bool XMLDetector::hasTagScanning(const ssa::SSAFunction& fn) const
{
	return ir_query::hasConstant(fn, '<') && ir_query::hasConstant(fn, '>') && ir_query::hasConstant(fn, '/');
}
...
SerialLibrary XMLDetector::detectLibrary(const ssa::SSAFunction& fn, const std::unordered_set<std::string>& sym) const
{
...
SerialResult XMLDetector::detect(const ssa::SSAFunction& fn, const std::unordered_set<std::string>& sym) const
{
	SerialResult res;
...
std::string ProtoEmitter::emit(const ProtoSchema& schema) const
{
	...
	if (!schema.packageName.empty()) os << "package " << schema.packageName << ";\n";
...
std::string FbsEmitter::emit(const FbsSchema& schema) const
{
	...
	for (auto& tbl: schema.tables)
		os << emitTable(tbl) << "\n";
	if (!schema.rootType.empty()) os << "root_type " << schema.rootType << ";\n";
	return os.str();
}
...
SerialResult
SerialDetector::analyseFunction(const ssa::SSAFunction& fn, const std::unordered_set<std::string>& symTable) const
{
	...
	const std::unordered_set<std::string>& symTable) const
{
	...
	for (auto* fn: functions)
	{
		...
		if (it != detections.end() && it->second.framework == SerialFramework::Protobuf)
		{
	...
	const std::unordered_set<std::string>& symTable) const
{
	...
	for (auto* fn: functions)
	{
		if (!fn) continue;
		auto it = detections.find(fn->name());
		if (it != detections.end() && it->second.framework == SerialFramework::FlatBuffers)
		{
			fbsFns.push_back(fn);
		}
	}
