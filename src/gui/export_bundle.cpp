...
{
	if (!QFileInfo::exists(diskPath)) return true;
	if (!zip.addFile(diskPath, zipEntryBasename(diskPath)))
	{
		if (errOut) *errOut = zip.errorString();
		return false;
	}
	return true;
}
...
QString formatDecompilerCommandText(const DecompileBundleInput& in)
{
	...
	if (!in.decompilerCwd.isEmpty()) line += QStringLiteral("cwd: %1\n").arg(in.decompilerCwd);
...
bool exportDecompileBundle(const DecompileBundleInput& in, const QString& zipPath, QString* errOut)
{
	if (in.cPath.isEmpty() || !QFileInfo::exists(in.cPath))
	{
		if (errOut) *errOut = QStringLiteral("Decompiled C file not found: %1").arg(in.cPath);
		return false;
	}
	...
	if (!addFileIfExists(zip, paths.cPath, errOut)) return false;
	if (!addFileIfExists(zip, paths.configPath, errOut)) return false;
	if (!addFileIfExists(zip, paths.dsmPath, errOut)) return false;
	if (!addFileIfExists(zip, paths.llPath, errOut)) return false;
	...
	if (!in.logFilePath.isEmpty())
	{
		...
		if (logFile.open(QIODevice::ReadOnly)) logBytes = logFile.readAll();
	}
	if (logBytes.isEmpty() && !in.logInline.isEmpty()) logBytes = in.logInline;
