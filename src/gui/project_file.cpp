...
ProjectFile::ProjectFile(const QString& binaryPath): binaryPath_(binaryPath) {}
...
std::optional<ProjectFile::Annotation> ProjectFile::annotation(uint64_t address) const
{
...
QJsonObject ProjectFile::toJson() const
{
	...
	if (!functionListFilter_.isEmpty()) root["functionListFilter"] = functionListFilter_;
	if (callGraphDepth_ != 0) root["callGraphDepth"] = callGraphDepth_;
	...
	for (auto it = annotations_.constBegin(); it != annotations_.constEnd(); ++it)
	{
		...
		QJsonArray tags;
		for (const QString& t: it.value().tags)
			if (!t.isEmpty()) tags.append(t);
		if (!tags.isEmpty()) a["tags"] = tags;
...
bool ProjectFile::fromJson(const QJsonObject& obj)
{
	...
	for (auto it = annsObj.constBegin(); it != annsObj.constEnd(); ++it)
	{
		...
		for (const QJsonValue& tv: a["tags"].toArray())
		{
			const QString t = tv.toString().trimmed();
			if (!t.isEmpty()) ann.tags.append(t);
		}
