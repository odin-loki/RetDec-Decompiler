...
void InferenceWorker::abortInference() { abort_.store(true); }
...
AIAssistantPanel::AIAssistantPanel(QWidget* parent)
    ...
    connect(this, &AIAssistantPanel::startInferenceRequest,
            worker_, &InferenceWorker::startInference, Qt::QueuedConnection);
    connect(worker_, &InferenceWorker::tokenGenerated,
            this, &AIAssistantPanel::onTokenGenerated, Qt::QueuedConnection);
    connect(worker_, &InferenceWorker::responseComplete,
            this, &AIAssistantPanel::onResponseComplete, Qt::QueuedConnection);
    connect(worker_, &InferenceWorker::errorOccurred,
            this, &AIAssistantPanel::onInferenceError, Qt::QueuedConnection);
    connect(worker_, &InferenceWorker::systemStatusMessage,
            this, &AIAssistantPanel::onWorkerSystemMessage, Qt::QueuedConnection);
    connect(worker_, &InferenceWorker::loadModelFinished,
            this, &AIAssistantPanel::onLoadModelFinished, Qt::QueuedConnection);
...
bool AIAssistantPanel::loadModel(const QString& ggufPath, bool useGpu) {
    ...
    QMetaObject::invokeMethod(worker_, "loadModelSlot", Qt::QueuedConnection,
                              Q_ARG(QString, ggufPath), Q_ARG(bool, useGpu), Q_ARG(int, 4096));
    return true;
}
...
bool AIAssistantPanel::isModelLoaded() const { return modelLoaded_; }
...
void AIAssistantPanel::rebuildChatLog() {
    ...
    for (const auto& msg : history_) {
        ...
        if (msg.role == ChatRole::User) color = "#4af";
        else if (msg.role == ChatRole::Assistant) color = "#afa";
...
void AIAssistantPanel::onClearHistory() { clear(); }
...
void AIAssistantPanel::onResponseFinished() { setInferenceBusy(false); }
