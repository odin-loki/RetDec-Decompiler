...
QString formatHexDump(const QByteArray& data, quint64 fileOffset)
{
	...
	for (int i = 0; i < data.size(); i += kBytesPerLine)
	{
		...
		out += QStringLiteral("%1  ").arg(fileOffset + static_cast<quint64>(i), 8, 16, QChar('0'));
...
BinaryBrowserPanel::BinaryBrowserPanel(QWidget* parent): PanelBase("Binary Browser", parent)
{
	setupUI();
}
...
void BinaryBrowserPanel::setupUI()
{
	...
	connect(sectionTree_, &QTreeWidget::itemDoubleClicked, this, &BinaryBrowserPanel::onItemDoubleClicked);
	connect(sectionTree_, &QTreeWidget::itemSelectionChanged, this, &BinaryBrowserPanel::onSelectionChanged);
	connect(sectionTree_, &QTreeWidget::customContextMenuRequested, this, &BinaryBrowserPanel::onSectionContextMenu);
}
...
void BinaryBrowserPanel::loadBinary(const QString& path)
{
	...
	auto* hint = new QTreeWidgetItem(sectionTree_, {QStringLiteral("Run Inspect or wait for fileinfo…"), "", ""});
...
void BinaryBrowserPanel::populateFromFileinfo(const QJsonObject& root)
{
	...
	if (sections.isEmpty())
	{
		auto* empty = new QTreeWidgetItem(sectionTree_, {QStringLiteral("(no sections in fileinfo)"), "", ""});
		empty->setFlags(empty->flags() & ~Qt::ItemIsSelectable);
		sectionTree_->addTopLevelItem(empty);
		return;
	}

	auto* rootItem = new QTreeWidgetItem(sectionTree_, {QFileInfo(binaryPath_).fileName(), "", ""});
...
{
	hexView_->clear();
	if (!item || binaryPath_.isEmpty()) return;

	const QVariant offVar = item->data(0, Qt::UserRole);
	if (!offVar.isValid()) return;
	...
	{
		hexView_->setPlainText(QStringLiteral("Cannot seek to offset 0x%1").arg(offset, 0, 16));
		return;
	}

	const QByteArray chunk = f.read(static_cast<qint64>(readSize));
	hexHeader_->setText(
		QStringLiteral("Hex View — %1 @ file 0x%2 (%3 bytes)").arg(item->text(0)).arg(offset, 0, 16).arg(size));
	QString dump = formatHexDump(chunk, offset);
	if (size > kMaxHexBytes)
		dump +=
			QStringLiteral("\n… [showing first %1 KiB of %2 KiB]").arg(kMaxHexBytes / 1024).arg((size + 1023) / 1024);
...
void BinaryBrowserPanel::onSectionContextMenu(const QPoint& pos)
{
	...
	QMenu menu;
	QAction* decompileAct = menu.addAction(QStringLiteral("Decompile this range…"));
	QAction* copyStartAct = menu.addAction(QStringLiteral("Copy start address"));
	QAction* copyRangeAct = menu.addAction(QStringLiteral("Copy VMA range"));
	QAction* copyHexAct = menu.addAction(QStringLiteral("Copy hex snippet"));

	QAction* chosen = menu.exec(sectionTree_->viewport()->mapToGlobal(pos));
	if (!chosen) return;

	if (chosen == decompileAct)
	{
		emit decompileRangeRequested(QStringLiteral("0x%1-0x%2").arg(start, 0, 16).arg(end, 0, 16));
	}
	else if (chosen == copyStartAct)
	{
		QApplication::clipboard()->setText(QStringLiteral("0x%1").arg(start, 0, 16));
	}
	else if (chosen == copyRangeAct)
	{
		QApplication::clipboard()->setText(QStringLiteral("0x%1-0x%2").arg(start, 0, 16).arg(end, 0, 16));
	}
	else if (chosen == copyHexAct)
	{
		const quint64 offset = item->data(0, Qt::UserRole).toULongLong();
		QFile f(binaryPath_);
		if (!f.open(QIODevice::ReadOnly)) return;
		if (!f.seek(static_cast<qint64>(offset))) return;
		const QByteArray chunk = f.read(static_cast<qint64>(qMin(size, quint64(64))));
		QString hex;
		hex.reserve(chunk.size() * 3);
		for (int i = 0; i < chunk.size(); ++i)
		{
			if (i) hex += QChar(' ');
			hex += QStringLiteral("%1").arg(static_cast<uchar>(chunk[i]), 2, 16, QChar('0'));
		}
		if (!hex.isEmpty()) QApplication::clipboard()->setText(hex);
	}
}
