...
LiveConsolePanel::LiveConsolePanel(QWidget* parent): PanelBase(QStringLiteral("Console"), parent)
{
	setupUi();
...
void LiveConsolePanel::setMaxBlocks(int blocks)
{
	...
	if (text_) text_->setMaximumBlockCount(maxBlocks_);
}
...
void LiveConsolePanel::setFlushHz(int hz)
{
	hz = qBound(2, hz, 250);
	flushMs_ = 1000 / hz;
	if (flushTimer_) flushTimer_->setInterval(flushMs_);
}
...
void LiveConsolePanel::attachProcess(QProcess* proc, const QString& label)
{
	...
	connect(
		proc, &QProcess::readyReadStandardOutput, this, &LiveConsolePanel::onProcessStdoutReady, Qt::UniqueConnection);
	connect(
		proc, &QProcess::readyReadStandardError, this, &LiveConsolePanel::onProcessStderrReady, Qt::UniqueConnection);
	connect(
		proc,
		QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
		this,
		&LiveConsolePanel::onProcessFinished,
		Qt::UniqueConnection);
	connect(proc, &QObject::destroyed, this, &LiveConsolePanel::onProcessDestroyed, Qt::UniqueConnection);
}
...
void LiveConsolePanel::appendBanner(const QString& tool, const QStringList& args, const QString& cwd)
{
	...
	if (!cwd.isEmpty()) line += QStringLiteral("==>   cwd: %1\n").arg(cwd);
...
{
	QString line = QStringLiteral("<== %1 finished (exit=%2, time=%3 ms)").arg(tool).arg(exitCode).arg(elapsedMs);
...
{
	Q_UNUSED(stream);
	if (bytes.isEmpty() || text_ == nullptr) return;
	...
	else
	{
		...
		while (pos < total)
		{
			...
			if (nl >= 0 && nl - pos < kInsertBudget * 2) end = nl + 1;
...
void LiveConsolePanel::flushAll()
{
	...
	auto takeCapped = [cap](QByteArray& src) -> QByteArray {
		if (src.size() <= cap) return std::exchange(src, {});
		int cut = cap;
...
void LiveConsolePanel::onSaveAs()
{
	const QString path = QFileDialog::getSaveFileName(
		this, QStringLiteral("Save console output"), QString(), QStringLiteral("Text files (*.txt);;All files (*)"));
...
void LiveConsolePanel::clear()
{
	...
	if (statusLabel_) statusLabel_->setText(QStringLiteral("(idle)"));
}
