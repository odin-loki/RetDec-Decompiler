...
InspectPanel::InspectPanel(QWidget* parent): PanelBase(QStringLiteral("Inspect"), parent)
{
	setupUi();
...
void InspectPanel::setupUi()
{
	...
	decompileModeBtn_->setToolTip(QStringLiteral("Switch workspace to Decompile (centre code tabs)."));
	...
	auto* autoOpen = new QCheckBox(QStringLiteral("Open when done"), this);
	autoOpen->setObjectName(QStringLiteral("inspectAutoOpenUnpacked"));
	autoOpen->setChecked(false);
	autoOpen->setToolTip(QStringLiteral("Load the unpacked file as the current binary when unpacker exits 0."));
...
{
	if (!lastBinaryPath_.isEmpty()) runFileinfo(lastBinaryPath_, fileinfoExe_);
}
...
{
	if (lastBinaryPath_.isEmpty()) return;
	...
	if (unpackProc_->state() != QProcess::NotRunning) return;
	...
	QStringList args;
	if (unpackBrute_->isChecked()) args << QStringLiteral("-b");
...
void InspectPanel::onUnpackFinished(int exitCode, QProcess::ExitStatus)
{
	...
	const QByteArray combined = unpackProc_->readAllStandardOutput() + unpackProc_->readAllStandardError();
	QString text = QString::fromUtf8(combined);
	if (text.size() > 4000) text = text.left(4000) + QStringLiteral("\n…");

	QStringList args;
	if (unpackBrute_->isChecked()) args << QStringLiteral("-b");
	args << lastBinaryPath_ << QStringLiteral("-o") << unpackOutEdit_->text().trimmed();

	emit cliToolFinished(
		QStringLiteral("retdec-unpacker"),
		args,
		QFileInfo(unpackProc_->program()).absolutePath(),
		exitCode,
		unpackTimer_.elapsed(),
		text);

	if (exitCode == 0)
	{
		status_->setText(
			QStringLiteral("Unpacker finished (exit 0). Output: %1").arg(unpackOutEdit_->text().trimmed()));
		const QString outp = unpackOutEdit_->text().trimmed();
		openUnpackedBtn_->setEnabled(!outp.isEmpty() && QFileInfo::exists(outp));
	}
	else
	{
		status_->setText(QStringLiteral("Unpacker exit %1 — see Command log / Diagnostics.").arg(exitCode));
		openUnpackedBtn_->setEnabled(false);
	}
	updateButtonStates();
	if (auto* cb = findChild<QCheckBox*>(QStringLiteral("inspectAutoOpenUnpacked")))
		if (cb->isChecked() && openUnpackedBtn_->isEnabled()) onOpenUnpackedClicked();
}

void InspectPanel::onOpenUnpackedClicked()
{
	const QString p = unpackOutEdit_->text().trimmed();
	if (!p.isEmpty() && QFileInfo::exists(p)) emit requestOpenBinary(QFileInfo(p).absoluteFilePath());
}
...
void InspectPanel::onFileinfoFinished(int exitCode, QProcess::ExitStatus)
{
	...
	emit cliToolFinished(
		QStringLiteral("retdec-fileinfo"),
		QStringList{QStringLiteral("--json"), QStringLiteral("--verbose"), lastBinaryPath_},
		QFileInfo(fileinfoExe_).absolutePath(),
		exitCode,
		elapsed,
		tail);
	...
	if (exitCode != 0)
	{
		...
		if (out.isEmpty())
			...
			raw_->setPlainText(QString::fromUtf8(out.size() > 256 * 1024 ? out.left(256 * 1024) : out));
		updateButtonStates();
		return;
	}
	...
	else
	{
		lastFileinfoJson_ = QJsonObject();
		lastFileinfoBinaryPath_.clear();
		summary_->setHtml(QStringLiteral("<p>Could not parse JSON (%1). Raw output in JSON tab.</p>")
