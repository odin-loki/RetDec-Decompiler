...
// ─── Uncompressed CFG cache (expand-on-click / toolbar) ──────────────────────
namespace {

struct UncompressedCFG
{
	std::vector<BasicBlockData> blocks;
	std::vector<CFGEdgeData> edges;
};

QHash<CFGScene*, UncompressedCFG> g_uncompressedCache;

bool isChainSummary(const BasicBlockData& data)
{
	return data.instrs.size() == 1 && data.instrs.front().text.startsWith(QLatin1String("[chain:"));
}

void ensureCfgCacheCleanup(CFGScene* scene)
{
...
void storeUncompressedCFG(
	CFGScene* scene, const std::vector<BasicBlockData>& blocks, const std::vector<CFGEdgeData>& edges)
{
	if (!scene) return;
	ensureCfgCacheCleanup(scene);
	UncompressedCFG cached;
	cached.blocks = blocks;
	cached.edges = edges;
	g_uncompressedCache.insert(scene, std::move(cached));
}

void refreshCfgInfoLabel(CFGScene* scene)
{
	if (!scene) return;
	auto* panel = qobject_cast<QWidget*>(scene->parent());
	if (!panel) return;

	const bool haveCache = g_uncompressedCache.contains(scene);
	const int count = haveCache ? static_cast<int>(g_uncompressedCache.value(scene).blocks.size()) : scene->nodeCount();
	QString info = QString("%1 blocks").arg(count);
	if (scene->isCompressed()) info += " (compressed)";

	for (auto* lab: panel->findChildren<QLabel*>())
	{
		const QString t = lab->text();
		if (t.contains(QLatin1String(" blocks")) || t.contains(QLatin1String("(compressed)")))
		{
			lab->setText(info);
			break;
		}
	...
	if (auto* btn = panel->findChild<QPushButton*>("cfgExpandChains"))
	{
		const bool forced = scene->property("forceUncompressed").toBool();
		const bool large = haveCache && static_cast<int>(g_uncompressedCache.value(scene).blocks.size()) > 200;
		btn->setText(
			(forced && large && !scene->isCompressed()) ? QStringLiteral("Compress chains")
														: QStringLiteral("Expand chains"));
	}
}

void expandSceneFromCache(CFGScene* scene)
{
	if (!scene || !g_uncompressedCache.contains(scene)) return;
	const UncompressedCFG data = g_uncompressedCache.value(scene);
	scene->setProperty("forceUncompressed", true);
	scene->loadCFG(data.blocks, data.edges);
	refreshCfgInfoLabel(scene);
}
...
BasicBlockItem::BasicBlockItem(const BasicBlockData& data, QGraphicsItem* parent): QGraphicsObject(parent), data_(data)
{
...
void BasicBlockItem::paint(QPainter* painter, const QStyleOptionGraphicsItem*, QWidget*)
{
	...
	// ── Body ────────────────────────────────────────────────────────────────
	QColor bodyColor = hovered_ ? clr::surface1 : isSelected() ? clr::surface2 : clr::surface0;
	...
	if (data_.isEntry)
		addrText += "  [entry]";
	else if (data_.isExit)
		addrText += "  [exit]";
	else if (data_.isLoopHeader)
		addrText += "  [loop header]";
	painter->drawText(QRectF(kPadX, 0, kWidth - kPadX * 2, kHeaderH), Qt::AlignVCenter | Qt::AlignLeft, addrText);
	...
	for (int i = 0; i < shown; ++i)
	{
		...
		painter->drawText(QRectF(kPadX, iy, kWidth - kPadX * 2, kInstrH), Qt::AlignVCenter | Qt::AlignLeft, line);
	}
	if (static_cast<int>(data_.instrs.size()) > kMaxPreview)
	{
		...
		painter->drawText(
			QRectF(kPadX, iy, kWidth - kPadX * 2, kInstrH),
...
void BasicBlockItem::mousePressEvent(QGraphicsSceneMouseEvent* event)
{
	...
	if (!isChainSummary(data_)) return;
	auto* cfgScene = qobject_cast<CFGScene*>(scene());
	if (!cfgScene) return;
	// Defer so this item is not destroyed mid-event when the scene reloads.
	QMetaObject::invokeMethod(cfgScene, [cfgScene]() { expandSceneFromCache(cfgScene); }, Qt::QueuedConnection);
}
...
void BasicBlockItem::contextMenuEvent(QGraphicsSceneContextMenuEvent* event)
{
	...
	if (isChainSummary(data_)) expandAct = menu.addAction(QStringLiteral("Expand chain"));
	...
	else if (expandAct && chosen == expandAct)
	{
		if (auto* cfgScene = qobject_cast<CFGScene*>(scene())) expandSceneFromCache(cfgScene);
	}
}
...
CFGEdgeItem::CFGEdgeItem(QPointF from, QPointF to, EdgeKind kind, bool isCurvedBack, QGraphicsItem* parent):
...
void CFGEdgeItem::buildPath(QPointF from, QPointF to, EdgeKind kind, bool isCurvedBack)
{
	...
	if (isCurvedBack)
	{
		...
		path.cubicTo(QPointF(leftX, from.y()), QPointF(leftX, to.y()), QPointF(to.x(), to.y()));
	}
	else
	{
		qreal cy = (from.y() + to.y()) / 2.0;
		path.cubicTo(QPointF(from.x(), cy), QPointF(to.x(), cy), QPointF(to.x(), to.y()));
	}
...
LoopRegionItem::LoopRegionItem(QRectF bounds, QColor color, QGraphicsItem* parent): QGraphicsRectItem(bounds, parent)
{
	QColor fill = color;
	fill.setAlpha(25);
	QColor border = color;
...
MiniMapView::MiniMapView(QGraphicsScene* scene, QGraphicsView* mainView, QWidget* parent):
	QGraphicsView(scene, parent), mainView_(mainView)
{
	...
	setStyleSheet(
		"background: rgba(24,24,37,210); border: 1px solid #45475a;"
		"border-radius: 4px;");
}
...
CFGView::CFGView(QGraphicsScene* scene, QWidget* parent): QGraphicsView(scene, parent)
{
	setDragMode(NoDrag);
...
void CFGView::repositionMiniMap()
{
	if (miniMap_)
	{
		miniMap_->move(viewport()->width() - miniMap_->width() - 8, 8);
...
CFGScene::CFGScene(QObject* parent): QGraphicsScene(parent)
{
	setBackgroundBrush(clr::base);
}
...
{
	auto barycentre = [&](uint64_t v,
						  const std::vector<uint64_t>& neighbours,
...
// ── Main entry point ──────────────────────────────────────────────────────────
void CFGScene::loadCFG(const std::vector<BasicBlockData>& blocks, const std::vector<CFGEdgeData>& edges)
{
	storeUncompressedCFG(this, blocks, edges);
	...
	const bool forceUncompressed = property("forceUncompressed").toBool();
	if (!forceUncompressed && static_cast<int>(blocks.size()) > kCompressThreshold)
	{
		compressed_ = true;
	...
	for (auto& b: workBlocks)
	{
		...
		connect(item, &BasicBlockItem::clicked, this, &CFGScene::blockClicked);
		connect(item, &BasicBlockItem::goToFunctionRequested, this, &CFGScene::blockGoToFunctionRequested);
	}
	...
	for (auto& e: workEdges)
	{
		...
		bool isBack = backSet.count(edgeKey(e.from, e.to)) > 0 || e.kind == EdgeKind::BackEdge;
...
CFGPanel::CFGPanel(QWidget* parent): PanelBase("Control Flow Graph", parent)
{
	setupUI();
}
...
void CFGPanel::setupUI()
{
	...
	auto* expandChainsBtn = new QPushButton("Expand chains", topBar);
	expandChainsBtn->setObjectName(QStringLiteral("cfgExpandChains"));
	expandChainsBtn->setToolTip(
		QStringLiteral("Show the full control-flow graph with compressed instruction chains expanded"));
	...
	for (auto* btn: {fitButton_, resetButton_, expandChainsBtn, svgButton_, pngButton_})
	{
	...
	topLayout->addWidget(expandChainsBtn);
	...
	connect(expandChainsBtn, &QPushButton::clicked, this, [this]() {
		if (!g_uncompressedCache.contains(scene_)) return;

		const UncompressedCFG cached = g_uncompressedCache.value(scene_);
		const bool forced = scene_->property("forceUncompressed").toBool();
		const bool large = static_cast<int>(cached.blocks.size()) > 200;

		if (forced && large)
			scene_->setProperty("forceUncompressed", false);
		else
			scene_->setProperty("forceUncompressed", true);

		scene_->loadCFG(cached.blocks, cached.edges);

		const int count = g_uncompressedCache.contains(scene_)
							? static_cast<int>(g_uncompressedCache.value(scene_).blocks.size())
							: scene_->nodeCount();
		QString info = QString("%1 blocks").arg(count);
		if (scene_->isCompressed()) info += " (compressed)";
		infoLabel_->setText(info);

		if (auto* btn = findChild<QPushButton*>(QStringLiteral("cfgExpandChains")))
		{
			const bool nowForced = scene_->property("forceUncompressed").toBool();
			btn->setText(
				(nowForced && large && !scene_->isCompressed()) ? QStringLiteral("Compress chains")
																: QStringLiteral("Expand chains"));
		}
	...
	connect(scene_, &CFGScene::blockGoToFunctionRequested, this, &CFGPanel::onBlockGoToFunction);
...
void CFGPanel::loadCFG(const std::vector<BasicBlockData>& blocks, const std::vector<CFGEdgeData>& edges)
{
	addressMap_.clear();
	for (auto& b: blocks)
		addressMap_[b.id] = b.address;
	...
	if (auto* btn = findChild<QPushButton*>(QStringLiteral("cfgExpandChains")))
	{
		const bool forced = scene_->property("forceUncompressed").toBool();
		btn->setText(
			(forced && static_cast<int>(blocks.size()) > 200 && !scene_->isCompressed())
				? QStringLiteral("Compress chains")
				: QStringLiteral("Expand chains"));
	}
...
{
#ifdef QT_SVG_LIB
	QString path = QFileDialog::getSaveFileName(this, "Export CFG as SVG", QString(), "SVG files (*.svg)");
	...
	painter.end();
#else
	QMessageBox::warning(this, "SVG Export", "SVG export requires Qt6::Svg. Rebuild with -DQT_SVG=ON.");
#endif
}

void CFGPanel::onExportPng()
{
	QString path = QFileDialog::getSaveFileName(this, "Export CFG as PNG", QString(), "PNG files (*.png)");
	if (path.isEmpty()) return;
...
{
	if (selectedFuncAddress_ != 0) emit addressNavigated(selectedFuncAddress_);
}
