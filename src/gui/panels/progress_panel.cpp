...
AnalysisBridge::AnalysisBridge(QObject* parent): QObject(parent) {}
...
ProgressPanel::ProgressPanel(QWidget* parent): PanelBase("Progress", parent)
{
...
void ProgressPanel::connectBridge(AnalysisBridge* bridge)
{
	...
	connect(bridge, &AnalysisBridge::stageStarted, this, &ProgressPanel::onStageStarted, Qt::QueuedConnection);
	connect(bridge, &AnalysisBridge::progressUpdated, this, &ProgressPanel::onProgressUpdated, Qt::QueuedConnection);
	connect(bridge, &AnalysisBridge::stageCompleted, this, &ProgressPanel::onStageCompleted, Qt::QueuedConnection);
	connect(bridge, &AnalysisBridge::stageErrored, this, &ProgressPanel::onStageErrored, Qt::QueuedConnection);
	connect(
		bridge,
		&AnalysisBridge::functionCountChanged,
		this,
		&ProgressPanel::onFunctionCountChanged,
		Qt::QueuedConnection);
	connect(
		bridge,
		&AnalysisBridge::instructionCountChanged,
		this,
		&ProgressPanel::onInstructionCountChanged,
		Qt::QueuedConnection);
	connect(
		bridge, &AnalysisBridge::throughputChanged, this, &ProgressPanel::onThroughputChanged, Qt::QueuedConnection);
	connect(bridge, &AnalysisBridge::analysisStarted, this, &ProgressPanel::onAnalysisStarted, Qt::QueuedConnection);
	connect(
		bridge, &AnalysisBridge::analysisCompleted, this, &ProgressPanel::onAnalysisCompleted, Qt::QueuedConnection);
	connect(
		bridge, &AnalysisBridge::analysisCancelled, this, &ProgressPanel::onAnalysisCancelled, Qt::QueuedConnection);
}
...
void ProgressPanel::setupUI()
{
	...
	connect(exportBtn, &QPushButton::clicked, this, &ProgressPanel::onExportClicked);
	...
	connect(elapsedTimer_, &QTimer::timeout, this, &ProgressPanel::onElapsedTick);
}
...
void ProgressPanel::setStageState(const QString& stageName, StageState state, int percent)
{
...
void ProgressPanel::setThroughput(double fnPerSec, double instrPerSec)
{
	...
	instrRateLabel_->setText(
		instrPerSec >= 1e6 ? QString("%1M").arg(instrPerSec / 1e6, 0, 'f', 1) : QString::number(instrPerSec, 'f', 0));
}
...
void ProgressPanel::onAnalysisCancelled()
{
	...
	for (auto& r: rows_)
	{
		if (r.state == StageState::Running) setStageState(r.name, StageState::Pending);
	}
}
...
{
	const QString json = exportJson();
	QApplication::clipboard()->setText(json);
	emit exportRequested();
	if (!qEnvironmentVariableIsEmpty("RETDEC_GUI_HEADLESS")) return;

	const QString path = QFileDialog::getSaveFileName(this, "Save progress", QString(), "JSON (*.json);;All files (*)");
...
void ProgressPanel::updateOverallBar()
{
	...
	{
		if (r.state == StageState::Done || r.state == StageState::Error) ++done;
	}
...
void ProgressPanel::renderWaterfall()
{
	...
	{
		if (t.startMs >= 0 && t.durationMs > 0) maxEnd = qMax(maxEnd, t.startMs + t.durationMs);
	}
	if (maxEnd <= 0) return;
	...
	for (const auto& t: timings_)
	{
		...
		// Stage name label
		auto* label = waterfallScene_->addText(t.name.left(18), QFont("monospace", 7));
		...
		auto* rect = waterfallScene_->addRect(x, y + 1, w, rowH - 2, QPen(Qt::NoPen), QBrush(stageColor(idx)));
		(void)rect;

		// Duration annotation
		auto* ann = waterfallScene_->addText(formatMs(t.durationMs), QFont("monospace", 7));
...
QString ProgressPanel::formatMs(qint64 ms) const
{
	...
	if (ms < 60'000) return QString("%1.%2s").arg(ms / 1000).arg((ms % 1000) / 100);
