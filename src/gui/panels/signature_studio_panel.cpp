...
SignatureStudioPanel::SignatureStudioPanel(QWidget* parent): PanelBase(QStringLiteral("Signature Studio"), parent)
{
...
void SignatureStudioPanel::setupUi()
{
	...
	auto* exBox = new QGroupBox(QStringLiteral("retdec-signature-from-library-creator.py (Python 3)"), exPage);
	...
	expressOut_->setObjectName(QStringLiteral("sigStudioYaraOut"));
	...
	pat2yaraOut_->setObjectName(QStringLiteral("sigStudioPatYaraOut"));
	...
	auto* useAsStatic = new QPushButton(QStringLiteral("Use YARA as static-code sigfile"), this);
	useAsStatic->setObjectName(QStringLiteral("sigStudioUseAsStatic"));
	useAsStatic->setToolTip(QStringLiteral("Next interactive decompile will use this path as --static-code-sigfile."));
	root->addWidget(useAsStatic);
}
...
{
	if (activeBinary_.isEmpty()) return;
	if (expressLib_->text().trimmed().isEmpty()) expressLib_->setText(activeBinary_);
	if (expressOut_->text().trimmed().isEmpty())
	{
		const QFileInfo fi(activeBinary_);
		expressOut_->setText(QDir(fi.absolutePath()).filePath(fi.completeBaseName() + QStringLiteral("-libsig.yar")));
	}
	if (arArchive_->text().trimmed().isEmpty()) arArchive_->setText(activeBinary_);
	if (machoFile_->text().trimmed().isEmpty()) machoFile_->setText(activeBinary_);
	const QString dir = QFileInfo(activeBinary_).absolutePath();
	const QString sub = QDir(dir).filePath(QStringLiteral("retdec-sig-work"));
	if (arOutDir_->text().trimmed().isEmpty()) arOutDir_->setText(sub);
	if (machoOutDir_->text().trimmed().isEmpty()) machoOutDir_->setText(sub);
}
...
bool SignatureStudioPanel::startCli(const QString& toolLabel, const QString& exe, const QStringList& args)
{
	if (exe.isEmpty())
	{
...
bool SignatureStudioPanel::startPythonCli(
	const QString& toolLabel, const QString& pythonExe, const QString& scriptPath, const QStringList& scriptArgs)
{
...
void SignatureStudioPanel::onProcessFinished(int exitCode, QProcess::ExitStatus)
{
	...
	if (tail.size() > 3500) tail = tail.right(3500);
	lastLog_->appendPlainText(QStringLiteral("--- %1 exit %2 ---\n%3").arg(pendingTool_).arg(exitCode).arg(tail));
	emit cliToolFinished(pendingTool_, pendingArgs_, pendingCwd_, exitCode, runTimer_.elapsed(), tail);
}
...
void SignatureStudioPanel::onBrowseExpressLib()
{
	const QString p = QFileDialog::getOpenFileName(
		this, QStringLiteral("Library / archive"), expressLib_->text(), QStringLiteral("All (*.*)"));
	if (!p.isEmpty()) expressLib_->setText(p);
}

void SignatureStudioPanel::onBrowseExpressOut()
{
	const QString p = QFileDialog::getSaveFileName(
		this, QStringLiteral("YARA output"), expressOut_->text(), QStringLiteral("YARA (*.yar);;All (*.*)"));
	if (!p.isEmpty()) expressOut_->setText(p);
}
...
{
	const QString p =
		QFileDialog::getOpenFileName(this, QStringLiteral("Archive"), arArchive_->text(), QStringLiteral("All (*.*)"));
	if (!p.isEmpty()) arArchive_->setText(p);
}

void SignatureStudioPanel::onBrowseArOut()
{
	const QString p = QFileDialog::getExistingDirectory(this, QStringLiteral("Output directory"), arOutDir_->text());
	if (!p.isEmpty()) arOutDir_->setText(p);
}

void SignatureStudioPanel::onBrowseMachoFile()
{
	const QString p = QFileDialog::getOpenFileName(
		this, QStringLiteral("Mach-O file"), machoFile_->text(), QStringLiteral("All (*.*)"));
	if (!p.isEmpty()) machoFile_->setText(p);
}

void SignatureStudioPanel::onBrowseMachoOut()
{
	const QString p = QFileDialog::getExistingDirectory(this, QStringLiteral("Output directory"), machoOutDir_->text());
	if (!p.isEmpty()) machoOutDir_->setText(p);
}

void SignatureStudioPanel::onBrowseBin2patIn()
{
	const QString p = QFileDialog::getOpenFileName(
		this, QStringLiteral("Input for bin2pat"), bin2patIn_->text(), QStringLiteral("All (*.*)"));
	if (!p.isEmpty()) bin2patIn_->setText(p);
}

void SignatureStudioPanel::onBrowseBin2patOut()
{
	const QString p = QFileDialog::getSaveFileName(
		this, QStringLiteral("bin2pat output"), bin2patOut_->text(), QStringLiteral("All (*.*)"));
	if (!p.isEmpty()) bin2patOut_->setText(p);
}

void SignatureStudioPanel::onBrowsePat2yaraIn()
{
	const QString p = QFileDialog::getOpenFileName(
		this, QStringLiteral("pat2yara input"), pat2yaraIn_->text(), QStringLiteral("All (*.*)"));
	if (!p.isEmpty()) pat2yaraIn_->setText(p);
}

void SignatureStudioPanel::onBrowsePat2yaraOut()
{
	const QString p = QFileDialog::getSaveFileName(
		this, QStringLiteral("YARA output"), pat2yaraOut_->text(), QStringLiteral("YARA (*.yar);;All (*.*)"));
	if (!p.isEmpty()) pat2yaraOut_->setText(p);
}
