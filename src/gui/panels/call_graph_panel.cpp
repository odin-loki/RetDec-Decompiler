...
CallGraphNodeItem::CallGraphNodeItem(const CallGraphNode& node, QColor moduleColor, QGraphicsItem* parent):
...
qreal CallGraphNodeItem::nodeHeight() const
{
	return kNodeH;
}
...
void CallGraphNodeItem::paint(QPainter* painter, const QStyleOptionGraphicsItem*, QWidget*)
{
	...
	QColor border = hovered_        ? clrCG::blue
				  ...
				  : node_.isLibrary ? clrCG::surface2
									: moduleColor_;
	...
	displayName = fm.elidedText(displayName, Qt::ElideRight, static_cast<int>(nodeWidth()) - 16);
	painter->drawText(r.adjusted(8, 0, -4, 0), Qt::AlignVCenter | Qt::AlignLeft, displayName);
	...
	if (node_.instrCount > 0)
	{
		...
		painter->setPen(nc);
		painter->drawText(
			r.adjusted(4, 0, -6, 0), Qt::AlignVCenter | Qt::AlignRight, QString::number(node_.instrCount));
	}
}
...
void CallGraphNodeItem::contextMenuEvent(QGraphicsSceneContextMenuEvent* event)
{
	...
	if (!chosen) return;
	if (chosen == focusAction1)
		emit focusRequested(node_.address, 1);
	else if (chosen == focusAction2)
		emit focusRequested(node_.address, 2);
	else if (chosen == focusAction3)
		emit focusRequested(node_.address, 3);
	else if (chosen == navAction)
		emit clicked(node_.address);
}
...
SccSuperNodeItem::SccSuperNodeItem(
	const std::vector<uint64_t>& members, const std::vector<QString>& names, QGraphicsItem* parent):
...
void SccSuperNodeItem::paint(QPainter* painter, const QStyleOptionGraphicsItem*, QWidget*)
{
	painter->setRenderHint(QPainter::Antialiasing);
	QRectF r(0, 0, 120, 40);
	...
	painter->drawText(r, Qt::AlignCenter, QString("SCC [%1]").arg(members_.size()));
	painter->setFont(QFont("Monospace", 7));
	...
	if (!names_.empty())
	{
		...
		painter->drawText(r.adjusted(4, 18, -4, 0), Qt::AlignTop | Qt::AlignHCenter, firstName);
	}
}
...
CallGraphEdgeItem::CallGraphEdgeItem(QPointF from, QPointF to, int callCount, bool isDirect, QGraphicsItem* parent):
...
void CallGraphEdgeItem::buildPath(QPointF from, QPointF to, int callCount, bool isDirect)
{
	// Line width scales with call count (log scale, clamped 1–4)
	qreal width = 1.0 + std::log2(1.0 + callCount) * 0.4;
	width = std::min(width, 4.0);

	QColor color = isDirect ? QColor(0x89, 0xb4, 0xfa, 160) : QColor(0xa6, 0xad, 0xc8, 120);
...
ModuleClusterItem::ModuleClusterItem(QRectF bounds, QColor color, const QString& label, QGraphicsItem* parent):
...
void ModuleClusterItem::paint(QPainter* painter, const QStyleOptionGraphicsItem* opt, QWidget* w)
{
	...
	tc.setAlpha(180);
	painter->setPen(tc);
	painter->drawText(r.adjusted(6, 4, -6, 0), Qt::AlignTop | Qt::AlignLeft, label_);
}
...
CallGraphView::CallGraphView(QGraphicsScene* scene, QWidget* parent): QGraphicsView(scene, parent)
{
	setDragMode(NoDrag);
...
CallGraphScene::CallGraphScene(QObject* parent): QGraphicsScene(parent)
{
	setBackgroundBrush(clrCG::base);
}
...
CallGraphScene::SccResult CallGraphScene::computeSccs(
	const std::vector<uint64_t>& nodeAddrs, const std::unordered_map<uint64_t, std::vector<uint64_t>>& succ) const
{
	SccResult result;
	...
	for (int iter = 0; iter < iterations; ++iter)
	{
		...
		for (auto& [u, succs]: edgeMap)
		{
			...
			for (uint64_t v: succs)
			{
				...
				qreal dx = nodes[static_cast<size_t>(uIt->second)].x - nodes[static_cast<size_t>(vIt->second)].x;
				qreal dy = nodes[static_cast<size_t>(uIt->second)].y - nodes[static_cast<size_t>(vIt->second)].y;
				qreal d = std::sqrt(dx * dx + dy * dy);
				if (d < 0.01) continue;
				qreal f = fa(d) / d;
...
// ── Main loadGraph ────────────────────────────────────────────────────────────
void CallGraphScene::loadGraph(const std::vector<CallGraphNode>& nodes, const std::vector<CallEdge>& edges)
{
	clearGraph();
	nodes_ = nodes;
	edges_ = edges;
	...
	for (auto& e: edges)
	{
		...
		if (u != v && nodeSet.count(e.callerAddress) && nodeSet.count(e.calleeAddress)) layoutEdges[u].push_back(v);
	}

	// ── Force-directed layout ─────────────────────────────────────────────
	forceDirectedLayout(layoutNodes, layoutEdges, layoutNodes.size() > 300 ? 20 : 60);
	...
	{
		minX = std::min(minX, ln.x);
		minY = std::min(minY, ln.y);
	}
	for (auto& ln: layoutNodes)
	{
		ln.x -= minX - kMargin;
	...
	for (auto& ln: layoutNodes)
		positions[ln.addr] = QPointF(ln.x, ln.y);
	...
	auto edgeKey = [](uint64_t a, uint64_t b) { return (a << 32) ^ b; };
	for (auto& e: edges)
	{
		uint64_t u = resolve(e.callerAddress);
		uint64_t v = resolve(e.calleeAddress);
...
void CallGraphScene::recomputeNodeDepths()
{
	...
	if (queue.empty() && !nodes_.empty()) queue.push_back(nodes_.front().address);
...
bool CallGraphScene::isNodeVisible(uint64_t addr) const
{
	...
	if (!nameFilter_.isEmpty())
	{
		...
		QRegularExpression re(
			QRegularExpression::wildcardToRegularExpression(nameFilter_), QRegularExpression::CaseInsensitiveOption);
...
CallGraphPanel::CallGraphPanel(QWidget* parent): PanelBase("Call Graph", parent)
{
	setupUI();
}
...
void CallGraphPanel::setupUI()
{
	...
	auto* moduleFilter = new QComboBox(toolbar);
	moduleFilter->setObjectName(QStringLiteral("callGraphModuleFilter"));
	moduleFilter->addItem(QStringLiteral("All modules"), -1);
	...
	tbLayout->addWidget(moduleFilter);
	...
	connect(minCallSpin_, QOverload<int>::of(&QSpinBox::valueChanged), this, &CallGraphPanel::onMinCallCountChanged);
	connect(depthSpin_, QOverload<int>::of(&QSpinBox::valueChanged), this, &CallGraphPanel::onDepthChanged);
	connect(hideLibCheck_, &QCheckBox::toggled, this, &CallGraphPanel::onHideLibraryToggled);
	connect(moduleFilter, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [this, moduleFilter](int) {
		scene_->setModuleFilter(moduleFilter->currentData().toInt());
	});
...
void CallGraphPanel::loadGraph(const std::vector<CallGraphNode>& nodes, const std::vector<CallEdge>& edges)
{
	scene_->loadGraph(nodes, edges);
	infoLabel_->setText(QString("%1 nodes").arg(nodes.size()));

	if (auto* combo = findChild<QComboBox*>(QStringLiteral("callGraphModuleFilter")))
	{
		const int previous = combo->currentData().toInt();
		combo->blockSignals(true);
		combo->clear();
		combo->addItem(QStringLiteral("All modules"), -1);

		std::vector<int> moduleIds;
		for (const auto& node: nodes)
		{
			if (node.moduleId >= 0) moduleIds.push_back(node.moduleId);
		}
		...
		for (int id: moduleIds)
			combo->addItem(QStringLiteral("Module %1").arg(id), id);

		const int restoreIdx = combo->findData(previous);
		combo->setCurrentIndex(restoreIdx >= 0 ? restoreIdx : 0);
		combo->blockSignals(false);
		scene_->setModuleFilter(combo->currentData().toInt());
	}

	// fitInView on a 0×0 offscreen viewport can stall headless GUI tests.
	if (qEnvironmentVariableIsEmpty("RETDEC_GUI_HEADLESS")) graphView_->fitGraph();
}

void CallGraphPanel::setCallGraph(const QStringList& functionNames, const std::vector<CallEdge>& edges)
{
...
void CallGraphPanel::clear()
{
	...
	if (auto* combo = findChild<QComboBox*>(QStringLiteral("callGraphModuleFilter")))
	{
		const int allIdx = combo->findData(-1);
		combo->setCurrentIndex(allIdx >= 0 ? allIdx : 0);
	}
	infoLabel_->setText("");
	focusLabel_->setText("");
	clearFocusBtn_->setEnabled(false);
...
void CallGraphPanel::onFitView()
{
	graphView_->fitGraph();
}
void CallGraphPanel::onResetZoom()
{
...
{
	QString path =
		QFileDialog::getSaveFileName(this, "Export Call Graph (DOT)", QString(), "Graphviz DOT (*.dot *.gv)");
	if (path.isEmpty()) return;

	QFile file(path);
	if (!file.open(QIODevice::WriteOnly | QIODevice::Text))
	{
		QMessageBox::warning(this, "Export Failed", "Cannot write to " + path);
		return;
	}
