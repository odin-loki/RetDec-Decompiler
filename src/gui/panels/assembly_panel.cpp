...
AssemblyPanel::AssemblyPanel(QWidget* parent): PanelBase("Assembly", parent)
{
	setupUI();
}
...
void AssemblyPanel::setupUI()
{
	...
	connect(view_, &QPlainTextEdit::customContextMenuRequested, this, &AssemblyPanel::onContextMenu);

	auto followJump = [this]() {
		if (!view_) return;
		const QString line = view_->textCursor().block().text();
		static const QRegularExpression insnRe(
			QStringLiteral(R"(\b(call|jmp|je|jne|jz|jnz|ja|jb|jae|jbe|jg|jl|jge|jle|jo|js)\b)"),
			QRegularExpression::CaseInsensitiveOption);
		if (!insnRe.match(line).hasMatch()) return;

		const auto currentAddr = retdec::gui::parseFirstAddress(line);

		static const QRegularExpression hexRe(QStringLiteral("(0x[0-9a-fA-F]+)"));
		QRegularExpressionMatchIterator it = hexRe.globalMatch(line);
		QString lastHex;
		int hexCount = 0;
		while (it.hasNext())
		{
			lastHex = it.next().captured(1);
			++hexCount;
		}
		...
		{
			bool ok = false;
			const uint64_t addr = lastHex.toULongLong(&ok, 16);
			if (ok) target = addr;
		}
		else
		{
			const QRegularExpressionMatch im = insnRe.match(line);
			if (im.hasMatch()) target = retdec::gui::parseFirstAddress(line.mid(im.capturedEnd()));
		}

		if (!target.has_value()) return;
		if (currentAddr.has_value() && *target == *currentAddr) return;

		QVariantList stack = view_->property("asmNavStack").toList();
		if (currentAddr.has_value()) stack.append(QVariant::fromValue(static_cast<qulonglong>(*currentAddr)));
		view_->setProperty("asmNavStack", stack);
		navigateTo(*target);
	};
	...
	auto navBack = [this]() {
		...
		if (stack.isEmpty())
		{
			if (searchBar_ && searchBar_->isVisible()) searchBar_->hide();
			return;
		}
		const qulonglong addr = stack.takeLast().toULongLong();
		view_->setProperty("asmNavStack", stack);
		navigateTo(static_cast<uint64_t>(addr));
	};

	auto* followReturn = new QShortcut(QKeySequence(Qt::Key_Return), view_);
	followReturn->setObjectName(QStringLiteral("asmFollowReturn"));
	followReturn->setContext(Qt::WidgetWithChildrenShortcut);
	connect(followReturn, &QShortcut::activated, this, followJump);

	auto* followEnter = new QShortcut(QKeySequence(Qt::Key_Enter), view_);
	followEnter->setObjectName(QStringLiteral("asmFollowEnter"));
	followEnter->setContext(Qt::WidgetWithChildrenShortcut);
	connect(followEnter, &QShortcut::activated, this, followJump);

	auto* navBackSc = new QShortcut(QKeySequence(Qt::Key_Escape), view_);
	navBackSc->setObjectName(QStringLiteral("asmNavBack"));
	navBackSc->setContext(Qt::WidgetWithChildrenShortcut);
	connect(navBackSc, &QShortcut::activated, this, navBack);
	...
	searchBar_->hide();
	connect(searchBar_, &QLineEdit::returnPressed, this, &AssemblyPanel::onSearchReturnPressed);
...
{
	if (view_->document()->isEmpty()) return;
	...
	{
		if (!lines[i].contains(needle, Qt::CaseInsensitive)) continue;
...
void AssemblyPanel::setAssemblyText(const QString& text)
{
	...
	updateEmptyState();
	if (pendingScrollAddr_ != 0) scrollToAddress(pendingScrollAddr_);
}
...
void AssemblyPanel::onSearchReturnPressed()
{
	...
	if (searchMode_ == SearchMode::GoTo)
	{
		bool ok = false;
		QString hex = query;
		if (hex.startsWith(QStringLiteral("0x"), Qt::CaseInsensitive)) hex = hex.mid(2);
		const uint64_t addr = hex.toULongLong(&ok, 16);
		if (ok) navigateTo(addr);
		return;
	}
...
void AssemblyPanel::onContextMenu(const QPoint& pos)
{
	...
	if (!chosen || !addr.has_value()) return;
	if (chosen == copyAct)
