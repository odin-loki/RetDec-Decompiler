...
QWidget* SettingsDialog::makeRow(const QString& label, QWidget* widget, const QString& tooltip)
{
...
SettingsDialog::SettingsDialog(QWidget* parent): QDialog(parent)
{
	...
	connect(
		buttonBox_->button(QDialogButtonBox::RestoreDefaults), &QPushButton::clicked, this, &SettingsDialog::onReset);

	auto* exportBtn = buttonBox_->addButton(QStringLiteral("Export…"), QDialogButtonBox::ActionRole);
	auto* importBtn = buttonBox_->addButton(QStringLiteral("Import…"), QDialogButtonBox::ActionRole);

	connect(exportBtn, &QPushButton::clicked, this, [this]() {
		const QString path = QFileDialog::getSaveFileName(
			this, QStringLiteral("Export settings"), QString(), QStringLiteral("JSON (*.json);;All files (*)"));
		if (path.isEmpty()) return;
		applyToSettings();
		if (!AppSettings::instance().exportToFile(path))
		{
			QMessageBox::warning(
				this, QStringLiteral("Settings"), QStringLiteral("Failed to export settings to:\n%1").arg(path));
		}
	});

	connect(importBtn, &QPushButton::clicked, this, [this]() {
		const QString path = QFileDialog::getOpenFileName(
			this, QStringLiteral("Import settings"), QString(), QStringLiteral("JSON (*.json);;All files (*)"));
		if (path.isEmpty()) return;
		if (!AppSettings::instance().importFromFile(path))
		{
			QMessageBox::warning(
				this, QStringLiteral("Settings"), QStringLiteral("Failed to import settings from:\n%1").arg(path));
			return;
		}
...
QWidget* SettingsDialog::buildGeneralTab()
{
	...
	langCombo_->addItems({"English (en)", "German (de)", "French (fr)", "Spanish (es)", "Chinese (zh)"});
	...
	restoreCheck_->setToolTip(QStringLiteral("Re-open the last binary from File → Open Binary when RetDec starts."));
	l->addWidget(restoreCheck_);

	l->addStretch();

	connect(fontCombo_, &QFontComboBox::currentFontChanged, this, &SettingsDialog::onFontChanged);
...
QWidget* SettingsDialog::buildAnalysisTab()
{
...

	{
		auto* banner = new QLabel(
			QStringLiteral("F5 runs retdec-decompiler externally; these toggles affect "
...
QWidget* SettingsDialog::buildCUDATab()
{
	...
	wgSizeSpin_->setRange(32, 1024);
	wgSizeSpin_->setSingleStep(32);
...
QWidget* SettingsDialog::buildMLTab()
{
...

	{
		auto* hint = new QLabel(
			QStringLiteral("When this GGUF path exists on disk, Run Full Analysis sets "
						   "RETDEC_NEURAL_REFINE on the decompiler child (llama.cpp). "
						   "Context length and max new tokens come from this tab. "
						   "CPU device forces RETDEC_NEURAL_N_GPU_LAYERS=0; GPU/Auto use -1. "
						   "Empty path leaves refinement off."),
			w);
...
QWidget* SettingsDialog::buildRecoveryTab()
{
	...
	stlConfSpin_->setRange(0.0, 1.0);
	stlConfSpin_->setSingleStep(0.05);
	l->addWidget(makeRow("STL confidence", stlConfSpin_));

	cryptoConfSpin_ = new QDoubleSpinBox;
	cryptoConfSpin_->setRange(0.0, 1.0);
	cryptoConfSpin_->setSingleStep(0.05);
	l->addWidget(makeRow("Crypto confidence", cryptoConfSpin_));

	patConf2Spin_ = new QDoubleSpinBox;
	patConf2Spin_->setRange(0.0, 1.0);
	patConf2Spin_->setSingleStep(0.05);
	l->addWidget(makeRow("Pattern confidence", patConf2Spin_));

	concurConfSpin_ = new QDoubleSpinBox;
	concurConfSpin_->setRange(0.0, 1.0);
	concurConfSpin_->setSingleStep(0.05);
...
QWidget* SettingsDialog::buildAdvancedTab()
{
	...
	for (auto* c: {dumpIRCheck_, dumpASMCheck_, dumpCFGCheck_, dumpSSACheck_, colorOutCheck_})
		l->addWidget(c);
	...
	l->addWidget(makeRow("Max functions", maxFuncSpin_, "0 = decompile all functions"));
...
QWidget* SettingsDialog::buildDecompilerTab()
{
	...
	connect(llvmPassesAllBtn_, &QPushButton::clicked, this, &SettingsDialog::onDecompilerPassesAll);
	connect(llvmPassesNoneBtn_, &QPushButton::clicked, this, &SettingsDialog::onDecompilerPassesNone);
	connect(llvmPassesDefaultBtn_, &QPushButton::clicked, this, &SettingsDialog::onDecompilerPassesDefaults);
...

	{
		...
		l->addWidget(row);
		l->addWidget(
			new QLabel("<span style='color:#888;'>Set a folder outside OneDrive to keep large "
					   "`.gui-decompiled.*` artifacts out of sync. Leave empty to write beside the binary.</span>"));
	}

	connect(decompileOutputDirBtn_, &QToolButton::clicked, this, &SettingsDialog::onBrowseDecompileOutputDir);
...

	{
		...
		l->addWidget(row);
		l->addWidget(
			new QLabel("<span style='color:#888;'>Passed as <code>--output-lang</code> for native "
					   "binaries. Managed inputs (.pyc, .class, …) use format-specific emitters.</span>"));
	}

	l->addWidget(makeSeparator("Console"));

	liveConsoleTailCheck_ = new QCheckBox("Stream decompiler log to console (may slow decompile on huge logs)");
	...
	connect(decompilerConfigBtn_, &QToolButton::clicked, this, &SettingsDialog::onBrowseDecompilerConfig);

	llvmPassUniqueNames_.clear();
	llvmPassCheckboxes_.clear();
	if (!loadLlvmPassUniqueNames(llvmPassUniqueNames_))
	{
		l->addWidget(new QLabel("Could not load pass list resource (:/retdec/llvm_passes_default.json)."));
		l->addStretch();
		return w;
	}
...
QWidget* SettingsDialog::buildPluginsTab()
{
	...
	connect(pluginList_, &QListWidget::currentItemChanged, [this](QListWidgetItem* cur, QListWidgetItem*) {
		if (!cur) return;
		...
		if (lp)
		{
			pluginDetailLabel_->setText(
				"<b>" + lp->meta.name + "</b> v" + lp->meta.version + " — " + lp->meta.description
				+ "<br><i>Author: " + lp->meta.author + "</i>" + "<br><i>File: " + lp->filePath + "</i>");
		}
	});
...
void SettingsDialog::populateFromSettings()
{
	...
	if (useCustomLlvmPassesCheck_)
	{
		...
		const QSet<QString> disabled(s.decompiler.llvmPassesDisabled.begin(), s.decompiler.llvmPassesDisabled.end());
	...
	if (decompilerConfigEdit_) decompilerConfigEdit_->setText(s.decompiler.extraConfigPath);
	if (decompileOutputDirEdit_) decompileOutputDirEdit_->setText(s.decompiler.decompileOutputDir);
	if (liveConsoleTailCheck_) liveConsoleTailCheck_->setChecked(s.decompiler.liveConsoleTail);
	if (outputLangCombo_)
	{
...
void SettingsDialog::applyToSettings()
{
	...
	if (decompilerConfigEdit_) s.decompiler.extraConfigPath = decompilerConfigEdit_->text();
	if (decompileOutputDirEdit_) s.decompiler.decompileOutputDir = decompileOutputDirEdit_->text().trimmed();
...
void SettingsDialog::onApply()
{
	...
	applyToSettings();
	if (auto* app = qobject_cast<QApplication*>(QApplication::instance())) applyThemeFromSettings(*app);
...
void SettingsDialog::onBrowseModelPath()
{
	QString path = QFileDialog::getOpenFileName(
		this, "Select GGUF model", modelPathEdit_->text(), "GGUF models (*.gguf);;All files (*)");
	if (!path.isEmpty()) modelPathEdit_->setText(path);
}

void SettingsDialog::onBrowseKernelCache()
{
	QString path = QFileDialog::getExistingDirectory(this, "Kernel cache directory", kernelCacheEdit_->text());
	if (!path.isEmpty()) kernelCacheEdit_->setText(path);
}

void SettingsDialog::onBrowseIRDump()
{
	QString path = QFileDialog::getExistingDirectory(this, "IR dump directory", irDumpEdit_->text());
	if (!path.isEmpty()) irDumpEdit_->setText(path);
}

void SettingsDialog::onBrowseIntermediateDir()
{
	QString path = QFileDialog::getExistingDirectory(this, "Intermediate output directory", intermDirEdit_->text());
...
{
	if (useCustomLlvmPassesCheck_) useCustomLlvmPassesCheck_->setChecked(false);
...
void SettingsDialog::onBrowseDecompilerConfig()
{
	QString path = QFileDialog::getOpenFileName(
		this,
		QStringLiteral("Decompiler configuration JSON"),
		decompilerConfigEdit_ ? decompilerConfigEdit_->text() : QString(),
		QStringLiteral("JSON (*.json);;All files (*)"));
	if (!path.isEmpty() && decompilerConfigEdit_) decompilerConfigEdit_->setText(path);
}
...
void SettingsDialog::onBrowseDecompileOutputDir()
{
	QString path = QFileDialog::getExistingDirectory(
		this,
		QStringLiteral("Decompile output directory"),
		decompileOutputDirEdit_ ? decompileOutputDirEdit_->text() : QString());
	if (!path.isEmpty() && decompileOutputDirEdit_) decompileOutputDirEdit_->setText(path);
}
