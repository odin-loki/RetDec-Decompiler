...
bool CategoryDetector::isFormatString(const QString& v)
{
	static const QRegularExpression re(
		R"(%[-+0 #]?\d*(\.\d+)?[diouxXeEfgGaAcsSpn%])", QRegularExpression::CaseInsensitiveOption);
	return re.match(v).hasMatch();
}

bool CategoryDetector::isURL(const QString& v)
{
	return v.startsWith("http://", Qt::CaseInsensitive) || v.startsWith("https://", Qt::CaseInsensitive)
		|| v.startsWith("ftp://", Qt::CaseInsensitive) || v.startsWith("ftps://", Qt::CaseInsensitive)
		|| v.startsWith("file://", Qt::CaseInsensitive) || v.startsWith("ws://", Qt::CaseInsensitive)
...
bool CategoryDetector::isFilePath(const QString& v)
{
	...
	if (v.length() >= 3 && v[1] == ':' && (v[2] == '\\' || v[2] == '/')) return true;
	// UNC path
	if (v.startsWith("\\\\")) return true;
	...
	static const QStringList exts = {
		".exe", ".dll", ".so", ".dylib", ".sys", ".conf", ".cfg", ".ini", ".log", ".txt", ".xml", ".json", ".yaml"};
...
QString ConstantLabeler::lookupPort(uint64_t val)
{
	static const std::unordered_map<uint32_t, const char*> ports = {
		...
		{8080, "HTTP-Alt"}, {8443, "HTTPS-Alt"}, {27017, "MongoDB"}, {1433, "MSSQL"},      {1521, "Oracle"},
	};
...
void ConstantLabeler::label(ConstantEntry& e)
{
	...
	e.hexStr = QString("%1").arg(e.value, static_cast<int>(e.size) * 2, 16, QChar('0')).toUpper();
	...
	{
		e.label = lbl;
		e.labelKind = ConstantLabel::MagicNumber;
		return;
	}
	...
	{
		e.label = lbl;
		e.labelKind = ConstantLabel::CryptoConstant;
		return;
	}
	...
	{
		e.label = lbl;
		e.labelKind = ConstantLabel::FloatSpecial;
		return;
	}
	if (e.value > 0 && e.value <= 65535)
	{
		...
		{
			e.label = lbl;
			e.labelKind = ConstantLabel::PortNumber;
			return;
		}
	}
	e.label = {};
...
StringsModel::StringsModel(QObject* parent): QAbstractTableModel(parent) {}
...
QVariant StringsModel::data(const QModelIndex& index, int role) const
{
	...
	if (role == Qt::ToolTipRole && index.column() == 3) return e.value;
...
void StringsModel::setStrings(std::vector<StringEntry> entries)
{
	...
	for (auto& e: entries)
		if (e.category == StringCategory::Plain) e.category = CategoryDetector::classify(e.value);
...
ConstantsModel::ConstantsModel(QObject* parent): QAbstractTableModel(parent) {}
...
int ConstantsModel::columnCount(const QModelIndex&) const
{
	return 6;
}
...
QVariant ConstantsModel::data(const QModelIndex& index, int role) const
{
	...
	if (role == Qt::UserRole) return static_cast<int>(e.labelKind);
...
TypeFilterProxy::TypeFilterProxy(QObject* parent): QSortFilterProxyModel(parent)
{
...
void TypeFilterProxy::setTextFilter(const QString& text, bool useRegex)
{
	textFilter_ = text;
	useRegex_ = useRegex;
	if (useRegex_) re_ = QRegularExpression(text, QRegularExpression::CaseInsensitiveOption);
	invalidateFilter();
}
...
bool TypeFilterProxy::filterAcceptsRow(int source_row, const QModelIndex& parent) const
{
	...
	if (!typeFilter_.isEmpty())
	{
		...
		if (!src->data(idx).toString().contains(typeFilter_, Qt::CaseInsensitive)) return false;
	}
	...
	if (!catFilter_.isEmpty())
	{
		...
		if (!src->data(idx).toString().contains(catFilter_, Qt::CaseInsensitive)) return false;
	}
...
StringsBrowserPanel::StringsBrowserPanel(QWidget* parent): PanelBase("Strings & Constants", parent)
{
...
void StringsBrowserPanel::setupStringTab(QWidget* tab)
{
	...
	connect(copyAddrAction, &QAction::triggered, this, [this] {
		...
		QString addr = QString("0x%1").arg(strModel_->entry(src.row()).address, 16, 16, QChar('0'));
	...
	// ── Connections ──────────────────────────────────────────────────────────
	connect(strSearchBox_, &QLineEdit::textChanged, this, &StringsBrowserPanel::onStringFilterChanged);
	connect(
		typeCombo_,
		QOverload<int>::of(&QComboBox::currentIndexChanged),
		this,
		&StringsBrowserPanel::onTypeFilterChanged);
	connect(
		catCombo_, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &StringsBrowserPanel::onCatFilterChanged);
	connect(regexBtn_, &QToolButton::toggled, this, &StringsBrowserPanel::onRegexToggled);
	connect(strTable_, &QTableView::doubleClicked, this, &StringsBrowserPanel::onStringDoubleClicked);
	connect(exportBtn, &QToolButton::clicked, this, &StringsBrowserPanel::onExportStrings);
	connect(strProxy_, &QSortFilterProxyModel::rowsInserted, this, &StringsBrowserPanel::updateStatusBar);
	connect(strProxy_, &QSortFilterProxyModel::rowsRemoved, this, &StringsBrowserPanel::updateStatusBar);
	connect(strProxy_, &QSortFilterProxyModel::modelReset, this, &StringsBrowserPanel::updateStatusBar);
}
...
void StringsBrowserPanel::setupConstTab(QWidget* tab)
{
	...
	// Label-kind filter. Search box applies only when this combo is All
	// (one QSortFilterProxyModel cannot easily combine UserRole + DisplayRole).
	auto* constLabelFilter = new QComboBox(tab);
	constLabelFilter->setObjectName(QStringLiteral("constLabelFilter"));
	constLabelFilter->addItem(QStringLiteral("All"), -1);
	constLabelFilter->addItem(QStringLiteral("Magic"), static_cast<int>(ConstantLabel::MagicNumber));
	constLabelFilter->addItem(QStringLiteral("Crypto"), static_cast<int>(ConstantLabel::CryptoConstant));
	constLabelFilter->addItem(QStringLiteral("Float"), static_cast<int>(ConstantLabel::FloatSpecial));
	constLabelFilter->addItem(QStringLiteral("Port"), static_cast<int>(ConstantLabel::PortNumber));
	constLabelFilter->addItem(QStringLiteral("Errno"), static_cast<int>(ConstantLabel::Errno));
	constLabelFilter->addItem(QStringLiteral("WinError"), static_cast<int>(ConstantLabel::WinError));
	constLabelFilter->addItem(QStringLiteral("Syscall"), static_cast<int>(ConstantLabel::SyscallNr));
	constLabelFilter->addItem(QStringLiteral("Other"), static_cast<int>(ConstantLabel::OtherKnown));
	toolbar->addWidget(constLabelFilter);
	...
	constProxy_->setFilterRole(Qt::DisplayRole);
	constProxy_->setFilterKeyColumn(-1); // all columns (text search when combo is All)
	...
	connect(constSearchBox_, &QLineEdit::textChanged, this, &StringsBrowserPanel::onConstFilterChanged);
	connect(constLabelFilter, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [this, constLabelFilter](int) {
		const int kind = constLabelFilter->currentData().toInt();
		if (kind == -1)
		{
			constProxy_->setFilterRole(Qt::DisplayRole);
			constProxy_->setFilterKeyColumn(-1);
			constProxy_->setFilterFixedString(constSearchBox_ ? constSearchBox_->text() : QString());
		}
		else
		{
			constProxy_->setFilterRole(Qt::UserRole);
			constProxy_->setFilterKeyColumn(0);
			constProxy_->setFilterFixedString(QString::number(kind));
		}
		updateStatusBar();
	});
	connect(constTable_, &QTableView::doubleClicked, this, &StringsBrowserPanel::onConstDoubleClicked);
	connect(exportBtn, &QToolButton::clicked, this, &StringsBrowserPanel::onExportConstants);
	connect(constProxy_, &QSortFilterProxyModel::modelReset, this, &StringsBrowserPanel::updateStatusBar);
}
...
{
	auto* combo = findChild<QComboBox*>(QStringLiteral("constLabelFilter"));
	if (combo && combo->currentData().toInt() != -1)
		return; // kind filter owns UserRole; search box applies only when All
	constProxy_->setFilterRole(Qt::DisplayRole);
	constProxy_->setFilterKeyColumn(-1);
...
{
	if (strSearchBox_) onStringFilterChanged(strSearchBox_->text());
	(void)checked;
}
...
{
	QString path = QFileDialog::getSaveFileName(this, "Export Strings", "strings.csv", "CSV files (*.csv)");
...
{
	QString path = QFileDialog::getSaveFileName(this, "Export Constants", "constants.csv", "CSV files (*.csv)");
	if (path.isEmpty()) return;
	QFile f(path);
	if (!f.open(QIODevice::WriteOnly | QIODevice::Text))
	{
...
{
	if (strStatusLabel_)
		strStatusLabel_->setText(QString("%1 / %2 strings").arg(strProxy_->rowCount()).arg(strModel_->totalCount()));
	if (constStatusLabel_)
		constStatusLabel_->setText(
			QString("%1 / %2 constants").arg(constProxy_->rowCount()).arg(constModel_->rowCount({})));
}
...
void StringsBrowserPanel::clear()
{
	...
	if (auto* combo = findChild<QComboBox*>(QStringLiteral("constLabelFilter"))) combo->setCurrentIndex(0);
	updateStatusBar();
}
