...
DecompiledCPanel::DecompiledCPanel(QWidget* parent): PanelBase("Decompiled C", parent)
{
	setupUI();
}
...
void DecompiledCPanel::setupUI()
{
	...
	auto* refinedAction = toolbar_->addAction(QStringLiteral("Refined"));
	refinedAction->setObjectName(QStringLiteral("showRefinedC"));
	refinedAction->setCheckable(true);
	refinedAction->setEnabled(false);
	refinedAction->setToolTip(QStringLiteral("Show accepted neural refine sidecar (.refined.c) when present"));
	connect(refinedAction, &QAction::toggled, this, [this](bool on) {
		const QString original = property("originalCPath").toString();
		const QString refined = property("refinedCPath").toString();
		const QString path = on ? refined : original;
		if (path.isEmpty() || !QFileInfo::exists(path)) return;
		if (property("displayedCPath").toString() == path) return;
		if (setSourceFromPath(path)) setProperty("displayedCPath", path);
	});
	...
	searchBar_->hide();
	connect(searchBar_, &QLineEdit::returnPressed, this, &DecompiledCPanel::onSearchReturnPressed);
	...
	connect(escSc, &QShortcut::activated, this, [this]() {
		if (searchBar_ && searchBar_->isVisible()) hideFindBar();
	});
...
void DecompiledCPanel::setSource(const QString& cSource)
{
	...
	if (cSource.size() <= kAsyncThreshold)
	{
		...
		while (pos < cSource.size())
		{
			...
			if (nl != -1 && nl - pos < kSyncChunk * 2) end = nl + 1;
...
bool DecompiledCPanel::setSourceFromPath(const QString& path)
{
	...
	setProperty("displayedCPath", path);

	const QString refinedSuffix = QStringLiteral(".refined.c");
	const bool isRefined = path.endsWith(refinedSuffix);
	if (isRefined)
	{
		QString original = path;
		original.chop(refinedSuffix.size());
		if (!original.isEmpty()) setProperty("originalCPath", original);
		setProperty("refinedCPath", path);
	}
	else
	{
		const QString sibling = path + refinedSuffix;
		if (QFileInfo::exists(sibling))
		{
			setProperty("originalCPath", path);
			setProperty("refinedCPath", sibling);
		}
	}

	if (auto* action = findChild<QAction*>(QStringLiteral("showRefinedC")))
	{
		const QString refinedPath = property("refinedCPath").toString();
		action->setEnabled(!refinedPath.isEmpty() && QFileInfo::exists(refinedPath));
		const bool blocked = action->blockSignals(true);
		action->setChecked(isRefined);
...
void DecompiledCPanel::onAsyncLoadTick()
{
	...
	if (remaining <= 0)
	{
		funcLabel_->setText(QStringLiteral("Loaded %1 KiB").arg(pendingLoadTotal_ / 1024));
		pendingLoad_.clear();
		pendingLoadPos_ = 0;
		pendingLoadTotal_ = 0;
		return;
	...
	if (nl != -1 && nl - pendingLoadPos_ < kAsyncChunk * 2) take = nl - pendingLoadPos_ + 1;
	...
	funcLabel_->setText(QStringLiteral("Loading decompiled C… %1 / %2 KiB").arg(doneKiB).arg(totKiB));
...
void DecompiledCPanel::scrollToFunction(const QString& name, int startLine, int endLine)
{
	...
	const QRegularExpression sigRe(QStringLiteral(R"(\b%1\s*\()").arg(escaped));
...
{
	if (!view_ || view_->document()->isEmpty()) return;
...
{
	QString path =
		QFileDialog::getSaveFileName(this, "Save Decompiled C", QString(), "C Source Files (*.c);;All Files (*)");
...
{
	if (query.isEmpty() || !hasSearchableText()) return;
...
{
	const QString query = searchBar_->isVisible() ? searchBar_->text().trimmed() : lastFindQuery_;
	if (query.isEmpty())
	{
		showFindBar();
		return;
	}
	if (!hasSearchableText()) return;
	lastFindQuery_ = query;
	searchInView(query, true);
}
