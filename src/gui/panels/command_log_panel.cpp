...
CommandLogPanel::CommandLogPanel(QWidget* parent): PanelBase(QStringLiteral("Command log"), parent)
{
	...
	copyBtn_->setToolTip(QStringLiteral("Copy the full log to the clipboard. Ctrl+Shift+C when this panel has focus."));
	...
	connect(saveBtn, &QPushButton::clicked, this, [this] {
		const QString path = QFileDialog::getSaveFileName(
			this, QStringLiteral("Save command log"), QString(), QStringLiteral("Text (*.txt);;All files (*)"));
		if (path.isEmpty()) return;
		QFile f(path);
		if (!f.open(QIODevice::WriteOnly | QIODevice::Truncate))
		{
			QMessageBox::warning(this, QStringLiteral("Save failed"), f.errorString());
			return;
		}
...
void CommandLogPanel::appendRun(
	const QString& tool,
