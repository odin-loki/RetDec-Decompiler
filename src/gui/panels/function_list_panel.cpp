...
QVariant FunctionListModel::data(const QModelIndex& index, int role) const
{
	...
	case Qt::DisplayRole:
		...
		case ColName: {
			if (!retdec::gui::AppSettings::instance().advanced.demangleNames && !e.rawName.isEmpty()) return e.rawName;
			return e.name.isEmpty() ? e.rawName : e.name;
		}
		case ColSize: return e.sizeBytes > 0 ? QString("%1 B").arg(e.sizeBytes) : "—";
		case ColInstrs: return e.instrCount > 0 ? QString::number(e.instrCount) : "—";
		case ColConfidence: return QString("%1 %").arg(static_cast<int>(e.confidence.composite() * 100));
		case ColPatterns: return {}; // painted by delegate
		case ColCC: return e.cc.isEmpty() ? "—" : e.cc;
		case ColNotes: {
			QString s = e.notes;
			if (!e.tags.isEmpty()) s += QString(s.isEmpty() ? "" : "  ") + "[" + e.tags.join(", ") + "]";
			return s;
		}
		}
		break;
	...
	case Qt::ToolTipRole:
		...
		case ColName: {
			QString tip = e.name;
			if (!e.rawName.isEmpty() && e.rawName != e.name) tip += "\n" + e.rawName;
			if (!e.signature.isEmpty()) tip += "\n" + e.signature;
			return tip;
		}
		case ColConfidence:
			return QString(
		...
		case ColCC: return "Calling convention: " + e.cc;
		}
		break;

	case Qt::UserRole:
		// Return the raw address for sorting
		if (index.column() == ColAddress) return static_cast<qulonglong>(e.address);
		if (index.column() == ColConfidence) return e.confidence.composite();
		break;
...
Qt::ItemFlags FunctionListModel::flags(const QModelIndex& index) const
{
	...
	if (index.column() == ColName || index.column() == ColNotes) f |= Qt::ItemIsEditable;
	return f;
}
...
void FunctionListModel::applyTag(const std::vector<uint64_t>& addresses, const QString& tag)
{
	...
	emit dataChanged(createIndex(0, ColNotes), createIndex(static_cast<int>(fns_.size()) - 1, ColNotes));
}
...
FunctionPatternDelegate::FunctionPatternDelegate(FunctionListModel* model, QObject* parent):
	QStyledItemDelegate(parent), model_(model)
{}

QSize FunctionPatternDelegate::sizeHint(const QStyleOptionViewItem& opt, const QModelIndex& index) const
{
	if (index.column() == FunctionListModel::ColPatterns) return QSize(5 * 16 + 4, 20);
	if (index.column() == FunctionListModel::ColConfidence) return QSize(80, 20);
	return QStyledItemDelegate::sizeHint(opt, index);
}

void FunctionPatternDelegate::paint(QPainter* painter, const QStyleOptionViewItem& opt, const QModelIndex& index) const
{
...
void FunctionPatternDelegate::paintConfidence(
	QPainter* painter, const QStyleOptionViewItem& opt, float confidence) const
{
	painter->save();
	...
	QColor barColor;
	if (confidence >= 0.75f)
		barColor = clrFL::green;
	else if (confidence >= 0.40f)
		barColor = clrFL::yellow;
	else
		barColor = clrFL::red;
	barColor.setAlpha(200);

	painter->setBrush(barColor);
	if (fill.width() > 0) painter->drawRoundedRect(fill, 3, 3);

	painter->restore();
}

void FunctionPatternDelegate::paintPatterns(
	QPainter* painter, const QStyleOptionViewItem& opt, const PatternFlags& patterns) const
{
	painter->save();
	painter->setRenderHint(QPainter::Antialiasing);
...
void FunctionFilterProxy::setPatternFilter(bool stl, bool crypto, bool algo, bool design, bool lib)
{
	filterStl_ = stl;
...
bool FunctionFilterProxy::filterAcceptsRow(int sourceRow, const QModelIndex&) const
{
	...
	{
		bool match = (filterStl_ && e.patterns.stl) || (filterCrypto_ && e.patterns.crypto)
				  || (filterAlgo_ && e.patterns.algo) || (filterDesign_ && e.patterns.design)
	...
	if (!classFilter_.isEmpty())
	{
		...
		if (!e.name.startsWith(prefix) && !e.rawName.startsWith(prefix)) return false;
	}
	...
	if (!nameGlob_.isEmpty())
	{
		QRegularExpression re(
			QRegularExpression::wildcardToRegularExpression(nameGlob_), QRegularExpression::CaseInsensitiveOption);
		if (!re.match(e.name).hasMatch() && !re.match(e.rawName).hasMatch()) return false;
	}

	return true;
}

static QString functionListCsvText(FunctionListModel* model, FunctionFilterProxy* proxy)
{
	QString out;
	QTextStream ts(&out);
	ts << "Address,Name,SizeBytes,Instrs,Confidence,CC,STL,Crypto,Algo,Design,Lib,Notes,Tags\n";

	for (int r = 0; r < proxy->rowCount(); ++r)
	{
		QModelIndex src = proxy->mapToSource(proxy->index(r, 0));
		const auto& e = model->entry(src.row());
		ts << QString("0x%1").arg(e.address, 0, 16) << ',' << e.name << ',' << e.sizeBytes << ',' << e.instrCount << ','
		   << QString::number(static_cast<double>(e.confidence.composite()), 'f', 3) << ',' << e.cc << ','
		   << (e.patterns.stl ? 1 : 0) << ',' << (e.patterns.crypto ? 1 : 0) << ',' << (e.patterns.algo ? 1 : 0) << ','
		   << (e.patterns.design ? 1 : 0) << ',' << (e.patterns.library ? 1 : 0) << ',' << e.notes << ','
		   << e.tags.join(QLatin1Char(';')) << '\n';
	}
	return out;
}
...
FunctionListPanel::FunctionListPanel(QWidget* parent): PanelBase("Functions", parent)
{
	setupUI();
}
...
void FunctionListPanel::setupUI()
{
	...
	// ── Connections ───────────────────────────────────────────────────────
	connect(tableView_, &QTableView::doubleClicked, this, &FunctionListPanel::onDoubleClicked);
	connect(tableView_, &QWidget::customContextMenuRequested, this, &FunctionListPanel::onContextMenu);
	connect(
		tableView_->selectionModel(),
		&QItemSelectionModel::selectionChanged,
		this,
		&FunctionListPanel::selectionChanged);
	connect(model_, &FunctionListModel::functionRenamed, this, &FunctionListPanel::functionRenamed);
	connect(proxy_, &QSortFilterProxyModel::rowsInserted, this, &FunctionListPanel::updateStatusLabel);
	connect(proxy_, &QSortFilterProxyModel::rowsRemoved, this, &FunctionListPanel::updateStatusLabel);
	connect(proxy_, &QSortFilterProxyModel::modelReset, this, &FunctionListPanel::updateStatusLabel);
	updateStatusLabel();
}
...
void FunctionListPanel::setupFilterBar(QWidget* parent, QHBoxLayout* layout)
{
	...
	auto* copyButton = new QPushButton("Copy", parent);
	copyButton->setObjectName(QStringLiteral("functionListCopyCsv"));
	copyButton->setProperty("compact", true);
	...
	connect(confSpin_, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, [onFilter](double) { onFilter(); });
	...
	connect(copyButton, &QPushButton::clicked, this, [this]() {
		QApplication::clipboard()->setText(functionListCsvText(model_, proxy_));
		statusLabel_->setText(QString("Copied %1 functions").arg(proxy_->rowCount()));
	});
}
...
{
	if (!nameFilter_ || nameFilter_->text() == text) return;
...
void FunctionListPanel::selectFunction(uint64_t address)
{
	for (int r = 0; r < model_->rowCount(); ++r)
	{
		...
		tableView_->selectionModel()->select(proxyIdx, QItemSelectionModel::ClearAndSelect | QItemSelectionModel::Rows);
...
void FunctionListPanel::onFilterChanged()
{
	...
	proxy_->setPatternFilter(
		ckStl_->isChecked(), ckCrypto_->isChecked(), ckAlgo_->isChecked(), ckDesign_->isChecked(), ckLib_->isChecked());
	updateStatusLabel();
}
...
void FunctionListPanel::onContextMenu(const QPoint& pos)
{
	...
	else if (chosen == copyAct && idx.isValid())
	{
		...
		{
			const auto& e = model_->entry(src.row());
			QApplication::clipboard()->setText(retdec::gui::formatAddressHex(e.address));
		}
	}
...
void FunctionListPanel::onExportCsv()
{
	...
	QString path = QFileDialog::getSaveFileName(this, "Export Function List (CSV)", QString(), "CSV files (*.csv)");
	...
	QTextStream ts(&file);
	ts << functionListCsvText(model_, proxy_);
}
...
void FunctionListPanel::onExportJson()
{
	...
	QString path = QFileDialog::getSaveFileName(this, "Export Function List (JSON)", QString(), "JSON files (*.json)");
	if (path.isEmpty()) return;
...
std::vector<uint64_t> FunctionListPanel::selectedAddresses() const
{
	...
	for (auto& proxyIdx: tableView_->selectionModel()->selectedRows())
	{
		...
		if (src.isValid()) addrs.push_back(model_->entry(src.row()).address);
	}
	return addrs;
}
...
void FunctionListPanel::updateStatusLabel()
{
	...
	else
	{
		statusLabel_->setText(
			visible == total ? QString("%1 functions").arg(total)
