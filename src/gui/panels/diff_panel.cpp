...
std::string DiffResult::toUnifiedDiff(const std::string& leftName, const std::string& rightName, int context) const
{
	...
	while (i < n)
	{
		...
		while (i < n && ops[i].kind == DiffOpKind::Equal)
			++i;
		if (i >= n) break;
		...
		while (end < n)
		{
			if (ops[end].kind != DiffOpKind::Equal)
				end++;
			else
			{
				// Count equal lines ahead
				int eq = 0;
				for (int j = end; j < n && ops[j].kind == DiffOpKind::Equal; ++j)
					eq++;
				if (eq <= 2 * context)
					end += eq; // merge nearby hunks
				else
				{
					end += context;
					break;
		...
		os << "@@ -" << leftStart << "," << leftLen << " +" << rightStart << "," << rightLen << " @@\n";
		for (int j = start; j < end; ++j)
		{
			char prefix = ' ';
...
std::string DiffResult::toHtml() const
{
	...
	for (const auto& op: ops)
	{
		...
		if (op.kind == DiffOpKind::Delete)
			cls = "background:#3d1212;color:#ff9999";
		else if (op.kind == DiffOpKind::Insert)
			cls = "background:#122d12;color:#99ff99";
		else
			cls = "background:#1e1e2e;color:#cdd6f4";
		char prefix = op.kind == DiffOpKind::Delete ? '-' : op.kind == DiffOpKind::Insert ? '+' : ' ';
		// Escape HTML
		std::string escaped;
		for (char c: op.line)
		{
...
MyersDiff::Snake MyersDiff::midpoint(
	const std::vector<std::string>& /*a*/, const std::vector<std::string>& /*b*/, int aLo, int aHi, int bLo, int bHi)
{
...
DiffResult MyersDiff::diff(const std::vector<std::string>& left, const std::vector<std::string>& right)
{
	DiffResult result;
...
DiffResult MyersDiff::diffText(const std::string& leftText, const std::string& rightText)
{
...
DiffLineNumberArea::DiffLineNumberArea(DiffPane* editor): QWidget(editor), editor_(editor) {}
...
DiffPane::DiffPane(QWidget* parent): QPlainTextEdit(parent), lineNumberArea_(new DiffLineNumberArea(this))
{
	setReadOnly(true);
	...
	connect(this, &QPlainTextEdit::cursorPositionChanged, this, &DiffPane::highlightCurrentLine);
...
void DiffPane::lineNumberAreaPaintEvent(QPaintEvent* event)
{
	...
	while (block.isValid() && top <= event->rect().bottom())
	{
		...
		{
			QColor bg = (blockNum < static_cast<int>(lineColors_.size()) && lineColors_[blockNum].isValid())
						  ? lineColors_[blockNum].darker(130)
						  : QColor("#181825");
			painter.fillRect(0, top, lineNumberArea_->width() - 2, lineH, bg);
			painter.setPen(kColLineNum);
			painter.drawText(
				0, top, lineNumberArea_->width() - 4, lineH, Qt::AlignRight, QString::number(blockNum + 1));
		}
		block = block.next();
		top = bottom;
...
void DiffPane::setLineColors(const std::vector<QColor>& colors)
{
	...
	while (!cursor.atEnd() && line < static_cast<int>(colors.size()))
	{
		...
		if (colors[line].isValid()) fmt.setBackground(colors[line]);
...
DiffPanel::DiffPanel(QWidget* parent): PanelBase("Diff / Compare", parent)
{
	setupUI();
}
...
void DiffPanel::setupUI()
{
	...
	// ── Scroll sync ──────────────────────────────────────────────────────────
	connect(leftPane_->verticalScrollBar(), &QScrollBar::valueChanged, this, &DiffPanel::onLeftScrollChanged);
	connect(rightPane_->verticalScrollBar(), &QScrollBar::valueChanged, this, &DiffPanel::onRightScrollChanged);

	// ── Toolbar connections ───────────────────────────────────────────────────
	connect(stageCombo_, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &DiffPanel::onStageSelected);
...
void DiffPanel::updateStats()
{
	...
	statsLabel_->setText(QString("<span style='color:#a6e3a1'>+%1</span>  "
...
{
	if (hunkLines_.empty()) return;
	currentHunk_ = (currentHunk_ - 1 + static_cast<int>(hunkLines_.size())) % static_cast<int>(hunkLines_.size());
	int line = hunkLines_[currentHunk_];
	leftPane_->scrollToLine(line);
	rightPane_->scrollToLine(line);
}
...
{
	QString path = QFileDialog::getSaveFileName(this, "Export Diff", "diff.patch", "Patch files (*.patch *.diff)");
	if (path.isEmpty()) return;
	QFile f(path);
	if (!f.open(QIODevice::WriteOnly | QIODevice::Text)) return;
	f.write(QByteArray::fromStdString(currentDiff_.toUnifiedDiff("before", "after")));
}

void DiffPanel::onExportHtml()
{
	QString path = QFileDialog::getSaveFileName(this, "Export Diff HTML", "diff.html", "HTML files (*.html)");
	if (path.isEmpty()) return;
	QFile f(path);
	if (!f.open(QIODevice::WriteOnly | QIODevice::Text)) return;
...
{
	QApplication::clipboard()->setText(QString::fromStdString(currentDiff_.toUnifiedDiff("before", "after")));
}

// ── Public API ────────────────────────────────────────────────────────────────

void DiffPanel::setDiff(const QString& before, const QString& after, const QString& stageName)
{
...
void DiffPanel::addStage(const QString& name, const QString& before, const QString& after)
{
	stages_.push_back({name, before, after});
	if (stageCombo_) stageCombo_->addItem(name);
