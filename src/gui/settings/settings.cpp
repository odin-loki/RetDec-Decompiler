...
void AppSettings::loadGeneral(QSettings& s)
{
	...
	general.theme =
		static_cast<GeneralSettings::Theme>(s.value("theme", static_cast<int>(GeneralSettings::Theme::Dark)).toInt());
	QFont font;
...
bool AppSettings::exportToFile(const QString& path) const
{
	...
	gen["restoreSession"] = general.restoreSession;
	root["General"] = gen;

	QJsonObject ana;
	ana["enableTyping"] = analysis.enableTyping;
	ana["enablePatternMatch"] = analysis.enablePatternMatch;
	ana["enableConcurrency"] = analysis.enableConcurrency;
	ana["enableCudaRecovery"] = analysis.enableCudaRecovery;
	ana["enableSerialDetect"] = analysis.enableSerialDetect;
	ana["enableModuleCluster"] = analysis.enableModuleCluster;
	ana["enableCxxLifter"] = analysis.enableCxxLifter;
	ana["minTypeConfidence"] = analysis.minTypeConfidence;
	ana["minPatternConfidence"] = analysis.minPatternConfidence;
	ana["minRecoveryConfidence"] = analysis.minRecoveryConfidence;
	...
	ml_obj["quantLevel"] = static_cast<int>(ml.quantLevel);
	ml_obj["inferenceDevice"] = static_cast<int>(ml.inferenceDevice);
	...
	cuda_obj["deviceName"] = cuda.deviceName;
	cuda_obj["deviceIndex"] = cuda.deviceIndex;
	cuda_obj["kernelCacheDir"] = cuda.kernelCacheDir;
	cuda_obj["enableProfiling"] = cuda.enableProfiling;
	cuda_obj["useGPU"] = cuda.useGPU;
	cuda_obj["blockSize"] = cuda.blockSize;
	root["CUDA"] = cuda_obj;

	QJsonObject rec;
	rec["detectSTL"] = recovery.detectSTL;
	rec["detectCrypto"] = recovery.detectCrypto;
	rec["detectPatterns"] = recovery.detectPatterns;
	rec["detectConcurrency"] = recovery.detectConcurrency;
	rec["detectCuda"] = recovery.detectCuda;
	rec["detectRTTI"] = recovery.detectRTTI;
	rec["detectExceptions"] = recovery.detectExceptions;
	rec["detectVirtual"] = recovery.detectVirtual;
	rec["stlConfidence"] = recovery.stlConfidence;
	rec["cryptoConfidence"] = recovery.cryptoConfidence;
	rec["patternConfidence"] = recovery.patternConfidence;
	rec["concurrencyConfidence"] = recovery.concurrencyConfidence;
	root["Recovery"] = rec;

	QJsonObject adv;
	adv["verbosity"] = static_cast<int>(advanced.verbosity);
	adv["irDumpPath"] = advanced.irDumpPath;
	adv["intermediateDir"] = advanced.intermediateDir;
	adv["dumpIR"] = advanced.dumpIR;
	adv["dumpASM"] = advanced.dumpASM;
	adv["dumpCFG"] = advanced.dumpCFG;
	adv["dumpSSA"] = advanced.dumpSSA;
	adv["colorOutput"] = advanced.colorOutput;
	adv["maxFunctions"] = advanced.maxFunctions;
	adv["demangleNames"] = advanced.demangleNames;
	root["Advanced"] = adv;
...
bool AppSettings::importFromFile(const QString& path)
{
	...
	if (root.contains("General"))
	{
		...
		QFont font;
		font.fromString(gen.value("font").toString());
		general.editorFont = font;
		...
		general.restoreSession = gen.value("restoreSession").toBool(true);
	}
	...
	if (root.contains("Analysis"))
	{
		...
		analysis.enablePatternMatch = ana.value("enablePatternMatch").toBool(true);
		analysis.enableConcurrency = ana.value("enableConcurrency").toBool(true);
		analysis.enableCudaRecovery = ana.value("enableCudaRecovery").toBool(true);
		analysis.enableSerialDetect = ana.value("enableSerialDetect").toBool(true);
		analysis.enableModuleCluster = ana.value("enableModuleCluster").toBool(true);
		analysis.enableCxxLifter = ana.value("enableCxxLifter").toBool(true);
		analysis.minTypeConfidence = ana.value("minTypeConfidence").toDouble(0.5);
		analysis.minPatternConfidence = ana.value("minPatternConfidence").toDouble(0.6);
		analysis.minRecoveryConfidence = ana.value("minRecoveryConfidence").toDouble(0.4);
	...
	if (root.contains("ML"))
	{
		...
		ml.quantLevel = static_cast<MLSettings::QuantLevel>(
			m.value("quantLevel").toInt(static_cast<int>(MLSettings::QuantLevel::Q4_K_M)));
		ml.inferenceDevice = static_cast<MLSettings::InferenceDevice>(
			m.value("inferenceDevice").toInt(static_cast<int>(MLSettings::InferenceDevice::Auto)));
	...
	{
		auto c = root["CUDA"].toObject();
		cuda.deviceName = c.value("deviceName").toString();
		cuda.deviceIndex = c.value("deviceIndex").toInt(0);
		cuda.kernelCacheDir = c.value("kernelCacheDir").toString();
		cuda.enableProfiling = c.value("enableProfiling").toBool(false);
		cuda.useGPU = c.value("useGPU").toBool(true);
		cuda.blockSize = c.value("blockSize").toInt(256);
	}

	if (root.contains("Recovery"))
	{
		auto r = root["Recovery"].toObject();
		recovery.detectSTL = r.value("detectSTL").toBool(true);
		recovery.detectCrypto = r.value("detectCrypto").toBool(true);
		recovery.detectPatterns = r.value("detectPatterns").toBool(true);
		recovery.detectConcurrency = r.value("detectConcurrency").toBool(true);
		recovery.detectCuda = r.value("detectCuda").toBool(true);
		recovery.detectRTTI = r.value("detectRTTI").toBool(true);
		recovery.detectExceptions = r.value("detectExceptions").toBool(true);
		recovery.detectVirtual = r.value("detectVirtual").toBool(true);
		recovery.stlConfidence = r.value("stlConfidence").toDouble(0.6);
		recovery.cryptoConfidence = r.value("cryptoConfidence").toDouble(0.7);
		recovery.patternConfidence = r.value("patternConfidence").toDouble(0.5);
		recovery.concurrencyConfidence = r.value("concurrencyConfidence").toDouble(0.6);
	}

	if (root.contains("Advanced"))
	{
		auto a = root["Advanced"].toObject();
		advanced.verbosity = static_cast<AdvancedSettings::Verbosity>(
			a.value("verbosity").toInt(static_cast<int>(AdvancedSettings::Verbosity::Normal)));
		advanced.irDumpPath = a.value("irDumpPath").toString();
		advanced.intermediateDir = a.value("intermediateDir").toString();
		advanced.dumpIR = a.value("dumpIR").toBool(false);
		advanced.dumpASM = a.value("dumpASM").toBool(false);
		advanced.dumpCFG = a.value("dumpCFG").toBool(false);
		advanced.dumpSSA = a.value("dumpSSA").toBool(false);
		advanced.colorOutput = a.value("colorOutput").toBool(true);
		advanced.maxFunctions = a.value("maxFunctions").toInt(0);
		advanced.demangleNames = a.value("demangleNames").toBool(true);
	}
