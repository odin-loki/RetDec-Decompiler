...
makeLlvmPassesJsonTempFile(const DecompilerSettings& d, bool fastPreset, QString* errOut)
{
	const QString resource = fastPreset ? QStringLiteral(":/retdec/llvm_passes_fast.json")
										: QStringLiteral(":/retdec/llvm_passes_default.json");
	QFile f(resource);
	if (!f.open(QIODevice::ReadOnly))
	{
		if (errOut) *errOut = QStringLiteral("Missing resource %1").arg(resource);
		return nullptr;
	}
	QJsonParseError pe{};
	...
	if (!doc.isArray())
	{
		if (errOut) *errOut = QStringLiteral("Invalid LLVM passes resource %1").arg(resource);
		return nullptr;
	}
	const QSet<QString> disabled(d.llvmPassesDisabled.begin(), d.llvmPassesDisabled.end());
	QJsonArray out;
	for (const QJsonValue& v: doc.array())
	{
		if (!v.isString()) continue;
		const QString s = v.toString();
		if (!disabled.contains(s)) out.append(s);
	}
	auto t = std::make_unique<QTemporaryFile>();
	t->setAutoRemove(true);
	if (!t->open())
	{
		if (errOut) *errOut = QStringLiteral("Could not create temporary LLVM passes file.");
		return nullptr;
	}
	const QJsonDocument outDoc(out);
	if (t->write(outDoc.toJson(QJsonDocument::Compact)) < 0)
	{
		if (errOut) *errOut = QStringLiteral("Could not write LLVM passes JSON.");
		return nullptr;
	}
	t->flush();
	return t;
...
{
	static const QRegularExpression elapsedRe(QStringLiteral(R"(\(\s*[0-9.]+\s*s\s*\)$)"));
...
QString extractPhaseText(const QString& line)
{
	...
	if (runIdx >= 0) return stripElapsedSuffix(t.mid(runIdx + 14).trimmed());

	const int subIdx = t.lastIndexOf(QStringLiteral(" -> "));
	if (subIdx >= 0) return stripElapsedSuffix(t.mid(subIdx + 4).trimmed());
...
bool matchLogStage(const QString& phaseText, QString* stageOut, int* orderOut)
{
	...
	for (const LogStageRule& rule: kLogStageRules)
	{
		...
		if (kwLen > bestKeywordLen || (kwLen == bestKeywordLen && rule.order > bestOrder))
		{
	...
	if (bestOrder < 0) return false;

	if (stageOut) *stageOut = bestStage;
	if (orderOut) *orderOut = bestOrder;
	return true;
}
...
{
	static const QRegularExpression progressRe(QStringLiteral(R"(\[progress\][^\n]*?(\d+)%)"));
...
bool parsePhaseLine(const QString& line, DecompileLogProgress* out)
{
	...
	QString stage;
	int order = -1;
	if (!matchLogStage(phaseText, &stage, &order)) return false;
...
QStringList buildDecompilerArguments(
	const DecompilerLaunchRequest& req, QString* errOut, std::unique_ptr<QTemporaryFile>* llvmPassesOut)
{
	QStringList args;
	...
	if (!req.decompiler.extraConfigPath.isEmpty())
	{
		...
		{
			*errOut = QStringLiteral("Configured --config file missing: %1").arg(req.decompiler.extraConfigPath);
		}
	}
	...
	if (profileName.isEmpty()) profileName = QStringLiteral("balanced");
	if (req.fastDecompile) profileName = QStringLiteral("fast");
	if (!req.decompiler.useCustomLlvmPasses) args << QStringLiteral("--profile") << profileName;
	...
	{
		QString passErr;
		auto passes = makeLlvmPassesJsonTempFile(req.decompiler, req.fastDecompile, &passErr);
		if (!passes)
		{
			if (errOut) *errOut = passErr;
			return {};
		}
		passes->close();
...
QProcessEnvironment buildDecompilerProcessEnvironment(const AppSettings& settings, bool applyInteractiveOverrides)
{
	...
	{
		const QString cache = settings.cuda.kernelCacheDir.trimmed();
		if (!cache.isEmpty()) env.insert(QStringLiteral("RETDEC_OCL_CACHE_DIR"), QDir(cache).absolutePath());
	}
	if (settings.cuda.enableProfiling) env.insert(QStringLiteral("RETDEC_PROFILE_JSON"), QStringLiteral("1"));
	return env;
}

void appendDecompilerLogToConsole(panels::LiveConsolePanel* panel, const QString& logPath, qint64 maxBytes)
{
	if (!panel || logPath.isEmpty()) return;
	...
	if (nl >= 0) tail.remove(0, nl + 1);

	panel->appendLine(
		panels::LiveConsolePanel::Stream::Stdout,
		QStringLiteral("… (%1 KiB log omitted, showing last %2 KiB) …").arg(size / 1024).arg(maxBytes / 1024));
	if (!tail.isEmpty()) panel->appendChunk(panels::LiveConsolePanel::Stream::Stdout, tail);
}

bool appendDecompilerLogIncrementalToConsole(
	panels::LiveConsolePanel* panel, const QString& logPath, qint64* ioFileOffset, qint64 maxBytesPerTick)
{
	if (!panel || !ioFileOffset || logPath.isEmpty() || maxBytesPerTick <= 0) return false;

	QFile f(logPath);
	if (!f.open(QIODevice::ReadOnly)) return false;

	const qint64 size = f.size();
	if (*ioFileOffset > size) *ioFileOffset = size;
...
void scanDecompilerLogDiagnostics(panels::DiagnosticsPanel* diagnostics, const QString& logPath, int maxEntries)
{
	if (!diagnostics || logPath.isEmpty() || maxEntries <= 0) return;
...
void populateSemanticDetectionsFromConfig(
	panels::DiagnosticsPanel* diagnostics, const QJsonObject& configRoot, int maxEntries)
{
	if (!diagnostics || maxEntries <= 0) return;
	...
	for (const QJsonValue& fv: fnArr)
	{
		...
		const QJsonArray dets = fnObj.value(QStringLiteral("semanticDetections")).toArray();
		if (dets.isEmpty()) continue;

		uint64_t fnAddr = 0;
		{
			bool ok = false;
			const uint64_t a = fnObj.value(QStringLiteral("startAddr")).toString().toULongLong(&ok, 0);
			if (ok) fnAddr = a;
		}
		...
		for (const QJsonValue& dv: dets)
		{
			...
			QString msg = label;
			if (msg.isEmpty()) msg = kind;
			if (msg.isEmpty()) msg = QStringLiteral("detection");
			msg += QStringLiteral(" (confidence %1%)").arg(QString::number(conf * 100.0, 'f', 0));
			if (!detail.isEmpty()) msg += QStringLiteral(" — ") + detail;

			diagnostics->addMessage(sev, fnName.isEmpty() ? QStringLiteral("semantic") : fnName, msg, fnAddr);
			++added;
		}
	}
}
...
{
	if (!ioFileOffset || !out || logPath.isEmpty()) return false;
...
QString resolveGuiDecompiledCPath(const QString& binaryPath, const QString& outputDir)
{
	...
	const QString fileName = bfi.completeBaseName() + QStringLiteral(".gui-decompiled.c");
	const QString trimmed = outputDir.trimmed();
	if (trimmed.isEmpty()) return QDir::fromNativeSeparators(QDir(bfi.path()).filePath(fileName));
	return QDir::fromNativeSeparators(QDir(trimmed).filePath(fileName));
}

QString locateGuiDecompiledCPath(const QString& binaryPath, const QString& outputDir)
{
	const QString trimmed = outputDir.trimmed();
	if (!trimmed.isEmpty())
	{
		const QString configured = resolveGuiDecompiledCPath(binaryPath, trimmed);
		if (QFileInfo::exists(configured)) return QFileInfo(configured).absoluteFilePath();
	}
