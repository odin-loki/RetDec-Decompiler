...
std::vector<DecompileArtifacts::SemanticDetectionEntry> parseSemanticDetections(const QJsonArray& fnArr)
{
	...
	for (const QJsonValue& v: fnArr)
	{
		...
		for (const QJsonValue& dv: fo.value(QStringLiteral("semanticDetections")).toArray())
		{
...
std::vector<panels::FunctionEntry> parseFunctions(const QJsonArray& fnArr)
{
	...
	for (const QJsonValue& v: fnArr)
	{
		...
		if (ok && end > e.address) e.sizeBytes = static_cast<int>(end - e.address);
...
std::vector<panels::StringEntry> parseStringGlobals(const QJsonArray& globals)
{
	...
	for (const QJsonValue& v: globals)
	{
		...
		if (!llvmIr.contains(QStringLiteral("i8")) && !wide) continue;
		...
		if (e.value.isEmpty()) e.value = o.value(QStringLiteral("name")).toString();
...
uint64_t parseJsonInt(const QJsonValue& v, bool* okOut = nullptr)
{
	if (v.isUndefined() || v.isNull() || v.isArray() || v.isObject() || v.isBool())
	{
		if (okOut) *okOut = false;
		return 0;
	}
	if (v.isString()) return parseAddr(v.toString(), okOut);
	const QString s = v.toString();
	bool ok = false;
	uint64_t n = s.toULongLong(&ok);
	if (!ok)
	{
		n = QString::number(v.toDouble(), 'f', 0).toULongLong(&ok);
	}
	if (okOut) *okOut = ok;
	return ok ? n : 0;
}

std::vector<panels::ConstantEntry> parseConstantGlobals(const QJsonArray& globals)
{
	std::vector<panels::ConstantEntry> out;
	for (const QJsonValue& v: globals)
	{
		if (!v.isObject()) continue;
		const QJsonObject o = v.toObject();
		...
		panels::ConstantEntry e;
		const QJsonObject storage = o.value(QStringLiteral("storage")).toObject();
		e.address = parseAddr(storage.value(QStringLiteral("value")).toString());

		if (llvmIr.contains(QStringLiteral("i64")))
			e.size = panels::ConstantSize::Qword;
		else if (llvmIr.contains(QStringLiteral("i32")))
			e.size = panels::ConstantSize::Dword;
		else if (llvmIr.contains(QStringLiteral("i16")))
			e.size = panels::ConstantSize::Word;
		else
			e.size = panels::ConstantSize::Dword;

		bool valOk = false;
		e.value = parseJsonInt(o.value(QStringLiteral("value")), &valOk);
		if (!valOk) e.value = parseJsonInt(o.value(QStringLiteral("init")), &valOk);
		if (!valOk) e.value = parseJsonInt(o.value(QStringLiteral("integer")), &valOk);
		if (!valOk) e.value = parseJsonInt(storage.value(QStringLiteral("init")), &valOk);
		if (!valOk) e.value = parseJsonInt(storage.value(QStringLiteral("integer")), &valOk);

		e.hexStr = QStringLiteral("%1").arg(e.value, static_cast<int>(e.size) * 2, 16, QChar('0')).toUpper();
		e.decStr = QString::number(e.value);
		panels::ConstantLabeler::label(e);

		if (e.address != 0 || e.value != 0) out.push_back(std::move(e));
	}
	return out;
}

std::vector<panels::BlockInstr>
extractDsmInstructionsForRange(const QString& fullDsm, uint64_t startAddr, uint64_t endAddr)
{
	std::vector<panels::BlockInstr> instrs;
	if (fullDsm.isEmpty() || endAddr <= startAddr) return instrs;

	static const QRegularExpression insRe(QStringLiteral("^(0x[0-9a-fA-F]+):\\s*(.*)$"));
...
void fillBlockInstructionsFromDsm(std::vector<panels::BasicBlockData>& blocks, const QString& fullDsm)
{
	if (fullDsm.isEmpty()) return;
	for (auto& bb: blocks)
	{
		if (bb.endAddress <= bb.address) continue;
...
{
	const QHash<QString, VtableParseData> vtables = parseVtables(config.value(QStringLiteral("vtables")).toArray());
	...
	for (const QJsonValue& cv: classArr)
	{
		...
		info.name = !demangled.isEmpty() ? demangled : co.value(QStringLiteral("name")).toString();
		...
		for (const QJsonValue& vtNameVal: co.value(QStringLiteral("virtualTables")).toArray())
		{
			...
			if (vtIt == vtables.constEnd()) continue;
			if (info.vtableAddress == 0) info.vtableAddress = vtIt->address;
			if (info.vtable.isEmpty()) info.vtable = vtIt->vtableSlots;
...
void buildCallGraph(
	const QJsonArray& fnArr, std::vector<panels::CallGraphNode>* nodes, std::vector<panels::CallEdge>* edges)
{
	nodes->clear();
	edges->clear();
	...
	auto ensureNode = [&](uint64_t addr, const QString& name, bool isLib, int instrCount) {
		if (nodeIndex.contains(addr)) return;
	...
	for (const QJsonValue& fv: fnArr)
	{
		...
		const int instrEst = (ok && end > start) ? static_cast<int>((end - start) / 4) : 1;
		const QString fncType = fo.value(QStringLiteral("fncType")).toString();
		const bool isLib =
			(fncType == QStringLiteral("staticallyLinked") || fncType == QStringLiteral("dynamicallyLinked"));
	...
	std::unordered_map<uint64_t, std::vector<panels::CFGEdgeData>>* edges)
{
	...
	for (const QJsonValue& fv: fnArr)
	{
		...
		for (const QJsonValue& bv: bbArr)
		{
			...
			if (ok && bbEnd > bb.address) bb.endAddress = bbEnd;
...
DecompileArtifactPaths pathsFromOutputC(const QString& cPath)
{
	...
	p.cPath = cPath;
	QString stem = cPath;
	if (stem.endsWith(QStringLiteral(".c"), Qt::CaseInsensitive)) stem.chop(2);
...
bool loadDecompileArtifactsFromPaths(const DecompileArtifactPaths& paths, DecompileArtifacts& out, QString* errOut)
{
	...
	{
		if (errOut) *errOut = QStringLiteral("Missing config: %1").arg(paths.configPath);
		return false;
	}
	QJsonParseError pe{};
	...
	if (!doc.isObject())
	{
		if (errOut) *errOut = QStringLiteral("Invalid config JSON: %1").arg(paths.configPath);
		return false;
	}
	...
	out.constants = parseConstantGlobals(out.config.value(QStringLiteral("globals")).toArray());
...
QString extractDsmForFunction(const QString& fullDsm, uint64_t startAddr, uint64_t endAddr, const QString& funcName)
{
	if (fullDsm.isEmpty()) return {};
	...
	for (const QString& line: lines)
	{
		...
		if (inFn)
		{
			if (line.startsWith(QStringLiteral("; function:"))) break;
			out << line;
		}
	}
	...
	if (out.isEmpty())
	{
		...
		for (const QString& line: lines)
		{
			...
			if (!m.hasMatch()) continue;
			const uint64_t a = parseAddr(m.captured(1));
			if (a >= startAddr && (endAddr == 0 || a < endAddr)) out << line;
		}
	}
...
{
	if (fullLl.isEmpty() || funcName.isEmpty()) return {};
...
{
	if (startLine < 1 || path.isEmpty()) return {};
	...
	while (!ts.atEnd())
	{
		...
		if (endLine > 0 && line > endLine) break;
		out << l;
	}
	return out.join(QLatin1Char('\n'));
}
...
{
	if (fullC.isEmpty() || funcName.isEmpty()) return {};

	const QString escaped = QRegularExpression::escape(funcName);
	const QRegularExpression sigRe(QStringLiteral(R"((?:^|\n)([\w\s\*]+?\b%1\s*\())").arg(escaped));
...
QString extractCForFunction(const QString& cPath, int startLine, int endLine, const QString& funcName)
{
	if (startLine > 0) return readLineRangeFromFile(cPath, startLine, endLine);

	if (funcName.isEmpty() || cPath.isEmpty()) return {};

	const QString fullC = readTextFileIfExists(cPath);
	return extractCForFunctionByName(fullC, funcName);
}

panels::LineMapping buildIdentityLineMapping(const QString& asmText, const QString& irText, const QString& cText)
{
...
void populateFunctionViews(
	const DecompileArtifacts& art,
	uint64_t funcAddr,
	...
	panels::TriPaneCodeView* triPane)
{
	...
	if (cfg)
	{
		...
		if (bIt != art.cfgBlocks.end() && !bIt->second.empty())
		{
			...
			const auto& edges = (eIt != art.cfgEdges.end()) ? eIt->second : std::vector<panels::CFGEdgeData>{};
