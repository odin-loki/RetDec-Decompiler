...
{
	LuaExpr e;
	e.kind = LuaExpr::Kind::Nil;
	return e;
}
LuaExpr makeTrue()
{
	LuaExpr e;
	e.kind = LuaExpr::Kind::True;
	return e;
}
LuaExpr makeFalse()
{
	LuaExpr e;
	e.kind = LuaExpr::Kind::False;
	return e;
}
LuaExpr makeInt(int64_t v)
{
	LuaExpr e;
	e.kind = LuaExpr::Kind::Int;
	e.intVal = v;
	return e;
}
...
{
	LuaExpr e;
	e.kind = LuaExpr::Kind::String;
	e.raw = s;
	return e;
}
LuaExpr makeLocal(const std::string& n)
{
	LuaExpr e;
	e.kind = LuaExpr::Kind::Local;
	e.raw = n;
	return e;
}
...
LuaEmitter::LuaEmitter(LuaEmitOptions opts): opts_(std::move(opts)) {}
...
std::string LuaEmitter::rkExpr(const LuaProto& proto, uint16_t rk, const std::vector<std::string>& regs) const
{
	if (rk & 0x100)
	{
		// Constant index
		uint16_t ki = rk & 0xFF;
		if (ki < proto.constants.size()) return constStr(proto.constants[ki]);
		return "nil";
	}
...
std::string LuaEmitter::regOrConst(const LuaProto& proto, uint16_t x, const std::vector<std::string>& regs) const
{
...
std::string LuaEmitter::decodeInstrLua51(const LuaProto& proto, int pc, const std::vector<std::string>& regs) const
{
	...
	auto kstr = [&](int ki) -> std::string {
		if (ki >= 0 && ki < (int)proto.constants.size()) return constStr(proto.constants[ki]);
		return "nil";
	};
	...
	case 5: {
		std::string gname = (Bx < (int)proto.constants.size()) ? rawStr(proto.constants[Bx]) : kstr(Bx);
		return dst + " = " + gname;
	}
	...
	// 31 FORLOOP   A sBx
	case 31:
		return "-- for step: " + reg(A) + " += " + reg(A + 2) + "; if <= " + reg(A + 1) + " { " + reg(A + 3) + " = "
			 + reg(A) + "; goto " + std::to_string(pc + 1 + sBx) + " }";
	// 32 FORPREP   A sBx
	case 32: return "-- for init: " + reg(A) + " -= " + reg(A + 2) + "; goto " + std::to_string(pc + 1 + sBx);
	...
	case 34:
		return "-- setlist: " + reg(A) + "[" + std::to_string(C) + "*50+i] = R(" + std::to_string(A) + "+i), i=1.."
			 + std::to_string(B);
	// 35 CLOSE     A      close upvalues >= R(A)
	case 35: return "-- close upvalues from " + reg(A);
	// 36 CLOSURE   A Bx   R(A) = closure(KPROTO[Bx])
	case 36: return dst + " = function() --[[ proto[" + std::to_string(Bx) + "] ]] end";
	...
	default:
		return "-- op51_" + std::to_string(op) + " A=" + std::to_string(A) + " B=" + std::to_string(B)
			 + " C=" + std::to_string(C);
	}
}
...
LuaEmitter::decodeInstrLua52(const LuaProto& proto, int pc, const std::vector<std::string>& regs, int opOverride) const
{
	...
	case 32:
		return "-- for step: " + reg(A) + " += " + reg(A + 2) + "; if <= " + reg(A + 1) + " goto "
			 + std::to_string(pc + 1 + sBx); // FORLOOP
	case 33:
		return "-- for init: " + reg(A) + " -= " + reg(A + 2) + "; goto " + std::to_string(pc + 1 + sBx); // FORPREP
	...
	default:
		return "-- op52_" + std::to_string(op) + " A=" + std::to_string(A) + " B=" + std::to_string(B)
			 + " C=" + std::to_string(C);
	}
}
...
std::string LuaEmitter::decodeInstr54(const LuaProto& proto, int pc, const std::vector<std::string>& regs) const
{
	const LuaInstr& ins = proto.code[pc];
	...
	auto fmtImm = [](int32_t v) -> std::string { return (v >= 0 ? " + " : " - ") + std::to_string(std::abs(v)); };
	...
	case 61:
		return "if (" + reg(A) + " == " + std::to_string(sB) + ") ~= " + std::to_string(k) + " then skip"; // EQI A sB k
	case 62:
		return "if (" + reg(A) + " < " + std::to_string(sB) + ") ~= " + std::to_string(k) + " then skip"; // LTI A sB k
	case 63:
		return "if (" + reg(A) + " <= " + std::to_string(sB) + ") ~= " + std::to_string(k) + " then skip"; // LEI A sB k
	case 64:
		return "if (" + reg(A) + " > " + std::to_string(sB) + ") ~= " + std::to_string(k) + " then skip"; // GTI A sB k
	case 65:
		return "if (" + reg(A) + " >= " + std::to_string(sB) + ") ~= " + std::to_string(k) + " then skip"; // GEI A sB k
	...
	case 73: {                          // FORLOOP A Bx (=73 in Lua 5.4): step + test; if continuing: pc -= Bx
		...
		return "-- forloop " + reg(A) + " += " + reg(A + 2) + "; if <= " + reg(A + 1) + " goto "
			 + std::to_string(pc + 1 - bx);
	}
	...
	default:
		return "-- op54_" + std::to_string(op) + " A=" + std::to_string(A) + " B=" + std::to_string(B)
			 + " C=" + std::to_string(C);
	}
}
...
void LuaEmitter::disassemble(const LuaProto& proto, std::ostream& out, int indent_) const
{
...
void LuaEmitter::emitProto(const LuaProto& proto, std::ostream& out, int ind, const std::string& funcName) const
{
	...
	// Function header
	if (!funcName.empty())
		out << indent(ind) << "local function " << funcName << "(" << params << ")\n";
	else
		out << indent(ind) << "-- anonymous function (" << params << ")\n";

	// Source info
	if (!proto.source.empty()) out << indent(ind + 1) << "-- source: " << proto.source << "\n";
	if (proto.lineDefined > 0)
		out << indent(ind + 1) << "-- lines: " << proto.lineDefined << "-" << proto.lastLineDefined << "\n";

	// Upvalue declarations
	for (int i = 0; i < (int)proto.upvalues.size(); ++i)
		out << indent(ind + 1) << "-- upvalue[" << i << "]: " << proto.upvalueName(i) << "\n";
	...
	{
		out << "\n";
		std::string nested = funcName.empty() ? "inner" + std::to_string(i) : funcName + "_f" + std::to_string(i);
		emitProto(proto.protos[i], out, ind + 1, nested);
	}

	if (!funcName.empty()) out << indent(ind) << "end\n";
}
...
void LuaEmitter::emitModule(const LuaModule& mod, std::ostream& out) const
{
	if (opts_.emitFileHeader)
	{
		...
		if (!mod.topLevel.source.empty()) out << "-- Source: " << mod.topLevel.source << "\n";
		out << "\n";
	}
