...
static std::string readTypeInfoName(const IBinaryView& view, uint64_t typeInfoVma)
{
	...
	if (n[0] == '_' && n[1] == 'Z' && n[2] == 'T' && (n[3] == 'S' || n[3] == 'I')) n += 4;
...
LsdaResult parseLSDA(const IBinaryView& view, uint64_t lsdaVma, uint64_t funcStart)
{
	LsdaResult res;
	...
	if (lpstartEnc != DW_EH_PE_omit) lpstart = view.readEncodedPtr(cur, lpstartEnc, cur);
	...
	res.actionTableBase = csTableEnd;

	for (const auto& s: res.sites)
	{
		if (s.action == 0) continue;
		uint64_t cur2 = res.actionTableBase + (s.action - 1);
		while (true)
		{
			if (res.actionByVma.count(cur2)) break;
			uint64_t before = cur2;
			int64_t tf = view.readSLEB128(cur2);
			int64_t nx = view.readSLEB128(cur2);

			const std::size_t idx = res.actions.size();
			res.actionByVma[before] = idx;
			LsdaResult::Action a;
			a.typeFilter = tf;
			a.nextOffset = nx;
			// Where the record was read from, and where the chain goes next.
			// cur2 already points just past the ar_next field, which is what
			// ar_next is an offset from, so that sum is the successor's VMA.
			a.recordVma = before;
			a.nextVma = (nx == 0) ? 0 : static_cast<uint64_t>(static_cast<int64_t>(cur2) + nx);
	...
	if (ttypeBase != 0 && ttypeEnc != DW_EH_PE_omit)
	{
		...
		if (lower == DW_EH_PE_udata4 || lower == DW_EH_PE_sdata4)
			ptrSize = 4;
...
void lsdaToTryCatch(const IBinaryView& view, const LsdaResult& lsda, EHFunction& fn)
{
	...
	for (const auto& site: lsda.sites)
	{
		...
		if (block.tryBegin == 0 || site.csStart < block.tryBegin) block.tryBegin = site.csStart;
		if (site.csEnd > block.tryEnd) block.tryEnd = site.csEnd;
		...
		{
			// The site names a byte offset into the action table; the map says
			// which record that is. Walking `actions` by index instead treated
			// an offset as a position in a vector ordered by discovery.
			auto actIt = lsda.actionByVma.find(lsda.actionTableBase + (site.action - 1));
			std::size_t guard = 0;
			while (actIt != lsda.actionByVma.end() && actIt->second < lsda.actions.size()
				   && guard++ <= lsda.actions.size())
			{
				const std::size_t actionIdx = actIt->second;
				...
				{
					if (existing.handlerVma == ch.handlerVma && existing.catchType == ch.catchType)
					{
						dup = true;
						break;
					}
				...
				if (act.nextVma == 0) break;
				actIt = lsda.actionByVma.find(act.nextVma);
			}
		}
	}
