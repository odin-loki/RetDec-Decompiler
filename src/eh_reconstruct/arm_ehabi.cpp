 ...
 */
static void parseArmUnwindOpcodes(const std::vector<uint8_t>& opcodes, UnwindInfo& info)
{
	std::size_t i = 0;
	int32_t vspOffset = 0;
...
static std::optional<EHFunction> processExidxEntry(const IBinaryView& view, uint64_t entryVma, uint64_t nextFnVma)
{
	...
	else
	{
		...
		else
		{
			...
			fn.personalityFn = "personality@" + std::to_string(decodePrel31(extabWord0 & 0x7FFFFFFF, extabVma));
