...
uint64_t IBinaryView::readEncodedPtr(uint64_t& cursor, uint8_t encoding, uint64_t pcrelBase) const
{
	...
	uint64_t base = 0;
	if (application == 0x10)
		base = pcrelBase; // pcrel
	else if (application == 0x30)
		base = imageBase(); // datarel
...
static std::vector<TryCatchBlock> nestBlocks(std::vector<TryCatchBlock> flat)
{
	...
	std::sort(flat.begin(), flat.end(), [](const TryCatchBlock& a, const TryCatchBlock& b) {
	...
	insertInto = [&](std::vector<TryCatchBlock>& siblings, TryCatchBlock blk) -> bool {
...
std::size_t EHReconstructor::reconstruct(const IBinaryView& view)
{
	...
	std::sort(functions_.begin(), functions_.end(), [](const EHFunction& a, const EHFunction& b) {
...
const EHFunction* EHReconstructor::findFunction(uint64_t vma) const
{
	...
	auto it = std::upper_bound(functions_.begin(), functions_.end(), vma, [](uint64_t v, const EHFunction& fn) {
	...
	{
		--it;
		if (vma >= it->functionVma && vma < it->functionEnd) return &*it;
	}
	return nullptr;
}

const TryCatchBlock* EHReconstructor::findInnermostTry(const EHFunction& func, uint64_t vma)
{
