...
{
	return std::find_if(str.begin(), str.end(), [](WideCharType c) { return !isPrintable(c); }) == str.end();
}
...
{
	return std::find_if(str.begin(), str.end(), [](WideCharType c) { return !isPrintableOrZeroByte(c); }) == str.end();
}
...
std::string charToHexaStringRepr(WideCharType c, std::size_t charSize)
{
	...
	stringRepr << "\\x" << std::setfill('0') << std::setw(widthOf(charSize)) << std::hex << c;
 ...
 *                     it is printable.
 */
std::string charToEscapedCStringRepr(WideCharType c, std::size_t charSize, bool& lastWasHex, bool forceHex)
{
	if (forceHex)
	{
		lastWasHex = true;
...
{
	return std::all_of(str.begin(), str.end(), [](const unsigned char c) { return std::isxdigit(c); });
}
...
{
	return std::any_of(str.begin(), str.end(), [](const unsigned char c) { return isNonprintableChar(c); });
}
...
{
	return std::any_of(str.begin(), str.end(), [](const unsigned char c) { return isNonasciiChar(c); });
}
...
{
	for (std::string::size_type i = 0, e = std::min(str1.size(), str2.size()); i < e; ++i)
	{
...
bool containsCaseInsensitive(const std::string& str, const std::string& sub)
{
	...
	auto it = std::search(str.begin(), str.end(), sub.begin(), sub.end(), [](unsigned char ch1, unsigned char ch2) {
 ...
 * @return Converted string in ASCII.
 */
std::string readNullTerminatedAscii(
	const std::uint8_t* bytes, std::size_t bytesLen, std::size_t offset, std::size_t maxBytes, bool failOnExceed)
{
	std::string result;
	if (!bytes)
	{
 ...
 */
bool hasSubstringInArea(
	const std::string& str, const std::string& withWhat, std::string::size_type start, std::string::size_type stop)
{
	if (start > stop)
	{
		return false;
	...
	const auto stopIterator = stopIndex < str.size() ? str.begin() + stopIndex : str.end();
	return std::search(str.begin() + start, stopIterator, withWhat.begin(), withWhat.end()) != stopIterator;
}
...
{
	s.erase(std::remove_if(s.begin(), s.end(), [](const unsigned char c) { return std::isspace(c); }), s.end());
	return s;
}
...
std::pair<std::size_t, std::size_t> getLineAndColumnFromPosition(const std::string& json, std::size_t position)
{
	...
	if (position >= json.size()) return {line, column};
...
bool isIdentifier(const std::string& str)
{
	...
	if (str[0] != '_' && !isalpha(static_cast<unsigned char>(str[0]))) return false;

	for (std::size_t i = 1; i < str.size(); ++i)
	{
		if (str[i] != '_' && !isalnum(static_cast<unsigned char>(str[i]))) return false;
	}
...
bool isNiceString(const std::string& str, double minRatio)
{
	...
	std::string s = str;
	if (!s.empty() && s.back() == '\00') s.pop_back();

	const auto niceCharCount = std::count_if(s.begin(), s.end(), isNiceCharacter);
	return !s.empty() && fpred::ratioAtLeast(static_cast<std::size_t>(niceCharCount), s.size(), minRatio);
}
...
bool isNiceAsciiWideString(const std::vector<unsigned long long>& str, double minRatio)
{
	...
	return !s.empty() && fpred::ratioAtLeast(static_cast<std::size_t>(niceCnt), s.size(), minRatio);
}
...
std::string normalizeName(const std::string& name)
{
	...
	for (unsigned i = 0; i < name.length(); i++)
	{
		...
		case ')':
			...
			break;
		case ',': res += "_comma_"; break;
		case '~': res += "_destructor_"; break;
		case '*': res += "_ptr_"; break;
		case '&': res += "_ampersand_"; break;
		case '=': res += "_eq_"; break;
		case '!': res += "_not_"; break;
		case '?':
...
{
	static const std::vector<std::string> prefixesToRemove = {"__GI_", "__isoc99_", "_isoc99_"};
 ...
 */
bool findFirstInEmbeddedLists(
	std::size_t& pos, const std::string& str, char c, const std::vector<std::pair<char, char>>& pairs)
{
	if (str.empty())
	{
		pos = std::string::npos;
