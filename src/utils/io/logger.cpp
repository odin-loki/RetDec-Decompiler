...
Logger::Logger(std::ostream& stream, bool verbose): _out(stream), _verbose(verbose)
{
#ifdef OS_WINDOWS
...
Logger::~Logger()
{
	if (_currentBrush != Color::Default) *this << Color::Default;
}
...
Logger& Logger::operator<<(const Color& lc)
{
	...
	if (_terminalNotSupported || isRedirected(_out)) return *this;
