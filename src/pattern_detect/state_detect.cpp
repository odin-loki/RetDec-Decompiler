...
{
	return countOp(fn, ssa::IrInstr::Op::Load) >= 1 && countOp(fn, ssa::IrInstr::Op::Store) >= 1
...
{
	return countOp(fn, ssa::IrInstr::Op::Store) >= 2 && countOp(fn, ssa::IrInstr::Op::Compare) >= 2;
}
	...
	if (ev.confidence >= 0.45f)
	{
		...
		r.emittedForm = stateEnum
					  + "\n"
						...
						"    }\n"
						"}";
		r.comment = "// Design pattern: State Machine (" + std::to_string(ev.stateCount) + " states)";
	}
	return r;
}
