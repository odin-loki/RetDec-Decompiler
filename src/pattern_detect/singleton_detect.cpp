	...
	for (uint32_t b = 0; b < fn.blockCount(); ++b)
	{
		...
		for (const auto* i: blk->instrs)
		{
			...
			for (const auto& u: i->uses)
			{
				...
				if (v && v->kind == ssa::ValueKind::Immediate && v->imm == 0) return true;
			}
		}
	}
	return false;
...
static bool hasLockAcquire(const ssa::SSAFunction& fn)
{
	for (uint32_t b = 0; b < fn.blockCount(); ++b)
	{
		...
		for (const auto* i: blk->instrs)
		{
			...
			const auto& cn = i->calleeName;
			if (cn.find("mutex_lock") != std::string::npos || cn.find("EnterCriticalSection") != std::string::npos
				|| cn.find("__cxa_guard_acquire") != std::string::npos
				|| cn.find("pthread_mutex_lock") != std::string::npos || cn.find("AcquireSRWLock") != std::string::npos)
				return true;
		}
	}
	return false;
...
bool SingletonDetector::hasDoubleLock(const ssa::SSAFunction& fn) const
{
	...
	for (uint32_t b = 0; b < fn.blockCount(); ++b)
	{
		...
		for (const auto* i: blk->instrs)
		{
			...
			{
				const auto* v = fn.value(u.valueId);
				if (v && v->kind == ssa::ValueKind::Immediate && v->imm == 0) ++nullChecks;
			}
		}
	}
...
PatternResult SingletonDetector::detect(const ssa::SSAFunction& fn) const
{
	...
	if (ev.confidence >= 0.45f)
	{
		r.emittedForm =
			...
			"    return instance;\n"
			"}";
		r.comment = "// Design pattern: Singleton" + std::string(ev.hasDoubleLock ? " (double-checked locking)" : "");
	}
	return r;
}

PatternResult SingletonDetector::detectGroup(const std::vector<const ssa::SSAFunction*>& fns) const
{
	PatternResult best;
