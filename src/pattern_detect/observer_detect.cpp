	...
	for (uint32_t b = 0; b < fn.blockCount(); ++b)
	{
		...
		for (const auto* i: blk->instrs)
		{
			...
			if (cn.find("push_back") != std::string::npos || cn.find("emplace_back") != std::string::npos
				|| cn.find("insert") != std::string::npos || cn.find("subscribe") != std::string::npos
				|| cn.find("addListener") != std::string::npos || cn.find("addObserver") != std::string::npos)
				return true;
		}
	}
	return false;
...
bool ObserverDetector::hasNotifyPattern(const ssa::SSAFunction& fn) const
{
	...
	for (uint32_t b = 0; b < fn.blockCount(); ++b)
	{
		...
		for (const auto* i: blk->instrs)
		{
			...
			const auto& cn = i->calleeName;
			if (cn.empty() || cn[0] == '*' || cn.find("notify") != std::string::npos
				|| cn.find("emit") != std::string::npos || cn.find("broadcast") != std::string::npos
...
PatternResult ObserverDetector::detectGroup(const std::vector<const ssa::SSAFunction*>& fns) const
{
	PatternResult r;
	r.kind = PatternKind::Observer;
