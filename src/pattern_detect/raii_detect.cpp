...
void RAIIDetector::collectCalls(
...
void RAIIDetector::pairUp(
	RAIIEvidence& ev, const std::vector<std::string>& acquires, const std::vector<std::string>& releases) const
{
...
PatternResult RAIIDetector::detectGroup(const std::vector<const ssa::SSAFunction*>& fns) const
{
	PatternResult r;
	r.kind = PatternKind::RAII;
