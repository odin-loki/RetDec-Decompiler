 ...
 *
 * Experimental (audit A9): opcode-count / IR-shape heuristics for GoF
 * patterns. Not a product claim; do not advertise Singleton/Strategy/Observer
...
std::string PatternResult::toString() const
{
	...
	if (hasVariant && !variantName.empty()) os << " [" << variantName << "]";
...
PatternDetector::ResultList PatternDetector::detectFunction(const ssa::SSAFunction& fn) const
{
...
PatternDetector::ResultList PatternDetector::detectGroup(const std::vector<const ssa::SSAFunction*>& fns) const
{
	...
	for (const auto* fn: fns)
	{
		...
		for (auto& r: intra)
		{
			...
			if (!alreadyFound) results.push_back(std::move(r));
		}
	}
	return results;
}
