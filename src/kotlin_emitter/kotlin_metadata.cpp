...
static constexpr int kBuiltinCount = static_cast<int>(sizeof(kBuiltinNames) / sizeof(kBuiltinNames[0]));

// ─── ProtobufReader ───────────────────────────────────────────────────────────

ProtobufReader::ProtobufReader(const uint8_t* data, size_t size): data_(data), size_(size), pos_(0) {}

ProtobufReader::ProtobufReader(const std::string& bytes):
	data_(reinterpret_cast<const uint8_t*>(bytes.data())), size_(bytes.size()), pos_(0)
{}

bool ProtobufReader::atEnd() const
{
	return pos_ >= size_;
}
...
const BcAnnotation* KotlinMetadataDetector::findMetadataAnnotation(const BcClass& cls)
{
	...
	{
		if (ann.typeName == "kotlin/Metadata" || ann.typeName == "Lkotlin/Metadata;"
...
std::vector<std::string> KotlinMetadataDetector::extractStringArray(const BcAnnotationValue& val)
{
	...
	if (val.kind == BcAnnotationValue::Kind::Array)
	{
		...
		{
			if (elem.kind == BcAnnotationValue::Kind::String) result.push_back(elem.stringValue);
		}
	}
...
std::vector<int> KotlinMetadataDetector::extractIntArray(const BcAnnotationValue& val)
{
	...
	if (val.kind == BcAnnotationValue::Kind::Array)
	{
		for (const auto& elem: val.arrayValue)
		{
			if (elem.kind == BcAnnotationValue::Kind::Int) result.push_back(static_cast<int>(elem.intValue));
		}
	}
...
KotlinClassMetadata KotlinMetadataDetector::detect(const BcClass& cls)
{
	...
	if (kIt != ann->elements.end() && kIt->second.kind == BcAnnotationValue::Kind::Int)
	...
	if (mvIt != ann->elements.end()) meta.metadataVersion = extractIntArray(mvIt->second);
	...
	if (d1It != ann->elements.end())
	{
		...
		if (!d1.empty()) meta.d1 = d1[0];
	}

	// d2: string table
	auto d2It = ann->elements.find("d2");
	if (d2It != ann->elements.end()) meta.stringTable = extractStringArray(d2It->second);
...
std::string KotlinMetadataDetector::lookupString(const std::vector<std::string>& strings, int32_t idx)
{
	if (idx < 0)
	{
			...
			while (argReader.readField(af))
			{
				...
				else if (af.number == kTypeArgVariance && af.wireType == 0)
				{
					...
					if (var == 1)
						...
						arg.variance = KotlinTypeArg::Variance::Inv;
				}
...
KotlinProperty KotlinMetadataDetector::decodeProperty(ProtobufReader& reader, const std::vector<std::string>& strings)
{
	KotlinProperty prop;
	ProtobufReader::Field f;
	while (reader.readField(f))
...
KotlinFunction KotlinMetadataDetector::decodeFunction(ProtobufReader& reader, const std::vector<std::string>& strings)
{
	KotlinFunction fn;
	ProtobufReader::Field f;
	while (reader.readField(f))
...
KotlinTypeParam KotlinMetadataDetector::decodeTypeParam(ProtobufReader& reader, const std::vector<std::string>& strings)
{
	KotlinTypeParam tp;
	ProtobufReader::Field f;
	while (reader.readField(f))
...
void KotlinMetadataDetector::decodeClassProto(
	ProtobufReader& reader, const std::vector<std::string>& strings, KotlinClassMetadata& out)
{
	ProtobufReader::Field f;
	while (reader.readField(f))
	{
		...
		{
			out.nestedClasses.push_back(lookupString(strings, static_cast<int32_t>(f.varint)));
		}
		else if (f.number == kClassEnumEntry && f.wireType == 2)
		{
			...
			while (er.readField(ef))
			{
				if (ef.number == 1 && ef.wireType == 0)
					out.enumEntries.push_back(lookupString(strings, static_cast<int32_t>(ef.varint)));
			}
		}
		else if (f.number == kClassSealedSubclass && f.wireType == 0)
		{
			out.sealedSubclasses.push_back(lookupString(strings, static_cast<int32_t>(f.varint)));
		}
		else if (f.number == kClassCompanion && f.wireType == 0)
		{
			out.companionName = lookupString(strings, static_cast<int32_t>(f.varint));
		}
	}
...
void KotlinMetadataDetector::decodeProto(
	const std::string& protoBytes, const std::vector<std::string>& stringTable, KotlinClassMetadata& out)
{
