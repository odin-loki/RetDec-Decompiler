...
static UseDef buildUseDef(const SSAFunction& fn)
{
	...
	for (const auto& blk: fn.blocks())
	{
		...
		{
			if (!i) continue;
			if (i->defValue != kInvalidValue) ud.defOf[i->defValue] = i;
			for (const auto& u: i->uses)
			{
				if (u.valueId != kInvalidValue) ud.users[u.valueId].push_back(i);
			}
		}
	}
	...
	{
		if (!p) continue;
		if (p->result != kInvalidValue) ud.phiDefOf[p->result] = p.get();
		for (const auto& op: p->operands)
		{
			if (op.second != kInvalidValue) ud.phiUsers[op.second].push_back(p.get());
		}
	}
	for (const auto& vp: fn.values())
	{
		if (!vp) continue;
		if (vp->defInstr && vp->id != kInvalidValue) ud.defOf.emplace(vp->id, vp->defInstr);
		if (vp->defPhi && vp->id != kInvalidValue) ud.phiDefOf.emplace(vp->id, vp->defPhi);
	}
	return ud;
}
...
{
	if (i.op != IrInstr::Op::Shr && i.op != IrInstr::Op::Sar && i.op != IrInstr::Op::Div) return false;
	for (const auto& u: i.uses)
	{
		const IrValue* imm = asImm(fn, u.valueId);
		if (!imm) continue;
		if ((i.op == IrInstr::Op::Shr || i.op == IrInstr::Op::Sar) && imm->imm == 1) return true;
		if (i.op == IrInstr::Op::Div && imm->imm == 2) return true;
	}
	return false;
}
...
{
	if (const IrInstr* d = defInstrOf(ud, id)) return d->block;
	auto pit = ud.phiDefOf.find(id);
	if (pit != ud.phiDefOf.end() && pit->second) return pit->second->block;
	const IrValue* v = fn.value(id);
...
static bool
isLoopInvariant(const SSAFunction& fn, const UseDef& ud, ValueId id, const std::unordered_set<BlockId>& loop)
{
	const IrValue* v = fn.value(id);
	if (!v) return false;
	if (v->kind == ssa::ValueKind::Immediate) return true;
	const BlockId b = defBlockOf(fn, ud, id);
	if (b == kInvalidBlock) return true; // parameter / no in-function def
...
{
	return op == IrInstr::Op::Add || op == IrInstr::Op::Sub || op == IrInstr::Op::Shl || op == IrInstr::Op::Mul
		|| op == IrInstr::Op::Assign;
}

/// mid and values computed from it (scaled index, base+mid, mid±1).
static std::unordered_set<ValueId> derivedFrom(const UseDef& ud, ValueId root, int hops)
{
	...
	for (int h = 0; h < hops; ++h)
	{
		...
		for (ValueId v: frontier)
		{
			...
			for (const IrInstr* i: it->second)
			{
				...
				if (out.insert(i->defValue).second) next.push_back(i->defValue);
			}
		}
		frontier.swap(next);
	}
...
static MidEvidence
analyseMid(const SSAFunction& fn, const UseDef& ud, const IrInstr& half, const std::unordered_set<BlockId>& loop)
{
	MidEvidence ev;
	ev.hasHalving = true;
	...
	if (loadI && loadI->defValue != kInvalidValue)
	{
		...
		if (uit != ud.users.end())
		{
			for (const IrInstr* i: uit->second)
				if (i && i->op == IrInstr::Op::Compare) cmps.push_back(i);
		}
		...
		for (const IrInstr* cmp: cmps)
		{
			...
			for (const auto& u: cmp->uses)
			{
				...
				if (isLoopInvariant(fn, ud, u.valueId, loop)) ev.targetInvariant = true;
			}
		}
	}
	...
	if (mit != ud.users.end())
	{
		...
		{
			if (!i) continue;
			if (i->op == IrInstr::Op::Compare) ev.midUsedByCompare = true;
			if (i->op == IrInstr::Op::Add || i->op == IrInstr::Op::Sub)
			{
				...
				if (i->defValue != kInvalidValue && operandIs(half, i->defValue)) continue;
				ev.midUsedByBoundArith = true;
			}
			if (i->op == IrInstr::Op::Store) ev.midUpdatesBound = true;
		}
	}

	if (ud.phiUsers.count(mid)) ev.midUpdatesBound = true;
	for (ValueId v: fromMid)
	{
		if (v == mid) continue;
		if (ud.phiUsers.count(v)) ev.midUpdatesBound = true;
	}
	if (ev.midUsedByBoundArith) ev.midUpdatesBound = true;
...
static float score(const MidEvidence& ev)
{
	...
	if (inv1 && inv2 && inv3 && inv4) return 0.90f;

	// Conservative: def-use of the shr/div result to a Load index and
	// to a Compare (of the load or of mid) or a bound Add/Sub.
	const bool conservative = ev.hasHalving && inv2 && (inv3 || ev.midUsedByCompare || ev.midUsedByBoundArith);
	if (conservative) return 0.55f;
