...
void silenceLlamaLogs(enum ggml_log_level, const char*, void*) {}
    ...
    GenerationResult generate(const std::string& prompt,
        ...
        if (config.reuseKvPrefix && !g_lastPromptTokens.empty()) {
            ...
            while (common < limit && g_lastPromptTokens[common] == tokens[common])
                ++common;
            if (common < g_lastPromptTokens.size())
        ...
        if (common < tokens.size()) {
            llama_batch batch = llama_batch_get_one(
                tokens.data() + common,
                static_cast<int32_t>(tokens.size() - common));
        ...
        for (int i = 0; i < config.maxTokens; ++i) {
            const llama_token tok = llama_sampler_sample(smpl, g_context, -1);
            if (llama_vocab_is_eog(vocab, tok)) break;
            char piece[64];
            const int len = llama_token_to_piece(
                vocab, tok, piece, sizeof(piece), 0, true);
