...
struct MemMap
{
	...
	static uint64_t pageBase(uint64_t va)
	{
		return va & ~(uint64_t)(kPageSize - 1);
	}
	static uint64_t pageOff(uint64_t va)
	{
		return va & (uint64_t)(kPageSize - 1);
	}
...
struct MiniEmu::Impl
{
	...
	bool decodeModRM(bool rexB, bool rexR, bool rexX, int& reg, bool& isReg, uint64_t& ea)
	{
		uint8_t modrm;
		if (!fetchByte(modrm)) return false;
		int mod = modrm >> 6;
		int rm = modrm & 7;
		...
		int64_t disp = 0;
		if (hasDisp8)
			disp = static_cast<int8_t>(fetchSImm8());
		else if (hasDisp32)
		{
			uint32_t d;
			if (!fetchU32(d)) return false;
	...
	retry_prefix:
		...
		if (b == 0x26 || b == 0x2E || b == 0x36 || b == 0x3E || b == 0x64 || b == 0x65) goto retry_prefix;
		...
		if (b == 0x0F)
		{
			...
			if (op2 >= 0x80 && op2 <= 0x8F)
			{
				int32_t rel;
				uint32_t r;
				if (!fetchU32(r)) return false;
				rel = static_cast<int32_t>(r);
				if (evalCond(cpu, op2 & 0xF)) cpu.rip = static_cast<uint64_t>(static_cast<int64_t>(cpu.rip) + rel);
				return true;
			}
			...
			if (op2 == 0xB6 || op2 == 0xB7)
			{
				...
				if (isReg)
				{
					v8 = static_cast<uint8_t>(regRef(cpu, static_cast<int>(ea)));
				}
				else
				{
					mem.read(ea, v8);
			...
			if (op2 == 0xBE || op2 == 0xBF)
			{
				...
				if (isReg)
					...
					mem.read(ea, v8);
				regRef(cpu, reg) = rexW ? static_cast<uint64_t>(static_cast<int64_t>(static_cast<int8_t>(v8)))
		...
		case 0xE8: {
			...
			if (!pg)
			{
				stop = StopReason::EnteredNewCode;
				stopped = true;
				return true;
			}
		...
		case 0xC2: {
			...
			uint64_t ra;
			if (!pop(ra)) return true; // stack underflow → just stop
			cpu.rip = ra;
			return true;
		}
		...
		case 0x7F: {
			int32_t rel = fetchSImm8();
			if (evalCond(cpu, b & 0xF)) cpu.rip = static_cast<uint64_t>(static_cast<int64_t>(cpu.rip) + rel);
			return true;
		}
		...
		case 0xE2: {
			int32_t rel = fetchSImm8();
			--cpu.rcx;
			if ((cpu.rcx & 0xFFFFFFFF) != 0) cpu.rip = static_cast<uint64_t>(static_cast<int64_t>(cpu.rip) + rel);
			return true;
		}
		...
		case 0xC6: {
			...
			if (!decodeModRM(rexB, rexR, rexX, reg, isReg, ea)) return false;
			uint8_t imm;
			if (!fetchByte(imm)) return false;
			if (isReg)
			{
				regRef(cpu, static_cast<int>(ea)) = imm;
			}
			else
			{
				mem.write(ea, imm);
		...
		case 0xBF: {
			...
			{
				uint64_t v;
				if (!fetchU64(v)) return false;
				regRef(cpu, r) = v;
			}
			else
			{
				uint32_t v;
				if (!fetchU32(v)) return false;
		...
		case 0xAB: { // STOSD / STOSQ
			...
			if (rep)
			{
				while (cpu.rcx > 0)
				{
					if (rexW)
					{
						if (!mem.writeU64(cpu.rdi, cpu.rax)) break;
					}
					else
					{
						if (!mem.writeU32(cpu.rdi, static_cast<uint32_t>(cpu.rax))) break;
					}
			...
			{
				if (rexW)
					mem.writeU64(cpu.rdi, cpu.rax);
				else
					mem.writeU32(cpu.rdi, static_cast<uint32_t>(cpu.rax));
				cpu.rdi += dir ? static_cast<uint64_t>(-stride) : stride;
			}
			return true;
		}
		...
		case 0x6A: {
			uint8_t imm;
			fetchByte(imm);
			push(static_cast<uint64_t>(static_cast<int8_t>(imm)));
			return true;
		}
		case 0x68: {
			uint32_t imm;
			fetchU32(imm);
			push(static_cast<uint64_t>(static_cast<int32_t>(imm)));
			return true;
		}
		...
		while (mapped < vsz)
		{
			...
			if (bounds::addFits(fo, pageOff))
			{
				...
				copyLen = bounds::clamp(
					std::min(bounds::remaining(srcOff, size), bounds::remaining(pageOff, fsz)), kPageSize);
...
UnpackResult MiniEmu::run(uint64_t entryPoint, uint64_t maxInsns)
{
	...
	{
		// Sort by VA
		std::sort(result.regions.begin(), result.regions.end(), [](const UnpackedRegion& a, const UnpackedRegion& b) {
...
bool MiniEmu::readByte(uint64_t va, uint8_t& out) const
{
	return impl_->mem.read(va, out);
}
bool MiniEmu::writeByte(uint64_t va, uint8_t val)
{
	return impl_->mem.write(va, val);
}
const CPUState& MiniEmu::cpuState() const
{
	return impl_->cpu;
}
