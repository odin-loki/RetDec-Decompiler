...
static AnnotationElem parseAnnotationElement(BinaryReader& r, const ConstPool& pool, unsigned depth);
...
static AnnotationValue parseElementValue(BinaryReader& r, const ConstPool& pool, unsigned depth)
{
	if (depth > MAX_ANNOTATION_DEPTH)
		throw JvmParseError("annotation nested deeper than " + std::to_string(MAX_ANNOTATION_DEPTH));
	...
	case 'Z': {
		uint16_t idx = r.u2();
		av.value = static_cast<int32_t>(std::get<CpInt>(pool.entry(idx)).value);
		break;
	}
	case 'J': {
		uint16_t idx = r.u2();
...
static RawAnnotation parseAnnotation(BinaryReader& r, const ConstPool& pool, unsigned depth = 0)
{
	RawAnnotation ann;
...
static PermittedSubclassesAttr parsePermittedSubclasses(BinaryReader& r)
{
	...
	uint16_t n = r.u2();
	for (uint16_t i = 0; i < n; ++i)
		ps.classIndices.push_back(r.u2());
	return ps;
}

// ─── MethodParameters attribute ──────────────────────────────────────────────

static MethodParametersAttr parseMethodParameters(BinaryReader& r, const ConstPool& pool)
{
...
ParsedAttr parseAttribute(BinaryReader& r, const ConstPool& pool, const std::string& /*context*/)
{
	uint16_t nameIdx = r.u2();
	...
	else if (name == "NestMembers")
	{
		...
		uint16_t n = r.u2();
		for (uint16_t i = 0; i < n; ++i)
			nm.memberIndices.push_back(r.u2());
		result = nm;
		parsed = true;
	}
	...
	{
		uint16_t idx = r.u2();
		result = RawAttr{name, {static_cast<uint8_t>(idx >> 8), static_cast<uint8_t>(idx & 0xff)}};
		parsed = true;
	}
	else if (
		name == "RuntimeVisibleAnnotations" || name == "RuntimeInvisibleAnnotations"
		|| name == "RuntimeVisibleParameterAnnotations" || name == "RuntimeInvisibleParameterAnnotations")
	{
...
std::vector<ParsedAttr>
parseAttributes(BinaryReader& r, const ConstPool& pool, uint16_t count, const std::string& context)
{
...
const CodeAttr* getCode(const std::vector<ParsedAttr>& attrs)
{
	for (const auto& a: attrs)
		if (const auto* c = std::get_if<CodeAttr>(&a)) return c;
	return nullptr;
}
const BootstrapMethodsAttr* getBootstrap(const std::vector<ParsedAttr>& attrs)
{
	for (const auto& a: attrs)
		if (const auto* b = std::get_if<BootstrapMethodsAttr>(&a)) return b;
	return nullptr;
}
const InnerClassesAttr* getInnerClasses(const std::vector<ParsedAttr>& attrs)
{
	for (const auto& a: attrs)
		if (const auto* c = std::get_if<InnerClassesAttr>(&a)) return c;
	return nullptr;
}
const EnclosingMethodAttr* getEnclosing(const std::vector<ParsedAttr>& attrs)
{
	for (const auto& a: attrs)
		if (const auto* e = std::get_if<EnclosingMethodAttr>(&a)) return e;
	return nullptr;
}
const RecordAttr* getRecord(const std::vector<ParsedAttr>& attrs)
{
	for (const auto& a: attrs)
		if (const auto* r = std::get_if<RecordAttr>(&a)) return r;
	return nullptr;
}
const PermittedSubclassesAttr* getPermittedSubclasses(const std::vector<ParsedAttr>& attrs)
{
	for (const auto& a: attrs)
		if (const auto* p = std::get_if<PermittedSubclassesAttr>(&a)) return p;
	return nullptr;
}
const NestHostAttr* getNestHost(const std::vector<ParsedAttr>& attrs)
{
	for (const auto& a: attrs)
		if (const auto* n = std::get_if<NestHostAttr>(&a)) return n;
	return nullptr;
}
const NestMembersAttr* getNestMembers(const std::vector<ParsedAttr>& attrs)
{
	for (const auto& a: attrs)
		if (const auto* n = std::get_if<NestMembersAttr>(&a)) return n;
	return nullptr;
}
const MethodParametersAttr* getMethodParameters(const std::vector<ParsedAttr>& attrs)
{
	for (const auto& a: attrs)
		if (const auto* m = std::get_if<MethodParametersAttr>(&a)) return m;
	return nullptr;
}

std::string getSignature(const std::vector<ParsedAttr>& attrs, const ConstPool& /*pool*/)
{
	for (const auto& a: attrs)
	{
		const auto* raw = std::get_if<RawAttr>(&a);
		if (raw && raw->name == "Signature") return std::string(raw->data.begin(), raw->data.end());
	}
	return "";
}

std::string getSourceFile(const std::vector<ParsedAttr>& attrs, const ConstPool& /*pool*/)
{
	for (const auto& a: attrs)
	{
		const auto* raw = std::get_if<RawAttr>(&a);
		if (raw && raw->name == "SourceFile") return std::string(raw->data.begin(), raw->data.end());
	}
	return "";
}

std::vector<RawAnnotation> getAnnotations(const std::vector<ParsedAttr>& attrs, const ConstPool& pool)
{
	std::vector<RawAnnotation> out;
	for (const auto& a: attrs)
	{
		const auto* raw = std::get_if<RawAttr>(&a);
		if (!raw || raw->data.size() < 2) continue;
		if (raw->name != "RuntimeVisibleAnnotations" && raw->name != "RuntimeInvisibleAnnotations") continue;
		try
		{
...
std::vector<std::vector<RawAnnotation>> getParamAnnotations(const std::vector<ParsedAttr>& attrs, const ConstPool& pool)
{
	std::vector<std::vector<RawAnnotation>> out;
	for (const auto& a: attrs)
	{
		const auto* raw = std::get_if<RawAttr>(&a);
		if (!raw || raw->data.empty()) continue;
		if (raw->name != "RuntimeVisibleParameterAnnotations" && raw->name != "RuntimeInvisibleParameterAnnotations")
			continue;
		try
		{
			BinaryReader br(raw->data.data(), raw->data.size());
