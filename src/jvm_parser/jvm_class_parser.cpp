...
static std::uint32_t clampedEnd(std::uint32_t startPc, std::uint32_t length, std::size_t codeSize)
{
...
{
	if (t.size() >= 2 && t.front() == 'L' && t.back() == ';') return t.substr(1, t.size() - 2);
	return t;
}
...
static void
fillAnnotations(std::vector<BcAnnotation>& dest, const std::vector<ParsedAttr>& attrs, const ConstPool& pool)
{
	...
	for (const auto& a: dest)
		if (a.typeName.find("Deprecated") != std::string::npos) hasDeprecated = true;
	if (hasDeprecated) return;
	for (const auto& a: attrs)
	{
		const auto* raw = std::get_if<RawAttr>(&a);
		if (!raw || raw->name != "Deprecated") continue;
		BcAnnotation d;
...
static void fillParamAnnotations(BcMethod& m, const std::vector<ParsedAttr>& attrs, const ConstPool& pool)
{
	std::vector<ParsedAttr> vis, invis;
	for (const auto& a: attrs)
	{
	...
	if (m.paramAnnotations.size() < n) m.paramAnnotations.resize(n);
...
static BcField parseField(BinaryReader& r, const ConstPool& pool, const JvmParseOptions& opts, BcClass& cls)
{
	BcField f;
	uint16_t flags = r.u2();
	...
	if (opts.parseAnnotations) fillAnnotations(f.annotations, attrs, pool);
	for (const auto& a: attrs)
	{
		const auto* raw = std::get_if<RawAttr>(&a);
		if (!raw || raw->name != "ConstantValue" || raw->data.size() != 2) continue;
	...
	if (flags & 0x4000) cls.enumConstants.push_back(f.name);
	return f;
}
...
static BcMethod
parseMethod(BinaryReader& r, const ConstPool& pool, const JvmParseOptions& opts, LiftOptions liftOpts = {})
{
	BcMethod m;
	uint16_t flags = r.u2();
	uint16_t nameIdx = r.u2();
	...
	{
		const auto* raw = std::get_if<RawAttr>(&a);
		if (!raw || raw->name != "Exceptions" || raw->data.empty()) continue;
		...
		while (i < d.size())
		{
			...
			while (j < d.size() && d[j] != 0)
				++j;
			m.throwsList.emplace_back(reinterpret_cast<const char*>(d.data() + i), j - i);
			i = j + (j < d.size() ? 1 : 0);
			if (j >= d.size()) break;
		}
	}
		...
		try
		{
			...
			for (const auto& t: ms.throwsTypes)
			{
				...
				else
				{
					...
					if (!s.empty() && s != "?") m.throwsList.push_back(std::move(s));
				}
			}
		}
		catch (const JvmParseError&)
	...
	{
		const auto* raw = std::get_if<RawAttr>(&a);
		if (!raw || raw->name != "AnnotationDefault" || raw->data.size() < 3) continue;
		char tag = static_cast<char>(raw->data[0]);
		uint16_t idx = (static_cast<uint16_t>(raw->data[1]) << 8) | static_cast<uint16_t>(raw->data[2]);
		BcAnnotation def;
		...
		else if (tag == 'Z')
		{
			...
			if (const auto* cp = std::get_if<CpInt>(&pool.entry(idx))) val.boolValue = cp->value != 0;
		}
		else if (tag == 'B' || tag == 'C' || tag == 'I' || tag == 'S')
		{
			val.kind = BcAnnotationValue::Kind::Int;
			if (const auto* cp = std::get_if<CpInt>(&pool.entry(idx))) val.intValue = cp->value;
		}
		else if (tag == 'J')
		{
			val.kind = BcAnnotationValue::Kind::Int;
			if (const auto* cp = std::get_if<CpLong>(&pool.entry(idx))) val.intValue = cp->value;
		}
		else if (tag == 'F')
		{
			val.kind = BcAnnotationValue::Kind::Float;
			if (const auto* cp = std::get_if<CpFloat>(&pool.entry(idx))) val.floatValue = cp->value;
		}
		else if (tag == 'D')
		{
			val.kind = BcAnnotationValue::Kind::Float;
			if (const auto* cp = std::get_if<CpDouble>(&pool.entry(idx))) val.floatValue = cp->value;
		}
		else
		{
			continue;
		...
		if (const auto* code = getCode(attrs))
		{
			...
			if (opts.parseDebugInfo)
			{
				for (const auto& lv: code->localVarTable)
				{
					...
					v.type = JvmSignatureParser::parseDescriptor(pool.utf8(lv.descOrSigIndex));
					...
					// from.
					v.endOffset = clampedEnd(lv.startPc, lv.length, code->bytecode.size());
				...
				for (const auto& lv: code->localVarTypeTable)
				{
					for (auto& v: m.locals)
					{
						if (v.index != lv.index || v.startOffset != lv.startPc) continue;
						try
						{
							v.type = JvmSignatureParser::parseFieldSig(pool.utf8(lv.descOrSigIndex));
						}
						catch (const JvmParseError&)
						{}
						break;
...
JvmParseResult parseClassFile(const uint8_t* data, size_t size, const JvmParseOptions& opts)
{
	JvmParseResult res;
	try
	{
		...
		// Magic number.
		uint32_t magic = r.u4();
		if (magic != 0xCAFEBABE) throw JvmParseError("invalid class file magic: " + std::to_string(magic));

		res.minorVersion = r.u2();
		res.majorVersion = r.u2();

		if (opts.strictVersion && res.majorVersion > 65)
			throw JvmParseError("unsupported class file version " + std::to_string(res.majorVersion));
		...
		res.cls.name = (lastSlash == std::string::npos) ? res.cls.fqName : res.cls.fqName.substr(lastSlash + 1);
		res.cls.packageName = (lastSlash == std::string::npos) ? "" : res.cls.fqName.substr(0, lastSlash);
		...
		{
			uint16_t idx = r.u2();
			res.cls.interfaces.push_back(bc_module::types::Class(res.pool.className(idx)));
		}
		...
		if (opts.parseAnnotations) fillAnnotations(res.cls.annotations, classAttrs, res.pool);
		...
		{
			if (nh->hostClassIndex != 0) res.cls.nestHost = res.pool.className(nh->hostClassIndex);
		}
		if (const auto* nm = getNestMembers(classAttrs))
		{
			for (uint16_t idx: nm->memberIndices)
			{
				if (idx != 0) res.cls.nestMembers.push_back(res.pool.className(idx));
			}
		}
		if (const auto* ps = getPermittedSubclasses(classAttrs))
		{
			...
			{
				if (idx != 0) res.cls.permittedSubclasses.push_back(res.pool.className(idx));
			}
			if (!res.cls.permittedSubclasses.empty()) res.cls.access = res.cls.access | BcAccess::Sealed;
		}
		if (!res.cls.signature.empty())
		{
			try
			{
				...
				for (const auto& tp: cs.typeParams)
				{
					...
					if (!bound.empty() && bound != "java/lang/Object" && bound != "Object") s += " extends " + bound;
...
JvmParseResult parseClassFile(const std::vector<uint8_t>& data, const JvmParseOptions& opts)
{
...
{
	return "Java " + std::to_string(javaRelease(major)) + " (class version " + std::to_string(major) + "."
		 + std::to_string(minor) + ")";
}
