...
{
	if (pos >= s.size()) throw JvmParseError("unexpected end of signature: " + s);
	return s[pos++];
}
...
BcType JvmSignatureParser::parseBaseType(char tag)
{
	...
	case 'Z': return Bool();
	case 'V': return Void();
	default: throw JvmParseError(std::string("unknown base type '") + tag + "'");
	}
}
...
BcType JvmSignatureParser::parseTypeVariableSignature(Cursor& c)
{
	...
	if (!c.expect(';')) throw JvmParseError("expected ';' after type variable " + id);
	return TypeVar(id);
}
...
BcType JvmSignatureParser::parseTypeArgument(Cursor& c)
{
	...
	{
		c.get();
		return BoundedAbove(parseReferenceTypeSignature(c));
	}
	if (ch == '-')
	{
		c.get();
		return BoundedBelow(parseReferenceTypeSignature(c));
	}
...
BcType JvmSignatureParser::parseClassTypeSignature(Cursor& c)
{
	...
	if (c.peek() == '<')
	{
		c.get(); // consume '<'
		while (c.peek() != '>')
			args.push_back(parseTypeArgument(c));
		c.get(); // consume '>'
	}
	...
	if (!c.expect(';')) throw JvmParseError("expected ';' after class type signature for " + name);
...
BcType JvmSignatureParser::parseReferenceTypeSignature(Cursor& c)
{
	...
	struct DepthGuard
	{
		...
		explicit DepthGuard(Cursor& c_): cur(c_)
		{
			...
			if (++cur.depth > MAX_SIGNATURE_DEPTH)
				throw JvmParseError("signature nested deeper than " + std::to_string(MAX_SIGNATURE_DEPTH));
		}
		~DepthGuard()
		{
			--cur.depth;
	...
	if (ch == '[')
	{
		return parseArrayTypeSignature(c);
	}
...
{
	char ch = c.peek();
	if (ch == 'L' || ch == 'T' || ch == '[') return parseReferenceTypeSignature(c);
	c.get();
...
JvmTypeParam JvmSignatureParser::parseTypeParameter(Cursor& c)
{
	...
	// ':' ClassBound
	if (!c.expect(':')) throw JvmParseError("expected ':' in type parameter " + tp.name);
...
std::vector<JvmTypeParam> JvmSignatureParser::parseTypeParameters(Cursor& c)
{
	...
	c.get(); // consume '<'
	while (c.peek() != '>')
		params.push_back(parseTypeParameter(c));
	c.get(); // consume '>'
	return params;
}
...
ParsedClassSignature JvmSignatureParser::parseClassSig(const std::string& sig)
{
	...
	{
		c.get();
		cs.superclass = parseClassTypeSignature(c);
	}
	else
		cs.superclass = Object();
...
ParsedMethodSignature JvmSignatureParser::parseMethodSig(const std::string& sig)
{
	...
	if (!c.expect('(')) throw JvmParseError("expected '(' in method signature: " + sig);
	while (c.peek() != ')')
...
bc_module::BcFuncType JvmSignatureParser::parseMethodDescriptor(const std::string& desc)
{
	...
	Cursor c{desc, 0};
	if (!c.expect('(')) throw JvmParseError("method descriptor must start with '(': " + desc);
	while (c.peek() != ')')
	{
