...
void BinaryReader::check(size_t n) const
{
	...
	if (!utils::bounds::rangeFits(pos_, size_, n))
		throw JvmParseError("unexpected end of file at offset " + std::to_string(pos_));
}
...
std::string BinaryReader::mutf8(uint16_t len)
{
	...
	out.resize(cap);
	const size_t written = utils::txt::mutf8ToUtf8(data_ + pos_, len, len, &out[0], cap);
	out.resize(written);
	pos_ += len;
	return out;
}
...
ConstPool ConstPool::read(BinaryReader& r)
{
	...
	for (uint16_t i = 1; i < count; ++i)
	{
		...
		case CpTag::Integer: pool.entries_.push_back(CpInt{r.s4()}); break;
		case CpTag::Float: pool.entries_.push_back(CpFloat{r.f4()}); break;
		case CpTag::Long:
		...
		case CpTag::Double:
			...
			++i;
			break;
		case CpTag::Class: pool.entries_.push_back(CpClass{r.u2()}); break;
		case CpTag::String: pool.entries_.push_back(CpString{r.u2()}); break;
		case CpTag::Fieldref: pool.entries_.push_back(CpFieldref{r.u2(), r.u2()}); break;
		case CpTag::Methodref: pool.entries_.push_back(CpMethodref{r.u2(), r.u2()}); break;
		case CpTag::InterfaceMethodref: pool.entries_.push_back(CpInterfaceMethodref{r.u2(), r.u2()}); break;
		case CpTag::NameAndType: pool.entries_.push_back(CpNameAndType{r.u2(), r.u2()}); break;
		case CpTag::MethodHandle: pool.entries_.push_back(CpMethodHandle{r.u1(), r.u2()}); break;
		case CpTag::MethodType: pool.entries_.push_back(CpMethodType{r.u2()}); break;
		case CpTag::Dynamic: pool.entries_.push_back(CpDynamic{r.u2(), r.u2()}); break;
		case CpTag::InvokeDynamic: pool.entries_.push_back(CpInvokeDynamic{r.u2(), r.u2()}); break;
		case CpTag::Module: pool.entries_.push_back(CpModule{r.u2()}); break;
		case CpTag::Package: pool.entries_.push_back(CpPackage{r.u2()}); break;
		default:
			throw JvmParseError(
				"unknown constant pool tag " + std::to_string(tag) + " at cp index " + std::to_string(i));
		}
	}
	return pool;
}
...
{
	if (idx == 0 || idx >= entries_.size()) throw JvmParseError("cp index " + std::to_string(idx) + " out of range");
	return entries_[idx];
}
...
{
	const auto& e = entry(idx);
	if (const auto* u = std::get_if<CpUtf8>(&e)) return u->value;
...
std::string ConstPool::className(uint16_t classIdx) const
{
	...
	if (const auto* c = std::get_if<CpClass>(&e)) return utf8(c->nameIndex);
...
std::string ConstPool::string(uint16_t stringIdx) const
{
	...
	if (const auto* s = std::get_if<CpString>(&e)) return utf8(s->utf8Index);
...
std::string ConstPool::name(uint16_t natIdx) const
{
	...
	if (const auto* n = std::get_if<CpNameAndType>(&e)) return utf8(n->nameIndex);
...
{
	const auto& e = entry(natIdx);
	if (const auto* n = std::get_if<CpNameAndType>(&e)) return utf8(n->descriptorIndex);
	throw JvmParseError("cp[" + std::to_string(natIdx) + "] is not NameAndType");
}

static CpRef asRef(const CpEntry& e, uint16_t idx)
{
	if (const auto* r = std::get_if<CpFieldref>(&e)) return {r->classIndex, r->nameAndTypeIndex};
	if (const auto* r = std::get_if<CpMethodref>(&e)) return {r->classIndex, r->nameAndTypeIndex};
	if (const auto* r = std::get_if<CpInterfaceMethodref>(&e)) return {r->classIndex, r->nameAndTypeIndex};
	throw JvmParseError("cp[" + std::to_string(idx) + "] is not a Fieldref/Methodref/InterfaceMethodref");
}
