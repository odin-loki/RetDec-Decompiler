...
PyStackSimulator::PyStackSimulator(const PyCodeObject& code, Options opts): code_(code), opts_(opts) {}
...
std::vector<PyStackSimulator::RawInstr> PyStackSimulator::decode() const
{
	...
	while (pos + 1 < bc.size())
	{
		...
		int32_t fullArg = full > static_cast<uint32_t>(INT32_MAX) ? INT32_MAX : static_cast<int32_t>(full);
		extArg = 0;
		extCount = 0;

		OpcodeInfo info = opcodeInfo(op, code_.version);
		result.push_back({std::string(info.name), fullArg, static_cast<uint32_t>(pos - 2)});
	}
	return result;
}
...
std::vector<uint32_t> PyStackSimulator::findLeaders(const std::vector<RawInstr>& instrs) const
{
	...
	for (size_t i = 0; i < instrs.size(); ++i)
	{
		...
		bool isJump = nm.find("JUMP") != std::string::npos || nm.find("POP_JUMP") != std::string::npos
				   || nm == "FOR_ITER" || nm == "SEND";
		...
		if (nm == "RETURN_VALUE" || nm == "RETURN_CONST" || nm == "RAISE_VARARGS" || nm == "RERAISE")
		{
			if (nextOff != UINT32_MAX) leaders.push_back(nextOff);
		}
	}
...
{
	if (idx < 0 || static_cast<size_t>(idx) >= code_.co_consts.size()) return makeNone();
...
std::string PyStackSimulator::nameFromIdx(int32_t idx, const std::vector<std::string>& table) const
{
	if (idx >= 0 && static_cast<size_t>(idx) < table.size()) return table[static_cast<size_t>(idx)];
	return "?";
}
...
int32_t PyStackSimulator::boundedPopCount(int32_t declared, const Stack& stack, int32_t perItem) const
{
	if (declared <= 0 || perItem <= 0) return 0;

	const int64_t affordable = static_cast<int64_t>(stack.size()) / perItem + kUnderflowAllowance;
	if (declared <= affordable) return declared;

	warn(
		"Operand count " + std::to_string(declared) + " exceeds what the stack can hold; truncated to "
...
PyExprPtr PyStackSimulator::buildBinOp(const std::string& opName, PyExprPtr lhs, PyExprPtr rhs) const
{
...
PyExprPtr PyStackSimulator::buildCollection(const std::string& opName, int32_t count, Stack& stack) const
{
	ExprList elts;
	...
	else if (opName == "BUILD_TUPLE")
		e->kind = PyExpr::Kind::Tuple;
	else if (opName == "BUILD_SET")
...
PyExprPtr PyStackSimulator::buildCompare(int32_t cmpIdx, PyExprPtr lhs, PyExprPtr rhs) const
{
...
bool PyStackSimulator::applyInstr(const RawInstr& instr, Stack& stack, StmtList& stmts)
{
	const std::string& nm = instr.name;
	...
	if (nm == "LOAD_DEREF" || nm == "LOAD_CLOSURE" || nm == "LOAD_CLASSDEREF")
	{
		...
		else
		{
			...
			if (ci < code_.co_freevars.size())
				n = code_.co_freevars[ci];
			else
				n = "?";
		}
	...
	if (nm == "STORE_FAST")
	{
		...
		stmts.push_back(makeAssign({makeName(nameFromIdx(arg, code_.co_varnames), ExprCtx::Store)}, val));
		return true;
	}
	if (nm == "STORE_NAME" || nm == "STORE_GLOBAL")
	{
		auto val = popExpr(stack);
		stmts.push_back(makeAssign({makeName(nameFromIdx(arg, code_.co_names), ExprCtx::Store)}, val));
		return true;
	}
	if (nm == "STORE_ATTR")
	{
	...
	if (nm == "POP_TOP")
	{
		if (!stack.empty())
		{
			...
			if (e->kind == PyExpr::Kind::Call || e->kind == PyExpr::Kind::Yield || e->kind == PyExpr::Kind::YieldFrom
	...
	if (nm.rfind("UNARY_", 0) == 0)
	{
		...
		else if (nm == "UNARY_INVERT")
			e->unaryOp = UnaryOp::Invert;
		else if (nm == "UNARY_NEGATIVE")
			e->unaryOp = UnaryOp::USub;
		else
			e->unaryOp = UnaryOp::UAdd;
		e->children = {operand};
		pushExpr(stack, e);
		return true;
	}
	...
	if (nm == "BUILD_CONST_KEY_MAP")
	{
		...
		if (keys_tuple->kind == PyExpr::Kind::Tuple || keys_tuple->kind == PyExpr::Kind::Constant)
		{
			e->keys = {keys_tuple};
		}
	...
	if (nm == "CALL_FUNCTION_KW")
	{
		...
		// Convert kw args
		int numKw = 0;
		if (kw_names_expr->kind == PyExpr::Kind::Tuple || kw_names_expr->kind == PyExpr::Kind::Constant)
		{
			...
			// first element.
			numKw = static_cast<int>(std::min<size_t>(kw_names_expr->values.size(), static_cast<size_t>(nargs)));
		}
		...
		for (int i = numPos; i < nargs; ++i)
		{
			...
			if (i - numPos < numKw && kw_names_expr->values[i - numPos])
			{
				...
				if (kname->kind == PyExpr::Kind::Constant) kw.arg = kname->sval;
			}
			kw.value = allArgs[i];
	...
	if (nm == "MAKE_FUNCTION")
	{
		...
		if (name_expr && name_expr->kind == PyExpr::Kind::Constant && !name_expr->sval.empty()) fname = name_expr->sval;
	...
	// ── NOP / RESUME / CACHE ─────────────────────────────────────────────────
	if (nm == "NOP" || nm == "RESUME" || nm == "CACHE" || nm == "PRECALL" || nm == "COPY_FREE_VARS"
		|| nm == "RETURN_GENERATOR")
	{
		return true;
	}
	...
	if (nm == "LIST_EXTEND" || nm == "SET_UPDATE" || nm == "DICT_MERGE" || nm == "DICT_UPDATE")
	{
		...
		if (!stack.empty())
		{
			auto& top = stack.back();
			if (top.expr
				&& (top.expr->kind == PyExpr::Kind::List || top.expr->kind == PyExpr::Kind::Set
	...
	if (nm.find("JUMP") != std::string::npos || nm.find("POP_JUMP") != std::string::npos || nm == "FOR_ITER"
		|| nm == "SEND")
	{
		// POP_JUMP_IF_* pops condition
		if (nm.find("POP_JUMP") != std::string::npos && !stack.empty()) popExpr(stack);
		return true;
	}
...
BlockResult PyStackSimulator::simulateBlock(
	const std::vector<RawInstr>& instrs, size_t blockStart, size_t blockEnd, Stack entryStack)
{
	BlockResult result;
...
StmtList PyStackSimulator::simulate()
{
	...
	for (size_t bi = 0; bi < leaders.size(); ++bi)
	{
		...
		{
			if (instrs[i].offset >= blockEndOff)
			{
				endIdx = i;
				break;
			}
	...
	if (allStmts.empty()) allStmts.push_back(makePass());
