...
{
	if (id < slots.size()) return &slots[id];
	return nullptr;
}

// ─── JvmStackSim ─────────────────────────────────────────────────────────────

JvmStackSim::JvmStackSim(StackSimOptions opts): opts_(opts) {}

uint32_t JvmStackSim::newSlot(StackSimResult& result, const BcType& type, uint32_t defBlock, uint32_t defOffset)
{
...
BcType JvmStackSim::meetTypes(const BcType& a, const BcType& b)
{
	...
	if (a.isRef() && b.isRef())
	{
		...
		if (a.isArray() && b.isArray())
		{
			auto& ra = a.ref();
			auto& rb = b.ref();
			if (ra.elementType && rb.elementType && *ra.elementType == *rb.elementType) return a;
...
BcType
JvmStackSim::inferPushType(const BcInstruction& insn, const StackState& stateBefore, const StackSimResult& result) const
{
	switch (insn.opcode)
	{
	// Constants
	...
	case BcOpcode::Xor:
		...
		{
			if (auto* s = result.slot(stateBefore.slotIds.back())) return s->type;
		}
		return types::Int();
	...
	case BcOpcode::FDiv:
	case BcOpcode::FRem:
	case BcOpcode::FNeg: return types::Float();
	...
	case BcOpcode::I2L:
	case BcOpcode::F2L:
	case BcOpcode::D2L: return types::Long();
	case BcOpcode::I2F:
	case BcOpcode::L2F:
	case BcOpcode::D2F: return types::Float();
	case BcOpcode::I2D:
	case BcOpcode::L2D:
	case BcOpcode::F2D: return types::Double();
	case BcOpcode::L2I:
	case BcOpcode::F2I:
	case BcOpcode::D2I:
	case BcOpcode::I2B:
	case BcOpcode::I2C:
	case BcOpcode::I2S: return types::Int();
		...
		if (stateBefore.size() >= 2)
		{
			...
			if (auto* s = result.slot(arrSlotId))
			{
				if (s->type.isRef() && s->type.isArray())
				{
					...
					if (ref.elementType) return *ref.elementType;
				}
			}
		}
	...
	case BcOpcode::MultiNewArray:
		...
		{
			if (auto* t = std::get_if<BcTypeOperand>(&insn.operands[0])) return t->type;
		}
	...
	case BcOpcode::GetStatic:
		if (!insn.operands.empty())
		{
			if (auto* f = std::get_if<BcFieldRef>(&insn.operands[0])) return f->type;
		}
		return types::Int();
	...
	// CLR-specific
	case BcOpcode::Ldstr: return BcType{BcRefType{BcRefKind::Class, "System.String"}};
	case BcOpcode::Box:
		if (!insn.operands.empty())
		{
			if (auto* t = std::get_if<BcTypeOperand>(&insn.operands[0])) return t->type;
...
void JvmStackSim::processBlock(const BcBasicBlock& blk, StackState& state, StackSimResult& result)
{
	for (const auto& insn: blk.instrs)
	{
		...
		if (insn.opcode == BcOpcode::InvokeVirtual || insn.opcode == BcOpcode::InvokeInterface
			|| insn.opcode == BcOpcode::InvokeSpecial || insn.opcode == BcOpcode::Callvirt)
		{
			// instance + args
			if (!insn.operands.empty())
			{
		...
		else if (insn.opcode == BcOpcode::InvokeStatic || insn.opcode == BcOpcode::Call)
		{
			if (!insn.operands.empty())
			{
				if (auto* m = std::get_if<BcMethodRef>(&insn.operands[0]))
		...
		if ((insn.opcode == BcOpcode::InvokeVirtual || insn.opcode == BcOpcode::InvokeInterface
			 || insn.opcode == BcOpcode::InvokeSpecial || insn.opcode == BcOpcode::InvokeStatic
			 || insn.opcode == BcOpcode::InvokeDynamic || insn.opcode == BcOpcode::Callvirt
		...
		if (insn.opcode == BcOpcode::Dup2 || insn.opcode == BcOpcode::Dup2X1 || insn.opcode == BcOpcode::Dup2X2)
		{
...
StackSimResult JvmStackSim::simulate(const BcCFG& cfg, const BcMethod& method)
{
	(void)method;
	...
	for (long long iter = 0; iter < visitBudget && !worklist.empty(); ++iter)
	{
		...
		if (blockId >= cfg.blockCount()) continue;

		if (visited.count(blockId) && !opts_.meetAtJoins) continue;
		visited[blockId] = true;

		// Get the entry state for this block.
		StackState state;
		if (result.blockEntryStates.count(blockId)) state = result.blockEntryStates[blockId];
		...
		for (uint32_t succId: blk.succs)
		{
			...
			else if (opts_.meetAtJoins)
			{
				...
				bool changed = false;
				size_t minLen = std::min(existingState.slotIds.size(), state.slotIds.size());
				...
				if (changed && !visited.count(succId)) worklist.push(succId);
			}
		}
