...
std::string ItaniumRttiReconstructor::readTiName(const BinaryView& view, uint64_t tiVma) const
{
...
void ItaniumRttiReconstructor::discoverTiVtables(const BinaryView& view, ClassHierarchyGraph& out)
{
	uint32_t ps = view.ptrSize();
	...
	uint32_t subIdx) const
{
	...
	for (std::size_t i = 0; i < 256; ++i, slotVma += ps)
	{
		...
		if (i > 0 && possibleOtt >= -0x100000 && possibleOtt <= 0x100000)
		{
			...
			if (possibleTi && view.inData(possibleTi)) break; // next sub-vtable header
		}
...
bool ItaniumRttiReconstructor::isValidVtable(
	const BinaryView& view, const std::unordered_set<uint64_t>& funcSet, uint64_t vtableVma) const
{
	uint32_t ps = view.ptrSize();
	...
	if (tiPtr != 0 && !view.inData(tiPtr) && !view.inReadOnly(tiPtr)) return false;
	...
	if (firstSlot != 0 && !view.inExecutable(firstSlot)) return false;
...
void ItaniumRttiReconstructor::scanVtables(
	const BinaryView& view, const std::unordered_set<uint64_t>& funcSet, ClassHierarchyGraph& out)
{
	uint32_t ps = view.ptrSize();
	...
	for (const auto& sec: view.sections)
	{
		...
		while (p + 2 * ps <= end)
		{
			...
			if (!view.inData(tiPtr) && !view.inReadOnly(tiPtr))
			{
				p += ps;
				continue;
			}
			...
			uint64_t firstSlot = view.readPtr(firstSlotVma);
			bool slotOk = (firstSlot == 0) || view.inExecutable(firstSlot);
			if (!slotOk)
			{
				p += ps;
				continue;
			...
			VtableInfo vt = parseVtableSlots(view, funcSet, firstSlotVma, tiPtr, ott, subIdx);
...
void ItaniumRttiReconstructor::parseSiClassTypeInfo(
	const BinaryView& view, uint64_t tiVma, ClassNode& node, ClassHierarchyGraph& out)
{
	uint32_t ps = view.ptrSize();
	...
	const BinaryView& view, uint64_t tiVma, ClassNode& node, ClassHierarchyGraph& out)
{
	...
	for (uint32_t i = 0; i < count; ++i)
	{
		...
		InheritanceEdge e;
		e.baseName = baseName;
		e.kind = (offsetFlags & 1) ? InheritanceKind::Virtual : InheritanceKind::Direct;
...
bool ItaniumRttiReconstructor::reconstruct(
	const BinaryView& view, const std::vector<uint64_t>& funcVmas, ClassHierarchyGraph& out)
{
