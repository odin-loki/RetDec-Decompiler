...
{
	return std::string(stringKindName(kind)) + " [" + encodingName(encoding) + "] " + "\"" + value + "\" @ 0x" +
		   [&] {
			   char buf[32];
			   std::snprintf(buf, sizeof(buf), "%llx", (unsigned long long)address);
...
EncodingResult detectEncoding(const uint8_t* buf, std::size_t len) noexcept
{
	...
	if (pairs >= 2)
	{
		...
		if (leScore > 0.80f && beScore < 0.50f) return {EncodingKind::UTF16LE, leScore};
		if (beScore > 0.80f && leScore < 0.50f) return {EncodingKind::UTF16BE, beScore};
	}
	...
	while (i < len)
	{
		...
		if (c >= 0x80)
		{
			...
			{
				if (i + 1 + e >= len || (buf[i + 1 + e] & 0xC0) != 0x80)
				{
					validUtf8 = false;
					break;
				}
...
readUtf16(const IBinaryView& view, uint64_t vma, bool le, std::size_t maxBytes, std::size_t* unitsOut)
{
	...
	for (std::size_t i = 0; i < maxBytes - 1; i += 2)
	{
		...
		uint16_t wc = le ? (uint16_t)(buf[0] | (buf[1] << 8)) : (uint16_t)((buf[0] << 8) | buf[1]);
		if (wc == 0)
		{
			// NUL terminator
...
std::optional<StringLiteral> typeString(const IBinaryView& view, uint64_t vma, std::size_t maxBytes)
{
	...
	// ── 5. C NUL-terminated string ────────────────────────────────────────────
	{
		...
		if (enc.kind == EncodingKind::Unknown && nulPos < 4) return std::nullopt; // too short and unrecognised
