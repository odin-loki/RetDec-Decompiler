...
StringDetector::StringDetector(const IBinaryView& view, const StringDetectorConfig& cfg): view_(view), cfg_(cfg) {}

StringDetector::~StringDetector() = default;

bool StringDetector::isKnownString(uint64_t vma) const
{
	return std::binary_search(knownStringAddrs_.begin(), knownStringAddrs_.end(), vma);
}

bool StringDetector::isDataMarked(uint64_t vma) const
{
	return std::binary_search(dataMarkedAddrs_.begin(), dataMarkedAddrs_.end(), vma);
}

void StringDetector::emitString(StringLiteral s)
{
	// Insert address into known set (keep sorted)
	auto pos = std::lower_bound(knownStringAddrs_.begin(), knownStringAddrs_.end(), s.address);
...
void StringDetector::processLiteralPoolEntry(const InstrRef& ref)
{
	...
	auto pos = std::lower_bound(dataMarkedAddrs_.begin(), dataMarkedAddrs_.end(), ref.targetVma);
...
void StringDetector::processCompareBranch(
	int64_t compareImm, uint64_t branchVma, uint64_t inlinePath, uint64_t heapPath)
{
