		...
		bool constExpr)
{
	...
	else
	{
		errs() << "\nconvertValueToType(): unhandled type conversion\n"
		       ...
		       << *val->getType() << "\n"
		       << *type << "\n\n";
		// This path represents a programmer error — an LLVM type conversion
		// that the caller should have ensured never occurs.  Abort in all
		// build configurations rather than returning nullptr and letting
		// callers dereference it.
		llvm_unreachable("convertValueToType(): unhandled type conversion");
	}
		...
		llvm::ArrayRef<llvm::Value*> args)
{
	...
	if (call->getNumUses())
	{
		...
		else
		{
			...
			{
				// Unknown user kind — skip; the result value may be used in
				// ways we don't need to clean up here.
				errs() << "[ir_modifier] unhandled call-result user: " << *u << "\n";
			}
			}
		...
		Type* preferredPointeeType)
{
	...
	Constant* c = nullptr;
	Type* t = preferredPointeeType && preferredPointeeType->isSized()
			? preferredPointeeType
		...
		const std::vector<std::string>& argNames)
{
	...
	// beginning of function body. Skip when there is no body (e.g. external
	// declarations after signature recovery in @c ParamReturn).
	//
	if (!nf->empty())
	{
