/**
 * @file src/bin2llvmir/providers/abi/riscv64.cpp
 * @brief ABI information for RISC-V RV64 (psABI).
...
AbiRiscv64::AbiRiscv64(llvm::Module* m, Config* c) :
		Abi(m, c)
{
...
bool AbiRiscv64::isGeneralPurposeRegister(const llvm::Value* val) const
{
	uint32_t rid = getRegisterId(val);
	return RISCV_REG_X0 <= rid && rid <= RISCV_REG_X31;
}

bool AbiRiscv64::isNopInstruction(cs_insn* insn)
{
