...
llvm::FunctionType* Demangler::getFunctionTypeFromDemangledName(const std::string &mangled)
{
	auto ctypesFunction = _demangler->demangleFunctionToCtypes(
		mangled,
		_ctypesModule,
	...
	auto *ty = getLlvmType(ctypesFunction->getType());
	return dyn_cast<FunctionType>(ty);
}
