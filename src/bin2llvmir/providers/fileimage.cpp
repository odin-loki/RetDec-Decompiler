		...
		bool wideString)
{
	...
	{
		// Vector types arise from auto-vectorisation at higher optimisation
		// levels.  Read the raw bytes as an integer of the same total width
		// and return a splat-zero constant so compilation can continue.
		unsigned bits = vt->getPrimitiveSizeInBits();
		if (bits == 0) bits = 128;
		auto* intTy = IntegerType::get(type->getContext(), bits);
		c = getConstantInt(intTy, addr);
		if (c)
		{
			// Re-interpret as a zero-filled vector to match the declared type.
