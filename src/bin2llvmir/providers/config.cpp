		...
		const std::string& realName)
{
	...
	if (cf == nullptr)
	{
		// The alloca's parent function has no config entry; the stack variable
		// cannot be recorded.  Return null so callers can handle this gracefully.
		errs() << "[config] insertStackVariable(): no config function for "
		       << sv->getFunction()->getName() << "\n";
		return nullptr;
	}
