...
CallingConventionProvider::CallingConventionProvider()
{
	...
	// __vectorcall: passes first two integer args in ECX/EDX (x86) like fastcall,
	// plus vector/float args in XMM registers.  Use fastcall as the integer-register
	// approximation; XMM handling is resolved by the dataflow filter.
	registerCC(CallingConvention::ID::CC_VECTORCALL, &FastcallCallingConvention::create);
	// Intel __regcall: maximises use of registers; closest existing model is fastcall.
	registerCC(CallingConvention::ID::CC_REGCALL, &FastcallCallingConvention::create);
}
...
{
	// CallingConventionProvider has no per-module cached data — the _id2cc
	// table holds constructor function pointers that are registered once in
	// the singleton constructor and must persist for the lifetime of the
	// process.  Clearing it (as was incorrectly done before) would destroy
	// all registered constructors, causing createCallingConvention() to
	// return nullptr and triggering an assertion in FilterProvider::createFilter.
