		...
		bool linear)
{
	if (auto* l = dyn_cast<LoadInst>(value))
	{
		...
		else if (RDA && RDA->wasRun())
		{
			...
			{
				// Too many reaching definitions — mark this node as invalid
				// and leave ops empty.  Callers must check isInvalid() before
				// treating the tree as a concrete, analysable value.
				_invalid = true;
				return;
			}
			else
...
void SymbolicTree::_simplifyNode()
{
	...
	// MIPS: t9 holds the function's own entry address for PIC calls.
	// Only substitute if we can actually resolve the function address — a zero
	// address would produce a wrong constant and silently misguide callers.
	else if (_abi->isMips()
			&& match(*this, m_Load(
...
void SymbolicTree::solveMemoryLoads(FileImage* image)
{
	...
	if (isa<LoadInst>(value)
			...
			&& cast<ConstantInt>(ops[0].value)->getBitWidth() <= 64)
	{
...
SymbolicTree* SymbolicTree::getMaxIntValue()
{
	...
	for (auto &o : ops)
	{
		...
		else if (m && mc && mc->getBitWidth() <= 64 && maxc->getBitWidth() <= 64
		         && mc->getSExtValue() > maxc->getSExtValue())
		{
			max = m;
		}
	}
	...
	if (auto* c = dyn_cast<ConstantInt>(value))
	{
		...
		else if (c->getBitWidth() <= 64 && maxc->getBitWidth() <= 64
		         && c->getSExtValue() > maxc->getSExtValue())
		{
			max = this;
		}
	}
