...
/**
 * Stage 8: Convergent function boundaries — higher priority overrides.
 * DEBUG > SYMBOL > CONFIG. Use higher-priority sources for more accurate boundaries.
 */
void Decoder::addFunctionSize(llvm::Function* f, std::optional<std::size_t> sz,
		FunctionSizePriority prio)
{
	if (!sz.has_value())
		return;
	auto it = _fnc2sz.find(f);
	if (it == _fnc2sz.end())
	{
		_fnc2sz.emplace(f, std::make_pair(sz.value(), prio));
		return;
	}
	// Override if new source has higher priority
	if (static_cast<int>(prio) > static_cast<int>(it->second.second))
		it->second = std::make_pair(sz.value(), prio);
}
