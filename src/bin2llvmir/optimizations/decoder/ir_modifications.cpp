		...
		llvm::Instruction* from)
{
	...
	{
		// Stage 9: Eager constraint CFG — when strict, only create blocks for addr in ranges.
		if (_ranges.isStrict() && !_ranges.primaryEmpty() && !_ranges.get(addr))
		{
			LOG << "\t\t\t\t" << "B: addr outside ranges (strict) @ " << addr << std::endl;
			return;
		}
		tBb = createBasicBlock(addr, bb->getParent(), bb);
	...
	{
		// Stage 9: Eager constraint CFG — when strict, only create functions for addr in ranges.
		if (_ranges.isStrict() && !_ranges.primaryEmpty() && !_ranges.get(addr))
		{
			LOG << "\t\t\t\t" << "B: addr outside ranges (strict) @ " << addr << std::endl;
			return;
		...
		std::set<llvm::BasicBlock*>& newFncStarts)
{
	...
	{
		if (fAddr <= addr && addr < (fAddr + fSzIt->second.first))
		{
