...
bool simpleTypesNextRunIsFullAnalysis = true;

/// Returns true for values that must never enter the EqSet graph.
/// This covers:
///  - Integer values wider than 64 bits (i128, i256, …)
///  - AllocaInst objects that allocate a >64-bit integer type
///    (XMM register backing stores from the SSE instruction translator)
/// Inserting such values triggers unordered_set hash-chain corruption because
/// the propagation / apply phases were never written to handle them.
static bool isWideValue(const llvm::Value* v)
{
	if (!v) return false;
	if (const auto* it = dyn_cast<IntegerType>(v->getType()))
		if (it->getBitWidth() > 64) return true;
	if (const auto* ai = dyn_cast<AllocaInst>(v))
		if (const auto* at = dyn_cast<IntegerType>(ai->getAllocatedType()))
			if (at->getBitWidth() > 64) return true;
	return false;
}

} // namespace

void SimpleTypesAnalysis::resetAlternationForTests()
{
	simpleTypesNextRunIsFullAnalysis = true;
}
...
bool SimpleTypesAnalysis::runOnModule(Module& M)
{
	...
	if (simpleTypesNextRunIsFullAnalysis)
	{
		simpleTypesNextRunIsFullAnalysis = false;

		// Detect whether the module contains i128 (or wider) alloca backing
		// stores inserted by the SSE instruction translator.  Building the
		// equivalence-set graph in the presence of such types causes
		// unordered_set hash-table corruption; skip the EqSet phase when
		// such allocas are present and go straight to the bookkeeping steps.
		bool hasWideAllocas = false;
		for (auto& F : M)
		{
			for (auto& B : F)
			{
				for (auto& I : B)
				{
					...
					{
						if (auto* at = dyn_cast<IntegerType>(ai->getAllocatedType()))
						{
							if (at->getBitWidth() > 64)
							{
								hasWideAllocas = true;
								break;
							}
...
void SimpleTypesAnalysis::pruneEmptyEqSets()
{
	std::unordered_set<EqSet*> referenced;
	referenced.reserve(processedObjs.size() * 2u + 16);
	for (const auto& p : processedObjs)
	{
		referenced.insert(p.second);
	}
	for (auto& es : eqSets.eqSets)
	{
		for (const auto& eq : es.equationSet)
		{
			referenced.insert(eq.other);
		}
	}
	for (auto it = eqSets.eqSets.begin(); it != eqSets.eqSets.end(); )
	{
		EqSet& es = *it;
		if (es.valSet.empty() && es.typeSet.empty() && es.equationSet.empty()
				&& referenced.count(&es) == 0)
		{
			it = eqSets.eqSets.erase(it);
		}
		else
		{
			++it;
...
{
	// Reject wide-integer values and i128 alloca backing stores.
	if (isWideValue(root))
		return;
	...
	if (processedObjs.find(root) == processedObjs.end())
	{
		...
		toProcess.push(root);
		EqSet* active = &eqSet;
		processValue(toProcess, active);

		auto& back = eqSets.eqSets.back();
		if (back.valSet.size() <= 1 && back.typeSet.size() <= 1
				&& back.equationSet.size() <= 1)
		{
 ...
 * @param eqSetPtr In/out: equivalence set for this walk; updated if merged with an existing set.
 */
void SimpleTypesAnalysis::remapEquationOtherPointers(EqSet* from, EqSet* to)
{
	for (auto& es : eqSets.eqSets)
	{
		EquationEntrySet rebuilt;
		for (const auto& e : es.equationSet)
		{
			EqSet* o = e.other;
			if (o == from)
			{
			...
			rebuilt.insert(e.withRemappedOther(o));
		}
		es.equationSet = std::move(rebuilt);
	}
}

void SimpleTypesAnalysis::mergeEqSetInto(EqSet& sink, EqSet& victim)
{
	if (&sink == &victim)
	{
		return;
	}
	remapEquationOtherPointers(&victim, &sink);
	for (const auto& ve : victim.valSet)
	{
		// Skip any wide-integer value that may have slipped in previously.
		if (isWideValue(ve.value))
			continue;
		sink.valSet.insert(ve);
		processedObjs[ve.value] = &sink;
	}
	for (const auto& te : victim.typeSet)
	{
		sink.typeSet.insert(te);
	}
	for (const auto& e : victim.equationSet)
	{
		sink.equationSet.insert(e);
	}
	victim.valSet.clear();
	victim.typeSet.clear();
	victim.equationSet.clear();
}

void SimpleTypesAnalysis::processValue(std::queue<Value*>& toProcess, EqSet*& eqSetPtr)
{
	while (!toProcess.empty())
	{
		...
		// Skip wide-integer values and i128 alloca backing stores.
		if (isWideValue(current))
			continue;

		auto found = processedObjs.find(current);
		if (found != processedObjs.end())
		{
			if (found->second != eqSetPtr)
			{
				mergeEqSetInto(*found->second, *eqSetPtr);
				eqSetPtr = found->second;
			}
			continue;
		}
...
void SimpleTypesAnalysis::processUse(llvm::Value* current, Value* u, std::queue<Value*>& toProcess, EqSet& eqSet)
{
	...
	//
	// Unhandled:
	// Invoke, Resume, Unreachable, Fence,
	...
	{
		// Category 2.1: pointer base and GEP result share the same equivalence walk.
		if (gep->getPointerOperand() == current) {
			toProcess.push(gep);
		}
		// Category 2.1: link GEP indices of the same LLVM type (not pointer vs index).
		const unsigned numIdx = gep->getNumIndices();
		for (unsigned ii = 0; ii < numIdx; ++ii) {
			Value* idxI = gep->getOperand(1 + ii);
			if (idxI != current) {
				continue;
			}
			for (unsigned jj = 0; jj < numIdx; ++jj) {
				if (jj == ii) {
					continue;
				}
				Value* idxJ = gep->getOperand(1 + jj);
				if (idxJ->getType() != current->getType()) {
					continue;
				}
				if (auto* ij = dyn_cast<Instruction>(idxJ)) {
					toProcess.push(ij);
				}
			}
		}
	...
	{
		simpleTypesFpExt(user, toProcess);
	}
	...
	{
		// Delegated to simpleTypesFpExt (see simple_types_fp_ext.cpp).
		simpleTypesFpExt(user, toProcess);
	}
	...
	else if (auto* asc = dyn_cast<AddrSpaceCastInst>(user))
	{
		// Category 2.1: same pointer in a different address space.
	...
	{
		// Category 2.1: compared operands usually share scalar/pointer kind and width.
	...
	else if (auto* cas = dyn_cast<AtomicCmpXchgInst>(user))
	{
		// Category 2.1: expected value and new value share the pointee's type.
		Value* cmp = cas->getCompareOperand();
		Value* nw = cas->getNewValOperand();
		if (current == cmp) {
			if (auto* nwI = dyn_cast<Instruction>(nw)) {
				toProcess.push(nwI);
			}
		} else if (current == nw) {
			if (auto* cmpI = dyn_cast<Instruction>(cmp)) {
				toProcess.push(cmpI);
			}
		}
	}
	else if (auto* armw = dyn_cast<AtomicRMWInst>(user))
	{
		// Category 2.1: RMW result type matches the operand written to memory.
		Value* val = armw->getValOperand();
		if (current == val) {
			toProcess.push(armw);
		} else if (current == armw) {
	...
	{
		// Category 2.1: incoming values of the same LLVM type are merged by the PHI.
		const unsigned n = phi->getNumIncomingValues();
		for (unsigned i = 0; i < n; ++i) {
			if (phi->getIncomingValue(i) != current) {
				continue;
			}
			for (unsigned j = 0; j < n; ++j) {
				...
				Value* other = phi->getIncomingValue(j);
	...
	else if (auto* insv = dyn_cast<InsertValueInst>(user))
	{
		// Category 2.1: inserted value and aggregate share a structural update site.
		Value* agg = insv->getAggregateOperand();
		Value* val = insv->getInsertedValueOperand();
		if (current == val) {
			if (auto* ai = dyn_cast<Instruction>(agg)) {
				toProcess.push(ai);
			}
		} else if (current == agg) {
			if (auto* vi = dyn_cast<Instruction>(val)) {
	...
	else if (auto* call = dyn_cast<CallBase>(user))
	{
		...
		// Category 2.1: memory intrinsics — use CallBase so @c invoke is covered.
		// memcmp: name prefix only (@c Intrinsic::memcmp missing on some LLVM).
		if (fnc && fnc->isIntrinsic()) {
			const Intrinsic::ID id = fnc->getIntrinsicID();
...
{
	// Guard: never insert wide-integer values or i128 alloca backing stores.
	if (isWideValue(v))
		return;
...
EquationEntry EquationEntry::withRemappedOther(EqSet* newOther) const
{
	assert(newOther);
	return EquationEntry(newOther, type);
}
