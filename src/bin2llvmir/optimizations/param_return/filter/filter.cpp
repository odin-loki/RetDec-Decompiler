...
int countPrintfStyleArgs(const std::string& fmt)
{
	...
	for (std::size_t i = 0; i < fmt.size(); ++i)
	{
				...
				|| fmt[i] == 'q' || fmt[i] == 'j' || fmt[i] == 'z' || fmt[i] == 't'))
		{
			char lm = fmt[i];
			++i;
			if (i < fmt.size() && fmt[i] == lm && (lm == 'h' || lm == 'l'))
			{
				++i;
			}
		}
...
void Filter::filterCalls(DataFlowEntry* de) const
{
	...
	for (auto& call : de->callEntries())
	{
		...
		auto groupedArgs = createGroupedArgValues(callArgsCopy.front());
		applyExtendedCallArgFiltering(call, de, _abi, groupedArgs);
		call.setArgs(std::move(groupedArgs));
		if (!callRets.empty())
		{
...
void Filter::filterCallsVariadic(DataFlowEntry* de, const Collector* collector) const
{
	...
	for (auto& call : de->callEntries())
	{
		...
		auto groupedArgs = createGroupedArgValues(callArgs.front());
		applyExtendedCallArgFiltering(call, de, _abi, groupedArgs);
		call.setArgs(std::move(groupedArgs));
		call.setRetValues(createGroupedRetValues(callRets.front()));
...
void Filter::filterRetsByKnownTypes(FilterableLayout& lay) const
{
	...
	if (retType->isVectorTy() && !vecRegs.empty())
	{
		...
		auto* firstReg = _abi->getRegister(vecRegs.front());
		if (firstReg == nullptr)
		{
	...
	{
		std::size_t typeSize = _abi->getTypeByteSize(retType);
		auto* firstReg = _abi->getRegister(doubleRegs.front());
		if (firstReg == nullptr)
		{
			lay.knownTypes = {nullptr};
			return;
	...
	{
		std::size_t typeSize = _abi->getTypeByteSize(retType);
		auto* firstReg = _abi->getRegister(fpRegs.front());
		if (firstReg == nullptr)
		{
			lay.knownTypes = {nullptr};
			return;
		...
		const std::vector<uint32_t>& vecRegs) const
{
	...
	for (auto pv: paramValues)
	{
		...
		else if (isa<GlobalVariable>(pv) && !_abi->isRegister(pv))
		{
			// Some lifted IR models GPRs as plain globals without Abi::isRegister
			// (e.g. certain ARM/PPC tests). Do not bucket ABI register globals here:
			// that would duplicate them as both "stack" layout slots and gpRegisters.
			layout.stacks.push_back(pv);
			continue;
		}
