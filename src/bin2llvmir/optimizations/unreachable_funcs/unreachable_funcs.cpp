...
void stripDeadFunctionBodiesAndEraseUnused(const std::vector<Function*>& toRemove, CallGraph& callGraph)
{
	(void)callGraph;
...
bool userPreventsElim(User* user, const std::set<Function*>& liveFuncs, int depth = 0)
{
...
RegisterPass<UnreachableFuncs>
	UnreachableFuncsRegistered("retdec-unreachable-funcs", "Unreachable functions optimization", false, false);
...
bool UnreachableFuncs::run()
{
	...
	bool isSharedOrObj = config->getConfig().fileType.isShared() || config->getConfig().fileType.isObject();
	...
	if (isSharedOrObj)
	{
		for (auto& fn: *module)
		{
			...
			if (fn.getLinkage() == GlobalValue::ExternalLinkage || fn.getLinkage() == GlobalValue::WeakAnyLinkage
				|| fn.getLinkage() == GlobalValue::WeakODRLinkage)
			{
				liveFuncs.insert(&fn);
				addToSet(ReachableFuncsAnalysis::getReachableDefinedFuncsFor(fn, *module, *callGraph), liveFuncs);
			}
		}
	}

	// 3. Globally reachable (e.g. via function pointers in live data).
	addToSet(ReachableFuncsAnalysis::getGloballyReachableFuncsFor(*module), liveFuncs);
...
void UnreachableFuncs::getFuncsThatCannotBeOptimized(std::set<Function*>& out)
{
...
void UnreachableFuncs::removeFuncsThatCanBeOptimized(const std::set<Function*>&) {}
