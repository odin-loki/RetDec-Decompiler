...
void dumpEntryOrder(llvm::Function& f)
{
	if (!valueProtectTraceEnabled() || f.empty())
	{
		return;
	}
	llvm::errs() << "[value-protect] entry order for " << f.getName() << ":\n";
	unsigned idx = 0;
	for (llvm::Instruction& i : f.getEntryBlock())
	{
		if (idx++ > 140)
		{
			llvm::errs() << "[value-protect] ... truncated ...\n";
			break;
		}
		llvm::errs() << "  [" << idx << "] ";
		if (i.hasName())
...
bool ValueProtect::protect()
{
	...
	// Normalize typed stack-slot accesses. Some earlier transformations can end
	// up reusing one stack slot with different pointee expectations (e.g. i32*
	// vs i32), which leaves malformed load/store pointer operands.
	...
	for (inst_iterator it = inst_begin(&f), eIt = inst_end(&f); it != eIt; ++it)
	{
		...
		{
			auto* base = llvm_utils::skipCasts(l->getPointerOperand());
			if (isa<AllocaInst>(base))
			{
				auto* expectedPtrTy = PointerType::get(
						l->getType(),
						l->getPointerAddressSpace());
				if (l->getPointerOperand()->getType() != expectedPtrTy)
				{
					auto* castPtr = CastInst::CreatePointerBitCastOrAddrSpaceCast(
							l->getPointerOperand(),
							expectedPtrTy,
							"",
							l);
		...
		else if (auto* s = dyn_cast<StoreInst>(insn))
		{
			auto* base = llvm_utils::skipCasts(s->getPointerOperand());
			if (isa<AllocaInst>(base))
			{
				auto* expectedPtrTy = PointerType::get(
						s->getValueOperand()->getType(),
						s->getPointerAddressSpace());
				if (s->getPointerOperand()->getType() != expectedPtrTy)
				{
					auto* castPtr = CastInst::CreatePointerBitCastOrAddrSpaceCast(
							s->getPointerOperand(),
							expectedPtrTy,
	...
	// Repair malformed CFG fragments before downstream LLVM optimizations.
	// Unreachable blocks may still contain uses that violate dominance.
	...
	do
	{
		...
		for (Function& f : *_module)
		{
			if (!f.empty() && removeUnreachableBlocks(f))
			{
				changed = true;
				removedAny = true;
			}
	...
	// Hoist all constant-size allocas from non-entry blocks to the function
	// entry block.  LLVM requires allocas to dominate all uses; keeping them
	// in the entry block (which dominates every other block) is the simplest
	// guarantee.  We must NOT use isStaticAlloca() here because that predicate
	// only returns true for allocas that are ALREADY in the entry block.
	for (Function& f : *_module)
	{
		if (f.empty())
		{
		...
		for (auto* a : toHoist)
		{
			a->moveBefore(entryInsert);
			changed = true;
		}

		// Additionally, for allocas already in the entry block, ensure they
		// come before the earliest use that is also in the entry block.
		...
		for (auto* a : entryAllocas)
		{
			...
			for (User* u : a->users())
			{
				...
				if (ui == nullptr || ui->getParent() != &f.getEntryBlock())
				{
					continue;
				}
				// Check if ui comes before a in the entry block.
				bool uiBefore = false;
				for (Instruction& bi : f.getEntryBlock())
				{
					if (&bi == ui)
					{
						uiBefore = true;
				...
				if (uiBefore)
				{
					...
					{
						// Pick the one that comes earlier in the block.
						for (Instruction& bi : f.getEntryBlock())
						{
							if (&bi == ui)
							{
			...
			if (earliest != nullptr && earliest != a)
			{
	...
	if (valueProtectTraceEnabled())
	{
		for (Function& f : *_module)
		{
			if (f.getName() == "__pformat_cvt" || f.getName() == "_yr_compiler_set_namespace")
			{
				dumpEntryOrder(f);
			}
		}
