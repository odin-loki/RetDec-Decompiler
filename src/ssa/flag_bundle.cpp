...
void FlagBundleAnalysis::run(const SSAFunction& fn)
{
	...
	for (auto& valPtr: fn.values())
	{
		if (valPtr->kind == ValueKind::FlagBundle && valPtr->kind != ValueKind::Undef)
		{
			...
			info.defBlock = valPtr->defInstr ? valPtr->defInstr->block : kInvalidBlock;
	...
	for (auto& phi: fn.phis())
	{
		...
		for (auto& [predId, valId]: phi->operands)
		{
			...
			if (it2 != bundleIdx.end()) bundles_[it2->second].useSites.push_back({predId, UINT32_MAX});
		}
	}
	...
	{
		if (info.defBlock == kInvalidBlock)
		{
			info.sameBlockOnly = false;
			continue;
		}
		info.sameBlockOnly = true;
		for (auto& [blk, _]: info.useSites)
		{
			if (blk != info.defBlock)
			{
				info.sameBlockOnly = false;
				break;
			}
...
std::vector<SSAVerifier::Error> SSAVerifier::verify(const SSAFunction& fn, const DominatorTree& dom) const
{
		...
		for (auto* instr: blkPtr->instrs)
		{
			if (instr->defValue != kInvalidValue)
			{
				if (defined.count(instr->defValue))
				{
					...
					e.message = "Value " + std::to_string(instr->defValue) + " defined more than once";
		...
		for (auto* instr: blkPtr->instrs)
		{
			for (auto& use: instr->uses)
			{
				...
				if (!dom.dominates(fn, defB, useB))
				{
					...
					e.instr = instr->id;
					e.value = vid;
					e.message = "Use of v" + std::to_string(vid) + " in block " + std::to_string(useB)
							  + " not dominated by def in block " + std::to_string(defB);
					errors.push_back(std::move(e));
				}
			}
		}
	...
	for (auto& phi: fn.phis())
	{
		...
		for (BlockId pred: blk->preds)
		{
			if (!covered.count(pred))
			{
				...
				e.value = phi->result;
				e.message = "Phi for var" + std::to_string(phi->varId) + " in block " + std::to_string(phi->block)
						  + " missing operand from pred " + std::to_string(pred);
				errors.push_back(std::move(e));
			}
		}
	}
