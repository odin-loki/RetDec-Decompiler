...
ValueId SSARename::currentDef(const std::unordered_map<VarId, std::vector<ValueId>>& stacks, VarId var) const
{
...
void SSARename::pushDef(std::unordered_map<VarId, std::vector<ValueId>>& stacks, VarId var, ValueId val)
{
	if (var == kInvalidVar) return;
	stacks[var].push_back(val);
}

void SSARename::popDefs(
	std::unordered_map<VarId, std::vector<ValueId>>& stacks, const std::vector<std::pair<VarId, ValueId>>& pushed)
{
	for (auto it = pushed.rbegin(); it != pushed.rend(); ++it)
	{
		...
		if (sit != stacks.end() && !sit->second.empty()) sit->second.pop_back();
	}
}

// ─── Fill phi operands ────────────────────────────────────────────────────────

void SSARename::fillPhiOperands(SSAFunction& fn, BlockId blk, std::unordered_map<VarId, std::vector<ValueId>>& stacks)
{
...
void SSARename::renameBlock(SSAFunction& fn, BlockId blkId, std::unordered_map<VarId, std::vector<ValueId>>& stacks)
{
