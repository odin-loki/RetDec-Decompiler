...
std::uint64_t decodeUnsigned(const std::vector<std::uint8_t>& data, std::uint64_t& bytesRead)
{
	...
	else if ((data[0] & 0xC0) == 0x80)
	{
		...
		result = ((static_cast<std::uint64_t>(data[0]) & 0x3F) << 8) | data[1];
		bytesRead = 2;
	}
	...
	else if ((data[0] & 0xE0) == 0xC0)
	{
		...
		result = ((static_cast<std::uint64_t>(data[0]) & 0x1F) << 24) | (static_cast<std::uint64_t>(data[1]) << 16)
...
std::int64_t decodeSigned(const std::vector<std::uint8_t>& data, std::uint64_t& bytesRead)
{
	...
	if ((data[0] & 0x80) == 0)
	{
		...
		std::int8_t result8 = (data[0] & 0x01 ? 0x80 : 0x00) | static_cast<std::uint64_t>(data[0]);
		result = result8 >> 1;
		bytesRead = 1;
	}
	// If highest bit set and second highest not set, it is 2-byte number
	else if ((data[0] & 0xC0) == 0x80)
	{
		...
		std::int16_t result16 = (data[1] & 0x01 ? 0xC000 : 0x0000) | ((static_cast<std::uint64_t>(data[0]) & 0x1F) << 8)
	...
	else if ((data[0] & 0xE0) == 0xC0)
	{
		...
		std::int32_t result32 = (data[3] & 0x01 ? 0xE0000000 : 0x00000000)
							  ...
							  | (static_cast<std::uint64_t>(data[1]) << 16) | (static_cast<std::uint64_t>(data[2]) << 8)
 ...
 */
DotnetTypeReconstructor::DotnetTypeReconstructor(
	const MetadataStream* metadata, const StringStream* strings, const BlobStream* blob):
...
{
	if (!metadataStream || !stringStream || !blobStream) return false;
	...
	// - Reconstruction of fields and properties needs to know which classes are defined and which generic parameters
	// they contain
	// - Reconstruction of nested classes and base types needs to know all the classes that are defined
	return reconstructClasses() && reconstructMethods() && reconstructGenericParameters()
		&& reconstructMethodParameters() && reconstructFields() && reconstructProperties() && reconstructNestedClasses()
 ...
 */
void DotnetTypeReconstructor::linkReconstructedClassesDo(
	size_t i,
...
{
	auto typeDefTable =
		static_cast<const MetadataTable<TypeDef>*>(metadataStream->getMetadataTable(MetadataTableType::TypeDef));
	auto typeRefTable =
		static_cast<const MetadataTable<TypeRef>*>(metadataStream->getMetadataTable(MetadataTableType::TypeRef));
	auto fieldTable =
		static_cast<const MetadataTable<Field>*>(metadataStream->getMetadataTable(MetadataTableType::Field));
	auto methodDefTable =
		static_cast<const MetadataTable<MethodDef>*>(metadataStream->getMetadataTable(MetadataTableType::MethodDef));
	if (typeDefTable == nullptr || typeRefTable == nullptr) return false;
	...
	for (std::size_t i = 1; i <= typeDefTable->getNumberOfRows(); ++i)
	{
		...
		{
			fieldsCount = nextTypeDef ? nextTypeDef->fieldList.getIndex() - typeDef->fieldList.getIndex()
		...
		{
			methodsCount = nextTypeDef ? nextTypeDef->methodList.getIndex() - typeDef->methodList.getIndex()
...
bool DotnetTypeReconstructor::reconstructMethods()
{
	auto methodDefTable =
		static_cast<const MetadataTable<MethodDef>*>(metadataStream->getMetadataTable(MetadataTableType::MethodDef));
	if (methodDefTable == nullptr) return true;
	...
	for (const auto& kv: defClassTable)
	{
		...
		for (auto i = methodStartIndex; i < methodStartIndex + classType->getDeclaredMethodsCount(); ++i)
		{
			...
			if (methodDef == nullptr) break;

			auto newMethod = createMethod(methodDef, classType.get());
			if (newMethod == nullptr) continue;
...
{
	auto genericParamTable = static_cast<const MetadataTable<GenericParam>*>(
		metadataStream->getMetadataTable(MetadataTableType::GenericParam));
	if (genericParamTable == nullptr) return true;

	for (const auto& genericParam: *genericParamTable)
	{
		// Obtain generic parameter name
		std::string genericParamName;
		if (!stringStream->getString(genericParam.name.getIndex(), genericParamName)) continue;
		...
		else if (classOrMethod == MetadataTableType::MethodDef)
		{
			...
			if (itr == methodTable.end()) continue;
...
bool DotnetTypeReconstructor::reconstructMethodParameters()
{
	...
	for (const auto& kv: defClassTable)
	{
		...
		for (auto&& method: classToMethodTable[classType.get()])
		{
			...
			if (returnType == nullptr) continue;
			...
			if (methodOk) classType->addMethod(std::move(method));
		}
	}
...
bool DotnetTypeReconstructor::reconstructFields()
{
	auto fieldTable =
		static_cast<const MetadataTable<Field>*>(metadataStream->getMetadataTable(MetadataTableType::Field));
	if (fieldTable == nullptr) return true;
...
bool DotnetTypeReconstructor::reconstructProperties()
{
	...
	// Property table contains information about every single property, however it does not contain the reference to the
	// class it belongs to PropertyMap table actually contains mapping of properties to classes
	auto propertyTable =
		static_cast<const MetadataTable<Property>*>(metadataStream->getMetadataTable(MetadataTableType::Property));
	auto propertyMapTable = static_cast<const MetadataTable<PropertyMap>*>(
		metadataStream->getMetadataTable(MetadataTableType::PropertyMap));
	if (propertyTable == nullptr || propertyMapTable == nullptr) return true;
	...
	for (std::size_t i = 1; i <= propertyMapTable->getNumberOfRows(); ++i)
	{
		...
		for (std::size_t propertyIndex = startIndex; propertyIndex < startIndex + propertyCount; ++propertyIndex)
		{
			...
			if (newProperty == nullptr) continue;
...
bool DotnetTypeReconstructor::reconstructNestedClasses()
{
	...
	auto nestedClassTable = static_cast<const MetadataTable<NestedClass>*>(
		metadataStream->getMetadataTable(MetadataTableType::NestedClass));
	if (nestedClassTable == nullptr) return true;
	...
	for (std::size_t i = 1; i <= nestedClassTable->getNumberOfRows(); ++i)
	{
		...
		if (nestedItr == defClassTable.end() || !nestedItr->second->isNested()) continue;

		auto enclosingItr = defClassTable.find(nestedClass->enclosingClass.getIndex());
		if (enclosingItr == defClassTable.end()) continue;

		// Ignore self-references
		if (nestedItr == enclosingItr) continue;
		...
		{
			nestedItr->second->setNameSpace(
				enclosingItr->second->getFullyQualifiedName() + "." + nestedItr->second->getNameSpace());
		}
	}
...
bool DotnetTypeReconstructor::reconstructBaseTypes()
{
	...
	for (const auto& kv: defClassTable)
	{
		...
		if (!typeDef->extends.getTable(extendsTable)) continue;
		...
		else if (extendsTable == MetadataTableType::TypeRef)
		{
			...
			if (itr == refClassTable.end()) continue;
	...
	auto interfaceImplTable = static_cast<const MetadataTable<InterfaceImpl>*>(
		metadataStream->getMetadataTable(MetadataTableType::InterfaceImpl));
	if (interfaceImplTable == nullptr) return true;

	for (std::size_t i = 1; i <= interfaceImplTable->getSize(); ++i)
	{
		auto interfaceImpl = interfaceImplTable->getRow(i);
		if (interfaceImpl == nullptr) continue;
		...
		if (!interfaceImpl->interfaceType.getTable(interfaceTable)) continue;
		...
		{
			// TypeSpec table is used when class implements some generic interface like Interface<T>, Interface<int> or
			// similar
 ...
 */
std::unique_ptr<DotnetClass> DotnetTypeReconstructor::createClassDefinition(
	const TypeDef* typeDef, std::size_t fieldsCount, std::size_t methodsCount, std::size_t typeDefIndex)
{
	std::string className, classNameSpace;
	if (!stringStream->getString(typeDef->typeName.getIndex(), className)
		|| !stringStream->getString(typeDef->typeNamespace.getIndex(), classNameSpace))
		return nullptr;
	...
	if (className.empty() || className == "<Module>") return nullptr;
 ...
 */
std::unique_ptr<DotnetClass>
DotnetTypeReconstructor::createClassReference(const TypeRef* typeRef, std::size_t typeRefIndex)
{
	...
	if (!typeRef->resolutionScope.getTable(resolutionScopeType)
		|| resolutionScopeType != MetadataTableType::AssemblyRef)
	{
		classLibName = "";
	}
...
std::unique_ptr<DotnetField> DotnetTypeReconstructor::createField(const Field* field, const DotnetClass* ownerClass)
{
	...
	if (!stringStream->getString(field->name.getIndex(), fieldName)) return nullptr;

	fieldName = retdec::utils::replaceNonprintableChars(fieldName);
	auto signature = blobStream->getElement(field->signature.getIndex());

	if (signature.empty() || signature[0] != FieldSignature) return nullptr;
...
DotnetTypeReconstructor::createProperty(const Property* property, const DotnetClass* ownerClass)
{
	std::string propertyName;
	if (!stringStream->getString(property->name.getIndex(), propertyName)) return nullptr;
	...
	if (signature.size() < 2 || (signature[0] & ~HasThis) != PropertySignature) return nullptr;
	...
	// This seems like a weird thing, because I don't think that C# allows any parameters in getters/setters and
	// therefore this will always be 0
...
DotnetTypeReconstructor::createMethod(const MethodDef* methodDef, const DotnetClass* ownerClass)
{
	std::string methodName;
	if (!stringStream->getString(methodDef->name.getIndex(), methodName)) return nullptr;
	...
	if (methodName.empty() || signature.empty()) return nullptr;
	...
	if (signature[0] & Generic)
	{
		...
		// We ignore this value just because we have this information already from the class name in format
		// 'ClassName`N'
	...
	// However, we can't reconstruct generic types until we know all classes and methods, so we first create method just
	// with its name, properties and parameter count
	...
	std::vector<std::uint8_t>& signature)
{
	...
	auto paramTable =
		static_cast<const MetadataTable<Param>*>(metadataStream->getMetadataTable(MetadataTableType::Param));
	const Param* param;
 ...
 */
template <typename T>
std::unique_ptr<T> DotnetTypeReconstructor::createDataTypeFollowedByType(
	std::vector<std::uint8_t>& data, const DotnetClass* ownerClass, const DotnetMethod* ownerMethod)
{
...
std::unique_ptr<T> DotnetTypeReconstructor::createGenericReference(std::vector<std::uint8_t>& data, const U* owner)
{
	...
	if (index >= genericParams.size()) return nullptr;
 ...
 * @return New data type or @c nullptr in case of failure.
 */
std::unique_ptr<DotnetDataTypeGenericInst> DotnetTypeReconstructor::createGenericInstantiation(
	std::vector<std::uint8_t>& data, const DotnetClass* ownerClass, const DotnetMethod* ownerMethod)
{
	...
	for (std::size_t i = 0; i < genericCount; ++i)
	{
		...
		if (genericType == nullptr) return nullptr;
 ...
 * @return New data type or @c nullptr in case of failure.
 */
std::unique_ptr<DotnetDataTypeArray> DotnetTypeReconstructor::createArray(
	std::vector<std::uint8_t>& data, const DotnetClass* ownerClass, const DotnetMethod* ownerMethod)
{
	...
	if (bytesRead == 0 || numOfSizes > rank) return nullptr;
	data.erase(data.begin(), data.begin() + bytesRead);
	...
	if (bytesRead == 0 || numOfLowBounds > rank) return nullptr;
	data.erase(data.begin(), data.begin() + bytesRead);
 ...
 */
template <typename T>
std::unique_ptr<T> DotnetTypeReconstructor::createModifier(
	std::vector<std::uint8_t>& data, const DotnetClass* ownerClass, const DotnetMethod* ownerMethod)
{
 ...
 * @return New data type or @c nullptr in case of failure.
 */
std::unique_ptr<DotnetDataTypeFnPtr> DotnetTypeReconstructor::createFnPtr(
	std::vector<std::uint8_t>& data, const DotnetClass* ownerClass, const DotnetMethod* ownerMethod)
{
	if (data.empty()) return nullptr;
 ...
 * @return New data type or @c nullptr in case of failure.
 */
std::unique_ptr<DotnetDataTypeBase> DotnetTypeReconstructor::dataTypeFromSignature(
	std::vector<std::uint8_t>& signature, const DotnetClass* ownerClass, const DotnetMethod* ownerMethod)
{
	...
	switch (type)
	{
	case ElementType::Void: result = std::make_unique<DotnetDataTypeVoid>(); break;
	case ElementType::Boolean: result = std::make_unique<DotnetDataTypeBoolean>(); break;
	case ElementType::Char: result = std::make_unique<DotnetDataTypeChar>(); break;
	case ElementType::Int8: result = std::make_unique<DotnetDataTypeInt8>(); break;
	case ElementType::UInt8: result = std::make_unique<DotnetDataTypeUInt8>(); break;
	case ElementType::Int16: result = std::make_unique<DotnetDataTypeInt16>(); break;
	case ElementType::UInt16: result = std::make_unique<DotnetDataTypeUInt16>(); break;
	case ElementType::Int32: result = std::make_unique<DotnetDataTypeInt32>(); break;
	case ElementType::UInt32: result = std::make_unique<DotnetDataTypeUInt32>(); break;
	case ElementType::Int64: result = std::make_unique<DotnetDataTypeInt64>(); break;
	case ElementType::UInt64: result = std::make_unique<DotnetDataTypeUInt64>(); break;
	case ElementType::Float32: result = std::make_unique<DotnetDataTypeFloat32>(); break;
	case ElementType::Float64: result = std::make_unique<DotnetDataTypeFloat64>(); break;
	case ElementType::String: result = std::make_unique<DotnetDataTypeString>(); break;
	...
	case ElementType::ByRef:
		...
		break;
	case ElementType::ValueType: result = createDataTypeFollowedByReference<DotnetDataTypeValueType>(signature); break;
	case ElementType::Class: result = createDataTypeFollowedByReference<DotnetDataTypeClass>(signature); break;
	case ElementType::GenericVar:
		result = createGenericReference<DotnetDataTypeGenericVar>(signature, ownerClass);
		break;
	case ElementType::Array: result = createArray(signature, ownerClass, ownerMethod); break;
	case ElementType::GenericInst: result = createGenericInstantiation(signature, ownerClass, ownerMethod); break;
	case ElementType::TypedByRef: result = std::make_unique<DotnetDataTypeTypedByRef>(); break;
	case ElementType::IntPtr: result = std::make_unique<DotnetDataTypeIntPtr>(); break;
	case ElementType::UIntPtr: result = std::make_unique<DotnetDataTypeUIntPtr>(); break;
	case ElementType::FnPtr: result = createFnPtr(signature, ownerClass, ownerMethod); break;
	case ElementType::Object: result = std::make_unique<DotnetDataTypeObject>(); break;
	...
	case ElementType::CModRequired:
		...
		break;
	case ElementType::Internal: return nullptr;
	case ElementType::Modifier: return nullptr;
	case ElementType::Sentinel: return nullptr;
	case ElementType::Pinned: return nullptr;
	case ElementType::MetaType: return nullptr;
	case ElementType::BoxedObject: return nullptr;
	case ElementType::CustomField: return nullptr;
	case ElementType::CustomProperty: return nullptr;
	case ElementType::CustomEnum: return nullptr;
	default: break;
	}
