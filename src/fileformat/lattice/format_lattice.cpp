...
static inline uint8_t u8(const uint8_t* p)
{
	return p[0];
}
...
{
	return static_cast<uint32_t>(p[3]) | (static_cast<uint32_t>(p[2]) << 8) | (static_cast<uint32_t>(p[1]) << 16)
...
static FormatResult parseELF(const uint8_t* data, size_t size, bool is64, bool isLE, const std::string& name)
{
	FormatResult res;
	res.name = name;
	...
	if (e_phoff != 0 && e_phnum > 0 && e_phentsize >= (is64 ? 56u : 32u))
	{
		...
		if (minLoadVA != UINT64_MAX) imageBase = minLoadVA;
	}
	...
	for (uint16_t si = 0; si < e_shnum; ++si)
	{
		...
		std::string secName;
		if (shstr_data) secName = safeStr(shstr_data, shstr_size, sh_name);
		...
		if ((sh_type == SHT_RELA || sh_type == SHT_REL) && (secName == ".rela.plt" || secName == ".rel.plt"))
		{
			relaplt_idx = si;
	...
	if (relaplt_idx != UINT32_MAX && dynsym_idx != UINT32_MAX && dynstr_idx != UINT32_MAX && e_shentsize > 0)
	{
		// Get section info
		...
		if (inBounds(static_cast<size_t>(rela_off), static_cast<size_t>(rela_sz), size)
			...
			&& inBounds(static_cast<size_t>(str_off), static_cast<size_t>(str_sz), size) && rela_ent > 0 && sym_ent > 0)
		{
...
static FormatResult parsePE(const uint8_t* data, size_t size, const std::string& name)
{
	...
	if (imp_off != 0 && inBounds(static_cast<size_t>(imp_off), 20, size))
	{
		...
		for (size_t di = 0;; di += 20)
		{
			...
			{
				uint64_t sec_rva_start = sec.virtualAddress - image_base;
				if (name_rva >= sec_rva_start && name_rva < sec_rva_start + sec.virtualSize)
				{
			...
			{
				uint64_t sec_rva_start = sec.virtualAddress - image_base;
				if (thunk_rva >= sec_rva_start && thunk_rva < sec_rva_start + sec.virtualSize)
				{
					...
					for (size_t ti = 0;; ti += entry_sz)
					{
						...
						uint64_t thunk = is64 ? u64le(data + thunk_fo + ti) : u32le(data + thunk_fo + ti);
						if (thunk == 0) break;
						...
						{
							uint32_t hint_rva =
								static_cast<uint32_t>(is64 ? (thunk & 0x7FFF'FFFF'FFFF'FFFFULL) : (thunk & 0x7FFFFFFF));
							for (auto& sec2: res.sections)
							{
								...
								if (hint_rva >= s2 && hint_rva < s2 + sec2.virtualSize)
								{
									...
									if (fo2 + 2 < size) imp2.functionName = safeStr(data, size, fo2 + 2);
									break;
								}
							}
						}
	...
	if (exp_off != 0 && inBounds(static_cast<size_t>(exp_off), 40, size))
	{
		...
		else
		{
			...
			{
				uint64_t sec_rva_start = sec.virtualAddress - image_base;
				if (addr_rva >= sec_rva_start && addr_rva < sec_rva_start + sec.virtualSize)
				{
...
static FormatResult parseMachO(const uint8_t* data, size_t size, const std::string& name)
{
	...
	for (uint32_t ci = 0; ci < num_cmds; ++ci)
	{
		...
		if (cmd == 0x1 || cmd == 0x19)
		{
			...
			if (!inBounds(cmd_off, min_sz, size))
			{
				cmd_off += cmdsize;
				continue;
			}
...
static FormatResult parseAR(const uint8_t* data, size_t size, const std::string& name, const FormatLattice& lattice)
{
	...
	while (off + 60 <= size)
	{
		...
		for (int i = 15; i >= 0 && memberName[i] == ' '; --i)
			memberName[i] = 0;
...
static DetectedFormat scoreCafeBabe(const uint8_t* data, size_t size)
{
	...
	if (second_word > 30) return DetectedFormat::Unknown; // Java .class — not a supported binary
	...
	if (size >= 8 + second_word * 20) return DetectedFormat::MachOFat;
...
FormatResult FormatLattice::classify(const uint8_t* data, size_t size, const std::string& name) const
{
	...
	if (size >= 8 && data[0] == 0xCA && data[1] == 0xFE && data[2] == 0xBA && data[3] == 0xBE)
	{
	...
	if (size >= 2)
	{
		...
		static const uint16_t coffMachines[] = {0x014C, 0x014D, 0x014E, 0x014F, 0x0160, 0x0162, 0x0166, 0x0169,
