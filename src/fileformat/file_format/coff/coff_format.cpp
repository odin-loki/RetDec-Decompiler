...
CoffFormat::CoffFormat(std::string pathToFile, LoadFlags loadFlags):
	FileFormat(pathToFile, loadFlags), fileBuffer(MemoryBuffer::getFile(Twine(pathToFile)))
{
	initStructures();
}
...
void CoffFormat::initStructures()
{
	...
	{
		auto objOrErr = ObjectFile::createCOFFObjectFile(fileBuffer.get()->getMemBufferRef());
		if (!objOrErr)
		{
...
std::size_t CoffFormat::getBytesPerWord() const
{
	...
	case PELIB_IMAGE_FILE_MACHINE_R4000: return getFileFlags() & IMAGE_FILE_32BIT_MACHINE ? 4 : 8;
	...
	case PELIB_IMAGE_FILE_MACHINE_POWERPCFP: return getFileFlags() & IMAGE_FILE_32BIT_MACHINE ? 4 : 8;
...
std::size_t CoffFormat::getDeclaredFileLength() const
{
	...
	{
		std::size_t symTabMaxOffset =
			getCoffSymbolTableOffset() + (getNumberOfCoffSymbols() * file->getSymbolTableEntrySize());
