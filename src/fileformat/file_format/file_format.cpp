...
/**
 * Stage 12: Reference-anchored string detection.
 * Returns a String if there is a valid null-terminated string at the given address.
 * Uses same validation as loadStrings (printable chars, min length).
 */
std::optional<String> FileFormat::getStringAtAddress(std::uint64_t address, StringType type) const
{
	...
	return String(type, fileOffset, secSeg->getName(), std::move(content));
}
