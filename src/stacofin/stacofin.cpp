...
void Finder::solveReferences()
{
	for (auto& p : _allDetections)
	{
				...
				&& utils::containsCaseInsensitive(sigPath, "thumb"))
		{
			...
			{
			std::cerr << "[stacofin] failed to switch Capstone to THUMB mode for "
			          << sigPath << "; skipping signature\n";
				continue;
			}
			modeSwitch = true;
		}
		...
		if (modeSwitch)
		{
			...
			{
			std::cerr << "[stacofin] failed to restore Capstone mode after "
			             "THUMB detection\n";
			}
		}
	}
...
common::Address Finder::getAddressFromRef(common::Address ref)
{
	...
	{
		// Unknown architecture: cannot resolve signature reference address.
		return Address();
	}
}
