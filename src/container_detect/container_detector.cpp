...
std::string ContainerResult::toString() const
{
	...
	os << kindName() << " (confidence=" << confidence << ")" << " element=" << elementType.toString();
...
RecoveredType TemplateTypeRecoverer::fromComparatorParam(const ssa::SSAFunction& fn) const
{
	...
	for (uint32_t b = 0; b < fn.blockCount(); ++b)
	{
		...
		for (const auto* instr: blk->instrs)
		{
			...
			if (cn.find("compare") == std::string::npos && cn.find("less") == std::string::npos) continue;
...
RecoveredType TemplateTypeRecoverer::fromHashParam(const ssa::SSAFunction& fn) const
{
	for (uint32_t b = 0; b < fn.blockCount(); ++b)
	{
		...
		for (const auto* instr: blk->instrs)
		{
			if (!instr || instr->op != ssa::IrInstr::Op::Call) continue;
			const auto& cn = instr->calleeName;
			if (cn.find("hash") == std::string::npos && cn.find("Hash") == std::string::npos) continue;
			if (!instr->uses.empty())
			{
				const auto* v = fn.value(instr->uses[0].valueId);
				if (v) return fromByteWidth(static_cast<uint8_t>(v->memWidth));
...
RecoveredType TemplateTypeRecoverer::recoverElementType(
	const ssa::SSAFunction& fn, const ContainerResult& partial, uint8_t elementByteWidth) const
{
	...
	if (partial.elementType.kind != RecoveredType::Kind::Unknown) return partial.elementType;
...
RecoveredType TemplateTypeRecoverer::recoverKeyType(const ssa::SSAFunction& fn, const ContainerResult& partial) const
{
...
ContainerResult ContainerDetector::analyseFunction(const ssa::SSAFunction& fn) const
{
	...
	if (best.kind == ContainerKind::Map || best.kind == ContainerKind::Set || best.kind == ContainerKind::UnorderedMap
		|| best.kind == ContainerKind::UnorderedSet)
	{
	...
	{
		const bool nameOnlyHash = best.emittedType.find("evidence:symbol_name") != std::string::npos;
		best.emittedType = "std::unordered_map<" + best.keyType.toString() + ", " + best.elementType.toString() + ">";
		if (nameOnlyHash) best.emittedType = "evidence:symbol_name " + best.emittedType;
	}
...
ContainerDetector::DetectionMap
ContainerDetector::analyseModule(const std::vector<const ssa::SSAFunction*>& functions) const
{
	DetectionMap result;
	for (const auto* fn: functions)
	{
		...
		if (r.kind != ContainerKind::Unknown) result[fn->name()] = std::move(r);
	}
	return result;
}
