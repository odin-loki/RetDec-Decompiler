	...
	for (uint32_t b = 0; b < fn.blockCount(); ++b)
	{
		...
		for (const auto* instr: blk->instrs)
		{
			...
			for (const auto& use: instr->uses)
			{
				...
				if (iv && iv->kind == ssa::ValueKind::Immediate && iv->imm < 32) return true;
			}
		}
	}
	return false;
...
bool StringDetector::hasSSOThresholdCompare(const ssa::SSAFunction& fn, int& threshold) const
{
	for (uint32_t b = 0; b < fn.blockCount(); ++b)
	{
		const auto* blk = fn.block(b);
...
static bool hasNamedAtomic(const ssa::SSAFunction& fn)
{
	for (uint32_t b = 0; b < fn.blockCount(); ++b)
	{
		const auto* blk = fn.block(b);
		...
		for (const auto* instr: blk->instrs)
		{
			...
			const auto& cn = instr->calleeName;
			if (cn.find("atomic") != std::string::npos || cn.find("__atomic") != std::string::npos
...
static bool hasStructuralAtomic(const ssa::SSAFunction& fn)
{
	return countOp(fn, ssa::IrInstr::Op::Sub) >= 1 && countOp(fn, ssa::IrInstr::Op::Compare) >= 1;
}

bool SharedPtrDetector::hasAtomicOperation(const ssa::SSAFunction& fn) const
{
	return hasNamedAtomic(fn) || hasStructuralAtomic(fn);
}
...
SharedPtrEvidence SharedPtrDetector::analyseStructure(const ssa::SSAFunction& fn) const
{
	...
	ev.hasAtomicIncrement = ev.hasAtomicDecrement && countOp(fn, ssa::IrInstr::Op::Add) >= 1;
...
ContainerResult SharedPtrDetector::detect(const ssa::SSAFunction& fn) const
{
	...
	if (hasNamedAtomic(fn) && !hasStructuralAtomic(fn))
		result.emittedType = "evidence:symbol_name " + result.emittedType;
