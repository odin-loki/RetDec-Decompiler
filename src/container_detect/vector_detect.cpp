...
{
	return countOp(fn, ssa::IrInstr::Op::Load) >= 3 && countOp(fn, ssa::IrInstr::Op::Sub) >= 1
		&& countOp(fn, ssa::IrInstr::Op::Add) >= 1;
}

// Growth pattern: malloc/realloc/new call + memmove/copy loop + free/delete call.
static bool hasGrowthPattern(const ssa::SSAFunction& fn, float& growthFactor, CompilerVariant& variant)
{
	...
	for (uint32_t b = 0; b < fn.blockCount(); ++b)
	{
		...
		for (const auto* instr: blk->instrs)
		{
			...
			if (instr->op == ssa::IrInstr::Op::Call)
			{
				...
				if (cn == "malloc" || cn == "realloc" || cn == "_Znwm" || cn == "calloc"
					|| cn.find("allocate") != std::string::npos)
					hasMalloc = true;
				if (cn == "free" || cn == "_ZdlPv" || cn == "operator delete"
					|| cn.find("deallocate") != std::string::npos)
					hasFree = true;
				if (cn == "memmove" || cn == "memcpy" || cn.find("copy") != std::string::npos) hasCopy = true;
			}
...
static bool hasElementAccess(const ssa::SSAFunction& fn, uint8_t& elementByteWidth)
{
...
VectorEvidence VectorDetector::analyseStructure(const ssa::SSAFunction& fn) const
{
	...
	ev.hasSizeArith = countOp(fn, ssa::IrInstr::Op::Sub) >= 1 && countOp(fn, ssa::IrInstr::Op::Load) >= 2;
...
std::vector<AccessPattern> VectorDetector::recoverAccessPatterns(const ssa::SSAFunction& fn) const
{
	...
	if (countOp(fn, ssa::IrInstr::Op::Load) >= 2 && countOp(fn, ssa::IrInstr::Op::Add) >= 1)
	{
		AccessPattern ap;
...
ContainerResult VectorDetector::detect(const ssa::SSAFunction& fn) const
{
	...
	if (ev.elementByteWidth > 0)
	{
		...
		{
		case 1:
			elem.kind = RecoveredType::Kind::Int8;
			elem.byteWidth = 1;
			break;
		case 2:
			elem.kind = RecoveredType::Kind::Int16;
			elem.byteWidth = 2;
			break;
		case 4:
			elem.kind = RecoveredType::Kind::Int32;
			elem.byteWidth = 4;
			break;
		case 8:
			elem.kind = RecoveredType::Kind::Int64;
			elem.byteWidth = 8;
			break;
		default: break;
		}
	...
	if (ev.hasGrowthPattern && !ev.hasBeginEndCap) result.emittedType = "evidence:symbol_name " + result.emittedType;
