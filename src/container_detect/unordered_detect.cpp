 ...
 *     `bucket_idx = hash & (bucket_count - 1)` → And with (2^k − 1), k<=16.
 *   Bare Div / rem is not modulo here: SRem/URem map to Div, and any-Div
 *   plus the 0xFFFFFFFF zext-mask was extra HashTable/Map on B9 heapsort.
 *
 * ### Chain traversal
...
static bool hasChainTraversal(const ssa::SSAFunction& fn)
{
	...
	return backEdge && countOp(fn, ssa::IrInstr::Op::Load) >= 1 && countOp(fn, ssa::IrInstr::Op::Compare) >= 1;
}
	...
	for (uint32_t b = 0; b < fn.blockCount(); ++b)
	{
		...
		for (const auto* instr: blk->instrs)
		{
			...
			for (const auto& u: instr->uses)
			{
				...
				const uint64_t c = iv->imm;
				// k<=16: 0xFFFFFFFF (zext) is a mask, not a bucket count.
				if (c == 0 || c > 0xffffULL) continue;
				if (((c + 1ULL) & c) == 0ULL) return true;
			}
		}
	}
	return false;
