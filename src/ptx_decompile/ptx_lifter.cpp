...
PtxOperand PtxParser::parseOperand(const std::string& tok) const
{
	...
	// Special registers
	if (startsWith(tok, "%tid") || startsWith(tok, "%ntid") || startsWith(tok, "%ctaid") || startsWith(tok, "%nctaid")
		|| startsWith(tok, "%laneid") || startsWith(tok, "%warpid") || startsWith(tok, "%lanemask")
		|| startsWith(tok, "%smid") || startsWith(tok, "%clock") || startsWith(tok, "%globaltimer"))
	{
...
PtxVarDecl PtxParser::parseVarDecl(const std::vector<std::string>& tokens) const
{
	...
	for (size_t i = 0; i < tokens.size(); ++i)
	{
		if (tokens[i].front() == '.')
		{
			...
			if (sp != PtxSpace::Unknown)
			{
				decl.space = sp;
				continue;
			}
			PtxType ty = parseType(tokens[i]);
			if (ty != PtxType::Unknown)
			{
				decl.type = ty;
				continue;
			}
...
PtxParam PtxParser::parseParam(const std::vector<std::string>& tokens) const
{
	...
	for (const auto& tok: tokens)
	{
		...
		{
			p.name = tok;
			if (ty == PtxType::U64 || ty == PtxType::B64) p.isPointer = true;
		}
	}
	// Heuristic: 64-bit params in kernel entries are likely pointers
	if (p.type == PtxType::U64 || p.type == PtxType::B64) p.isPointer = true;
	return p;
}
...
PtxInstr PtxParser::parseInstr(const std::vector<std::string>& tokens) const
{
	...
	if (start < tokens.size() && !tokens[start].empty() && tokens[start][0] == '@')
	{
	...
	while (std::getline(ss, line))
	{
		...
		if (toks[0] == ".target" && toks.size() >= 2)
		{
			...
			if (p != std::string::npos)
			{
				try
				{
					mod.smVersion = std::stoi(sm.substr(p + 3));
				}
				catch (const std::invalid_argument&)
				{}
				catch (const std::out_of_range&)
		...
		if (!toks.empty()
			&& (toks[0] == ".reg" || toks[0] == ".shared" || toks[0] == ".local" || toks[0] == ".param"
				|| toks[0] == ".const"))
		{
		...
		if (!toks.empty())
		{
			...
			if (!instr.mnemonic.empty()) currentKernel->instrs.push_back(instr);
		}
	}
...
std::string SharedMemDeclaration::emit(const PtxVarDecl& decl)
{
	...
	os << "__shared__ " << ptxCType(decl.type) << " " << decl.name.substr(1); // strip leading %
	if (decl.bytes > 0)
...
std::string InstrLifter::cTypeStr(PtxType t)
{
	return ptxCType(t);
}
...
std::string InstrLifter::typeStr(PtxType t)
{
	...
	{
	case PtxType::B8:
	case PtxType::U8: return "u8";
	case PtxType::B16:
	case PtxType::U16: return "u16";
	case PtxType::B32:
...
std::string InstrLifter::operandStr(const PtxOperand& op, PtxType)
{
	...
	case PtxOperandKind::Register: return op.name.substr(1); // strip %
	...
	case PtxOperandKind::Label: return op.name;
	}
	return "?";
}
...
std::string InstrLifter::liftStore(const PtxInstr& i, const std::string& ind) const
{
	...
	std::string dst = operandStr(i.operands[0], i.type);
	std::string src = operandStr(i.operands[1], i.type);
	if (i.operands[0].kind != PtxOperandKind::Address) dst = "*" + dst;
	return ind + dst + " = " + src + ";\n";
}
...
std::string InstrLifter::liftArith(const PtxInstr& i, const std::string& ind) const
{
	...
	if (op == "min" || op == "max") return ind + dst + " = " + op + "(" + a + ", " + b + ");\n";

	// mad: dst = a * b + c
	if (i.mnemonic == "mad" || i.mnemonic == "fma")
	{
		std::string c = i.operands.size() >= 4 ? operandStr(i.operands[3], i.type) : "0";
...
{
	if (i.modifier.find(".gl") != std::string::npos) return ind + "__threadfence();\n";
	if (i.modifier.find(".cta") != std::string::npos) return ind + "__threadfence_block();\n";
...
std::string InstrLifter::liftAtom(const PtxInstr& i, const std::string& ind) const
{
	...
	std::string fn;
	if (i.modifier.find(".add") != std::string::npos)
		fn = "atomicAdd";
	else if (i.modifier.find(".exch") != std::string::npos)
		fn = "atomicExch";
	else if (i.modifier.find(".min") != std::string::npos)
		fn = "atomicMin";
	else if (i.modifier.find(".max") != std::string::npos)
		fn = "atomicMax";
	else if (i.modifier.find(".and") != std::string::npos)
		fn = "atomicAnd";
	else if (i.modifier.find(".or") != std::string::npos)
		fn = "atomicOr";
	else if (i.modifier.find(".xor") != std::string::npos)
		fn = "atomicXor";
	else if (i.modifier.find(".cas") != std::string::npos)
	{
		std::string cmp = i.operands.size() >= 4 ? operandStr(i.operands[3], i.type) : "0";
...
std::string InstrLifter::liftVote(const PtxInstr& i, const std::string& ind) const
{
	...
	if (i.modifier.find(".all") != std::string::npos) return ind + dst + " = __all_sync(0xffffffff, " + pred + ");\n";
	if (i.modifier.find(".any") != std::string::npos) return ind + dst + " = __any_sync(0xffffffff, " + pred + ");\n";
...
std::string InstrLifter::liftMath(const PtxInstr& i, const std::string& ind) const
{
	...
	if (i.mnemonic == "rcp")
		return ind + dst + " = " + (isF32 ? "__frcp_rn(" : "1.0/(") + src + (isF32 ? ")" : ")") + ";\n";
...
std::string InstrLifter::liftBranch(const PtxInstr& i, const std::string& ind) const
{
	...
	if (!i.predReg.empty())
	{
		...
		if (i.negPred) return ind + "if (!" + cond + ") goto " + target + ";\n";
...
std::string InstrLifter::lift(const PtxInstr& i, const std::string& ind) const
{
	...
	if (mn == "add" || mn == "sub" || mn == "mul" || mn == "div" || mn == "rem" || mn == "shl" || mn == "shr"
		|| mn == "and" || mn == "or" || mn == "xor" || mn == "neg" || mn == "abs" || mn == "min" || mn == "max"
		|| mn == "mad" || mn == "fma")
		return emit(liftArith(i, ind));
	...
	if (mn == "sqrt" || mn == "rsqrt" || mn == "sin" || mn == "cos" || mn == "lg2" || mn == "ex2" || mn == "rcp")
		return emit(liftMath(i, ind));
	...
	if (mn == "not")
	{
		...
		{
			return emit(ind + operandStr(i.operands[0], i.type) + " = ~" + operandStr(i.operands[1], i.type) + ";\n");
		}
	}
