...
class DepthGuard {
public:
	explicit DepthGuard(unsigned& depth) noexcept: depth_(depth)
	{
		++depth_;
	}
	~DepthGuard()
...
PyCodeObject::Const MarshalObject::toConst(uint64_t& budget) const
{
	...
	case Type::Code:
		...
		c.code = asCode();
		break;
	default: c.kind = PyCodeObject::Const::Kind::None; break;
	}
	return c;
}
...
MarshalReader::MarshalReader(const uint8_t* data, size_t size, const PythonVersion& version):
...
bool MarshalReader::countFitsInRemainingInput(size_t n, const char* what, size_t bytesPerElement)
{
...
std::shared_ptr<MarshalObject> MarshalReader::readLong()
{
	...
	const size_t digits = (n < 0) ? static_cast<size_t>(-static_cast<int64_t>(n)) : static_cast<size_t>(n);
	if (!countFitsInRemainingInput(digits, "long", kBytesPerLongDigit)) return nullptr;
	...
	for (size_t i = 0; i < digits; ++i)
	{
		...
		if (shift < 64) magnitude |= static_cast<uint64_t>(d) << shift;
	}
	if (sign < 0) magnitude = ~magnitude + 1; // two's-complement negate without overflow
...
std::shared_ptr<MarshalObject> MarshalReader::readBytes()
{
	...
	if (len < 0)
	{
		setError("Negative bytes length");
		return nullptr;
	}
	std::string s;
...
{
	using DictT = std::vector<std::pair<std::shared_ptr<MarshalObject>, std::shared_ptr<MarshalObject>>>;
	auto obj = std::make_shared<MarshalObject>();
...
std::shared_ptr<PyCodeObject> MarshalReader::readCodeFields()
{
	...
	auto readInt32 = [&](int32_t& out) -> bool { return readS32LE(out); };

	auto readStrObj = [&](std::string& out) -> bool {
		auto obj = readObject();
		if (!obj) return !hasError_;
		if (obj->isStr() || obj->isBytes()) out = obj->asStr();
		return true;
	};
	...
	if (!code->co_linetable.empty())
	{
		...
		{
			code->lineTable =
				decodeLnotab311(code->co_linetable, code->co_firstlineno, static_cast<uint32_t>(code->co_code.size()));
		}
		else if (version_.atLeast(3, 10))
		{
			code->lineTable =
				decodeLnotab310(code->co_linetable, code->co_firstlineno, static_cast<uint32_t>(code->co_code.size()));
		}
		else
		{
			code->lineTable =
				decodeLnotab(code->co_linetable, code->co_firstlineno, static_cast<uint32_t>(code->co_code.size()));
		}
	}
...
std::shared_ptr<MarshalObject> MarshalReader::readObject()
{
	...
	default: {
		...
		ss << "Unknown marshal type byte 0x" << std::hex << static_cast<int>(typeChar) << " at offset " << std::dec
		   << (pos_ - 1) << " (pos_=" << pos_ << ")";
		setError(ss.str());
		return nullptr;
	}
