 ...
 *
 * The table is 3.10's numbering. 3.8, 3.9, 3.11 and 3.12 reach it through
 * their own override functions below, one entry per opcode they number
 * differently. Unmapped opcodes get name="<unknown>" and kind=Unknown.
 *
 * Sources:
 *   CPython/Lib/opcode.py for each version tag -- and
 *   tests/pyc_parser/pyc_parser_test.cpp pins every entry against it
...
static void applyVersion312Overrides(uint8_t opcode, OpcodeInfo& info)
{
	...
	case 238:
		info = {238, "INSTRUMENTED_POP_JUMP_IF_NONE", OpcodeKind::Normal, true, OpcodeInfo::kUnknownEffect};
		break;
	case 239:
		info = {239, "INSTRUMENTED_POP_JUMP_IF_NOT_NONE", OpcodeKind::Normal, true, OpcodeInfo::kUnknownEffect};
		break;
	...
	case 244:
		info = {244, "INSTRUMENTED_CALL_FUNCTION_EX", OpcodeKind::Normal, true, OpcodeInfo::kUnknownEffect};
		break;
	...
	case 249:
		info = {249, "INSTRUMENTED_POP_JUMP_IF_FALSE", OpcodeKind::Normal, true, OpcodeInfo::kUnknownEffect};
		break;
	case 250:
		info = {250, "INSTRUMENTED_POP_JUMP_IF_TRUE", OpcodeKind::Normal, true, OpcodeInfo::kUnknownEffect};
		break;
