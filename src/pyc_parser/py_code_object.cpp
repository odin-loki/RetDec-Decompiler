...
int32_t PyCodeObject::lineAt(uint32_t offset) const
{
	...
	{
		if (offset >= e.startOffset && offset < e.endOffset) return e.line;
	}
...
int32_t PyCodeObject::columnAt(uint32_t offset) const
{
	for (const auto& e: lineTable)
	{
		if (offset >= e.startOffset && offset < e.endOffset) return e.column;
	}
	return -1;
}
...
std::vector<LineEntry> decodeLnotab(const std::vector<uint8_t>& lnotab, int32_t firstLineno, uint32_t codeLen)
{
...
std::vector<LineEntry> decodeLnotab310(const std::vector<uint8_t>& linetable, int32_t firstLineno, uint32_t codeLen)
{
...
static uint32_t readUleb128(const uint8_t* data, size_t size, size_t& pos)
{
	...
	{
		uint8_t b = data[pos++];
		if (shift < 32) result |= static_cast<uint32_t>(b & 0x7F) << shift;
		if ((b & 0x80) == 0) break;
		shift += 7;
	}
	return result;
...
std::vector<LineEntry> decodeLnotab311(const std::vector<uint8_t>& linetable, int32_t firstLineno, uint32_t codeLen)
{
...
std::vector<ExceptionEntry> decodeExceptionTable(const std::vector<uint8_t>& table)
{
