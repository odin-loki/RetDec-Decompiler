...
static const MagicEntry kMagicTable[] = {
	...
	{3563, {3, 13, 3563, "3.13"}},
	{3564, {3, 13, 3564, "3.13"}},
	// Python 3.14 (CPython pycore_magic_number.h)
	{3625, {3, 14, 3625, "3.14"}},
	{3626, {3, 14, 3626, "3.14"}},
	{3627, {3, 14, 3627, "3.14"}},
};
...
std::optional<PythonVersion> detectVersion(const uint8_t* buf, size_t len)
{
	...
	uint32_t raw = static_cast<uint32_t>(buf[0]) | (static_cast<uint32_t>(buf[1]) << 8)
...
uint32_t magicForVersion(int major, int minor)
{
	...
	for (size_t i = 0; i < kMagicTableSize; ++i)
	{
		if (kMagicTable[i].ver.major == major && kMagicTable[i].ver.minor == minor)
		{
			...
			best = (static_cast<uint32_t>(kPycCRLFMarker) << 16) | kMagicTable[i].magic;
		}
	}
	return best;
}

std::string describeHeader(uint32_t magic, uint32_t bitField, uint32_t mtimeOrHash, uint32_t sourceSize)
{
