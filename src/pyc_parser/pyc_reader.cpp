...
PycReadResult PycReader::read(const uint8_t* data, size_t size, const std::string& srcPath)
{
	...
	{
		result.error = "File too small to be a valid .pyc (need " + std::to_string(kPycHeaderSize) + " bytes)";
		return result;
	}

	// 1. Read header
	uint32_t rawMagic = static_cast<uint32_t>(data[0]) | (static_cast<uint32_t>(data[1]) << 8)
					  | (static_cast<uint32_t>(data[2]) << 16) | (static_cast<uint32_t>(data[3]) << 24);
	uint32_t bitField = static_cast<uint32_t>(data[4]) | (static_cast<uint32_t>(data[5]) << 8)
...
void PycReader::buildModule(const PyCodeObject& root, const std::string& moduleName, PycReadResult& result)
{
	...
	const PyCodeObject& code, BcClass& cls, PycReadResult& result, const std::string& parentQual)
{
	...
	bool isCompGenerated = !code.co_name.empty()
						&& (code.co_name == "<genexpr>" || code.co_name == "<listcomp>" || code.co_name == "<dictcomp>"
	...
	std::string qualName = code.co_qualname.empty()
							 ? (parentQual.empty() ? code.co_name : parentQual + "." + code.co_name)
							 : code.co_qualname;
	...
	if (opts_.buildCFG)
	{
		...
		result.totalInstructions += static_cast<int>(method.cfg.blockCount() > 0 ? method.cfg.blocks().size() : 0);
	}
...
BcMethod PycReader::buildMethod(const PyCodeObject& code, const std::string& qualName) const
{
	BcMethod m;
	m.name = code.co_name;
	...
	for (size_t i = static_cast<size_t>(paramCount); i < code.co_varnames.size(); ++i)
	{
		BcLocalVar lv;
	...
	// Flags to BcAccess
	if (code.isGenerator()) m.access = m.access | BcAccess::Bridge;       // generator marker
	if (code.isCoroutine()) m.access = m.access | BcAccess::Synchronized; // async marker (reuse)
...
void PycReader::liftInstruction(
	uint8_t rawOp, int32_t arg, uint32_t offset, const PyCodeObject& code, BcInstruction& out) const
{
	OpcodeInfo info = opcodeInfo(rawOp, code.version);

	// Map opcode name → BcOpcode
	auto it = kPyOpcodeMap.find(info.name);
	BcOpcode bcOp = (it != kPyOpcodeMap.end()) ? it->second : BcOpcode::PYTHON_UNKNOWN;
	out.opcode = bcOp;
	out.offset = offset;

	// Line number from table
	out.line = static_cast<uint32_t>(std::max(0, code.lineAt(offset)));
	...
	case BcOpcode::PYTHON_LOAD_CONST: {
		if (arg >= 0 && static_cast<size_t>(arg) < code.co_consts.size())
		{
			...
			else if (c.kind == PyCodeObject::Const::Kind::Str || c.kind == PyCodeObject::Const::Kind::Unicode)
			{
	...
	default: operand = BcIntOperand{static_cast<int64_t>(arg)}; break;
	}
...
std::vector<uint32_t> PycReader::findLeaders(const PyCodeObject& code) const
{
	...
	while (pos < bytecode.size())
	{
		...
		if (info.isJump() && hasArg)
		{
			...
			int64_t target = arg;
			if (info.kind == OpcodeKind::JumpForward)
				target = static_cast<int64_t>(pos) + static_cast<int64_t>(instrSize) + static_cast<int64_t>(arg) * 2;
			else if (info.kind == OpcodeKind::JumpBackward)
				target = static_cast<int64_t>(pos) + static_cast<int64_t>(instrSize) - static_cast<int64_t>(arg) * 2;
			...
			if (info.kind != OpcodeKind::JumpAbsolute && info.kind != OpcodeKind::Jump && nextOff < bytecode.size())
		...
		else if (info.isReturn() || info.kind == OpcodeKind::Raise)
		{
			...
			if (nextOff < bytecode.size()) leaders.insert(nextOff);
		}
	...
	while (pos < bytecode.size())
	{
		...
		if (pos < bytecode.size())
		{
			...
			if (nextIt != offsetToBlock.end() && nextIt->second != curBlockIdx)
			{
				...
				{
					blocks[curBlockIdx]->succs.push_back(static_cast<uint32_t>(nextIt->second));
				}
			}
		}
	}
