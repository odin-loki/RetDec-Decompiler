...
retdec::common::Object DebugFormat::loadDwarf_variable(llvm::DWARFDie die)
{
	...
	if (auto o = llvm::dwarf::toBlock(die.find(llvm::dwarf::DW_AT_location)))
	{
		...
		if (expr.begin() != expr.end())
		{
			...
			else if (e.getCode() >= llvm::dwarf::DW_OP_reg0
					&& e.getCode() <= llvm::dwarf::DW_OP_reg31)
			{
				// Variable lives entirely in a register
				unsigned regNum = e.getCode() - llvm::dwarf::DW_OP_reg0;
