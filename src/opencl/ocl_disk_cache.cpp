 ...
 * @brief Persist compiled OpenCL program binaries under ~/.retdec/ocl-cache
 *        (or %USERPROFILE%\\.retdec\\ocl-cache), or RETDEC_OCL_CACHE_DIR when set.
 */
...
std::string oclCacheDir()
{
	...
	fs::path base;
	const char* overrideDir = std::getenv("RETDEC_OCL_CACHE_DIR");
	if (overrideDir && overrideDir[0])
	{
		base = fs::path(overrideDir);
	}
	else
	{
		...
		base = fs::path(home) / ".retdec" / "ocl-cache";
	}
	std::error_code ec;
