...
CliSigDecoder::CliSigDecoder(const ITypeNameResolver* resolver): resolver_(resolver) {}

std::string CliSigDecoder::tokenName(MetadataToken tok) const
{
	if (!resolver_) return "?";
	if (tok.table == static_cast<uint8_t>(TableId::TypeDef)) return resolver_->typeDefName(tok.index);
	if (tok.table == static_cast<uint8_t>(TableId::TypeRef)) return resolver_->typeRefName(tok.index);
	if (tok.table == static_cast<uint8_t>(TableId::TypeSpec)) return resolver_->typeSpecType(tok.index).toString();
	return "?";
}

MetadataToken CliSigDecoder::readTypeDefOrRef(std::span<const uint8_t> blob, size_t& pos) const
{
	...
	std::vector<MetadataToken>& opts) const
{
	...
	{
		uint8_t et = blob[pos];
		if (et != static_cast<uint8_t>(ElementType::CModReqd) && et != static_cast<uint8_t>(ElementType::CModOpt))
			break;
		++pos;
	...
	std::vector<int32_t>& loBounds) const
{
	...
	if (!numSizes || !utils::bounds::countFits(pos, blob.size(), *numSizes, kMinBytesPerCompressedInt)) return;
	...
	if (!numLoBounds || !utils::bounds::countFits(pos, blob.size(), *numLoBounds, kMinBytesPerCompressedInt)) return;
...
CliType CliSigDecoder::decodeType(std::span<const uint8_t> blob, size_t& pos) const
{
	CliType ct;
	...
	switch (et)
	{
	case ElementType::String: ct.base = types::ClrString(); return ct;

	case ElementType::Object: ct.base = types::ClrObject(); return ct;
	...
	case ElementType::Class: {
		...
		ct.base = (et == ElementType::ValueType) ? clrNameToType(name) : types::Class(name);
		return ct;
	}
	...
	case ElementType::GenericInst: {
		...
		bool isValue = (pos < blob.size() && blob[pos] == static_cast<uint8_t>(ElementType::ValueType));
		...
		if (!countOpt || !utils::bounds::countFits(pos, blob.size(), *countOpt, kMinBytesPerGenericArg))
		{
...
BcType CliSigDecoder::toBcType(const CliType& ct) const
{
	return ct.base;
}

std::optional<CliType> CliSigDecoder::decodeField(std::span<const uint8_t> blob) const
{
...
std::optional<CliMethodSig> CliSigDecoder::decodeMethod(std::span<const uint8_t> blob) const
{
...
std::optional<CliLocalVarSig> CliSigDecoder::decodeLocalVar(std::span<const uint8_t> blob) const
{
...
std::optional<CliType> CliSigDecoder::decodeTypeSpec(std::span<const uint8_t> blob) const
{
	if (blob.empty()) return std::nullopt;
	size_t pos = 0;
	return decodeType(blob, pos);
...
std::vector<CliType> CliSigDecoder::decodeMethodSpec(std::span<const uint8_t> blob) const
{
