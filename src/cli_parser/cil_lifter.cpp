...
uint8_t CILLifter::r8(std::span<const uint8_t> c, size_t p)
{
	return c[p];
}
...
{
	return static_cast<uint32_t>(c[p]) | (static_cast<uint32_t>(c[p + 1]) << 8)
		 | (static_cast<uint32_t>(c[p + 2]) << 16) | (static_cast<uint32_t>(c[p + 3]) << 24);
}
uint64_t CILLifter::r64(std::span<const uint8_t> c, size_t p)
{
	return static_cast<uint64_t>(r32(c, p)) | (static_cast<uint64_t>(r32(c, p + 4)) << 32);
}
...
CILLifter::CILLifter(const ITypeNameResolver* resolver, const Options& opts): resolver_(resolver), opts_(opts) {}

// ─── Header parsing ──────────────────────────────────────────────────────────

bool CILLifter::parseHeader(std::span<const uint8_t> body, CILMethodHeader& hdr, size_t& codeStart)
{
	...
	uint16_t flagsAndSize = static_cast<uint16_t>(body[0]) | (static_cast<uint16_t>(body[1]) << 8);
	...
	hdr.codeSize = static_cast<uint32_t>(body[4]) | (static_cast<uint32_t>(body[5]) << 8)
				 | (static_cast<uint32_t>(body[6]) << 16) | (static_cast<uint32_t>(body[7]) << 24);
	hdr.localVarSigTok = static_cast<uint32_t>(body[8]) | (static_cast<uint32_t>(body[9]) << 8)
					   | (static_cast<uint32_t>(body[10]) << 16) | (static_cast<uint32_t>(body[11]) << 24);
	...
	if (hasMoreSections && body.size() > codeStart + hdr.codeSize)
	{
		...
		size_t sectStart = utils::align::alignUpSaturating(codeStart + hdr.codeSize, kSectAlignment);
		...
		while (sectStart < body.size())
		{
			...
			if (!utils::bounds::rangeFits(sectStart, body.size(), kSectHeaderBytes)) break;

			if (isFat)
			{
				dataSize = static_cast<size_t>(body[sectStart + 1]) | (static_cast<size_t>(body[sectStart + 2]) << 8)
			...
			if (isEH)
			{
				for (size_t ci = 0; ci < numClauses && utils::bounds::rangeFits(sectStart, body.size(), clauseSize);
					 ++ci)
				{
					...
					if (isFat)
					{
						clause.kind = static_cast<EHClauseKind>(
							static_cast<uint32_t>(body[sectStart]) | (static_cast<uint32_t>(body[sectStart + 1]) << 8)
					...
					{
						clause.kind = static_cast<EHClauseKind>(
							static_cast<uint16_t>(body[sectStart]) | (static_cast<uint16_t>(body[sectStart + 1]) << 8));
...
bool CILLifter::decodeOne(std::span<const uint8_t> code, size_t& pos, RawInsn& out) const
{
	...
	auto addI4 = [&](int32_t v) { out.operands.push_back(BcIntOperand{static_cast<int64_t>(v)}); };
	auto addI8 = [&](int64_t v) { out.operands.push_back(BcIntOperand{v}); };
	auto addLocal = [&](uint32_t idx) { out.operands.push_back(BcLocalOperand{idx}); };
	auto addTok = [&](uint32_t tok) { out.operands.push_back(tokenToOperand(tok)); };
	auto addBlock = [&](uint32_t target) { out.operands.push_back(BcBlockOperand{target}); };
	...
	auto addRelBlock = [&](uint64_t base, int64_t delta) {
		uint64_t target = 0;
		if (utils::btgt::relative(base, delta, code.size(), target)) addBlock(static_cast<uint32_t>(target));
	};
	...
	auto need = [&](size_t n) { return utils::bounds::rangeFits(pos, code.size(), n); };
	...
	case kFEPrefix | 0x1A:
	case kFEPrefix | 0x1D:
	case kFEPrefix | 0x1E: break;

	// InlineI (int32)
	case 0x20:
		if (!need(4)) return false;
		addI4(static_cast<int32_t>(r32(code, pos)));
		pos += 4;
		break;
	// InlineI8 (int64)
	case 0x21:
		if (!need(8)) return false;
		addI8(static_cast<int64_t>(r64(code, pos)));
		pos += 8;
		break;
	// InlineR4 (float32)
	case 0x22:
		if (!need(4)) return false;
		out.operands.push_back(BcIntOperand{0});
		pos += 4;
		break;
	// InlineR8 (float64)
	case 0x23:
	...
	case 0x1F:
		...
		addI4(static_cast<int8_t>(code[pos++]));
		break;
	...
	case 0xDD: {
		if (!need(4)) return false;
		int32_t delta = static_cast<int32_t>(r32(code, pos));
		pos += 4;
		addRelBlock(pos, delta);
		break;
	}
	...
	case 0x45: {
		...
		if (!utils::btgt::tableEnd(pos, n, kSwitchLabelBytes, code.size(), afterSwitch)) return false;
...
BcOperand CILLifter::tokenToOperand(uint32_t token) const
{
	...
	if (tableId == static_cast<uint8_t>(TableId::MethodDef) || tableId == static_cast<uint8_t>(TableId::MemberRef))
	{
		BcMethodRef ref;
		ref.owner = "<class>";
	...
	if (tableId == static_cast<uint8_t>(TableId::TypeDef) || tableId == static_cast<uint8_t>(TableId::TypeRef)
...
std::string CILLifter::tokenToString(uint32_t token) const
{
	...
	uint8_t tableId = static_cast<uint8_t>(token >> 24);
	uint32_t idx = token & 0x00FFFFFF;
	if (tableId == static_cast<uint8_t>(TableId::TypeDef)) return resolver_->typeDefName(idx);
	if (tableId == static_cast<uint8_t>(TableId::TypeRef)) return resolver_->typeRefName(idx);
	if (tableId == static_cast<uint8_t>(TableId::TypeSpec)) return resolver_->typeSpecType(idx).toString();
	return "tok_" + std::to_string(token);
}

std::vector<CILLifter::RawInsn> CILLifter::decodeInstructions(std::span<const uint8_t> code) const
{
...
BcCFG CILLifter::buildCFG(const std::vector<RawInsn>& insns, const CILMethodHeader& hdr) const
{
	BcCFG cfg;
	...
	for (const auto& insn: insns)
	{
		...
		{
			if (auto* bb = std::get_if<BcBlockOperand>(&op)) leaders.insert(bb->blockId);
		}
		// Branches make the next instruction a leader
		switch (insn.opcode)
		{
		case BcOpcode::DOTNET_BR:
		case BcOpcode::DOTNET_BR_S:
		...
		case BcOpcode::DOTNET_LEAVE:
		case BcOpcode::DOTNET_LEAVE_S: leaders.insert(insn.offset + insn.size); break;
		case BcOpcode::DOTNET_BRTRUE:
		case BcOpcode::DOTNET_BRFALSE:
		case BcOpcode::DOTNET_BRTRUE_S:
		case BcOpcode::DOTNET_BRFALSE_S:
		case BcOpcode::DOTNET_BEQ:
		case BcOpcode::DOTNET_BEQ_S:
		case BcOpcode::DOTNET_BNE_UN:
		case BcOpcode::DOTNET_BNE_UN_S:
		case BcOpcode::DOTNET_BGT:
		case BcOpcode::DOTNET_BGT_S:
		case BcOpcode::DOTNET_BGE:
		case BcOpcode::DOTNET_BGE_S:
		case BcOpcode::DOTNET_BLT:
		case BcOpcode::DOTNET_BLT_S:
		case BcOpcode::DOTNET_BLE:
		case BcOpcode::DOTNET_BLE_S:
		case BcOpcode::DOTNET_BGT_UN:
		case BcOpcode::DOTNET_BGT_UN_S:
		case BcOpcode::DOTNET_BGE_UN:
		case BcOpcode::DOTNET_BGE_UN_S:
		case BcOpcode::DOTNET_BLT_UN:
		case BcOpcode::DOTNET_BLT_UN_S:
		case BcOpcode::DOTNET_BLE_UN:
		case BcOpcode::DOTNET_BLE_UN_S:
		case BcOpcode::DOTNET_SWITCH: leaders.insert(insn.offset + insn.size); break;
		default: break;
		}
	}
	...
	for (const auto& clause: hdr.exceptionClauses)
	{
		...
		if (clause.kind == EHClauseKind::Filter) leaders.insert(clause.filterOffset);
	}
	...
	for (const auto& insn: insns)
	{
		...
		if (it != offsetToBlock.end()) blockIdx = it->second;
	...
	for (size_t bi = 0; bi < cfg.blocks().size(); ++bi)
	{
		...
		for (const auto& op: lastInsn.operands)
		{
			if (const auto* bb = std::get_if<BcBlockOperand>(&op))
			{
				...
				if (tit != offsetToBlock.end()) blk.succs.push_back(tit->second);
			}
		}
		...
		case BcOpcode::DOTNET_BR_S:
		case BcOpcode::DOTNET_LEAVE:
		case BcOpcode::DOTNET_LEAVE_S: isUnconditionalBranch = true; break;
		case BcOpcode::DOTNET_RET:
		case BcOpcode::DOTNET_THROW:
		case BcOpcode::DOTNET_RETHROW:
		case BcOpcode::DOTNET_ENDFINALLY:
		case BcOpcode::DOTNET_ENDFILTER: isReturn = true; break;
		default: break;
		}
	...
	for (const auto& clause: hdr.exceptionClauses)
	{
		...
		uint64_t tryEnd = 0;
		if (!utils::btgt::region(clause.tryOffset, clause.tryLength, hdr.codeSize, tryEnd)) continue;
...
BcCFG CILLifter::lift(std::span<const uint8_t> body, CILMethodHeader& outHeader) const
{
	error_.clear();
	size_t codeStart = 0;
