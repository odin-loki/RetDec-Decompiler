...
std::optional<uint32_t> decodeCompressedUInt(std::span<const uint8_t> blob, size_t& pos)
{
	...
	const utils::cint::Result r = utils::cint::decodeUnsigned(blob.data(), blob.size(), pos);
...
std::optional<int32_t> decodeCompressedInt(std::span<const uint8_t> blob, size_t& pos)
{
	...
	const utils::cint::SResult r = utils::cint::decodeSigned(blob.data(), blob.size(), pos);
	if (!r.ok) return std::nullopt;
	pos += r.bytesRead;
	return r.value;
}
...
std::string UserStringsHeap::utf16leToUtf8(const uint8_t* src, size_t srcBytes, size_t chars)
{
	...
	const size_t written = utils::txt::utf16leToUtf8(src, inBytes, out.data(), out.size());
	out.resize(written);
	return out;
}
...
std::string UserStringsHeap::get(uint32_t offset) const
{
	...
	const utils::cint::Result len = utils::cint::decodeUnsigned(data_.data(), data_.size(), offset);
...
Guid GuidHeap::get(uint32_t index) const
{
	...
	size_t off = 0;
	if (!utils::idxmap::rowAt1Based(data_.size(), index, kGuidBytes, off)) return g;
...
CliHeaps::CliHeaps(
	...
	uint8_t hs): strings(strData), userStrings(usData), guids(guidData), blobs(blobData), heapSizes(hs)
{}
