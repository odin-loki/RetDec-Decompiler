...
uint16_t PeReader::read16(size_t off) const
{
	...
	return static_cast<uint16_t>(data_[off]) | (static_cast<uint16_t>(data_[off + 1]) << 8);
}

uint32_t PeReader::read32(size_t off) const
{
	if (!checkRange(off, 4)) return 0;
	return static_cast<uint32_t>(data_[off]) | (static_cast<uint32_t>(data_[off + 1]) << 8)
		 | (static_cast<uint32_t>(data_[off + 2]) << 16) | (static_cast<uint32_t>(data_[off + 3]) << 24);
}

uint64_t PeReader::read64(size_t off) const
{
	if (!checkRange(off, 8)) return 0;
	return static_cast<uint64_t>(read32(off)) | (static_cast<uint64_t>(read32(off + 4)) << 32);
}
...
bool PeReader::open(const uint8_t* data, size_t size)
{
	...
	if (size < 0x40)
	{
		error_ = "File too small for DOS header";
		return false;
	}
	...
	if (read16(0) != 0x5A4D)
	{
		error_ = "Not a PE file (no MZ signature)";
		return false;
	}

	// PE header offset stored at 0x3C
	uint32_t peOffset = read32(0x3C);
	if (!checkRange(peOffset, 4))
	{
		error_ = "PE offset out of bounds";
		return false;
	}
	if (read32(peOffset) != kPESignature)
	{
		error_ = "Invalid PE signature";
		return false;
	}
...
bool PeReader::parseCOFF(size_t peOffset)
{
	...
	if (!checkRange(peOffset, 20))
	{
		error_ = "COFF header truncated";
		return false;
	}
	...
	if (!checkRange(optOffset, optHdrSize))
	{
		error_ = "Optional header truncated";
		return false;
	}
...
bool PeReader::parseSections(size_t sectOffset, uint16_t count)
{
	...
	for (uint16_t i = 0; i < count; ++i)
	{
		...
		if (!checkRange(base, 40))
		{
			error_ = "Section header truncated";
			return false;
		}
...
uint64_t PeReader::rvaToOffset(uint32_t rva) const
{
	...
	const uint64_t off = utils::secmap::addressToOffset(rva, secs.data(), secs.size(), size_);
...
bool PeReader::parseMetadataRoot()
{
	...
	const uint64_t paddedVersionLength = (static_cast<uint64_t>(declaredVersionLength) + 3) & ~UINT64_C(3);
...
std::span<const uint8_t> PeReader::streamBytes(const std::string& name) const
{
	for (const auto& sh: streams_)
	{
		if (sh.name == name)
		{
			...
			if (off < size_ && off + sh.size <= size_) return {data_ + off, sh.size};
		}
	}
	return {};
}
