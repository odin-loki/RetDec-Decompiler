...
std::vector<uint8_t> TestBinary::serialiseELF64() const
{
	...
	const std::size_t shoffPos = out.size();
	write64(out, 0);  // e_shoff -- patched below, once the size is known
	...
	// The section header table starts here, and e_shoff has to say so.
	//
	// It was written as 0 with a "(filled later)" comment and nothing ever
	// filled it, while e_shnum said sections + 1 and that many 64-byte headers
	// were appended below. A reader that trusts e_shnum walked the table from
	// file offset 0 -- over the ELF header itself -- and never saw the headers
	// that were written, so every section it reported was nonsense.
	{
		const uint64_t shoff = static_cast<uint64_t>(out.size());
		for (int i = 0; i < 8; ++i)
			out[shoffPos + static_cast<std::size_t>(i)] = static_cast<uint8_t>((shoff >> (8 * i)) & 0xFF);
	}
...
std::vector<uint8_t> TestBinary::serialiseELF32() const
{
	...
	uint32_t sz = 0;
	for (const auto& s: sections_)
		sz += static_cast<uint32_t>(s.data.size());
	write32(out, sz);
	write32(out, sz);
	write32(out, 5);
	write32(out, 0x1000);
...
std::vector<uint8_t> TestBinary::serialisePE32() const
{
	...
	out.push_back(14);
	out.push_back(0); // linker version
	...
	write16(out, 6);
	write16(out, 0); // OS version
	write16(out, 0);
	write16(out, 0); // Image version
	write16(out, 6);
	write16(out, 0); // Subsystem version
	write32(out, 0);
...
TestBinary TestBinary::makeELF64(const std::vector<uint8_t>& text, const std::vector<Symbol>& syms)
{
	TestBinary b;
	b.format_ = Format::ELF64;
	Section s;
	s.name = ".text";
	s.virtualAddr = 0x401000;
	s.data = text;
	s.executable = true;
	if (s.data.empty()) s.data = {0xC3}; // ret
	b.sections_.push_back(s);
	for (const auto& sym: syms)
		b.symbols_.push_back(sym);
	return b;
}

TestBinary TestBinary::makeELF32(const std::vector<uint8_t>& text, const std::vector<Symbol>& syms)
{
	TestBinary b;
	b.format_ = Format::ELF32;
	Section s;
	s.name = ".text";
	s.virtualAddr = 0x8048000;
	s.data = text;
	s.executable = true;
...
std::string SnapshotTester::makeDiff(const std::string& expected, const std::string& actual)
{
	// Simple line diff
	std::ostringstream os;
	...
	std::string le, la;
	int line = 1;
	// Both streams are read every iteration.
	//
	// `while (getline(se, le) || getline(sa, la))` short-circuits: while the
	// expected stream still yields lines the actual stream is never read at
	// all, so `la` stayed empty for the whole first half and every expected
	// line was reported as removed; only once `se` was exhausted did the loop
	// start pulling from `sa`, emitting every actual line as an addition,
	// numbered by a counter already past the end of both files. Every snapshot
	// failure printed a diff that located nothing.
	for (;;)
	{
		const bool haveExpected = static_cast<bool>(std::getline(se, le));
		const bool haveActual = static_cast<bool>(std::getline(sa, la));
		if (!haveExpected && !haveActual) break;
		if (!haveExpected) le.clear();
		if (!haveActual) la.clear();
		if (le != la)
		{
			os << "@@ line " << line << " @@\n";
			if (haveExpected) os << "- " << le << "\n";
			if (haveActual) os << "+ " << la << "\n";
		}
		le.clear();
		la.clear();
...
SnapshotTester::CompareResult SnapshotTester::compare(const std::string& name, const std::string& output)
{
	CompareResult cr;
	cr.actual = output;
	...
	cr.expected = std::string(std::istreambuf_iterator<char>(snap), std::istreambuf_iterator<char>());
...
std::vector<CorpusEntry> CorpusRunner::collect(const std::string& ext) const
{
	...
	for (const auto& entry: fs::recursive_directory_iterator(corpusDir_))
	{
		...
		if (dir.find("x86_64") != std::string::npos)
			ce.architecture = "x86_64";
		else if (dir.find("x86") != std::string::npos)
			ce.architecture = "x86";
		else if (dir.find("arm") != std::string::npos)
			ce.architecture = "arm";
		else if (dir.find("mips") != std::string::npos)
			ce.architecture = "mips";
...
void CorpusRunner::iterate(const std::function<bool(const CorpusEntry&)>& fn, const std::string& ext) const
{
...
std::vector<CorpusEntry> CorpusRunner::syntheticCorpus()
{
	...
	std::vector<CorpusEntry> result;
	const std::vector<std::pair<std::string, std::string>> archs = {{"x86_64", "elf"}, {"x86", "elf"}, {"x86", "pe"}};
...
BenchmarkResult PerformanceAsserter::benchmark(
	const std::function<void()>& fn, int64_t iterations, int64_t inputBytes, const std::string&)
{
	...
	for (int64_t i = 0; i < iterations; ++i)
	{
		...
		auto t1 = Clock::now();
		times.push_back(
			static_cast<double>(std::chrono::duration_cast<std::chrono::nanoseconds>(t1 - t0).count()) / 1e6);
	}
...
bool PerformanceAsserter::assertMaxMs(const std::function<void()>& fn, double maxMs, const std::string& desc)
{
	auto t0 = Clock::now();
	fn();
	double ms =
		static_cast<double>(std::chrono::duration_cast<std::chrono::nanoseconds>(Clock::now() - t0).count()) / 1e6;
	bool ok = ms <= maxMs;
	if (!ok && !isSoftMode())
	{
...
bool PerformanceAsserter::assertThroughput(
	const std::function<void()>& fn, int64_t inputBytes, double minBps, const std::string& desc)
{
...
std::string BenchmarkResult::format() const
{
	...
	if (throughputBps > 0) os << "  throughput=" << throughputBps / 1e6 << " MB/s\n";
	return os.str();
}
...
void TestLogger::install()
{
	messages_.clear();
}
void TestLogger::uninstall()
{
	messages_.clear();
}
