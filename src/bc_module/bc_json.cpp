...
std::string serialiseType(const BcType& t)
{
	...
	else if (t.isRef())
	{
		...
		switch (r.kind)
		{
		case BcRefKind::Class: o << "\"class\",\"name\":" << jStr(r.className); break;
		case BcRefKind::TypeVariable: o << "\"typevar\",\"name\":" << jStr(r.className); break;
		case BcRefKind::Array:
			o << "\"array\",\"dims\":" << r.arrayDims
			  << ",\"element\":" << (r.elementType ? serialiseType(*r.elementType) : "null");
			break;
		case BcRefKind::Generic:
			o << "\"generic\",\"base\":" << (r.genericBase ? serialiseType(*r.genericBase) : "null") << ",\"args\":[";
			...
			o << "]";
			break;
		case BcRefKind::Wildcard: o << "\"wildcard\""; break;
		case BcRefKind::BoundedAbove:
			o << "\"bounded_above\",\"bound\":" << (r.bound ? serialiseType(*r.bound) : "null");
			break;
		case BcRefKind::BoundedBelow:
			o << "\"bounded_below\",\"bound\":" << (r.bound ? serialiseType(*r.bound) : "null");
			break;
		case BcRefKind::Null: o << "\"null_type\""; break;
		}
	}
	else if (t.isFunc())
	{
		...
		o << "],\"return\":" << (ft.returnType ? serialiseType(*ft.returnType) : "null");
	}
	o << "}";
	return o.str();
}
	...
	std::visit(
		[&](const auto& v) {
			...
			if constexpr (std::is_same_v<T, BcIntOperand>)
				...
				o << "{\"kind\":\"methodref\",\"owner\":" << jStr(v.owner) << ",\"name\":" << jStr(v.name) << "}";
			else if constexpr (std::is_same_v<T, BcFieldRef>)
				o << "{\"kind\":\"fieldref\",\"owner\":" << jStr(v.owner) << ",\"name\":" << jStr(v.name) << "}";
			...
			{
				o << "{\"kind\":\"switch\",\"default\":" << v.defaultBlock << ",\"cases\":[";
				for (size_t i = 0; i < v.cases.size(); ++i)
				{
					if (i) o << ',';
					o << "{\"key\":" << v.cases[i].first << ",\"block\":" << v.cases[i].second << "}";
				}
				o << "]}";
			}
		},
...
{
	std::ostringstream o;
	o << indent(ind) << "{" << "\"id\":" << i.id << ",\"off\":" << i.offset << ",\"op\":" << static_cast<int>(i.opcode)
	  << ",\"pop\":" << (int)i.effect.pop << ",\"push\":" << (int)i.effect.push;
...
{
	std::ostringstream o;
	o << indent(ind) << "{" << "\"id\":" << b.id;
	if (!b.label.empty()) o << ",\"label\":" << jStr(b.label);
	// Successors / predecessors
	o << ",\"succs\":[";
	for (size_t k = 0; k < b.succs.size(); ++k)
	{
		if (k) o << ',';
		o << b.succs[k];
	}
...
{
	std::ostringstream o;
	o << indent(ind) << "{" << "\"name\":" << jStr(f.name) << ",\"type\":" << serialiseType(f.type)
...
{
	std::ostringstream o;
	o << indent(ind) << "{" << "\"name\":" << jStr(m.name) << ",\"descriptor\":" << jStr(m.descriptor.jvmDescriptor())
	  << ",\"access\":" << static_cast<uint32_t>(m.access) << ",\"isCtor\":" << (m.isConstructor ? "true" : "false")
...
{
	std::ostringstream o;
	o << indent(ind) << "{" << "\"name\":" << jStr(c.name) << ",\"fqName\":" << jStr(c.fqName)
	  << ",\"access\":" << static_cast<uint32_t>(c.access) << ",\"isInterface\":" << (c.isInterface ? "true" : "false")
	  << ",\"isEnum\":" << (c.isEnum ? "true" : "false") << ",\"isRecord\":" << (c.isRecord ? "true" : "false");
...
static BcType parseType(Parser& p)
{
	...
	p.parseObject([&](const std::string& key) {
		...
		else if (key == "element")
		{
			if (t.isRef())
			{
				const_cast<BcRefType&>(t.ref()).elementType = std::make_shared<BcType>(parseType(p));
			}
			else
				p.skipValue();
		}
		...
		{
			if (t.isRef())
				const_cast<BcRefType&>(t.ref()).arrayDims = (int)p.parseInt();
			else
				p.parseInt();
		}
		else
	...
	try
	{
		p.parseObject([&](const std::string& key) {
			...
			else if (key == "sourceLang")
			{
				...
				if (l == "Java")
					res.module.setSourceLang(SourceLang::Java);
				else if (l == "CSharp")
					res.module.setSourceLang(SourceLang::CSharp);
				else if (l == "Python")
					res.module.setSourceLang(SourceLang::Python);
				else if (l == "WebAssembly")
					res.module.setSourceLang(SourceLang::WebAssembly);
				else if (l == "Lua")
					res.module.setSourceLang(SourceLang::Lua);
			}
			else if (key == "stringPool")
			{
				p.parseArray([&]() { res.module.internString(p.parseString()); });
			}
			else if (key == "classes")
			{
				p.parseArray([&]() {
					...
					p.parseObject([&](const std::string& k) {
						if (k == "name")
							cls.name = p.parseString();
						else if (k == "fqName")
							cls.fqName = p.parseString();
						else if (k == "isInterface")
							cls.isInterface = p.parseBool();
						else if (k == "isEnum")
							cls.isEnum = p.parseBool();
						else if (k == "isRecord")
							cls.isRecord = p.parseBool();
						else if (k == "fields")
						{
							p.parseArray([&]() {
								BcField f;
							...
							p.parseArray([&]() {
								...
								p.parseObject([&](const std::string& mk) {
									if (mk == "name")
										...
										m.isConstructor = p.parseBool();
									else if (mk == "maxStack")
										m.maxStack = (uint16_t)p.parseInt();
									else if (mk == "maxLocals")
										m.maxLocals = (uint16_t)p.parseInt();
									else
										p.skipValue();
								});
...
bool roundTripEquals(const BcModule& original, std::string* diffOut)
{
	...
	for (size_t i = 0; i < a.classes().size(); ++i)
	{
		...
		{
			if (diffOut) *diffOut = "class[" + std::to_string(i) + "] fqName mismatch";
			return false;
		}
