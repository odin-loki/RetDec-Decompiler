...
static IOSignature
cpuEmulateFunc(const FunctionBytecode& func, const std::array<std::uint64_t, kTestInputWidth>& inputs)
{
	IOSignature sig;
	...
	while (steps++ < EM_MAXSTEPS && s.status == 0)
	{
		...
		auto getImm8 = [&]() -> std::int64_t {
			if (remain < 1)
			{
				s.status = 2;
				return 0;
			}
			auto v = (std::int64_t)(signed char)*q;
			++q;
			--remain;
			return v;
		};
		auto getImm32 = [&]() -> std::int64_t {
			if (remain < 4)
			{
				s.status = 2;
				return 0;
			}
			std::uint32_t v = q[0] | (uint32_t)q[1] << 8 | (uint32_t)q[2] << 16 | (uint32_t)q[3] << 24;
			q += 4;
			remain -= 4;
			return (int32_t)v;
		};
		...
		{ // RET
			...
			if (sp <= EM_SCRATCH - 8u)
			{
				...
				std::memcpy(&ret_addr, s.scratch + sp, 8);
				s.regs[EM_RSP] = sp + 8;
		...
		{ // XOR r/m, r / XOR r, r/m
			...
			s.regs[ra] ^= s.regs[rb];
			cpuSetFlags64(s, s.regs[ra]);
		}
		else if (opc == 0x01 || opc == 0x03)
		{ // ADD
			if (remain < 1)
			{
				s.status = 2;
			...
			--remain;
			unsigned int ra = (mr >> 3) & 7, rb = mr & 7;
			uint64_t a = s.regs[ra], b = s.regs[rb],
					 r = (opc == 1) ? (s.regs[rb] += a, s.regs[rb]) : (s.regs[ra] += b, s.regs[ra]);
			cpuSetFlags64(s, r, a, b);
		}
		else if (opc == 0x29 || opc == 0x2B)
		{ // SUB
			if (remain < 1)
			{
				s.status = 2;
		...
		{ // Group1 r/m, imm8
			...
			std::int64_t imm = getImm8();
			if (s.status) break;
			switch (reg)
			{
			case 0:
				s.regs[rm] += (uint64_t)(int64_t)imm;
				cpuSetFlags64(s, s.regs[rm]);
				break;
			case 5: {
				uint64_t a = s.regs[rm], b = (uint64_t)(int64_t)imm;
				s.regs[rm] -= b;
				cpuSetFlags64(s, s.regs[rm], a, b, true);
				break;
			}
			case 1:
				s.regs[rm] |= (uint64_t)(int64_t)imm;
		...
		{ // JMP/CALL r/m
			...
			{
				s.rip += (std::size_t)(q - p);
				s.rip = (std::uint32_t)(s.regs[rm] - func.baseVMA);
				continue;
			}
			else if (op2 == 2)
			{
		...
		{ // PUSH rAX
			...
			if (sp >= 8 && sp <= EM_SCRATCH)
			{
				sp -= 8;
				s.regs[EM_RSP] = sp;
				std::memcpy(s.scratch + sp, &s.regs[EM_RAX], 8);
			}
		}
		else if (opc == 0x58)
		{ // POP rAX
			...
			if (sp <= EM_SCRATCH - 8u)
			{
				std::memcpy(&s.regs[EM_RAX], s.scratch + sp, 8);
				s.regs[EM_RSP] = sp + 8;
			}
		}
		else if (opc == 0x48 && remain > 0)
		{ // REX.W prefix followed by op
...
#define GETIMM32(v)                                                                                             \
	...
	{                                                                                                           \
		...
		unsigned int _u =                                                                                       \
			(unsigned int)q[0] | (unsigned int)q[1] << 8 | (unsigned int)q[2] << 16 | (unsigned int)q[3] << 24; \
		v = (long long)(int)_u;                                                                                 \
		q += 4;                                                                                                 \
		remain -= 4;                                                                                            \
	}                                                                                                           \
	while (0)
		...
		if (opc == 0xC3 || opc == 0xCB)
		{
			...
			if (sp <= EM_SCRATCH - 8u)
			{
				...
				for (int b = 0; b < 8; b++)
					ra |= ((unsigned long long)scratch[sp + b]) << (b * 8);
				regs[EM_RSP] = sp + 8;
		...
		else if (opc == 0xE8)
		{
			...
			if (target < func_sz && callDepth < EM_MAXCALL)
			{
				...
				if (sp >= 8 && sp <= EM_SCRATCH)
				{
					...
					for (int b = 0; b < 8; b++)
						scratch[sp + b] = (unsigned char)(retAddr >> (b * 8));
					++callDepth;
					rip = target;
					continue;
				}
		...
		else if (opc == 0xE9)
		{
			long long rel;
			GETIMM32(rel);
			rip = rip + (unsigned int)(q - p) + (unsigned int)(long long)rel;
			continue;
		}
		else if (opc == 0xEB)
		{
...
#undef GETIMM32
	...
	for (int i = 0; i < 8; i++)
		h = fnv1a_step_d(h, inp[i]);
	for (int i = 0; i < 16; i++)
		h = fnv1a_step_d(h, regs[i]);
	result_hashes[gid] = h;
...
std::vector<std::array<std::uint64_t, kTestInputWidth>> defaultTestVectors(std::uint32_t funcIdx)
{
...
std::vector<IOSignature> CUDASemanticHasher::hash(const std::vector<FunctionBytecode>& funcs)
{
	...
	if (gpuReady_ && ctx_)
	{
		...
		auto cleanup = [&]() {
			for (void* p: {dBytes, dOff, dSz, dInputs, dScratch, dHashes, dStatus})
				if (p) cudaFree(p);
		};

		bool ok =
			(cudaMalloc(&dBytes, allBytes.size()) == cudaSuccess && cudaMalloc(&dOff, N * 4) == cudaSuccess
			 && cudaMalloc(&dSz, N * 4) == cudaSuccess && cudaMalloc(&dInputs, N * 8 * 8) == cudaSuccess
			 && cudaMalloc(&dScratch, N * EM_SCRATCH) == cudaSuccess && cudaMalloc(&dHashes, N * 8) == cudaSuccess
...

		{
			...
			retdec_semantic_hash_kernel<<<grid, block, 0, stream>>>(
				...
				(const unsigned int*)dOff,
				(const unsigned int*)dSz,
				(const unsigned long long*)dInputs,
				(unsigned char*)dScratch,
				(unsigned long long*)dHashes,
		...
		CUDAProfiler::instance().record(
			"semantic_hash", (std::uint64_t)std::chrono::duration_cast<std::chrono::nanoseconds>(t1 - t0).count());
		return result;
	}
cpu_fallback:
#endif
...
{
	for (auto& e: entries_)
		if (e.hash == ioHash) return e.name;
	return {};
}
