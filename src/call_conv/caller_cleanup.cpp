...
std::vector<PhysReg> intArgRegs(CC cc)
{
	switch (cc)
	{
	case CC::SysVAmd64: return {PhysReg::RDI, PhysReg::RSI, PhysReg::RDX, PhysReg::RCX, PhysReg::R8, PhysReg::R9};
	case CC::Win64: return {PhysReg::RCX, PhysReg::RDX, PhysReg::R8, PhysReg::R9};
	case CC::Fastcall: return {PhysReg::ECX, PhysReg::EDX}; // first two args
	case CC::Thiscall: return {PhysReg::ECX};               // only `this`
	case CC::AArch64SysV:
		return {PhysReg::X0, PhysReg::X1, PhysReg::X2, PhysReg::X3, PhysReg::X4, PhysReg::X5, PhysReg::X6, PhysReg::X7};
	case CC::Arm32Aapcs: return {PhysReg::R0, PhysReg::R1, PhysReg::R2, PhysReg::R3};
	case CC::Cdecl:
	case CC::Stdcall:
	default: return {}; // all args on stack
	}
}
...
std::vector<PhysReg> fpArgRegs(CC cc)
{
	...
	case CC::SysVAmd64:
		return {
			...
			PhysReg::XMM6,
			PhysReg::XMM7};
	case CC::Win64: return {PhysReg::XMM0, PhysReg::XMM1, PhysReg::XMM2, PhysReg::XMM3};
	case CC::AArch64SysV:
		return {PhysReg::V0, PhysReg::V1, PhysReg::V2, PhysReg::V3, PhysReg::V4, PhysReg::V5, PhysReg::V6, PhysReg::V7};
	default: return {};
	}
}
...
std::vector<PhysReg> intRetRegs(CC cc)
{
	switch (cc)
	{
	case CC::SysVAmd64: return {PhysReg::RAX, PhysReg::RDX}; // RAX primary, RDX for 128-bit
	case CC::Win64: return {PhysReg::RAX};
	case CC::Cdecl:
	case CC::Stdcall:
	case CC::Fastcall:
	case CC::Thiscall: return {PhysReg::EAX};
	case CC::AArch64SysV: return {PhysReg::X0, PhysReg::X1};
	case CC::Arm32Aapcs: return {PhysReg::R0, PhysReg::R1};
	default: return {PhysReg::RAX};
	}
}
...
std::vector<PhysReg> fpRetRegs(CC cc)
{
	...
	{
	case CC::SysVAmd64:
	case CC::Win64: return {PhysReg::XMM0};
	case CC::AArch64SysV: return {PhysReg::V0};
	case CC::Arm32Aapcs: return {PhysReg::S0, PhysReg::D0};
	default: return {};
	}
}
...
static std::pair<bool, int32_t> checkAddEspAfterCall(const ssa::BasicBlock* bb, std::size_t callIdx)
{
...
CallerCleanupDetector::Result CallerCleanupDetector::run(const ssa::SSAFunction& fn) const
{
	Result res;
	...
	{
		auto it = std::max_element(cleanupHist.begin(), cleanupHist.end(), [](const auto& a, const auto& b) {
	...
	for (const auto& s: res.sites)
	{
		if (s.ecxUsed) ++ecxCount;
		if (s.edxUsed) ++edxCount;
	}
	...
	if (res.calleeCleanupVotes >= res.callerCleanupVotes)
	{
		...
		{
			res.cc = edxCount > static_cast<int>(res.sites.size() / 4) ? CC::Fastcall : CC::Thiscall;
		}
		else
		{
			res.cc = CC::Stdcall;
