...
ShPtr<Statement> Statement::getLastStatement(ShPtr<Statement> stmts) {
	...
	// Cycle-safe traversal: use a visited set to detect loops in the
	// successor chain that can occur when IR is corrupted by an optimizer.
	std::unordered_set<Statement*> seen;
	ShPtr<Statement> lastStmt(stmts);
	while (lastStmt->hasSuccessor()) {
		if (!seen.insert(lastStmt.get()).second) {
			// Cycle detected: break here so callers don't loop forever.
			std::cerr << "[Statement] WARNING: successor cycle in"
				" getLastStatement -> breaking\n";
			break;
		}
