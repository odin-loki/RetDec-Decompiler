...
/**
* Collapse @c call; return; and @c %t = call; return %t; into @c return call(...)
* (framework Cat. 1.5 — tail call shape for clearer C).
*/
ShPtr<Statement> collapseAdjacentTailCallReturns(ShPtr<Statement> first) {
	if (!first) {
		return first;
	}
	...
	while (again) {
		...
		while (cur && cur->getSuccessor()) {
			...
			if (auto as = cast<AssignStmt>(cur)) {
				if (auto rs = cast<ReturnStmt>(nxt)) {
					if (rs->hasRetVal() && isa<CallExpr>(as->getRhs()) &&
							rs->getRetVal()->isEqualTo(as->getLhs())) {
						auto nr = ReturnStmt::create(
							cast<CallExpr>(as->getRhs()), nullptr,
							rs->getAddress());
						const auto &meta = rs->getMetadata();
						nr->setMetadata(meta.empty() ? "tail call" : meta);
...
ShPtr<Statement> BasicBlockConverter::convert(llvm::BasicBlock &bb) {
	...
	convertedBody = collapseAdjacentTailCallReturns(convertedBody);
...
ShPtr<Statement> BasicBlockConverter::visitCallInst(llvm::CallInst &inst) {
	...
	if (inst.hasNUses(0)) {
		stmt = CallStmt::create(callExpr, nullptr, LLVMSupport::getInstAddress(&inst));
	} else {
		auto lhs = converter->convertValueToVariable(&inst);
		stmt = AssignStmt::create(lhs, callExpr, nullptr,
			LLVMSupport::getInstAddress(&inst));
	}

	// Category 2.5: indirect calls — surfaced as optional debug comments on the
	// emitted statement when debug comments are enabled in the HLL writer.
	if (!inst.getCalledFunction()) {
		stmt->setMetadata("indirect call");
	}
...
ShPtr<Statement> BasicBlockConverter::visitInstruction(llvm::Instruction &inst) {
	...
	// Guard: both lhs and rhs must be non-null for AssignStmt::create.
	// Unhandled LLVM instruction types (e.g. vector operations) may return
	// nullptr from the expression converter; fall back to an empty statement
