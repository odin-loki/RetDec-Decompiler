...
ShPtr<Expression> LLVMValueConverter::convertValueToExpression(llvm::Value *value) {
	...
	auto expr = convertValueToExpressionDirectly(value);
	// Guard against null expressions that could arise from unhandled LLVM
	// instruction or constant types (e.g. vector instructions).  An empty
	// expression here would crash AddressOpExpr::create or AssignStmt::create.
	if (!expr) {
		expr = ConstInt::create(llvm::APInt(32, 0), false);
	}
...
ShPtr<Expression> LLVMValueConverter::convertValueToExpressionDirectly(
	...
	if (auto constant = llvm::dyn_cast<llvm::Constant>(value)) {
		if (shouldBeConvertedAsConst(constant)) {
			auto expr = convertConstantToExpression(constant);
			// Guard: convertConstantToExpression can return null for unsupported
			// constant types (e.g. ConstantVector, ConstantAggregateZero for
			// vector types).  Fall through to variable conversion in that case.
			if (expr) return expr;
		}
	} else if (auto inst = llvm::dyn_cast<llvm::Instruction>(value)) {
		if (shouldBeConvertedAsInst(inst)) {
			auto expr = convertInstructionToExpression(inst);
			// Guard: convertInstructionToExpression can return null for
			// unhandled instruction types.  Fall through to variable conversion.
			if (expr) return expr;
		}
	}
