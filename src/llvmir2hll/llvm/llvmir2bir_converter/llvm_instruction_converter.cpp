...
/**
* @brief Converts an LLVM extractelement instruction to a BIR expression.
*
* extractelement is not representable in the HLL BIR, so we fall back to
* converting the vector operand as a variable (which the type converter maps
* to an integer) and wrapping it in a trunc/zext cast to the element type.
* This is semantically approximate but avoids null-pointer crashes in
* downstream HLL writers.
*/
ShPtr<Expression> LLVMInstructionConverter::visitExtractElementInst(
		llvm::ExtractElementInst &inst) {
	// Treat the vector operand as an integer variable and cast it to the
	// element integer type.  The entry_alloca pass should have lowered all
	// vector chains before this point; this handler is a safety net.
	auto vecExpr = getConverter()->convertValueToExpression(inst.getVectorOperand());
	auto dstType = getConverter()->convertType(inst.getType());
	return TruncCastExpr::create(vecExpr, dstType);
}

/**
* @brief Converts an LLVM insertelement instruction to a BIR expression.
*
* Approximation: return the base vector operand unchanged.  The type
* converter maps vectors to integers so this is safe.
*/
ShPtr<Expression> LLVMInstructionConverter::visitInsertElementInst(
		llvm::InsertElementInst &inst) {
	return getConverter()->convertValueToExpression(inst.getOperand(0));
}

/**
* @brief Converts an LLVM shufflevector instruction to a BIR expression.
*
* Approximation: return the first vector operand unchanged.  The type
* converter maps vectors to integers so this is safe.
*/
...
ShPtr<Expression> LLVMInstructionConverter::visitInstruction(llvm::Instruction &inst) {
	...
	// FAIL always aborts (debug and release); this is unreachable.
...
*
* String literal recovery with offset:
*
*   GEP @str, 0, N   (where @str is a string global and N is a constant)
*
* is emitted as  @c str + N  (an AddOpExpr) rather than as an array index
* expression, allowing the HLL writer to render it as @c "hello" + 3 or to
* recover the substring starting at that offset.  A zero offset still returns
* the bare ConstString, which is the most common case.
*/
ShPtr<Expression> LLVMInstructionConverter::convertGetElementPtrToExpression(
	...
	if (auto globVar = llvm::dyn_cast<llvm::GlobalVariable>(pointedValue)) {
		if (getConverter()->storesStringLiteral(*globVar)) {
			...
			if (it != e) {
				// First index: pointer-level dereference — must be 0.
				auto firstIdx = llvm::dyn_cast<llvm::ConstantInt>(it.getOperand());
				if (firstIdx && firstIdx->isZero()) {
					++it;
					if (it != e) {
						// Second index: byte offset into the string data.
						auto charIdx = llvm::dyn_cast<llvm::ConstantInt>(
							it.getOperand());
						if (charIdx) {
							auto strExpr = getInitializerAsConstString(globVar);
							if (charIdx->isZero()) {
								// GEP offset 0 — just return the string itself.
								return strExpr;
							}
						...
						// No further index — plain reference to the string.
						return getInitializerAsConstString(globVar);
					}
				}
			}
			// Fallback: non-constant or unsupported index pattern.
			return getInitializerAsConstString(globVar);
		}
	}
