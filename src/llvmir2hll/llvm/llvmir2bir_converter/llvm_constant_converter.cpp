...
ShPtr<Expression> LLVMConstantConverter::convertToExpression(llvm::Constant *constant) {
	...
	} else if (auto cVec = llvm::dyn_cast<llvm::ConstantVector>(constant)) {
		// SSE / auto-vectorisation produces vector constants (possibly with
		// undef elements).  We have no vector concept in the HLL IR, so
		// represent the whole thing as an appropriately-sized zero integer.
		return convertZeroInitializer(cVec->getType());
	} else if (auto cDV = llvm::dyn_cast<llvm::ConstantDataVector>(constant)) {
		return convertZeroInitializer(cDV->getType());
	}

	FAIL("unsupported constant: " << const_cast<llvm::Constant &>(*constant));
	llvm_unreachable("unsupported LLVM constant");
}
...
ShPtr<Expression> LLVMConstantConverter::convertZeroInitializer(
	...
	if (type->isIntegerTy()) {
		...
		// Vector types arise from auto-vectorisation in the LLVM front-end.
		// We have no vector IR concept, so represent the zero-initialiser as
		// a zero integer of the same total bit-width (e.g. <4 x i32> → 0u128).
		unsigned bits = type->getPrimitiveSizeInBits();
		if (bits == 0) bits = 64; // conservative fallback
		return ConstInt::create(0, bits);
	}
