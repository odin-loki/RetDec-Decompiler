...
/// If LLVM reports multiple exit blocks for this natural loop, attach a debug
/// comment on the structured loop (framework Cat. 1.2).
void attachMultiExitLoopComment(const ShPtr<Statement> &stmt, llvm::Loop *loop) {
	if (!stmt || !loop || !LLVMSupport::loopHasMultipleExits(loop)) {
		return;
	}
	auto n = LLVMSupport::countDistinctLoopExits(loop);
	stmt->setMetadata("loop: multi-exit (" + std::to_string(n) + " LLVM exit blocks)");
}
...
ShPtr<Statement> StructureConverter::convertFuncBody(llvm::Function &func) {
	...
	// Try Controlled Node Splitting to eliminate irreducible regions before
	// falling back to gotos. See DECOMPILATION_IMPROVEMENT_FRAMEWORK.md Cat 1.1.
	if (cfg->getSuccNum() != 0 && tryControlledNodeSplitting(cfg)) {
		while (cfg->getSuccNum() != 0 && reduceCFG(cfg)) {
			// Retry reduction after node splitting.
		}
	}
...
std::string StructureConverter::getDetectedCompilerOrPacker() const {
	return resModule ? resModule->getDetectedCompilerOrPacker() : std::string();
}
...
/**
* @brief Attempts controlled node splitting on merge points so @c reduceCFG can
*        make progress on irreducible regions. Clones one CFG node that shares an
*        LLVM basic block with the original and redirects all but one incoming
*        edge to the clone.
*
* Conservative: skips nodes with @c statementSuccessor wiring, skips bodies that
*        fail the same size heuristic as @c canBeCloned.
*
* @returns @c true if a split was applied.
*
* @par Preconditions
*  - @a cfg is non-null
*/
bool StructureConverter::tryControlledNodeSplitting(ShPtr<CFGNode> cfg) {
	PRECONDITION_NON_NULL(cfg);

	auto bodyAllowsSplitClone = [](const ShPtr<CFGNode> &node) -> bool {
	...
	for (const auto &v : order) {
		...
		if (v->hasStatementSuccessor()) {
			continue;
		}
		if (!bodyAllowsSplitClone(v)) {
			continue;
		}

		bool unsafePred = false;
		for (const auto &p : v->getPredecessors()) {
			if (p->hasStatementSuccessor() && p->getStatementSuccessor() == v) {
				unsafePred = true;
				break;
			}
		}
		...
		using EdgeRef = std::pair<ShPtr<CFGNode>, std::size_t>;
		std::vector<EdgeRef> edgeRefs;
		for (const auto &p : v->getPredecessors()) {
			for (std::size_t i = 0, n = p->getSuccNum(); i < n; ++i) {
				if (p->getSucc(i) == v) {
					edgeRefs.emplace_back(p, i);
				}
			}
		}
		if (edgeRefs.size() < 2) {
		...
		fixClonedGotos(bodyClone);

		auto clone = std::make_shared<CFGNode>(v->getFirstBB(), bodyClone);
		clone->setLastBB(v->getLastBB());

		for (std::size_t i = 0, n = v->getSuccNum(); i < n; ++i) {
			auto s = v->getSucc(i);
			clone->addSuccessor(s);
			if (v->isBackEdge(s)) {
				clone->markAsBackEdge(s);
			}
		}
...
void StructureConverter::reduceToForLoop(ShPtr<CFGNode> node) {
	...
	auto forStmt = ForLoopStmt::create(indVar, startValue, endCond, step,
		node->getBody(), nullptr, node->getBody()->getAddress());
	attachMultiExitLoopComment(forStmt, loop);
	node->setBody(forStmt);
...
void StructureConverter::reduceToWhileTrueLoop(ShPtr<CFGNode> node) {
	...
	auto whileStmt = WhileLoopStmt::create(ConstBool::create(true),
		node->getBody(), nullptr, node->getBody()->getAddress());
	attachMultiExitLoopComment(whileStmt, getLoopFor(node));
	node->setBody(whileStmt);
...
void StructureConverter::reduceToNestedWhileTrueLoopWithContinueInHeader(
	...
	auto nestedWhile = WhileLoopStmt::create(ConstBool::create(true),
		node->getBody(), nullptr, node->getBody()->getAddress());
	attachMultiExitLoopComment(nestedWhile, getLoopFor(node));
	node->setBody(nestedWhile);
