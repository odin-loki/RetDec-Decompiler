...
/**
* @brief Returns the number of distinct LLVM basic blocks that are loop exits.
*
* Used for multi-exit loop awareness (framework Cat. 1.2). Complements natural
* loop detection from @c LoopInfo.
*
* @par Preconditions
*  - @a loop is non-null
*/
std::size_t LLVMSupport::countDistinctLoopExits(const llvm::Loop *loop) {
...
/**
* @brief Returns @c true if @a loop has more than one distinct exit block.
*
* @par Preconditions
*  - @a loop is non-null
*/
bool LLVMSupport::loopHasMultipleExits(const llvm::Loop *loop) {
	return countDistinctLoopExits(loop) > 1;
}
