...
void FuncOptimizer::doOptimization() {
	...
	for (auto i = module->func_begin(), e = module->func_end(); i != e; ++i) {
		// Honour the global HLL optimisation deadline.  For very large (Rust/Go)
		// binaries with thousands of functions this avoids multi-minute runtimes.
		if (isGlobalDeadlineExceeded()) {
			Log::error() << Log::Warning
				<< "[" << getId() << "] global HLL time budget exceeded; "
				<< "skipping remaining functions." << std::endl;
			break;
		}
		runOnFunction(*i);
	}
