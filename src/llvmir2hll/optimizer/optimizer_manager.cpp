...
/// Global wall-clock budget for all HLL optimizer passes combined (seconds).
/// For typical C/C++ binaries the entire optimizer pipeline finishes in <30s.
/// For very large Rust/Go binaries with thousands of monomorphized functions
/// the passes can take many minutes; we stop early after this budget to keep
/// the decompiler responsive.  The output quality degrades gracefully: early
/// passes (SimpleCopyPropagation, simple structure optimisations) have already
/// run, so the code is still readable.
constexpr unsigned HLL_OPT_BUDGET_SECONDS = 60;
...
StringSet trimOptimizerSuffix(const StringSet& opts)
{
	...
	for (const auto& opt: opts)
	{
		...
		if (opt.size() > OPT_SUFFIX.size() && opt.substr(opt.size() - OPT_SUFFIX.size()) == OPT_SUFFIX)
		{
			// It does, so trim it.
...
OptimizerManager::OptimizerManager(
	...
	bool enableDebug):
	enabledOpts(trimOptimizerSuffix(enabledOpts)), disabledOpts(trimOptimizerSuffix(disabledOpts)),
	hllWriter(hllWriter), va(va), cio(cio), arithmExprEvaluator(arithmExprEvaluator), enableDebug(enableDebug),
...
{
	// Record the wall-clock start time and set a global deadline that both
	// runOptimizerProvidedItShouldBeRun and FuncOptimizer::doOptimization()
	// can use to enforce the HLL_OPT_BUDGET_SECONDS time budget.
	optStartTime = std::chrono::steady_clock::now();
	timeBudgetExceeded = false;
	Optimizer::setGlobalDeadline(optStartTime + std::chrono::seconds(HLL_OPT_BUDGET_SECONDS));
	...
	run<DeadLocalAssignCallOptimizer>(m, va);
	...
	// better results for simple integer induction variables). Also, run it only
	// for C because the Python HLL writer does not support emission of
	// universal for loops.
	...
	// Expression optimizations - run sequentially.
	// These passes all modify shared IR statement/successor chains without
	// internal locking; running them in parallel corrupts the IR (creates
	// cyclic successor chains), so they must be serialised.
	run<BitShiftOptimizer>(m);
	run<DerefAddressOptimizer>(m);
	// CharArrayToStringOptimizer must run before EmptyArrayToStringOptimizer
	// because the latter requires at least one ConstString in the outer array
	// to fire (isArrayOfStrings returns false if all elements are ConstArrays).
	// Running CharArray first converts printable sub-arrays to ConstString,
	// giving EmptyArray the ConstString anchors it needs for zero sub-arrays.
	run<CharArrayToStringOptimizer>(m);
...
void OptimizerManager::runOptimizerProvidedItShouldBeRun(ShPtr<Optimizer> optimizer)
{
	...
	// Enforce the global wall-clock budget for all HLL optimisation passes.
	// Once the budget is exhausted we skip all subsequent passes so that the
	// decompiler finishes in a bounded time even for very large (Rust/Go)
	// binaries.  The passes that already completed still improve readability.
	if (timeBudgetExceeded)
	{
		return;
	}
	auto elapsed =
		std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - optStartTime).count();
	if (elapsed >= static_cast<long long>(HLL_OPT_BUDGET_SECONDS))
	{
		Log::error() << Log::Warning << "HLL optimisation time budget (" << HLL_OPT_BUDGET_SECONDS
					 << "s) exceeded; skipping remaining passes." << std::endl;
...
{
	auto optimizer = std::make_shared<Optimization>(m, std::forward<Args>(args)...);
