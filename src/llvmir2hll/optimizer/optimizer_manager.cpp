...
void OptimizerManager::optimize(ShPtr<Module> m) {
	...
	Optimizer::setGlobalDeadline(optStartTime +
		std::chrono::seconds(HLL_OPT_BUDGET_SECONDS));
...
void OptimizerManager::runOptimizerProvidedItShouldBeRun(ShPtr<Optimizer> optimizer) {
	...
	auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(
		std::chrono::steady_clock::now() - optStartTime).count();
	if (elapsed >= static_cast<long long>(HLL_OPT_BUDGET_SECONDS)) {
		Log::error() << Log::Warning
			<< "HLL optimisation time budget (" << HLL_OPT_BUDGET_SECONDS
