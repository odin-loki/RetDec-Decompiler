...
/// Maximum number of fixed-point iterations per function.  Without a cap,
/// adversarial IR (e.g. very large dtoa/pformat functions) can loop for
/// tens of minutes.  50 iterations covers essentially all practical cases;
/// SSE-heavy Rust/Go functions that don't converge quickly tend to produce
/// very little additional simplification beyond the first 10-20 passes anyway.
const unsigned MAX_COPY_PROP_ITERATIONS = 50;

/// If a function has more than this many statements, reduce the per-function
/// iteration cap to avoid O(n²) runtimes on massive inlined functions.
const unsigned LARGE_FUNC_STMT_THRESHOLD = 500;
const unsigned MAX_COPY_PROP_ITERATIONS_LARGE = 10;

/// Minimum number of functions to enable parallel processing.  Below this,
/// thread spawning and per-thread analysis setup overhead exceeds the gain.
/// Windows has higher thread overhead; use a higher threshold there.
#ifdef _WIN32
...
// ---------------------------------------------------------------------------
// Memoised text-representation helpers
// ---------------------------------------------------------------------------
//
// getTextRepr() walks the entire expression tree of a statement and
// concatenates strings.  Called naively inside std::sort() comparators it
// becomes O(N * log(N) * stmtLength), which is catastrophic for large
// functions (hundreds of variables, thousands of def-use chains).
//
// We pre-populate a cache before each sort so that every statement is
// serialised at most once.
//
using ReprCache = std::unordered_map<const Statement*, std::string>;

/// Returns a memoised text representation for @a s.  If the statement has
/// not been seen before it is inserted into @a cache.
static const std::string& cachedRepr(const ShPtr<Statement>& s,
		ReprCache& cache) {
	if (!s) {
		static const std::string nullRepr;
		return nullRepr;
	}
	auto [it, inserted] = cache.emplace(s.get(), std::string{});
	if (inserted) {
		it->second = s->getTextRepr();
	}
	return it->second;
}
...
auto ordered(const StmtSet &stmts) {
	...
	for (auto& s : stmts) {
		cache.emplace(s.get(), s->getTextRepr());
	}
...
*        using the provided cache to avoid redundant getTextRepr() calls.
*/
auto textReprs(const StmtSet &stmts, ReprCache& cache) {
	StringSet reprs;
	for (auto &stmt : stmts) {
		reprs.insert(cachedRepr(stmt, cache));
	}
	return reprs;
}

/**
* @brief Compares the given two statements from DU chains using a memoised
*        text-representation cache.
*/
int compareStmtsInDUChains(const ShPtr<Statement> &s1,
		const ShPtr<Statement> &s2, ReprCache& cache) {
	if (!s1 && !s2) {
		return 0;
	} else if (s1 && !s2) {
		return 1;
	...
	return cachedRepr(s1, cache).compare(cachedRepr(s2, cache));
}

/**
* @brief Returns an ordered version of the given DU chain.
*        Pre-populates a text-repr cache before sorting to avoid O(N²)
*        string re-computation inside the comparator.
*/
auto ordered(const DefUseChains::DefUseChain &du) {
	// Pre-populate the cache for every statement and use that appears in du.
	// Additional entries (parents/successors/predecessors) are inserted lazily
	// inside the comparator via cachedRepr().
	ReprCache cache;
	cache.reserve(du.size() * 2);
	for (const auto& [svp, uses] : du) {
		const auto& stmt = svp.first;
		if (stmt) cache.emplace(stmt.get(), stmt->getTextRepr());
		for (const auto& use : uses) {
			if (use) cache.emplace(use.get(), use->getTextRepr());
		}
	}

	std::vector<std::pair<DefUseChains::StmtVarPair, StmtSet>> v(du.begin(), du.end());
	std::sort(v.begin(), v.end(), [&cache](const auto &p1, const auto &p2) {
		...
		cmpResult = compareStmtsInDUChains(s1, s2, cache);
		if (cmpResult != 0) {
			return cmpResult < 0;
		}

		// Compare use-set text representations (lazy: cache populated on demand).
		auto repr1 = textReprs(p1.second, cache);
		auto repr2 = textReprs(p2.second, cache);
		cmpResult = (repr1 < repr2) ? -1 : (repr2 < repr1 ? 1 : 0);
		if (cmpResult != 0) {
			return cmpResult < 0;
		}

		// Last resort: walk parent/successor/predecessor chains.
		cmpResult = compareStmtsInDUChains(s1->getParent(), s2->getParent(), cache);
		if (cmpResult != 0) {
			return cmpResult < 0;
		}
		cmpResult = compareStmtsInDUChains(s1->getSuccessor(), s2->getSuccessor(), cache);
		if (cmpResult != 0) {
			return cmpResult < 0;
		}
		...
		while (true) {
			...
			if (s1PredSize != s2PredSize) {
				...
				cmpResult = compareStmtsInDUChains(s1Pred, s2Pred, cache);
				if (cmpResult != 0) {
					return cmpResult < 0;
				}
				s1Seen.insert(s1);
...
void CopyPropagationOptimizer::doOptimization() {
	// Initialization: clear the cache and set up alias analysis.
	// Clearing the cache even when valid speeds up the optimization.
	va->clearCache();
	va->initAliasAnalysis(module);

	// Collect all function definitions to process.
	std::vector<ShPtr<Function>> funcs;
	for (auto i = module->func_begin(), e = module->func_end(); i != e; ++i) {
		funcs.push_back(*i);
	}

	// Use lazy caching: precomputeEverything() traverses the entire module
	// upfront; after loop/structure optimisations the IR is significantly
	// larger and this traversal becomes more expensive than the cache benefit.
	// Per-function lazy precompute is O(stmts) per function on first access.
	vuv = VarUsesVisitor::create(va, /*enableCaching=*/true, /*module=*/nullptr);
	dua = DefUseAnalysis::create(module, va, vuv);
	uda = UseDefAnalysis::create(module);

	// Adaptive parallelism: for small modules (few functions), thread overhead
	// exceeds gains; use serial.  For large modules, parallelize across functions.
	const bool useParallel = (funcs.size() >= PARALLEL_THRESHOLD);
	if (!useParallel) {
		for (auto& func : funcs)
			runOnFunction(func);
		return;
...
void CopyPropagationOptimizer::runOnFunction(ShPtr<Function> func) {
	// Skip mega-functions BEFORE the expensive CFG build.
	...
	constexpr std::size_t MAX_CFG_NODES_FOR_COPY_PROP = 200;
	if (currCFG->getNumberOfNodes() > MAX_CFG_NODES_FOR_COPY_PROP) {
		return;
	}

	// Keep optimizing until there are no changes, with a hard iteration cap
	// to prevent runaway behaviour on adversarial IR.  For very large
	// functions (many def-use chains, typical in SSE-heavy Rust/Go code),
	// use an even tighter cap since extra iterations add little value.
	//
	// Each individual function is also capped at PER_FUNC_BUDGET_SECONDS so
	// that a single monomorphized Rust/Go "mega-function" cannot block the
	// entire optimization phase.
	constexpr unsigned PER_FUNC_BUDGET_SECONDS = 20;
	auto funcStart = std::chrono::steady_clock::now();

	auto funcTimedOut = [&]() {
		auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(
			std::chrono::steady_clock::now() - funcStart).count();
		return elapsed >= static_cast<long long>(PER_FUNC_BUDGET_SECONDS);
	};

	unsigned iterations = 0;
	unsigned effectiveMax = MAX_COPY_PROP_ITERATIONS;
	do {
		// Honour the global HLL optimisation deadline so that large Rust/Go
		// binaries with thousands of monomorphized functions don't run forever.
		if (isGlobalDeadlineExceeded() || funcTimedOut()) {
			break;
		}
		...
		// Check after the potentially expensive getDefUseChains call.
		if (isGlobalDeadlineExceeded() || funcTimedOut()) {
			break;
		}
		...
		// Tighten the cap once we know the function is large.
		if (iterations == 1 && ducs->du.size() > LARGE_FUNC_STMT_THRESHOLD)
			effectiveMax = MAX_COPY_PROP_ITERATIONS_LARGE;
...
void CopyPropagationOptimizer::handleCaseSingleUse(ShPtr<Statement> stmt,
	...
	if (!callsInStmt.empty()) {
		...
		if (stmtSucc != use) {
			// Extended: allow propagation if the gap is safe.
			if (!canPropagateThroughStmts(stmt, stmtLhsVar, use, module, va)) {
				LOG << "\t" << "end 10" << std::endl;
				return;
			}
		}

		// stmtLhsVar has to be used precisely once in the use statement.
		...
		//    return func() + func()
		//
		// Use the actual `use` statement's data (not the immediate successor's)
		// so that we correctly handle the extended propagation case where
		// `use` may be several statements beyond `stmt`.
		const auto &useData = va->getValueData(use);
		if (useData->getDirNumOfUses(stmtLhsVar) != 1) {
			LOG << "\t" << "end 11" << std::endl;
			return;
		}
		...
		// use statement.
		for (auto cit = useData->call_begin(),
				e = useData->call_end(); cit != e; ++cit) {
		...
		// Check that the use statement does not use global variables.
		// These might be modified by the call in stmt.
		// This check is skipped if the call in stmt is calling a declared
		// (external) function.
		...
		if (!fnc->isDeclaration()) {
			for (auto i = useData->dir_read_begin(),
					e = useData->dir_read_end(); i != e ; ++i) {
