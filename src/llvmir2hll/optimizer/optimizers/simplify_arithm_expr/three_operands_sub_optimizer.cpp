...
/**
* @brief Maps a full-bit-width mask value to the corresponding integer width.
*
* Returns the number of bits for masks like 0xff (8), 0xffff (16),
* 0xffffffff (32), 0xffffffffffffffff (64). Returns 0 for anything else.
*/
unsigned maskToBitWidth(const llvm::APInt &val) {
	if (val == 0xff)               return 8;
	if (val == 0xffff)             return 16;
	if (val == 0xffffffff)         return 32;
	if (val == 0xffffffffffffffffULL) return 64;
	return 0;
}
...
// ---------------------------------------------------------------------------
// Bit mask pattern: x & 0xff  →  (uint8_t)x
//                  x & 0xffff  →  (uint16_t)x
//                  x & 0xffffffff  →  (uint32_t)x
//                  x & 0xffffffffffffffff  →  (uint64_t)x
//
// These patterns appear frequently in compiled code when the original source
// used a narrower type than the register width.  Replacing the mask with an
// explicit truncating cast makes the decompiled output dramatically cleaner
// and preserves the programmer's intent more faithfully.
// ---------------------------------------------------------------------------
void ThreeOperandsSubOptimizer::visit(ShPtr<BitAndOpExpr> expr) {
	OrderedAllVisitor::visit(expr);

	// Identify which operand, if any, is a constant full-bit-width mask.
	ShPtr<ConstInt> maskConst;
	ShPtr<Expression> maskedExpr;

	if (auto c = cast<ConstInt>(expr->getSecondOperand())) {
		maskConst  = c;
		maskedExpr = expr->getFirstOperand();
	} else if (auto c = cast<ConstInt>(expr->getFirstOperand())) {
		maskConst  = c;
		maskedExpr = expr->getSecondOperand();
	}

	if (!maskConst || !maskedExpr) {
		return;
	}

	unsigned targetBits = maskToBitWidth(maskConst->getValue());
	if (targetBits == 0) {
		// Not a full-width mask — leave the expression unchanged.
		return;
	}

	// Extended pattern: (x + C) & mask  →  (uintN_t)(x - k)
	// when C ≡ -k  (mod 2^N)  for a small positive k.
	//
	// This arises constantly in x86-64 decompilation:
	//   add i64 %n, 4294967295      ; n + (2^32 - 1) = n - 1  mod 2^32
	//   and i64 %result, 4294967295 ; mask to 32 bits
	// should decompile to  (uint32_t)(n - 1), not (uint32_t)(n + 0xffffffff).
	//
	// We apply this only for  k <= 256  to stay conservative.
	if (ShPtr<AddOpExpr> addExpr = cast<AddOpExpr>(maskedExpr)) {
		ShPtr<ConstInt> addConst;
		ShPtr<Expression> addVar;

		if (ShPtr<ConstInt> c = cast<ConstInt>(addExpr->getSecondOperand())) {
			addConst = c;
			addVar   = addExpr->getFirstOperand();
		} else if (ShPtr<ConstInt> c = cast<ConstInt>(addExpr->getFirstOperand())) {
			addConst = c;
			addVar   = addExpr->getSecondOperand();
		}

		if (addConst && addVar) {
			const uint64_t maskVal = maskConst->getValue().getZExtValue();
			const uint64_t base    = maskVal + 1;            // = 2^N
			const uint64_t C_mod   = addConst->getValue().getZExtValue() & maskVal;
			// k is how much to subtract: C ≡ -k (mod 2^N)
			const uint64_t k       = (C_mod == 0) ? 0 : (base - C_mod);

			if (k >= 1 && k <= 256) {
				auto kConst  = ConstInt::create(static_cast<std::int64_t>(k),
				                                targetBits, /* isSigned */ true);
				auto sub     = SubOpExpr::create(addVar, kConst);
				auto dstType = IntType::create(targetBits, /* isSigned */ false);
				auto newCast = TruncCastExpr::create(sub, dstType);
				optimizeExpr(expr, newCast);
				return;
			}
		}
	}

	// Replace  x & 0xff  with  (uintN_t)x  (a truncating cast to the
	// narrower unsigned integer type).
	auto dstType = IntType::create(targetBits, /* isSigned */ false);
	auto cast    = TruncCastExpr::create(maskedExpr, dstType);
	optimizeExpr(expr, cast);
}
