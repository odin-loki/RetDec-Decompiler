...
void WhileTrueToForLoopOptimizer::visit(ShPtr<WhileLoopStmt> stmt) {
	...
	// If the loop exit is via `return` (not `break`), insert a clone of that
	// return statement immediately after the for loop.  Without this, the
	// optimized loop falls through to the next statement when it should exit
	// the function.
	//
	// The loopEnd is an IfStmt whose body is one of:
	//   (a) return <expr>
	//   (b) <assign>; return <expr>
	// We walk to the last statement in the if-body to find the ReturnStmt.
	if (splittedLoop && splittedLoop->loopEnd) {
		auto ifBody = skipEmptyStmts(splittedLoop->loopEnd->getFirstIfBody());
		// Skip an optional assignment before the return.
		if (auto assignStmt = cast<AssignStmt>(ifBody)) {
			ifBody = skipEmptyStmts(assignStmt->getSuccessor());
		}
		if (auto retStmt = cast<ReturnStmt>(ifBody)) {
			// Clone the return and append it as the for loop's successor.
			auto retClone = cast<ReturnStmt>(retStmt->clone());
			retClone->setSuccessor(forLoop->getSuccessor());
			forLoop->setSuccessor(retClone);
		}
	}
}
...
ShPtr<Expression> WhileTrueToForLoopOptimizer::computeStepOfForLoop(
	...
	// Delegate to extension for mul/shl steps.
	return computeStepExt(updateRhs, indVarInfo->indVar);
}
