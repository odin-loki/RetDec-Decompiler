...
{
	if (!s) return false;
	while (s->getSuccessor())
		s = s->getSuccessor();
	return isa<ReturnStmt>(s) || isa<BreakStmt>(s) || isa<ContinueStmt>(s) || isa<GotoStmt>(s);
}
...
private:
	...
	//                 nullptr.
	//-----------------------------------------------------------------------
	void rewriteStmtList(ShPtr<Statement> stmt, ShPtr<Statement> loopHeader, ShPtr<Statement> loopExit)
	{
		while (stmt)
		{
			...
			if (auto is = cast<IfStmt>(stmt))
			{
				...
				if (is->hasElseClause()) rewriteStmtList(is->getElseClause(), loopHeader, loopExit);
			}
			...
			if (tryPatternC(stmt, loopHeader))
			{
				changed = true;
			}
			if (tryPatternD(stmt, loopExit))
			{
				changed = true;
			}
	...
	bool tryPatternA(ShPtr<Statement> stmt)
	{
		...
		while (body && isa<EmptyStmt>(body))
			body = body->getSuccessor();
