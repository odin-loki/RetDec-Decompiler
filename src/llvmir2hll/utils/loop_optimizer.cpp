...
*
* When multiple induction variables are present (e.g. nested-loop counters
* merged by the earlier passes), only the variable that is both written by the
* update statement and read by the exit condition is selected.  This prevents
* inner-loop counters from being mistaken for the outer-loop induction
* variable.
*/
ShPtr<IndVarInfo> getIndVarInfo(ShPtr<WhileLoopStmt> stmt) {
	...
	// Obtain the induction variable.  Among all variables written by the
	// update statement, prefer the one that also appears in the exit
	// condition.  This avoids confusion when multiple counters share the
	// same update block (e.g. nested loops that were not yet separated).
	auto modifiedVarsInUpdateStmt = UsedVarsVisitor::getUsedVars(
		updateStmt, false)->getWrittenVars();
	if (modifiedVarsInUpdateStmt.empty()) {
		return {};
	}

	auto exitCondVars = UsedVarsVisitor::getUsedVars(
		exitCond, true)->getReadVars();

	ShPtr<Variable> indVar;
	// Prefer a modified variable that also appears in the exit condition.
	for (auto &v : modifiedVarsInUpdateStmt) {
		if (hasItem(exitCondVars, v)) {
			indVar = v;
			break;
		}
	}
	// Fallback: take the first written variable (original behaviour).
	if (!indVar) {
		indVar = *modifiedVarsInUpdateStmt.begin();
	}
...
/**
* @brief Returns @c true if @a stmt is a do-while pattern.
*
* A do-while pattern is a "while true" loop where the exit condition check
* (isLoopEnd) is the last meaningful statement in the body — nothing follows
* it except possibly empty metadata statements.
*/
bool isDoWhileLoop(ShPtr<WhileLoopStmt> stmt) {
	...
	bool foundEnd = false;
	auto curr = stmt->getBody();
	while (curr) {
		if (isLoopEnd(curr)) {
			// There must be no non-empty statements after the loop end.
			if (skipEmptyStmts(curr->getSuccessor())) {
				return false;
			}
			foundEnd = true;
		}
...
/**
* @brief Returns the continuation condition for the do-while equivalent of
*        @a stmt.
*
* The exit condition of the inner @c if statement is logically negated so it
* expresses "keep looping" rather than "exit now".
*
* Returns the null pointer if @a stmt is not a do-while pattern.
*/
ShPtr<Expression> getDoWhileCondition(ShPtr<WhileLoopStmt> stmt) {
	if (!isDoWhileLoop(stmt)) {
		return {};
	}

	// Walk the body and collect the last loop-end's exit condition.
	ShPtr<Expression> exitCond;
	auto curr = stmt->getBody();
	while (curr) {
		if (isLoopEnd(curr)) {
			exitCond = getExitCondition(curr);
		}
		curr = curr->getSuccessor();
	}
	if (!exitCond) {
	...
	// Negate: the loop continues while the exit condition is false.
	return ExpressionNegater::negate(exitCond);
}
