...
/// Returns the `llvm::fltSemantics` corresponding to a FloatType size in bits.
static const llvm::fltSemantics &floatSemanticsForBits(unsigned bits) {
	switch (bits) {
		...
		default: return llvm::APFloat::IEEEdouble();  // 64-bit default
	}
}
...
void CArithmExprEvaluator::resolveCast(ShPtr<ExtCastExpr> expr,
	...
	if (ShPtr<IntType> intType = cast<IntType>(expr->getType())) {
		...
		// Float extension: promote a ConstFloat to a wider float type.
		if (ShPtr<ConstFloat> constFloat = cast<ConstFloat>(constant)) {
			llvm::APFloat apf = constFloat->getValue();
			bool losesInfo = false;
			apf.convert(floatSemanticsForBits(floatType->getSize()),
			            llvm::APFloat::rmNearestTiesToEven, &losesInfo);
			constant = ConstFloat::create(apf);
		} else {
			canBeEvaluated = false;
		}
	} else {
...
void CArithmExprEvaluator::resolveCast(ShPtr<TruncCastExpr> expr,
	...
	if (ShPtr<IntType> intType = cast<IntType>(expr->getType())) {
		...
		// Float truncation: narrow a ConstFloat to a smaller float type.
		if (ShPtr<ConstFloat> constFloat = cast<ConstFloat>(constant)) {
			llvm::APFloat apf = constFloat->getValue();
			bool losesInfo = false;
			apf.convert(floatSemanticsForBits(floatType->getSize()),
