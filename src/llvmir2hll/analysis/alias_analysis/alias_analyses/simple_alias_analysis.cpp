...
const VarSet &SimpleAliasAnalysis::mayPointTo(ShPtr<Variable> var) const {
	...
	// Type-sensitive filtering: narrow the candidate set by pointer element type.
	static thread_local VarSet filtered;
	filtered = filterByPointerElementType(var, mayPointToVarsIter->second);
	return filtered;
}
