...
void buildSeparatedAddressedSets(
	ShPtr<Module> module, VarSet& outGlobal, std::map<ShPtr<Function>, VarSet>& outFuncLocal, VarSet& outEscaped)
{
	outGlobal.clear();
	outFuncLocal.clear();
	outEscaped.clear();
	...
	// Per-function locals.
	for (auto fi = module->func_definition_begin(); fi != module->func_definition_end(); ++fi)
	{
		auto fn = *fi;
 ...
 */
VarSet filterByPointerElementType(ShPtr<Variable> ptr, const VarSet& candidates)
{
	...
	if (auto intElem = cast<IntType>(elemTy))
	{
		...
		for (auto& v: candidates)
		{
			...
			{
				if (intVar->getSize() >= intElem->getSize()) result.insert(v);
			}
		}
		return result.empty() ? candidates : result;
	}
	...
	if (auto floatElem = cast<FloatType>(elemTy))
	{
		...
		for (auto& v: candidates)
		{
			...
			{
				if (floatVar->getSize() == floatElem->getSize()) result.insert(v);
			}
		}
		return result.empty() ? candidates : result;
	}
...
public:
	...
	/// How many ifs, loops and switches enclose the statement being visited.
	/// An assignment inside one of them runs on some paths and not others, so
	/// it cannot establish what the pointer ALWAYS points to -- which is what
	/// pointsTo's contract promises and what ValueAnalysis relies on when it
	/// records a dereference as a MUST access.
	unsigned depth = 0;

	explicit SingleAssignFinder(ShPtr<Variable> p): OrderedAllVisitor(true, true), ptrVar(p) {}
	...
	/// Visit @a stmt's nested statements with the depth raised and its
	/// successor with the depth restored.  OrderedAllVisitor::visit walks the
	/// successor too, and the successor is not inside the construct.
	template <typename T>
	void descend(ShPtr<T> stmt, const std::function<void()>& visitNested)
	{
		++depth;
		visitNested();
		--depth;
	...
	{
		descend(stmt, [&] { visitStmt(stmt->getBody()); });
	}
	...
	void visit(ShPtr<AssignStmt> stmt) override
	{
		if (auto lhs = cast<Variable>(stmt->getLhs()))
		{
			if (lhs == ptrVar)
			{
				if (depth > 0)
				{
					// Runs on some paths and not others: invalidate, the same
					// way a non-address assignment does.
					count += 100;
				}
				else if (auto addr = cast<AddressOpExpr>(stmt->getRhs()))
				{
	...
	void visit(ShPtr<VarDefStmt> stmt) override
	{
		if (stmt->getVar() == ptrVar)
		{
			if (auto init = stmt->getInitializer())
			{
				...
				else if (auto addr = cast<AddressOpExpr>(init))
				{
					if (auto rhs = cast<Variable>(addr->getOperand()))
					{
						++count;
 ...
 */
ShPtr<Variable> singleAssignPointsTo(ShPtr<Variable> ptrVar, ShPtr<Function> fn)
{
