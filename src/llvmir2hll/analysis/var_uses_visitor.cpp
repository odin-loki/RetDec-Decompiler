...
ShPtr<VarUses> VarUsesVisitor::getUses(ShPtr<Variable> var,
	...
	if (cachingEnabled) {
		// If this function has not been precomputed yet, do a full single-pass
		// precompute for it now.  This is O(statements) per function and
		// amortises to O(1) per subsequent getUses() call for the same
		// function.  It is much cheaper than the naive per-variable lazy path
		// (O(N*statements) for N variables in the function) while avoiding
		// the expensive whole-module precompute at construction time.
		if (precomputedFunctions.find(func.get()) == precomputedFunctions.end()) {
			precomputeFunction(func);
		}
		...
		// Variable was not found during precompute (e.g. a newly added
		// variable).  Fall through to compute it lazily.
	}

	// Compute the uses (lazy path for uncached/non-cacheable case).
...
/**
* @brief Pre-computes uses of all variables in a single function.
*
* Similar to precomputeEverything() but scoped to one function so it can be
* called lazily on first access.  After this call, cache[func] contains an
* entry for every variable used in the function (plus empty entries for all
* global variables and function parameters, matching precomputeEverything()).
*
* @par Preconditions
*  - @a funcToCompute is non-null
*/
void VarUsesVisitor::precomputeFunction(ShPtr<Function> funcToCompute) {
	PRECONDITION_NON_NULL(funcToCompute);

	// Mark as precomputed BEFORE traversal to handle re-entrant calls.
	precomputedFunctions.insert(funcToCompute.get());

	precomputing = true;
	func = funcToCompute;

	// Initialise empty entries for all global variables so that getUses() for
	// an unused global returns an empty VarUses rather than triggering another
	// lazy traversal.
	// NOTE: We can't access the module here, so we skip global-var sentinel
	// initialisation.  The cache will simply be populated on demand for any
	// global variable that IS used in the function; unused globals will
	// trigger a single lazy traversal each (which is a minor overhead but
	// far cheaper than a whole-module pass).

	// Initialise empty entries for function parameters.
	for (const auto &param : funcToCompute->getParams()) {
		if (!cache[funcToCompute][param]) {
			cache[funcToCompute][param] = ShPtr<VarUses>(
				new VarUses(param, funcToCompute));
		}
	}

	restart();
	visitStmt(funcToCompute->getBody());
