...
CHLLWriter::CHLLWriter(llvm::raw_ostream& o, const std::string& outputFormat):
	HLLWriter(o, outputFormat), unnamedStructCounter(0), emittingGlobalVarDefs(false),
 ...
 */
ShPtr<HLLWriter> CHLLWriter::create(llvm::raw_ostream& out, const std::string& outputFormat)
{
...
bool CHLLWriter::emitFileHeader()
{
	...
	for (auto i = usedTypes->int_begin(), e = usedTypes->int_end(); i != e; ++i)
	{
	...
	{
		out->typedefLine(
			getCurrentIndent(), "int" + std::to_string(p.second) + "_t", "int" + std::to_string(p.first) + "_t");
	}
	// - unsigned
	for (const auto& p: emitUnsignedInt)
	{
		out->typedefLine(
			getCurrentIndent(), "uint" + std::to_string(p.second) + "_t", "uint" + std::to_string(p.first) + "_t");
	}
	...
	for (const auto& type: usedTypes->getFloatTypes())
	{
		...
		{
			FAIL("unsupported floating-point type of size " << floatType->getSize());
		}
		emitTypedefs = true;
	}
	// Emit them.
	...
	StructTypeVector usedStructTypes(StructTypesSorter::sort(usedTypes->getStructTypes()));
	// Make sure all structures have a name.
	for (const auto& type: usedStructTypes)
	{
		std::string structName(type->hasName() ? type->getName() : genNameForUnnamedStruct(usedStructTypes));
...
/// Returns a C character literal string (e.g. "'\\n'") for the given integer
/// value, or an empty string if the value has no commonly-used char literal.
static std::string toCharLiteralIfKnown(uint64_t v)
{
	switch (v)
	{
	case '\0': return "'\\0'";
...
/// Returns true when @a funcName is a well-known C function whose first
/// argument is a character (int-typed by convention, but semantically char).
static bool isCharArgFunction(const std::string& funcName)
{
	static const std::set<std::string> charFuncs = {
		"putchar",
		"putchar_unlocked",
		"fputc",
		"putc",
	...
	return charFuncs.count(funcName) > 0;
}
...
void CHLLWriter::visit(ShPtr<CallExpr> expr)
{
	...
	for (const auto& arg: args)
	{
		...
		if (charFirst)
		{
			// Try char literal for the first argument.
			charFirst = false;
			if (ShPtr<ConstInt> ci = cast<ConstInt>(arg))
			{
				if (!ci->isNegative())
				{
					std::string lit = toCharLiteralIfKnown(ci->getValue().getZExtValue());
					if (!lit.empty())
					{
						out->constantInt(lit);
						continue;
...
void CHLLWriter::visit(ShPtr<TruncCastExpr> expr)
{
	...
	if (isa<IntType>(expr->getType()) && (cast<IntType>(expr->getType())->isBool()))
	{
		out->punctuation('(');
...
void CHLLWriter::visit(ShPtr<ConstFloat> constant)
{
	...
	{
		out->constantFloat(constant->toMostReadableString() + getConstFloatSuffixIfNeeded(constant));
	}
}
...
{
	CompoundOpManager::CompoundOp compoundOp(compoundOpManager->tryOptimizeToCompoundOp(lhs, rhs));
	lhs->accept(this);
...
{
	// Emit a comment flagging the surviving goto so the reader understands
	// this is a decompiler artefact — the original control flow could not
	// be expressed as structured code at this point.
	out->space(getCurrentIndent());
	out->commentLine("decompiler: unstructured control flow");
	out->space(getCurrentIndent());
	out->keyword("goto");
	out->space();
...
{
	// The IR has proven this block unreachable; emit nothing so it does
	// not clutter the C output. The information is already implicit in
...
void CHLLWriter::visit(ShPtr<IntType> type)
{
	...
	if (type->getSize() == 8)
	{
		type->isUnsigned() ? out->dataType("unsigned char") : out->dataType("char");
		return;
	}

	out->dataType(
		(type->isUnsigned() ? std::string("u") : std::string("")) + "int" + std::to_string(type->getSize()) + "_t");
}
...
bool CHLLWriter::emitStandardFunctionPrototypes()
{
	...
	FuncSet funcsToEmit(module->func_definition_begin(), module->func_definition_end());
...
bool CHLLWriter::emitFunctionPrototypesForNonLibraryFuncs()
{
	...
	for (auto i = module->func_declaration_begin(), e = module->func_declaration_end(); i != e; ++i)
	{
		...
		{
			out->commentLine(
				"The following linked functions do not have "
...
bool CHLLWriter::emitFunctionPrototype(ShPtr<Function> func)
{
	...
	// not needed.
	if (module->isMainFunc(func) && !IndirectFuncRefAnalysis::isIndirectlyReferenced(module, func))
	{
		return false;
	}
...
void CHLLWriter::emitFunctionDefinition(ShPtr<Function> func)
{
	...
	// Category 9.3: provenance — optional address range when debug comments on.
	if (optionEmitDebugComments)
	{
		auto start = func->getStartAddress();
		auto end = func->getEndAddress();
		if (start.isDefined() && end.isDefined())
		{
			std::ostringstream oss;
			oss << "recovered range: " << func->getName() << " @ " << start.toHexPrefixString() << " .. "
				<< end.toHexPrefixString();
			emitDebugComment(oss.str(), true);
		}
	}
 ...
 */
void CHLLWriter::emitPointerToFunc(ShPtr<PointerType> pointerToFuncType, ShPtr<Variable> var)
{
	ShPtr<FunctionType> funcType(getFuncTypeFromPointerToFunc(pointerToFuncType));
	PRECONDITION(funcType, "pointerToFuncType is expected to be a pointer to a function");
 ...
 */
void CHLLWriter::emitArrayOfFuncPointers(ShPtr<ArrayType> arrayType, ShPtr<Variable> var)
{
	PRECONDITION(isArrayOfFuncPointers(arrayType), "arrayType is expected to be an array of function pointers");

	ShPtr<PointerType> ptrToFuncType(ucast<PointerType>(arrayType->getContainedType()));
 ...
 */
void CHLLWriter::emitPointerToArray(ShPtr<PointerType> pointerToArrayType, ShPtr<Variable> var)
{
	ShPtr<ArrayType> arrayType(getArrayTypeFromPointerToArray(pointerToArrayType));
	PRECONDITION(arrayType, "pointerToArrayType is expected to be a pointer to an array");
...
{
	for (auto i = funcType->param_begin(), e = funcType->param_end(); i != e; ++i)
	{
 ...
 */
void CHLLWriter::emitStructDeclaration(ShPtr<StructType> structType, bool emitInline)
{
	if (!emitInline)
	{
		out->space(getCurrentIndent());
