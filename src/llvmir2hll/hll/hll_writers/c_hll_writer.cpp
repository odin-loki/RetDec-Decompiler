...
static bool isCharArgFunction(const std::string &funcName) {
	static const std::set<std::string> charFuncs = {
		"putchar", "putchar_unlocked", "fputc", "putc",
		"ungetc",
	};
	...
	for (const auto &arg : args) {
		...
		if (charFirst) {
			...
			if (ShPtr<ConstInt> ci = cast<ConstInt>(arg)) {
				if (!ci->isNegative()) {
					std::string lit = toCharLiteralIfKnown(
					    ci->getValue().getZExtValue());
					if (!lit.empty()) {
						out->constantInt(lit);
						continue;
					}
...
void CHLLWriter::emitFunctionDefinition(ShPtr<Function> func) {
	...
	if (optionEmitDebugComments) {
		...
		if (start.isDefined() && end.isDefined()) {
			...
			oss << "recovered range: " << func->getName() << " @ "
				<< start.toHexPrefixString() << " .. " << end.toHexPrefixString();
