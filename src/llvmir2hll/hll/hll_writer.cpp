...
public:
	...
	*/
	bool operator()(ShPtr<Function> f1, ShPtr<Function> f2) {
		...
		if (f1LineRange.first == f2LineRange.first) {
...
public:
	...
	*/
	bool operator()(ShPtr<Function> f1, ShPtr<Function> f2) {
		...
		if (f1AddressRange.getStart() == f2AddressRange.getStart()) {
			return f1->getName() < f2->getName();
		}
...
bool HLLWriter::emitFileHeader() {
	out->commentLine("");
	out->commentLine("This file was generated by RetDec");
	out->commentLine("Copyright (c) Odin Loch Trading as Imortek");
	out->commentLine("");
...
void HLLWriter::sortFuncsForEmission(FuncVector &funcs) {
	...
	// Category 9.4: deterministic emission when line/address ordering unavailable.
	sortByName(funcs);
}
