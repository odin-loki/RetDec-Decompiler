...
*
* Implemented iteratively to avoid stack overflows on deeply-nested loops and
* to detect parent-chain cycles that can arise from IR corruption.
*/
ShPtr<Statement> findParentWhichIsNotLoopIfStmtIsInLoop(ShPtr<Statement> stmt) {
	std::unordered_set<Statement*> seen;
	seen.insert(stmt.get());
	ShPtr<Statement> cur = stmt;
	while (true) {
		ShPtr<Statement> parent(cur->getParent());
		if (!parent || !isLoop(parent)) {
			return cur;
		}
		// Parent-chain cycle detection.
		if (!seen.insert(parent.get()).second) {
			std::cerr << "[NonRecursiveCFGBuilder] WARNING: parent chain"
...
void NonRecursiveCFGBuilder::doJobs() {
	// Safety limit: a well-formed function cannot require more jobs than
	// a large multiple of its statement count.  If exceeded it means a bug
	// in the IR (e.g. an optimizer created a cyclic successor chain) caused
	// exponential job growth.  Cap at 2M to avoid infinite loops.
	static constexpr std::size_t kMaxJobs = 2000000;
	std::size_t jobCount = 0;
	while (!jobQueue.empty()) {
		if (++jobCount > kMaxJobs) {
			std::cerr << "[NonRecursiveCFGBuilder] WARNING: job limit exceeded"
				" (" << kMaxJobs << "), aborting CFG build early\n";
			break;
		}
...
void NonRecursiveCFGBuilder::doJob(const Job &job) {
	...
	// Track statements seen in this traversal to detect successor-chain cycles
	// that can appear in IR corrupted by certain optimizer transforms.
	std::unordered_set<Statement*> seenInJob;
	...
	for (ShPtr<Statement> stmt = job.stmt; stmt; stmt = stmt->getSuccessor()) {
		// Cycle detection: if we encounter the same statement pointer twice in
		// this linear successor walk, the IR has a corrupted successor chain.
		// Break out gracefully rather than looping forever.
		if (!seenInJob.insert(stmt.get()).second) {
			std::cerr << "[NonRecursiveCFGBuilder] WARNING: successor chain"
				" cycle detected at `" << stmt << "` in func `"
				<< (func ? func->getName() : "?") << "` -> breaking\n";
			break;
		}
