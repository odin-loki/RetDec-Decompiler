...
private:
	...
	bool independentOrder(const ShPtr<GlobalVarDef> &p1,
		...
		return p1->getVar()->getInitialName() <
			p2->getVar()->getInitialName();
	}
	...
	GlobalVarDefVector sortImpl(const GlobalVarDefVector &globalVars) {
		...
		std::stable_sort(base.begin(), base.end(),
			[this](const ShPtr<GlobalVarDef> &p1, const ShPtr<GlobalVarDef> &p2) {
		...
		while (!notEmitted.empty()) {
			...
			for (auto dep : dependents[next]) {
				if (waitingOn[dep] > 0 && --waitingOn[dep] == 0
						&& notEmitted.count(dep)) {
					ready.insert(dep);
				}
			}
		}
