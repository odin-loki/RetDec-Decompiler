...
/**
* @brief Constructs data from @a data (copy constructor).
*
* Both `this` and `data` reference the same external `APICallInfoSeq`.
* The pattern is duplicated so the two objects accumulate statements
* independently.
*/
APICallSeqData::APICallSeqData(const APICallSeqData &data):
	...
	pattern(StmtsPattern::create()) {
	// Deep-copy the accumulated statements into the new pattern.
	for (auto i = data.pattern->stmt_begin(), e = data.pattern->stmt_end();
	     i != e; ++i) {
		pattern->addStmt(*i);
	}
}
...
*
* Requires that both objects reference the same underlying `APICallInfoSeq`
* (the `allInfos` member is a reference and cannot be rebound).
*/
APICallSeqData &APICallSeqData::operator=(const APICallSeqData &data) {
	if (this == &data)
		return *this;

	PRECONDITION(&allInfos == &data.allInfos,
		"operator= requires both objects to reference the same APICallInfoSeq");

	currInfo     = data.currInfo;
	currInfoIter = data.currInfoIter;
	boundValues  = data.boundValues;

	// Rebuild the pattern from data.pattern's statements.
	pattern = StmtsPattern::create();
	for (auto i = data.pattern->stmt_begin(), e = data.pattern->stmt_end();
	     i != e; ++i) {
		pattern->addStmt(*i);
