...
bool llvmIr2HllDiagEnabled()
{
	const char *e = std::getenv("RETDEC_LLVMIR2HLL_DIAG");
	return e != nullptr && e[0] != '\0' && std::strcmp(e, "0") != 0;
}

/**
 * Wall-clock milliseconds between consecutive @c mark() calls (P4.5 tooling).
 */
class LlvmIr2HllPhaseTimer
{
	public:
		explicit LlvmIr2HllPhaseTimer(bool on)
				: _on(on), _last(std::chrono::steady_clock::now())
		{
		}
		...
		void mark(const char *phase)
		{
			...
			const auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(
					now - _last).count();
			_last = now;
			Log::info() << "[llvmir2hll-diag] phase_ms " << phase << "=" << ms
					<< std::endl;
		}
				...
				std::chrono::steady_clock::time_point start) const
		{
			...
			const auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(
					now - start).count();
			Log::info() << "[llvmir2hll-diag] " << label << "=" << ms << std::endl;
		}
...
/**
 * One-line snapshot of LLVM IR shape before llvmir2hll mutates the module (P4.5 / tooling).
 */
void logLlvmInputIrShapeDiag(const llvm::Module &m)
{
	if (!llvmIr2HllDiagEnabled())
	{
		return;
	}
	std::size_t fnDecl = 0;
	...
	Log::info() << "[llvmir2hll-diag] input_llvm module='" << m.getModuleIdentifier()
			<< "' fn_defined=" << fnDef << " fn_declaration=" << fnDecl
			<< " globals=" << globals << " indirectbr_insts=" << indirectBr
			<< std::endl;
}
...
{
	logLlvmInputIrShapeDiag(m);

	LlvmIr2HllPhaseTimer diagTimer(llvmIr2HllDiagEnabled());
	const auto llvmir2hllWallStart = std::chrono::steady_clock::now();

	const auto formatDuration = [](std::chrono::seconds s) -> std::string
	{
		auto total = s.count();
		const auto h = total / 3600;
		total %= 3600;
		const auto min = total / 60;
	...
	const auto shouldReportProgress = [&]() -> bool
	{
		return globalConfig && globalConfig->parameters.isVerboseOutput();
	};

	const bool removeLibraryFuncsEnabled = !globalConfig->parameters.isBackendKeepLibraryFuncs();
	const bool optimizationsEnabled = !globalConfig->parameters.isBackendNoOpts();
	const bool varRenamingEnabled = !globalConfig->parameters.isBackendNoVarRenaming();
	const bool symbolicNamesEnabled = !globalConfig->parameters.isBackendNoSymbolicNames();
	const bool validateModuleEnabled = ValidateModule;
	const bool findPatternsEnabled = !FindPatterns.empty();
	const bool emitCfgEnabled = globalConfig->parameters.isBackendEmitCfg();
	const bool emitCgEnabled = globalConfig->parameters.isBackendEmitCg();

	int totalSteps = 9; // Always executed phases.
	totalSteps += removeLibraryFuncsEnabled ? 1 : 0;
	totalSteps += optimizationsEnabled ? 2 : 0;
	totalSteps += varRenamingEnabled ? 1 : 0;
	totalSteps += symbolicNamesEnabled ? 1 : 0;
	totalSteps += validateModuleEnabled ? 1 : 0;
	totalSteps += findPatternsEnabled ? 1 : 0;
	totalSteps += emitCfgEnabled ? 1 : 0;
	totalSteps += emitCgEnabled ? 1 : 0;

	int completedSteps = 0;
	const auto decompilationStart = std::chrono::steady_clock::now();
	const auto reportProgress = [&](const std::string& phase)
	{
		Log::phase(phase);
		++completedSteps;
		...
		const auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(
				std::chrono::steady_clock::now() - decompilationStart
		);
		const int percent = (completedSteps * 100) / std::max(totalSteps, 1);
		const int barWidth = 24;
		const int filled = (percent * barWidth) / 100;
		std::string bar(static_cast<std::size_t>(filled), '#');
		bar += std::string(static_cast<std::size_t>(barWidth - filled), '-');

		std::chrono::seconds eta(0);
		if (completedSteps > 0 && completedSteps < totalSteps)
		{
			const auto etaSeconds = (elapsed.count() * (totalSteps - completedSteps)) / completedSteps;
			eta = std::chrono::seconds(etaSeconds);
		}
	...
	diagTimer.mark("initialize");

	reportProgress("conversion of LLVM IR into BIR");
	decompilationShouldContinue = convertLLVMIRToBIR();
	if (!decompilationShouldContinue)
	{
		return false;
	}
	diagTimer.mark("convert_llvm_ir_to_bir");

	if (!globalConfig->parameters.isBackendKeepLibraryFuncs())
	{
		reportProgress("removing functions from standard libraries");
		removeLibraryFuncs();
	}
	diagTimer.mark("remove_stdlib_funcs");
	...
	reportProgress("removing code that is not reachable in a CFG");
	removeCodeUnreachableInCFG();
	diagTimer.mark("remove_unreachable_cfg");

	reportProgress("signed/unsigned types fixing");
	fixSignedUnsignedTypes();
	diagTimer.mark("fix_signed_unsigned_types");

	reportProgress("converting LLVM intrinsic functions to standard functions");
	convertLLVMIntrinsicFunctions();
	diagTimer.mark("convert_llvm_intrinsics");

	if (resModule->isDebugInfoAvailable())
	{
		// Debug information is discovered after IR conversion, so account for
		// this optional phase dynamically.
		++totalSteps;
		reportProgress("obtaining debug information");
		obtainDebugInfo();
	}
	diagTimer.mark("debug_info");

	if (!globalConfig->parameters.isBackendNoOpts())
	{
		reportProgress("alias analysis [" + aliasAnalysis->getId() + "]");
		initAliasAnalysis();

		reportProgress("optimizations");
		runOptimizations();
	}
	diagTimer.mark("alias_analysis_and_opts");

	if (!globalConfig->parameters.isBackendNoVarRenaming())
	{
		reportProgress("variable renaming [" + varRenamer->getId() + "]");
		renameVariables();
	}
	diagTimer.mark("rename_variables");

	if (!globalConfig->parameters.isBackendNoSymbolicNames())
	{
		reportProgress("converting constants to symbolic names");
		convertConstantsToSymbolicNames();
	}
	diagTimer.mark("symbolic_constant_names");

	if (ValidateModule)
	{
		reportProgress("module validation");
		validateResultingModule();
	}
	diagTimer.mark("validate_module");

	if (!FindPatterns.empty())
	{
		reportProgress("finding patterns");
		findPatterns();
	}
	diagTimer.mark("find_patterns");

	if (globalConfig->parameters.isBackendEmitCfg())
	{
		reportProgress("emission of control-flow graphs");
		emitCFGs();
	}
	diagTimer.mark("emit_cfg");

	if (globalConfig->parameters.isBackendEmitCg())
	{
		reportProgress("emission of a call graph");
		emitCG();
	}
	diagTimer.mark("emit_cg");

	reportProgress("emission of the target code [" + hllWriter->getId() + "]");
	emitTargetHLLCode();
	diagTimer.mark("emit_target_hll");

	if (llvmIr2HllDiagEnabled() && resModule)
	{
		std::size_t birFuncs = 0;
		...
		Log::info() << "[llvmir2hll-diag] after_hll_emit bir_func_definitions=" << birFuncs
				<< std::endl;
	}

	reportProgress("finalization");
	finalize();
	diagTimer.mark("finalize");

	reportProgress("cleanup");
	cleanup();
	diagTimer.mark("cleanup");

	diagTimer.wallTotalMs("wall_clock_total_ms", llvmir2hllWallStart);
...
void LlvmIr2Hll::finalize()
{
	...
	if (outFile)
	{
		// Some MinGW/Windows runs may leave the underlying file descriptor in an
		// error state after code emission. Preserve the generated output file and
		// clear the stream error to avoid fatal termination in ToolOutputFile's
		// destructor.
		outFile->os().flush();
