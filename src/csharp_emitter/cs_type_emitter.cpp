...
CsTypeEmitter::CsTypeEmitter(CsWriter& writer, const CsExprEmitter& expr, const CsStmtEmitter& stmt, Options opts):
...
static std::string csharpAttrName(const std::string& typeName)
{
	...
	if (sep != std::string::npos) n = n.substr(sep + 1);
...
static std::string csharpAttrLit(const BcAnnotationValue& val)
{
	...
	case BcAnnotationValue::Kind::Type: return val.stringValue;
...
static std::string csharpAttrSuffix(const BcAnnotation& ann)
{
	...
	if (itVal == ann.elements.end()) itVal = ann.elements.find("value");
	bool first = true;
...
static void emitCsharpAttributes(CsWriter& w, const std::vector<BcAnnotation>& anns)
{
...
{
	if (cls.methods.size() == 1 && cls.methods[0].name == "Invoke") return true;
...
{
	return m.name.size() > 4 && (m.name.substr(0, 4) == "get_" || m.name.substr(0, 4) == "set_");
}

bool CsTypeEmitter::isEventAccessor(const BcMethod& m) const
{
	return m.name.size() > 4
		&& (m.name.substr(0, 4) == "add_" || m.name.size() > 7 && m.name.substr(0, 7) == "remove_");
}
...
std::vector<CsTypeEmitter::PropertyGroup> CsTypeEmitter::collectProperties(const BcClass& cls) const
{
	...
	for (const auto& m: cls.methods)
	{
		...
		if (it == nameToIdx.end())
		{
			...
			bool isStatic = (static_cast<uint32_t>(m.access) & static_cast<uint32_t>(BcAccess::Static)) != 0;
			g.isStatic = isStatic;
		...
		if (isGetter)
		{
			...
			if (m.descriptor.returnType) groups[idx].type = *m.descriptor.returnType;
		}
		else
		{
...
std::vector<CsTypeEmitter::EventGroup> CsTypeEmitter::collectEvents(const BcClass& cls) const
{
	...
	for (const auto& m: cls.methods)
	{
		...
		BcMethod* mptr = const_cast<BcMethod*>(&m);
		if (isAdder)
			groups[idx].adder = mptr;
		else
			groups[idx].remover = mptr;
		...
		if (groups[idx].type.isVoid() && !m.descriptor.params.empty() && m.descriptor.params.front())
		{
...
void CsTypeEmitter::emitTypeModifiers(const BcClass& cls)
{
	...
	if (opts_.emitPartialForSMs && (cls.name.find("d__") != std::string::npos))
	{
...
void CsTypeEmitter::emitTypeKeyword(const BcClass& cls)
{
	...
	else if (cls.isRecord)
	{
		// C# 9+ record
		using A = BcAccess;
		bool isValue = (static_cast<uint32_t>(cls.access) & static_cast<uint32_t>(A::Sealed)) != 0 && !cls.isAbstract;
		if (isValue)
			writer_.write("record struct ");
		else
...
void CsTypeEmitter::emitGenericParams(const std::vector<std::string>& typeParams)
{
	...
	for (size_t i = 0; i < typeParams.size(); ++i)
	{
		...
		if (pos == std::string::npos) pos = tp.find(" : ");
...
void CsTypeEmitter::emitBaseList(const BcClass& cls)
{
	...
	{
		std::string sc = cls.superClass->toString();
		if (sc != "System.Object" && sc != "object" && sc != "System.ValueType" && sc != "System.Enum"
...
void CsTypeEmitter::emitGenericConstraints(const std::vector<std::string>& typeParams)
{
	for (const auto& tp: typeParams)
	{
		...
		if (name.empty() || bound.empty()) continue;
		writer_.write(" where ");
...
void CsTypeEmitter::emitTypeHeader(const BcClass& cls)
{
	if (isDelegate(cls) && !cls.methods.empty())
	{
		...
		for (size_t i = 0; i < invoke.descriptor.params.size(); ++i)
		{
			...
			writer_.write(" ");
			std::string pname = (i < invoke.paramNames.size()) ? invoke.paramNames[i] : "arg" + std::to_string(i);
...
void CsTypeEmitter::emitField(const BcField& f)
{
	...
	bool isConst = has(A::Static) && has(A::Final)
				&& (f.constantIntValue.has_value() || f.constantFltValue.has_value() || f.constantStrValue.has_value());
	...
	if (isConst)
	{
		// Emit as const
		std::string val;
		if (f.constantIntValue)
			val = std::to_string(*f.constantIntValue);
		else if (f.constantFltValue)
			val = std::to_string(*f.constantFltValue);
		else if (f.constantStrValue)
			val = writer_.stringLiteral(*f.constantStrValue);
...
void CsTypeEmitter::emitEnum(const BcClass& cls)
{
	...
	emitTypeHeader(cls);
	{
		...
		for (size_t i = 0; i < cls.enumConstants.size(); ++i)
		{
			...
			{
				writer_.line(
					CsWriter::safeName(cls.enumConstants[i]) + " = " + std::to_string(*fld->constantIntValue) + comma);
			}
			else
			{
...
void CsTypeEmitter::emitAutoProperty(
	...
	const std::string& modifiers)
{
	...
	writer_.line(modifiers + " " + staticMod + typeName + " " + CsWriter::safeName(propName) + " { " + accessors + "}");
}
...
{
	std::string typeName = expr_.emitType(propType);
	bool isStatic = getter && (static_cast<uint32_t>(getter->access) & static_cast<uint32_t>(BcAccess::Static)) != 0;
	std::string staticMod = isStatic ? "static " : "";
...
void CsTypeEmitter::emitProperties(
	const BcClass& cls, const std::unordered_map<std::string, CilReconstructResult>& results)
{
	...
	{
		bool isAutoProperty = opts_.preferAutoProperties && pg.getter && pg.setter && pg.getter->cfg.blockCount() <= 1
						   && pg.setter->cfg.blockCount() <= 1;

		if (isAutoProperty)
		{
			emitAutoProperty(pg.name, pg.type, !!pg.getter, !!pg.setter, pg.isStatic, pg.modifiers);
		}
		else
		{
...
void CsTypeEmitter::emitEvents(const BcClass& cls, const std::unordered_map<std::string, CilReconstructResult>&)
{
	...
	for (const auto& eg: evs)
	{
		...
		writer_.line("public " + staticMod + "event " + typeName + " " + CsWriter::safeName(eg.name) + ";");
	}
...
void CsTypeEmitter::emitMethodSignature(const BcClass& cls, const BcMethod& m)
{
	...
	if (isOperator(m))
	{
		...
		if (it != opNames.end())
		{
			std::string retType = m.descriptor.returnType ? expr_.emitType(*m.descriptor.returnType) : "void";
	...
	for (size_t i = 0; i < m.descriptor.params.size(); ++i)
	{
		...
		if (i < m.paramAnnotations.size())
		{
			...
			{
				if (ann.typeName.empty()) continue;
				writer_.write("[" + csharpAttrName(ann.typeName) + csharpAttrSuffix(ann) + "] ");
			}
		}
		const auto& pt = *m.descriptor.params[i];
		writer_.write(expr_.emitType(pt));
		writer_.write(" ");
		std::string pname = (i < m.paramNames.size()) ? m.paramNames[i] : "arg" + std::to_string(i);
		writer_.write(CsWriter::safeName(pname));
	}
	writer_.write(")");
}
...
void CsTypeEmitter::emitClass(
	const BcClass& cls, const std::unordered_map<std::string, CilReconstructResult>& results, const BcModule& module)
{
	(void)module;
