...
CsStmtEmitter::CsStmtEmitter(CsWriter& writer, const CsExprEmitter& expr, Options opts):
...
std::string CsStmtEmitter::declType(const BcType& t, const CilExprPtr& rhs) const
{
	if (opts_.csharpVersion >= 3)
	{
		...
		if (rhs)
		{
			bool obvious = rhs->isNewobj() || rhs->isNewarr() || rhs->isConst() || rhs->isLocal() || rhs->isArg();
...
void CsStmtEmitter::emitExprStmt(const CilStmt& s)
{
	...
	if (!text.empty() && text != "null") writer_.line(text + ";");
}
...
void CsStmtEmitter::emitLeave(const CilStmt& s)
{
	...
	if (!s.labelName.empty()) writer_.line("goto " + s.labelName + ";");
}
...
void CsStmtEmitter::emitCatch(const CilStmt::CatchClause& cc)
{
	if (cc.isFilter && cc.filterExpr)
	{
		...
		if (!cc.varName.empty())
		{
			writer_.line("catch (Exception " + CsWriter::safeName(cc.varName) + ") when (" + filter + ")");
		}
		else
		{
	...
	else
	{
		...
		if (typeName == "object" || typeName == "System.Object" || typeName.empty()) typeName = "Exception";
		if (!cc.varName.empty())
		{
		...
		for (const auto& c: s.cases)
		{
			...
			}
			{
				...
				bool needsBreak = c.body.empty()
							   || (c.body.back().kind != StmtKind::Return && c.body.back().kind != StmtKind::Goto
								   && c.body.back().kind != StmtKind::Throw && c.body.back().kind != StmtKind::Rethrow);
