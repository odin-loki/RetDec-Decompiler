...
//		case Type::X86_FP80TyID:
		...
		case Type::PointerTyID:
			...
			{
				// Use the address of the LLVM Function object as a unique,
				// stable pointer value representing the function pointer.
				Result = PTOGV(const_cast<Function*>(F));
			}
			else if (const GlobalVariable *GV = dyn_cast<GlobalVariable>(C))
			{
				// Use the address of the LLVM GlobalVariable object as a unique,
				// stable pointer value representing the global's address.
...
bool LlvmIrEmulator::tryHandleExternalCall(
		llvm::Function* f,
		...
		llvm::GenericValue& result)
{
	...
	// VirtualAlloc(LPVOID, SIZE_T, DWORD, DWORD) -> returns allocated address
	if (baseName == "VirtualAlloc" || baseName == "VirtualAllocEx")
	{
		uint64_t size = args.size() >= 2 ? args[1].IntVal.getZExtValue() : 0;
		if (size == 0) size = 4096;
		// Align to page
	...
	// VirtualFree -> return 1 (success)
	if (baseName == "VirtualFree")
	{
		result.IntVal = APInt(f->getReturnType()->getIntegerBitWidth(), 1);
		return true;
	}

	// malloc(size) -> returns allocated address
	if (baseName == "malloc" || baseName == "_malloc" || baseName == "malloc_crt")
	{
	...
	// memcpy(dst, src, size) / RtlMoveMemory(dst, src, size) - copy and return dst
	if (baseName == "memcpy" || baseName == "RtlMoveMemory" || baseName == "_memcpy"
			|| baseName == "memmove")
	{
		if (args.size() >= 3)
		{
			uint64_t dst = args[0].IntVal.getZExtValue();
			uint64_t src = args[1].IntVal.getZExtValue();
			uint64_t size = args[2].IntVal.getZExtValue();
			// Copy 8-byte chunks
			for (uint64_t i = 0; i + 8 <= size; i += 8)
			{
				auto v = _globalEc.getMemory(src + i, false);
				_globalEc.setMemory(dst + i, v, false);
			}
			if (size % 8 != 0)
			{
				uint64_t a = size & ~uint64_t(7);
				auto v = _globalEc.getMemory(src + a, false);
				_globalEc.setMemory(dst + a, v, false);
			}
		}
		if (baseName == "RtlMoveMemory")
			result.IntVal = APInt(f->getReturnType()->getIntegerBitWidth(), 0);
		else
			result.IntVal = args.size() >= 1 ? args[0].IntVal : APInt(64, 0);
		return true;
	}

	// memset(dst, val, size) -> return dst
	if (baseName == "memset" || baseName == "_memset")
	{
		if (args.size() >= 3)
		{
			uint64_t dst = args[0].IntVal.getZExtValue();
			uint8_t v = static_cast<uint8_t>(args[1].IntVal.getZExtValue());
			uint64_t size = args[2].IntVal.getZExtValue();
			uint64_t pattern = 0;
			for (unsigned i = 0; i < 8; ++i)
				pattern |= static_cast<uint64_t>(v) << (i * 8);
			GenericValue gv;
			gv.IntVal = APInt(64, pattern);
			for (uint64_t i = 0; i + 8 <= size; i += 8)
				_globalEc.setMemory(dst + i, gv, false);
			if (size % 8 != 0)
				_globalEc.setMemory(dst + (size & ~7ULL), gv, false);
		}
		result.IntVal = args.size() >= 1 ? args[0].IntVal : APInt(64, 0);
		return true;
	}

	// Nop returns (void or 0)
	if (baseName == "Sleep" || baseName == "NtDelayExecution" || baseName == "GetTickCount")
	{
		result.IntVal = APInt(f->getReturnType()->getIntegerBitWidth(), 0);
		return true;
	}
...
void LlvmIrEmulator::visitCallInst(llvm::CallInst& I)
{
	...
	// Handle external (declaration) calls with stubs for packer APIs
	if (cf && cf->isDeclaration())
	{
		std::vector<GenericValue> callArgs;
		for (auto aIt = I.arg_begin(), eIt = I.arg_end(); aIt != eIt; ++aIt)
			callArgs.push_back(_globalEc.getOperandValue(*aIt, ec));

		GenericValue retVal;
		if (tryHandleExternalCall(cf, callArgs, retVal))
		{
			if (!I.getType()->isVoidTy())
			{
				...
				if (retTy->isIntegerTy())
				{
					...
					if (bw != retVal.IntVal.getBitWidth())
						retVal.IntVal = retVal.IntVal.zextOrTrunc(bw);
				}
				else if (retTy->isPointerTy())
				{
					retVal.PointerVal = reinterpret_cast<void*>(
							retVal.IntVal.getZExtValue());
				}
				_globalEc.setValue(&I, retVal);
			}
			return;
		}
...
void LlvmIrEmulator::visitInvokeInst(llvm::InvokeInst& I)
{
	...
	// Handle external (declaration) calls like CallInst - use stubs, then continue to normal dest
	if (cf && cf->isDeclaration())
	{
		std::vector<GenericValue> callArgs;
		for (auto aIt = I.arg_begin(), eIt = I.arg_end(); aIt != eIt; ++aIt)
		...
		switchToNewBasicBlock(I.getNormalDest(), ec, _globalEc);
		return;
	}

	// InvokeInst to a defined (non-external) function: treat like CallInst and
	...
	// Set a zero return value so the normal dest BB can read it if needed.
	if (!I.getType()->isVoidTy())
	{
		GenericValue defVal;
		if (I.getType()->isPointerTy())
...
{
	// VA arg access: return a zeroed value of the target type.
	// The emulator does not model va_list internals; this prevents crashes
	// while still allowing execution to continue past variadic calls.
	GenericValue dest;
	auto* ty = I.getType();
	if (ty->isIntegerTy())
...
{
	LocalExecutionContext& ec = _ecStack.back();
	// Start from the input vector value.
	GenericValue vec = _globalEc.getOperandValue(I.getOperand(0), ec);
...
void LlvmIrEmulator::visitShuffleVectorInst(llvm::ShuffleVectorInst& I)
{
	...
	for (unsigned i = 0; i < dstLen; ++i)
	{
		...
		if (mask < 0)
		{
			// Undef lane: zero-initialise.
			dest.AggregateVal[i] = GenericValue();
		}
		else if (static_cast<unsigned>(mask) < srcLen)
		{
			dest.AggregateVal[i] = v0.AggregateVal[mask];
		}
		else if (static_cast<unsigned>(mask) < 2 * srcLen)
		{
			dest.AggregateVal[i] = v1.AggregateVal[mask - srcLen];
		}
	}
	_globalEc.setValue(&I, dest);
}
...
{
	LocalExecutionContext& ec = _ecStack.back();
	// Start from the aggregate operand and splice in the new element.
	GenericValue agg = _globalEc.getOperandValue(I.getOperand(0), ec);
	GenericValue val = _globalEc.getOperandValue(I.getOperand(1), ec);
	// Walk the index path, expanding AggregateVal lazily if needed.
	GenericValue* cur = &agg;
	auto idxs = I.getIndices();
	for (unsigned d = 0; d < idxs.size(); ++d)
	{
		unsigned idx = idxs[d];
		if (cur->AggregateVal.size() <= idx)
			cur->AggregateVal.resize(idx + 1);
		if (d + 1 == idxs.size())
			cur->AggregateVal[idx] = val;
		else
			cur = &cur->AggregateVal[idx];
	}
	_globalEc.setValue(&I, agg);
}
