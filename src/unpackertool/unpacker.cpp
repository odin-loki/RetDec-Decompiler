...
bool detectPackers(const std::string& inputFile, std::vector<retdec::cpdetect::DetectResult>& detectedPackers)
{
	...
	// Only pass packers to unpackFile - compilers/linkers/installers have no
	// unpacker plugins and would emit spurious "No matching plugins found" errors.
	detectedPackers.clear();
	for (const auto& tool : toolInfo.detectedTools) {
		if (tool.isPacker())
			detectedPackers.push_back(tool);
	}
	return true;
}
...
ExitCode unpackFile(const std::string& inputFile, const std::string& outputFile, bool brute, const std::vector<retdec::cpdetect::DetectResult>& detectedPackers)
{
	...
	for (const auto& detectedPacker : detectedPackers)
	{
		...
		if (plugins.empty())
		{
			if (isHeuristicOnlyPackerLabel(detectedPacker.name))
			{
				Log::info() << "No static unpacker plugin for heuristic packer '"
						<< detectedPacker.name
						<< "'. Plugins target signature-known packers (UPX, MPRESS). "
						<< "Run retdec-decompiler on the packed file: it may apply "
						<< "emulation-based unpacking (Stage 3) when static unpack "
