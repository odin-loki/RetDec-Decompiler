...
static bool record_name_terminated(PDBGeneralSymbol* symbol, std::size_t nameOffset)
{
	...
	if (nameOffset >= total) return false;
...
{
	const std::size_t total = symbol_record_size(symbol);
	if (nameOffset >= total) return 0;
	const char* begin = reinterpret_cast<const char*>(symbol) + nameOffset;
	return static_cast<int>(retdec::utils::bstr::boundedLength(begin, total - nameOffset));
}
...
static PDBGeneralSymbol* symbol_at(char* data, std::size_t size, std::size_t position)
{
	...
	if (data == nullptr || !bounds::rangeFits(position, size, sizeof(PDBGeneralSymbol))) return nullptr;
	PDBGeneralSymbol* symbol = reinterpret_cast<PDBGeneralSymbol*>(data + position);
	if (symbol->size < PDB_MIN_SYMBOL_RECLEN || !bounds::rangeFits(position, size, symbol_record_size(symbol)))
		return nullptr;
	return symbol;
}
...
{
	namespace bounds = retdec::utils::bounds;
	if (data == nullptr || !bounds::rangeFits(position, size, sizeof(PDBBigSymbol))) return nullptr;
	PDBBigSymbol* symbol = reinterpret_cast<PDBBigSymbol*>(data + position);
	if (!bounds::rangeFits(position + sizeof(PDBBigSymbol), size, symbol->size)) return nullptr;
	return symbol;
}
...
static std::size_t symbol_struct_size(PDB_WORD type)
{
	...
	{
	case S_GPROC32:
	case S_LPROC32: return sizeof(PROCSYM32);
	case S_REGREL32: return sizeof(REGREL32);
	case S_BPREL32: return sizeof(BPRELSYM32);
	case S_BLOCK32: return sizeof(BLOCKSYM32);
	case S_REGISTER: return sizeof(REGSYM);
	case S_GDATA32:
	case S_LDATA32: return sizeof(DATASYM32);
	default:
...
void dump_local_variable(PDBLocalVariable& var)
{
	...
	case PDBLVLOC_REGREL32: {
		printf(
			"\t\tREGREL32 block: %d reg: %08x offset: %08x typind: %08x ",
			var.block,
			var.register_num,
			var.offset,
			var.type_index);
...
void PDBFunction::dump(void)
{
	...
	if (overload_index > 0) printf("\tFunction is overloaded. Index: %d\n", overload_index);
	printf("\tOffset : %08x\n", offset);
	printf("\tSection: %04x\n", section);
	printf("\tModule : %d\n", module_index);
...
bool PDBFunction::parse_symbol(PDBGeneralSymbol* symbol, PDBTypes* types, PDBSymbols* pdbsyms)
{
	...
	if (!record_holds(symbol, symbol_struct_size(symbol->type))) return false;

	switch (symbol->type)
	{
	case S_GPROC32:
	case S_LPROC32: { // Function definition
		PROCSYM32* func_sym = reinterpret_cast<PROCSYM32*>(symbol);
		if (!record_name_terminated(symbol, offsetof(PROCSYM32, name))) break;
		...
		if (type_def != nullptr)
		{
			...
			if (type_def->func_thistype_index) args_remain++; // This-parameter
			if (type_def->func_is_variadic) args_remain--;    // Variadic function
		}
	...
	case S_FRAMEPROC: { // Function stack frame
		break;
	}
	case S_REGREL32: { // Local variable register-relative
		REGREL32* lvar_sym = reinterpret_cast<REGREL32*>(symbol);
		if (!record_name_terminated(symbol, offsetof(REGREL32, name))) break;
		PDBLocalVariable new_var = {
			// Fill local variable structure
			reinterpret_cast<char*>(lvar_sym->name),
			PDBLVLOC_REGREL32,
			cur_block,
			lvar_sym->off,
			lvar_sym->reg,
			lvar_sym->typind,
			types->get_type_by_index(lvar_sym->typind),
		};
		if ((args_remain--) > 0)
	...
	case S_BPREL32: { // Local variable EBP-relative
		BPRELSYM32* lvar_sym = reinterpret_cast<BPRELSYM32*>(symbol);
		if (!record_name_terminated(symbol, offsetof(BPRELSYM32, name))) break;
		PDBLocalVariable new_var = {
			// Fill local variable structure
			reinterpret_cast<char*>(lvar_sym->name),
			PDBLVLOC_BPREL32,
	...
	case S_REGISTER: { // Local variable in register
		REGSYM* lvar_sym = reinterpret_cast<REGSYM*>(symbol);
		if (!record_name_terminated(symbol, offsetof(REGSYM, name))) break;
		PDBLocalVariable new_var = {
			// Fill local variable structure
			reinterpret_cast<char*>(lvar_sym->name),
			PDBLVLOC_REGISTER,
		...
		// case S_GDATA32:
	case S_LDATA32: { // Data inside function code
		DATASYM32* data_sym = reinterpret_cast<DATASYM32*>(symbol);
		if (!record_name_terminated(symbol, offsetof(DATASYM32, name))) break;
	...
	case S_END: { // Function or block end
		depth--;
		if (depth == 0) return true; // Function ended
		break;
	}
	default: break;
	}
...
{
	if (hdr == nullptr || hdr->off != unsigned(offset)) return;
	...
	if (!bounds::countFits(0, record_bytes, hdr->num_records, sizeof(LineInfoRecord))) return;
...
void PDBSymbols::parse_symbols(void)
{
	...
	while (PDBGeneralSymbol* symbol = symbol_at(pdb_sym_data, pdb_sym_size, position))
	{
		...
		if (symbol->type == S_GDATA32 /*|| symbol->type == S_LDATA32*/
			&& record_holds(symbol, sizeof(DATASYM32)) && record_name_terminated(symbol, offsetof(DATASYM32, name)))
		{ // Global variable
	...
	{
		if (modules[m].stream_num == 65535) continue;
		...
		{                                                         // Process all symbols in module stream
			if (symbol->size == 0xf4 || symbol->type == 0) break; // Determine the end of symbol list
			switch (symbol->type)
			{
			case S_GPROC32:
			case S_LPROC32: { // Symbol is function begin
				...
				if (new_function == nullptr || new_function->type_def == nullptr
					|| new_function->type_def->type_class != PDBTYPE_FUNCTION)
				{
					delete new_function;
			...
			case S_LDATA32: { // Data symbol
				...
				if (new_function != nullptr && !sections.empty() && sym->seg <= sections[0].file_address)
		...
		{ // Process all big symbols in module stream
			if (symbol->type == 0 || symbol->type > 0xFF) break;
			switch (symbol->type)
			{
			case 0xF2: { // Symbol is line info header
				// A subsection shorter than the line info header does not
				// carry one, so its fields are not there to be read.
				if (symbol->size + sizeof(PDBBigSymbol) < sizeof(LineInfoHeader)) break;
				...
				if (fIt != functions.end() && fIt->second != nullptr) fIt->second->parse_line_info(sym);
				break;
			}
			default: break;
			}
...
void PDBSymbols::dump_module_symbols(int index)
{
	...
	while (PDBGeneralSymbol* symbol = symbol_at(stream->data, stream_size, position))
	{ // Dump symbols
		if (symbol->size == 0xf4 || symbol->type == 0) break;
		...
		case S_FRAMEPROC: // 0x1012
		{
			...
			printf(
				"FRAMEPROC cbFrame: %08x cbPad: %08x offPad: %08x cbSaveRegs: %08x offExHdlr: %08x sectExHdlr: %04x "
				"flags: \n",
				sym->cbFrame,
				sym->cbPad,
				sym->offPad,
		...
		case S_BLOCK32: // 0x1103
		{
			...
			printf(
				"BLOCK32 pParent: %08x pEnd: %08x len: %08x off: %08x seg: %04x name: %s\n",
				sym->pParent,
				sym->pEnd,
				sym->len,
				sym->off,
		...
		case S_LABEL32: // 0x1105
		{
			...
			printf(
				"LABEL32 off: %08x seg: %04x flags: %02x name: %s\n", sym->off, sym->seg, sym->flags.bAll, sym->name);
			break;
		}
		...
		case S_PUB32: // 0x110e
		{
			...
			printf(
				"PUB32 pubsymflags: %08x off: %08x seg: %04x name: %s\n",
		...
		case S_LPROC32: // 0x110f
		{
			...
			printf(
				"LPROC32 pParent: %08x pEnd: %08x pNext: %08x len: %08x DbgStart: %08x DbgEnd: %08x typind: %08x off: "
				"%08x seg: %04x flags: %02x name: %s\n",
				sym->pParent,
				sym->pEnd,
				sym->pNext,
				sym->len,
		...
		case S_GPROC32: // 0x1110
		{
			PROCSYM32* sym = reinterpret_cast<PROCSYM32*>(symbol);
			printf(
				"GPROC32 pParent: %08x pEnd: %08x pNext: %08x len: %08x DbgStart: %08x DbgEnd: %08x typind: %08x off: "
				"%08x seg: %04x flags: %02x name: %s\n",
				sym->pParent,
				sym->pEnd,
				sym->pNext,
		...
		case S_COMPILE2: // 0x1116
		{
			...
			printf(
				"COMPILE2 machine: %04x verFEMajor: %04x verFEMinor: %04x verFEBuild: %04x verMajor: %04x verMinor: "
				"%04x verBuild: %04x verSt: %s\n",
				sym->machine,
				sym->verFEMajor,
				sym->verFEMinor,
				sym->verFEBuild,
		...
		case S_GMANPROC: // 0x112a
		{
			...
			printf(
				"GMANPROC pParent: %08x pEnd: %08x pNext: %08x len: %08x DbgStart: %08x DbgEnd: %08x token: %08x off: "
				"%08x seg: %04x flags: %02x retReg: %04x name: %s\n",
				sym->pParent,
				sym->pEnd,
				sym->pNext,
				sym->len,
		...
		case S_LMANPROC: // 0x112b
		{
			MANPROCSYM* sym = reinterpret_cast<MANPROCSYM*>(symbol);
			printf(
				"LMANPROC pParent: %08x pEnd: %08x pNext: %08x len: %08x DbgStart: %08x DbgEnd: %08x token: %08x off: "
				"%08x seg: %04x flags: %02x retReg: %04x name: %s\n",
				sym->pParent,
				sym->pEnd,
				sym->pNext,
		...
		case S_TRAMPOLINE: // 0x112c
		{
			...
			printf(
				"TRAMPOLINE trampType: %04x cbThunk: %04x offThunk: %08x offTarget: %08x sectThunk: %04x sectTarget: "
				"%04x\n",
				sym->trampType,
				sym->cbThunk,
				sym->offThunk,
		...
		case 0x1136: // 0x1136
		{
			...
			printf(
				"(?)SECTION seg: %04x ?: %04x off: %08x len: %08x ?: %08x name: %s\n",
				sym->seg,
				sym->unknown1,
				sym->off,
				sym->len,
		...
		case 0x1137: // 0x1137
		{
			...
			printf(
				"(?)COFFGROUP len: %08x ?: %08x off: %08x seg: %04x name: %s\n",
				sym->len,
				sym->unknown1,
				sym->off,
				sym->seg,
		...
		case 0x113c: // 0x113c
		{
			...
			printf(
				"(?)COMPILE3 %08x %04x %08x %08x %08x %08x %s\n",
				sym->unknown1,
				sym->unknown2,
				sym->unknown3,
				sym->unknown4,
		...
		case 0x1141: // 0x1141
		{
			...
			printf(
				"(?)DEFRANGE_REGISTER %08x %08x %04x %04x\n",
				sym->unknown1,
				sym->unknown2,
				sym->unknown3,
				sym->unknown4);
	...
	{ // Dump big symbols
		...
		case 0xF2: {
			...
			LineInfoHeader* sym = reinterpret_cast<LineInfoHeader*>(symbol);
			printf(
				"FUNCTION LINE INFO off: %08x seg: %08x len: %08x lines: %3d u1: %08x u2: %08x\n",
				sym->off,
				sym->seg,
				sym->len,
				sym->num_records,
...
void PDBSymbols::print_functions(void)
{
	...
	{
		if (it->second != nullptr) it->second->dump();
	}
}
...
void PDBSymbols::dump_symbol(PSYM Sym)
{
	...
	case S_UDT: printf("S_UDT| [%04x] typedef %s;", Sym->Udt.typind, Sym->Udt.name); break;

	case S_LPROCREF:
	case S_PROCREF:
		printf(
			"S_%sPROCREF| procref [%s; mod %04x; ibSym %x] %s",
	...
	case S_GDATA32: {
		printf(
			"S_%sDATA32| data [%s; type %04x] %04x = %s",
	...
	default: printf("unknown symbol len %04x type %04x\n", Sym->Sym.reclen, Sym->Sym.rectyp);
	}
